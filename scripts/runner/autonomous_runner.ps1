...
결정을 내리고, 중요하면 docs/DECISIONS.md에 이유를 남긴다.
...
사용자 권한·실제 Workflow를 망가뜨리면서 시각만 화려하게 만드는 것은 금지다.
...
"체크포인트에 도달했다"는 전부 종료 사유가 아니다. 멈추는 것이 정당한 경우는 둘뿐이다:
  (a) 지금 당장 실행 가능한 남은 작업이 정말로 하나도 없다 — 모든 후보가 진짜 외부 요인
      (사람만 풀 수 있는 것: MFA, 외부 승인, 접근 불가, 상충 요구)으로 막혔다. 한 blocker
      때문에 독립적으로 가능한 다른 작업까지 멈추는 것은 여기에 해당하지 않는다.
      TEST SERVER 접근·package 설치·QA 데이터 부재는 여기에 **해당하지 않는다**(4단계 참고).
    ...
    while ($true) {
        ...
        if ((Get-IntOr $state.consecutiveInfraRetries) -ge $MaxInfraRetries) {
            ...
            Write-Banner @(
                ...
                "실제 작업이 한 번도 진행되지 않고 있으므로 사람이 확인해야 합니다(네트워크·인증·구독 상태)."
            )
