...
- **먼저 전체 소비처를 다 찾고 나서 한 번에 고쳐라.** 한 곳 고치고 테스트하고, 또 한 곳 고치고
  테스트하는 식으로 돌지 마라.
- **테스트는 묶음이 끝난 뒤 한 번** 관련 스위트 전체를 돌린다. 파일 하나 바꿀 때마다 돌리지
  마라. 묶음이 크면 중간에 한 번 정도만 확인한다.
- **커밋도 묶음 단위**로 한다. 항목마다 커밋하지 마라.
- **상태 문서 갱신도 묶음이 끝난 뒤 한 번** 몰아서 한다. 항목마다 WORK_STATE를 고치지 마라.

### 테스트 강도 — 어디에 시간을 쓸지 고른다
사용자 전역 규칙에 "write-test-first가 MANDATORY"라고 되어 있지만, **이 무인 실행에서는 그것을
모든 항목에 기계적으로 적용하지 마라.** 아래 기준으로 판단한다.

- **버그 수정**: 재현 테스트를 **먼저** 만드는 것이 여전히 가장 빠르다 — 고쳤는지 확인할 유일한
  방법이기 때문이다. 이건 유지한다.
- **리팩터·신규 구현·UI 재설계**: 구현을 먼저 하고 관련 테스트를 뒤에 보강한다. 껍데기 테스트를
  먼저 쓰느라 시간을 쓰지 마라.
- **revert-to-verify(수정 전 실패 확인)**: 회귀 위험이 실제로 큰 것에만 한다 — 공유 component,
  권한/스코프 경로, transaction/동시성. 나머지는 생략한다.
- **커버리지 숫자를 목표로 삼지 마라.** 계약을 검증하는 테스트를 쓰고, 숫자를 맞추려고 테스트를
  양산하지 않는다.
...
BACKLOG.md·PROGRESS_STATUS.md·QA_COVERAGE.md도 함께)에 적고 git commit 한다. 항목마다가 아니다. 이 파일 기반 체크포인트는 **복구용**이지
