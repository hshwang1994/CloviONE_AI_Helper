...
PATTERNS: dict[str, str] = {
    ...
    # 최초 고객사 사내망 호스트명. 옛 이름은 S3 이 퇴역시켰지만 목록에서 빼지 않는다 —
    # 되살아나는 것을 잡는 것이 이 검사의 일이고, 지운 값은 아무것도 지키지 못한다.
    r"clovirone-ai\.gooddi\.lab": "고객사 호스트명(옛 이름)",
    # 같은 고객사의 현재 호스트명. **이름이 바뀌었다고 검사가 따라가지 않으면 S3 은 값을
    # 검사 밖으로 옮긴 것이 된다** — 새 이름을 소스 기본값에 박아도 아무 데서도 안 걸린다.
    r"clovirassist\.gooddi\.lab": "고객사 호스트명",
