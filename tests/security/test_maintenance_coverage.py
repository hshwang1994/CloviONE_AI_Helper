...
GATED_PREFIXES = (
    ...
    "/api/projects",
    # 작업 보드(S6). 티켓과 **같은 데이터를 같은 이유로** 바꾼다 — 카드를 옮기는 것은
    # 상태 변경이고, 그 변경은 저장소 seam 을 지나 외부 소스까지 나간다. 점검 중에
    # 그것이 나가면 점검이 끝난 뒤 미러와 소스가 어긋난 채로 남는다.
    "/api/work",
    # 지식 공간·문서(S7). 사용자가 쓰는 글이고, 저장은 **판을 쌓는다** — 점검 중에
    # 쌓인 판은 점검이 끝난 뒤에도 이력에 남아 「그때 무슨 일이 있었나」를 흐린다.
    # 라우터 전체에 이미 게이트가 걸려 있고(`app/knowledge/router.py`), 여기 적는 것은
    # 그것이 **의도**라는 기록이다.
    "/api/knowledge",
    # AI 작업공간(S10). 질의는 **구독 호출을 쓰고**, 초안은 `source_type=AI` 문서를
    # 만들어 판을 쌓는다 — 지식 도메인과 같은 이유로 같은 쓰기다. 점검 중에 쌓인 판은
    # 점검이 끝난 뒤에도 이력에 남아 「그때 무슨 일이 있었나」를 흐린다.
    #
    # ⚠️ **`GET /api/ai/search` 는 이 정책의 대상이 아니다** — 읽기라 애초에 안 걸린다.
    # 그래서 점검 중에도 근거 문서를 찾아보는 것은 그대로 된다.
    "/api/ai",
    "/api/board",
    "/api/team-docs",
    "/api/team-chat",
...
EXEMPT_PREFIXES: dict[str, str] = {
    ...
    # 운영자 전용 검색 재색인(C7). 사용자 내용을 만들거나 바꾸지 않고 검색 인덱스만
    # 재구축한다. 오히려 점검 중에 깨진 색인을 고치려고 쓸 수 있는 도구라 막으면 안 된다.
    "/api/search/reindex": "검색 색인 재구축뿐, 사용자 내용 쓰기가 아니다. 점검 중 복구 도구로도 쓰인다",
    # 파일 저장소 설정(S8). 사용자 내용을 만들지 않고, `STORAGE_CONFIGURE`(시스템 관리자)
    # 전용이다. 그리고 **점검 중에 써야 하는 도구다** — 저장소가 안 붙어서 점검을 켠
    # 상황이 정확히 이 화면을 쓰는 상황이다. 막으면 고치러 들어온 사람이 못 고친다.
    "/api/storage": "저장소 설정이다. 사용자 내용 쓰기가 아니고, 점검 중 복구 도구로 쓰인다",
}
