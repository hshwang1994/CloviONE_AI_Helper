...
def test_a_comment_on_a_ticket_nobody_owns_can_still_be_deleted(client, login_as, world, app):
    """**미할당 트리아지에 보이는 티켓의 댓글은 지워져야 한다.** 목록엔 있는데 못 지우면
    그건 보안이 아니라 고장이다."""
    csrf = login_as("user", email=ME)
    ...
    assert r.status_code == 200, (
        f"트리아지에 보이는 티켓의 댓글을 못 지운다({r.status_code}) — 목록엔 있는데 못 지우는 화면: "
        f"{r.text[:160]}"
    )
