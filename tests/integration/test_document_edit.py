...
**1 과 5 는 「정본과 원본이 다른 곳에 있다」를 전제로 하는 말**이었다. S14 가 그 원본을
걷어냈으므로(D-284) 확인할 대상 자체가 없어졌고, 그 두 절의 시험 여섯을 지웠다. 남은
성질(CSRF·낙관적 잠금·범위 404·작성자 미해석·휴지통·감사)은 소스와 무관하므로 실제로
배포되는 경로 위에서 그대로 선다.

`author_notion_ids` 를 채우던 동기화도 함께 사라졌다 — 4번은 그래서 **더** 중요해졌다.
그 칸은 이제 이관해 온 값에서 영영 안 움직이고, 막아 두면 그 문서들은 영영 못 고친다.

qa-contract-change: 여섯 시험이 Notion push 왕복(원본에서 되읽은 본문·저장이 원본을 부른다·원본 블록 보존·push 실패 배너와 그 지속·다음 저장이 그 배너를 지운다)을 확인했고 S14 가 그 구간을 통째로 없앴다(D-284) — 밀어 넣을 상대가 없으므로 지킬 대상이 없고, 자체 DB 쪽 성질은 tests/integration/test_native_document_repository.py 가 같은 이름으로 본다.
"""
...
# ── 1) 저장이 원본까지 간다 ───────────────────────────────────────────────────
#
# 여기 있던 세 시험(원본에서 되읽은 본문 · 저장이 원본을 부른다 · 저장이 원본 블록을
# 안 지운다)을 걷었다. 셋 다 **본문의 기록이 둘**일 때만 뜻이 있는 성질이고, S14 뒤로는
# 이 표가 유일한 기록이다(D-284). 자체 DB 쪽 성질은
# `tests/integration/test_native_document_repository.py` 가 본다 — 저장된 본문을 그대로
# 돌려주는가, 저장이 남은 오류를 비우는가, 없는 문서에 저장하면 404 인가.
...
# ── 5) 저장 결과가 화면에 보인다 ─────────────────────────────────────────────
#
# 여기 있던 세 시험(push 실패가 보인다 · 새로고침해도 남는다 · 다음 저장이 지운다)을
# 걷었다. push 라는 구간이 없어졌으므로(D-284) 실패도 없다. 자체 DB 소스는 저장할 때마다
# `body_sync_error` 를 오히려 **비운다** — 고칠 수 없는 경고를 영원히 띄우지 않기
# 위해서다. 그 비우기는 `test_native_document_repository.py` 가 본다.
# ── 6) 휴지통 문서는 편집 불가 ────────────────────────────────────────────────
