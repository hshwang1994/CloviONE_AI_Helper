...
def notion(fake_http) -> FakeNotionTasksDB:
    ...
    return FakeNotionTasksDB(
        rows=[
            task_row(page_id=PAGE_FREE, tid=901, title="미할당 티켓", status="진행",
                     due="2026-09-01", people=[]),
            task_row(page_id=PAGE_BOB, tid=902, title="밥이 맡은 티켓", status="진행",
                     due="2026-09-02", people=[NID["bob"]]),
        ],
...
def test_assigning_a_ticket_to_someone_notifies_them(client, app, people, notion):
...
def test_creating_a_ticket_for_someone_notifies_them(client, app, people, notion):
...
def test_assigning_a_ticket_to_myself_notifies_nobody(client, app, people, notion):
...
def test_claiming_a_ticket_notifies_nobody(client, app, people, notion):
...
def test_only_the_newly_added_assignee_hears_about_it(client, app, people, notion):
...
def test_a_swap_notifies_the_incoming_assignee_only(client, app, people, notion):
    """교체(밥 → 초). 들어온 사람에게만 간다.
...
def test_a_no_op_edit_notifies_nobody(client, app, people, notion):
...
def test_the_deep_link_points_at_the_ticket_screen(client, app, people, notion):
    csrf = _login(client, "amy@goodmit.co.kr")
...
def test_muting_the_type_keeps_it_out_of_the_badge(client, app, people, notion):
...
def test_a_broken_notifier_does_not_block_the_assignment(
    client, app, people, notion, monkeypatch
):
