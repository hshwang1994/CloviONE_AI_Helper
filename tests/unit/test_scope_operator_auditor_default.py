"""RBAC 재감사(2026-08-16)로 발견: `build_scope()`는 `role == ROLE_USER`만 따로 보고
나머지는 전부(operator/auditor 포함) `admin_scope` 컬럼으로 판정해 왔다. 그 컬럼의
기본값은 `global`(0024 마이그레이션의 의도된 선택)이고, 관리자가 사용자를 운영자/감사자로
바꿀 때 "관리 범위"를 함께 좁히는 것은 강제되지 않는 선택이다 — 그래서 `admin_scope`를
한 번도 명시적으로 좁힌 적 없는 운영자는 그 사실만으로 전역 범위가 됐다(실측:
...
이 파일은 그 원인 지점(`build_scope()`)을 API/화면 없이 함수 수준에서 직접 고정한다.
"""
