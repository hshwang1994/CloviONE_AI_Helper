...
APPR-01 계열 — 승인/스케줄/작업 큐/러너 알림이 서버가 이미 계산해 줄 수 있는 딥링크
(각 화면이 이미 `onQuery`로 소비하는 `?id=`/`?job_id=`)를 못 받고 있었다. 화면 쪽
(`onQuery`) 검증은 프런트 registry 시험이 이미 하므로, 여기서는 백엔드 표
(`RELATED_DESTINATIONS`)가 그 네 유형을 실제로 계산해 주는지, 그리고 아직 준비 안 된
유형(schedule_run/user)은 여전히 null인지를 못박는다.
"""
...
@pytest.mark.parametrize("object_type", ["schedule_run", "user", "unknown_type", None, ""])
def test_types_without_a_single_row_deep_link_stay_null(object_type):
    """schedule_run·user는 대상 화면에 id 딥링크(onQuery)가 아직 없다(destinations.py의
    표 뒤 주석 참고) — 여기 넣으면 화면이 그 id를 조용히 무시하고 무필터 목록만 연다."""
    assert destination_for(object_type, "some-id") is None


@pytest.mark.parametrize("object_type", ["approval", "schedule", "job", "runner"])
