...
export function MirrorNotice({ sync, canSync, onSync, syncing, unit = "항목" }) {
  ...
  if (!failed && !never) return null;

  const count = sync.doc_count != null ? sync.doc_count : sync.ticket_count;
  const text = failed
    ? "최근 동기화에 실패했습니다. 마지막 정상 데이터를 보고 있습니다"
      + (sync.last_success_at ? " (" + fmtDateTime(sync.last_success_at) + " 기준" + (count != null ? ", " + unit + " " + count + "개" : "") + ")." : ".")
  ...
  return (
    <Box sx={{ display: "flex", gap: 1.5, alignItems: "center", flexWrap: "wrap", mb: 2.5 }}>
      <Box sx={{ flex: 1, minWidth: "16rem" }}>
        <Callout tone={failed ? "warn" : "info"}>{text}</Callout>
      </Box>
      {canSync && onSync ? (
