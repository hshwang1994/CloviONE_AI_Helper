 ...
 *   5) 계획 논의       — "다음 주엔 뭘 하나"
 *
 * 그리고 화면이 스스로 **'날짜 범위 기준'** 이라고 말한다. 이 앱에는 스프린트라는 표가
 * 따로 없고, 여기의 '이번 주' 는 그 기간에 마감이 잡힌 티켓을 모은 것이다. 화면이 말하지
 * 않으면 팀이 따로 부르는 스프린트와 이 한 주를 같은 이름으로 부르며 회의를 한다. */
  ...
  return (
    <div className="c-screen">
      ...
      {/* 이 화면의 '주'가 무엇인지 못박는다. 아무 말이 없으면 회의에서 두 사람이 서로 다른
          것을 같은 이름으로 부른다 — 한 사람은 이 날짜 범위를, 다른 사람은 팀이 따로
          관리하는 스프린트를 떠올린다. */}
      <Box sx={{ mb: 2.5 }}>
        <Note sx={{ mt: 0 }}>
          이 화면은 <b>날짜 범위 기준</b>입니다. 위 기간에 마감이 잡힌 티켓을 모읍니다. 이 앱에는 스프린트를
          따로 관리하는 곳이 없으므로, 팀이 다른 곳에서 정한 스프린트와 이 화면의 한 주는 서로 다를 수 있습니다.
        </Note>
      </Box>
