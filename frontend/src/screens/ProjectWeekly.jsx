...
function Bucket({ title, bucket, onOpen }) {
  ...
  return (
    <Box sx={{ minWidth: 0 }}>
      <Typography component="h3" sx={{ fontSize: "0.9375rem", fontWeight: 750 }}>
        {title + " " + (b.count || 0) + "건"}
      </Typography>
      {items.length === 0 ? (
    ...
    <Stack gap={2.5}>
      ...
      <Card>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5, ...KO_WORD_BREAK }}>
          {data.source === "llm" ? "AI 가 쓴 요약입니다." : "규칙으로 만든 요약입니다."}
          ...
          {data.source !== "llm" && data.llm_notice ? " " + data.llm_notice : ""}
        </Typography>
        <BodyPreview text={data.summary_md || ""} wide />
        <Typography variant="body2" color="text.secondary" sx={{ mt: 1.5, ...KO_WORD_BREAK }}>
