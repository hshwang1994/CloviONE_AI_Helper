 ...
 * (clovirassist-worker-conversational.service)이다. 예전 이분법(web이 아니면
 * 무조건 clovirassist-worker)을 그대로 두면, 대화형 레인이 죽었을 때도 배치 워커
...
describe("ServiceStatusPanel — worker_conversational 유닛 안내 (D-118)", () => {
  it("대화형 레인만 죽었을 때 journalctl 안내가 배치 워커가 아니라 대화형 유닛을 가리킨다", () => {
    ...
    expect(screen.getByText(/clovirassist-worker-conversational/)).toBeInTheDocument();
    expect(screen.queryByText(/journalctl -u clovirassist-worker(?!-conversational)/)).toBeNull();
  });

  it("배치 워커만 죽었을 때는 여전히 배치 유닛을 가리킨다(회귀 방지)", () => {
    renderPanel({ web: "up", worker: "down", scheduler: "up" });
    expect(screen.getByText(/journalctl -u clovirassist-worker\b/)).toBeInTheDocument();
  });
