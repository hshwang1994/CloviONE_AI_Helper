 ...
 *
 * 여기서 지키는 것은 네 가지다:
 *   1) 화면이 스스로 **'날짜 범위 기준'** 이라고 말한다. 이 앱에는 스프린트라는 표가
 *      따로 없으므로, 포털의 '이번 주' 와 팀이 다른 곳에서 정한 스프린트는 **다른 것**이다.
 *      화면이 말하지 않으면 두 사람이 서로 다른 것을 같은 이름으로 부르며 회의를 한다.
...
describe("스프린트 회의 5단계 재설계", () => {
  ...
  it("무엇을 기준으로 모은 화면인지 말한다 — '날짜 범위 기준'", async () => {
    ...
    /* 왜 그렇게 말해야 하는지까지 화면에 있다. 예전에는 그 이유를 「Notion 의 스프린트
       데이터베이스가 이 포털 연동에 공유되어 있지 않다」로 적었는데, 그 연동 자체가
       없어졌으므로 지금은 사실이 아니다 - 없는 연동을 있는 것처럼 말하지 않는다. */
    expect(screen.getByText(/스프린트를\s*따로 관리하는 곳이 없으므로/)).toBeInTheDocument();
    expect(screen.queryByText(/Notion 에서 만든 스프린트/)).toBeNull();
  });
