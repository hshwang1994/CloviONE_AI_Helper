...
/* 본문(DocBody)의 legacy 산문 스타일은 screens.css가 px 토큰(--font-size-lg: 17px 등)으로 잡고
 * 있어 4K에서 글자만 그대로 남는다. 그 CSS는 문서 상세(TeamDoc)와 공유라 여기서 고칠 수 없으므로,
 * 이 화면 안에서만 rem으로 덮어써 루트 폰트사이즈 레버(styles/root.css)를 따르게 한다.
 * screens.css의 .doc-* 가 rem으로 옮겨지면 이 블록은 지운다. */
const PROSE_SX = {
...
function MetaRow({ label, children }) {
  return (
    <Box sx={{ display: "grid", gridTemplateColumns: { xs: "7rem minmax(0,1fr)" }, gap: 1, py: 1, borderBottom: 1, borderColor: "divider", "&:last-of-type": { borderBottom: 0 } }}>
  ...
  return (
    <div className="c-screen">
      ...
      <Box sx={DETAIL_GRID}>
        ...
        {/* 메타/활동 레일 — 좁은 화면에서는 본문 위로 올린다(order). 나중에 댓글(계획 §E)이
            들어오면 같은 열에 카드로 덧붙인다. */}
