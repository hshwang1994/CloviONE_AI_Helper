...
describe("사이드바 그룹이 저절로 접히지 않는다", () => {
  beforeEach(() => {
    ...
    // PA-RC-0031: 예전엔 독립 "문서" 그룹이 있었다. 2026-08-19(D-166)로 업무와 소통·놀이를 갈라 /team-docs 는 '팀 업무' 쪽이다
  ...
  it("접혀 있던 그룹이 활성 라우트로 강제로 펼쳐진 뒤, 다른 곳을 클릭해도 계속 펼쳐져 있다", async () => {
    // '팀 업무' 그룹의 라우트(/team-docs)로 바로 진입 —
    // 접혀 있었어도 강제로 펼쳐진다.
    renderShell("/team-docs");
    await waitFor(() => expect(screen.getByText("본문")).toBeInTheDocument());

    const groupHeader = screen.getByRole("button", { name: /팀 업무/ });
    ...
    // 사용자가 직접 접은 적이 없으므로 '팀 업무' 그룹은 계속 펼쳐져 있어야 한다.
    expect(groupHeader).toHaveAttribute("aria-expanded", "true");
  });
});
