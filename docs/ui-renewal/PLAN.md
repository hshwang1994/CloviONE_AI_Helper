...
**반증 가능한 판정 기준**: 로고를 가려도 화면이 ClovirAssist라고 말하는가. 이걸 희망이 아니라 **테스트**로 박는다 — `blue(chrome.shell) − red(chrome.shell) ≥ 24` (`#1E2758` → **58** ✓ / 현재 `#E7EAEE` → **7 ✗**). 이 단언이 없었기 때문에 지금 상태가 통과했다. *(W1 정정: 계획서 초안이 이 자리에 88 을 적었는데 88 은 `#1E2758` 의 blue 채널값이고 차이는 58 이다. 판정에는 영향이 없다 — 둘 다 24 를 넘는다. 독립 Requirement Reviewer 가 잡았다.)*
...
| **W1** ⚠️ | Brand Foundation — `theme.js` → 생성기 → 두 `tokens.css` → 두 Contract Test 재작성. **한 에이전트, 한 커밋 시퀀스** | `frontend/src/ui/theme.js` | W0 | ~~`brand_presence ≥5/7`~~ → **W1 실행 중 정정(D-180)**: `≥5/7`은 실측상 도달 불가한 기준이다(Before 1,494페이지에서 role 존재 수가 5를 넘는 페이지가 **0개**, 5개인 페이지가 22.5%). 대신 ① `blue(chrome.shell) − red(chrome.shell) ≥ 24`가 `theme-contract.test.js`의 단언으로 존재하고 통과 ② smoke set × 두 테마에서 **`brand_role_coverage`**(있는 role 이 전부 Brand)가 측정되고, 실패가 있으면 그 role 을 소유한 Wave 의 Finding 으로 등록 ③ `theme-contract.test.js`의 대비 단언이 **원래 강도 이상**으로 통과(`check_test_strength.py`가 강제) |
