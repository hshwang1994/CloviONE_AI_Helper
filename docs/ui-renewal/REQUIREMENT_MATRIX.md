...
- **Status**: DONE
- **Evidence**: 활성 신호가 넷(2px 레일 + `fontWeight 700 vs 600` + 색 + `icon opacity 1 vs .82`)에서 **둘**(3px `chrome.rail` inline-start 0 + 행 채움 `chrome.selected` · 잉크 `onShell`)로 줄었다 — 실브라우저 실측 `dist/ui-qa/w3-nav-e2e/anatomy.json` 8조합(1920·3840 × light·dark × 사용자·관리자) 전부에서 활성/비활성 굵기가 같은 500, 잉크는 `#EAEDFB` vs `#AFBBE8`, 활성 행 `rail.width=3 · rail.left=0 · background=rgba(255,255,255,.10)`. 라벨 시작선이 그룹 60px / 자식 64px 두 열에서 **한 열 42px** 로 합쳐졌다(8/8 조합 `labelStart=42`, 4K 는 루트 20px 기준 정규화). 그룹 헤더는 위치 신호를 갖지 않아 펼침과 선택이 갈린다. 같은 그림의 반복(`ticket` 4곳·`report` 4곳·`docs`/`policy`/`users` 3곳씩)이 자식 `icon:` 키 49개 삭제로 0이 됐다. 접힘 기본값과 메뉴 필터가 **역할이 아니라 콘솔**로 정해진다. 독립 검수가 이 위에서 회귀 하나를 잡았고(1366x768 에서 «내 정보» 랜드마크가 통째로 스크롤 밖으로 나갔다 — 펼침 기본을 1920 실측만 보고 전 뷰포트에 적용했다) 같은 Wave 에서 고쳤다: 셸이 전부 펼친 높이를 **재서** 정하고(`useLayoutEffect`, 뷰포트 상수 없음), 프로브가 «스크롤 없이 안 보이는 랜드마크 0건» 을 **18조합 전부**에서 확인한다. 랜드마크 경계 간격도 4px → 12px 로 올렸다. `nav-contract.test.jsx` 21 단언(선택 면 단언은 `.Mui-selected` 규칙 하나만 보도록 좁혔다 — 이웃 `:hover` 가 대신 답하던 구멍을 검수가 변이로 실증했다) · `--stage wave`(**W3** 범위) PASS · `ev:qa_run:w3-after`(664페이지, 번들 e4753159d7b26235 — W2 대비 악화 0 · 개선 2) · `ev:qa_run:w3-nav-e2e`(18조합) · `ev:review:wf_6fca7af0-570` · `ev:note:D-184`
