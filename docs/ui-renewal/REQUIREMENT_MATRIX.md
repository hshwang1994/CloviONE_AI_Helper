...
- **Wave**: W5
- **Requirement**: Search, Category, Sort가 서로 다른 줄에 이유 없이 흩어지거나 Filter 하나만 다음 줄에 고립되는 화면이 있다. Project, 사용자, 담당자처럼 이름으로 찾는 Entity가 단순 Dropdown으로 남아 있는 경우도 있다. 검색 가능한 Autocomplete/Combobox 요구를 실제 Browser Interaction까지 구현하고 Filter Grouping, Height, Baseline, 줄바꿈을 함께 재설계한다. **W5 결과**: 탐색 줄이 판을 잃고(지시 80) 컨트롤 폭을 **종류**가 정한다(C2 «폭 배분»). Entity 축은 검색형 Combobox(`EntityCombobox`) — 제품 전체에 0개였다. `SearchBox` 의 기본 인자 함정을 없애 `/board`·`/ideas` 의 «정렬 select 하나만 다음 줄» 이 사라졌다. 축 순서는 `FILTER_GROUP_ORDER` 한 곳이 정한다.
- **Affected**: ALL
- **Implementation**: `frontend/src/ui/FilterBar.jsx`, `frontend/src/ui/filters.jsx`, `frontend/src/ui/kit.jsx` (`FormField`·`DataTable` 카드 뷰·`cardFieldLabel`/`cardHeaderControls`), `frontend/src/ui/bulkSelect.jsx`, `frontend/src/ui/theme.js` (`CONTROL`·`MuiTabs`·`MuiIconButton`), `frontend/src/screens/TicketFilterBar.jsx`, `frontend/src/screens/DataScreen.jsx`, `frontend/src/screens/TeamDocs.jsx`, `frontend/src/screens/Board.jsx`, `frontend/src/screens/Users.jsx`, `frontend/src/screens/MyTickets.jsx`, `frontend/src/screens/Project.jsx`, `frontend/src/screens/SchedulerCalendar.jsx`, `frontend/src/screens/registry/governance.js`, `frontend/src/screens/registry/automation.js`, `scripts/ui_qa/assertions.py`, `scripts/ui_qa/probe_selftest.py`, `scripts/ui_qa/filter_e2e.py`
- **Verification**: `python -m scripts.ui_qa.run --fail-on isolated_control_row control_baseline_mismatch plain_dropdown_for_entity header_cell_alignment_mismatch numeric_alignment`, `python -m scripts.ui_qa.probe_selftest`, `python -m scripts.ui_qa.filter_e2e --base-url https://clovirassist.gooddi.lab`, `npx vitest run src/ui/filter-row-width.test.jsx src/ui/entity-combobox.test.jsx src/ui/department-filter.test.jsx src/ui/table-card-select.test.jsx src/ui/theme-contract.test.js`, `python scripts/check_ui_renewal_coverage.py --stage wave`
- **Status**: DONE
- **Evidence**: 탐색 줄이 **판을 잃고** 컨트롤 폭을 종류가 정한다. `CONTROL_KIND` 일곱(search·entity·enum·date·text·number·toggle)이 폭 계약을 들고, 균등 격자(`repeat(auto-fit, minmax(11rem,16rem))`)가 만들던 세 결함이 함께 사라졌다 — 값 길이와 무관한 균등 폭 · 고아 줄 · **검사 불능**(`width: fit-content` 가 «윗줄 여유» 를 구조적으로 0 으로 만들어 664 페이지에서 fail 이 0 이었다). Entity 축은 검색형 Combobox 다(`EntityCombobox`) — 제품 전체에 0개였고, 폼과 필터가 **같은 부품**을 쓴다(C8). 「‘사용자’ 화면에서 ID를 복사해 붙여 넣으세요」 자리 열 곳이 이름으로 고르는 자리가 됐고, 후보가 세상에 없는 자리는 `freeTextReason` 으로 이유를 선언한다(DOM 에 `data-free-text` 로 남는다 — 억제가 아니라 종류 선언이다). 키보드 사슬을 실브라우저에서 확인했다: ArrowDown 으로 열고 글자를 쳐서 후보를 좁히고 Enter 로 고른 값이 **주소(`project_id=`) → 서버 질의 → 결과 줄**까지 닿는다. `ev:qa_run:w5-after`(번들 `08b5525cb2f52618` · 83 Route × 2테마 × **9뷰포트** = 1,494페이지 — 승격 다섯 중 넷이 fail 0, `control_baseline_mismatch` 는 **1,381 pass / 0 fail**) · `ev:qa_run:w5-filter-e2e`(면 계약 32/32 조합 + 키보드 사슬 PASS) · `ev:test:scripts/ui_qa/probe_selftest.py`(반례 19사례) · `ev:review:wf_e886b628-93e`(독립 3 렌즈 24건 → 적대적 검증 10건 → 확정 6 · 기각 4, 확정 전부 반영) · `ev:note:D-186` · commit `(2차 커밋에서 채운다)`
- **Findings**: (없음 — W5 조사 Finding 은 `ROUTE_COVERAGE.json` 의 `w5_discovery_findings` 에 164건 등록)
- **Depends on**: (없음)
...
- **Wave**: W5
- **Requirement**: Portal 전체 페이지의 Search, Filter, Select, Sort, Condition 영역을 전수조사한다. 현재 페이지마다 검색창 크기, 필터 순서, Select 크기, 조건 배치가 제각각인 구조를 개선한다. 공통 Search/Filter Bar 패턴을 설계하되 모든 페이지에 동일한 필터 구성을 강제로 적용하지 않는다. 검색창은 주요 탐색 기능이라면 충분한 너비와 시각적 우선순위를 갖도록 한다. 필터가 많은 경우 검색, 주요 필터, 보조 필터, 정렬 기능의 위계를 명확하게 구분한다. 문서, 자유게시판, 기능 개선 제안 등 검색, 카테고리, 상태, 정렬이 뒤섞여 있는 페이지를 우선적으로 개선한다. 프로젝트처럼 데이터가 많고 이름으로 찾는 것이 자연스러운 항목은 검색 가능한 Autocomplete 또는 Combobox 방식을 사용한다. 입력 중 일치하는 후보를 아래에 표시하고 Keyboard와 Mouse로 선택할 수 있어야 한다. 다른 필터도 동일한 방식이 더 적합한지 조사하되 모든 Select를 무조건 검색형으로 변경하지 않는다. **W5 결과**: 검색·필터·정렬 부품이 화면마다 복제돼 있던 것을 공용 계층으로 모았다 — `TeamDocs` 의 사본 `FilterSelect`(라벨이 상자 안으로 내려앉던 원인)를 지우고, `Users` 의 손수 만든 검색창(디바운스 250ms 하드코딩)을 공용 `SearchBox`(토큰 300ms)로 바꿨다. 건수는 필터와 목록 **사이**의 결과 줄로 옮겨 «이 조건에 대한 결과» 라는 관계를 만든다.
- **Affected**: ALL
- **Implementation**: `frontend/src/ui/FilterBar.jsx`, `frontend/src/ui/filters.jsx`, `frontend/src/ui/kit.jsx` (`FormField`·`DataTable` 카드 뷰·`cardFieldLabel`/`cardHeaderControls`), `frontend/src/ui/bulkSelect.jsx`, `frontend/src/ui/theme.js` (`CONTROL`·`MuiTabs`·`MuiIconButton`), `frontend/src/screens/TicketFilterBar.jsx`, `frontend/src/screens/DataScreen.jsx`, `frontend/src/screens/TeamDocs.jsx`, `frontend/src/screens/Board.jsx`, `frontend/src/screens/Users.jsx`, `frontend/src/screens/MyTickets.jsx`, `frontend/src/screens/Project.jsx`, `frontend/src/screens/SchedulerCalendar.jsx`, `frontend/src/screens/registry/governance.js`, `frontend/src/screens/registry/automation.js`, `scripts/ui_qa/assertions.py`, `scripts/ui_qa/probe_selftest.py`, `scripts/ui_qa/filter_e2e.py`
- **Verification**: `python -m scripts.ui_qa.run --fail-on isolated_control_row control_baseline_mismatch plain_dropdown_for_entity header_cell_alignment_mismatch numeric_alignment`, `python -m scripts.ui_qa.probe_selftest`, `python -m scripts.ui_qa.filter_e2e --base-url https://clovirassist.gooddi.lab`, `npx vitest run src/ui/filter-row-width.test.jsx src/ui/entity-combobox.test.jsx src/ui/department-filter.test.jsx src/ui/table-card-select.test.jsx src/ui/theme-contract.test.js`, `python scripts/check_ui_renewal_coverage.py --stage wave`
- **Status**: DONE
- **Evidence**: 화면마다 복제돼 있던 검색·필터·정렬 부품을 공용 계층으로 모았다 — `TeamDocs` 의 사본 `FilterSelect`(라벨이 상자 안으로 내려앉던 원인)를 지우고, `Users` 의 손수 만든 검색창(디바운스 250ms 하드코딩)을 공용 `SearchBox`(토큰 300ms)로 바꿨다. `DataScreen` 의 자유 텍스트 필터도 `DebouncedTextField` 를 타면서 **서버 필터 키에서 타자 한 글자마다 나가던 요청**이 사라졌다. 건수는 필터와 목록 **사이**의 결과 줄로 옮겨 「이 조건에 대한 결과」라는 관계를 만든다. 사람 고르기 셋(그룹 초대·1:1·멤버 추가)은 디렉터리 전량을 쏟던 것을 `PeopleFilter` 로 좁힐 수 있게 했다. 회귀는 **선언을 읽는 시험**이 든다(`registry-entity-fields.test.js` — REGISTRY 전수에서 «Entity 어휘 라벨 + type:text» 0건, 예외에는 이유 문장을 요구한다). 브라우저 프로브는 그려진 것만 보므로 모달 안·조건부 필드를 원리적으로 놓친다. `ev:qa_run:w5-after`(번들 `08b5525cb2f52618` · 83 Route × 2테마 × **9뷰포트** = 1,494페이지 — 승격 다섯 중 넷이 fail 0, `control_baseline_mismatch` 는 **1,381 pass / 0 fail**) · `ev:qa_run:w5-filter-e2e`(면 계약 32/32 조합 + 키보드 사슬 PASS) · `ev:test:scripts/ui_qa/probe_selftest.py`(반례 19사례) · `ev:review:wf_e886b628-93e`(독립 3 렌즈 24건 → 적대적 검증 10건 → 확정 6 · 기각 4, 확정 전부 반영) · `ev:note:D-186` · commit `(2차 커밋에서 채운다)`
- **Findings**: (없음 — W5 조사 Finding 은 `ROUTE_COVERAGE.json` 의 `w5_discovery_findings` 에 164건 등록)
- **Depends on**: (없음)
...
- **Wave**: W5
- **Requirement**: Portal 전체 Form을 전수조사한다. Input, Textarea, Select, Autocomplete, Date Picker, Checkbox, Radio 등 Form Control의 크기, 높이, Label, Helper Text, Error State를 통일한다. 작은 Input 여러 개가 임의의 Grid에 배치되어 화면이 비어 보이거나 정렬이 맞지 않는 구조를 개선한다. 필드 중요도와 입력 순서에 맞게 Form Layout을 구성한다. 필수 항목, 선택 항목, Validation Error를 쉽게 이해할 수 있어야 한다. **W5 결과**: 폼의 Entity 필드에 `type:"entity"` 를 신설해 필터와 **같은 부품**을 쓰게 했다(C8). `FormField` 의 select 가 `displayEmpty` 를 빠뜨려 빈 값일 때 «잉크 0px 백지» 로 그려지던 것을 고쳤다. 컨트롤 높이 토큰을 rem 으로 옮겨 4K 에서 버튼(34px)과 아이콘 버튼(40px)이 갈리던 계약 파손을 닫았다.
- **Affected**: ALL
- **Implementation**: `frontend/src/ui/FilterBar.jsx`, `frontend/src/ui/filters.jsx`, `frontend/src/ui/kit.jsx` (`FormField`·`DataTable` 카드 뷰·`cardFieldLabel`/`cardHeaderControls`), `frontend/src/ui/bulkSelect.jsx`, `frontend/src/ui/theme.js` (`CONTROL`·`MuiTabs`·`MuiIconButton`), `frontend/src/screens/TicketFilterBar.jsx`, `frontend/src/screens/DataScreen.jsx`, `frontend/src/screens/TeamDocs.jsx`, `frontend/src/screens/Board.jsx`, `frontend/src/screens/Users.jsx`, `frontend/src/screens/MyTickets.jsx`, `frontend/src/screens/Project.jsx`, `frontend/src/screens/SchedulerCalendar.jsx`, `frontend/src/screens/registry/governance.js`, `frontend/src/screens/registry/automation.js`, `scripts/ui_qa/assertions.py`, `scripts/ui_qa/probe_selftest.py`, `scripts/ui_qa/filter_e2e.py`
- **Verification**: `python -m scripts.ui_qa.run --fail-on isolated_control_row control_baseline_mismatch plain_dropdown_for_entity header_cell_alignment_mismatch numeric_alignment`, `python -m scripts.ui_qa.probe_selftest`, `python -m scripts.ui_qa.filter_e2e --base-url https://clovirassist.gooddi.lab`, `npx vitest run src/ui/filter-row-width.test.jsx src/ui/entity-combobox.test.jsx src/ui/department-filter.test.jsx src/ui/table-card-select.test.jsx src/ui/theme-contract.test.js`, `python scripts/check_ui_renewal_coverage.py --stage wave`
- **Status**: DONE
- **Evidence**: 폼의 Entity 필드에 `type:"entity"` 를 신설해 필터와 **같은 부품**을 쓰게 했다(C8). `FormField` 의 select 가 `displayEmpty` 를 빠뜨려 빈 값일 때 «잉크 0px 백지» 로 그려지던 것을 고쳤다. 컨트롤 높이는 이제 **전부** 토큰에서 온다 — 예전에는 `theme.js` 에 override 가 넷뿐이라 MUI 기본값으로 서는 컨트롤이 있었고 한 줄에 Switch 38 · 입력 36 · 버튼 30 이 나란히 섰다(`/profile` 실측, 중심선 9.8px). `MuiSwitch`·`MuiCheckbox`·`MuiRadio`·`MuiTabs`·`MuiIconButton` 을 `CONTROL` 에 묶고 전부 rem 으로 내보낸다(4K 레버). 도움말 들여쓰기(`MuiFormHelperText` 기본 14px)를 0 으로 맞춰 라벨·입력·도움말의 좌측선을 하나로 했다. **한 오류에 표현 하나**(C5) — 필드를 지목한 오류는 인라인만, 전폭 상자는 지목할 칸이 없는 오류 전용이다. 모달 폭은 «필드 개수» 가 아니라 «구조» 가 정한다(한 열 폼은 언제나 md). 라벨 위치는 정적 검사가 지킨다(`check_label_above.py` — 첫 실행에서 13파일 60자리). `ev:qa_run:w5-after`(번들 `08b5525cb2f52618` · 83 Route × 2테마 × **9뷰포트** = 1,494페이지 — 승격 다섯 중 넷이 fail 0, `control_baseline_mismatch` 는 **1,381 pass / 0 fail**) · `ev:qa_run:w5-filter-e2e`(면 계약 32/32 조합 + 키보드 사슬 PASS) · `ev:test:scripts/ui_qa/probe_selftest.py`(반례 19사례) · `ev:review:wf_e886b628-93e`(독립 3 렌즈 24건 → 적대적 검증 10건 → 확정 6 · 기각 4, 확정 전부 반영) · `ev:note:D-186` · commit `(2차 커밋에서 채운다)`
- **Findings**: (없음 — W5 조사 Finding 은 `ROUTE_COVERAGE.json` 의 `w5_discovery_findings` 에 164건 등록)
- **Depends on**: (없음)
...
- **Affected**: ALL
- **Implementation**: `frontend/src/app/navConfig.js`, `frontend/src/app/navIcons.js`, `frontend/src/app/AppShell.jsx`(사이드바부), `frontend/src/app/CommandPalette.jsx`, `frontend/src/ui/theme.js`(`NAV_ANATOMY`·`ICON`)
- **Verification**: `frontend/src/app/nav-contract.test.jsx`, `frontend/src/app/nav-active.test.js`, `frontend/src/app/nav-features.test.js`, `frontend/src/app/registry-area-matches-nav-group.test.js`, `python -m scripts.ui_qa.nav_e2e`
- **Status**: DONE
- **Evidence**: 활성 신호가 넷(2px 레일 + `fontWeight 700 vs 600` + 색 + `icon opacity 1 vs .82`)에서 **둘**(3px `chrome.rail` inline-start 0 + 행 채움 `chrome.selected` · 잉크 `onShell`)로 줄었다 — 실브라우저 실측 `dist/ui-qa/w3-nav-e2e/anatomy.json` **16조합**(390·1366·1920·3840 × light·dark × 사용자·관리자) 전부에서 활성/비활성 굵기가 같은 500, 잉크는 `#EAEDFB` vs `#AFBBE8`, 활성 행 `rail.width=3 · rail.left=0 · background=rgba(255,255,255,.10)`. 라벨 시작선이 그룹 60px / 자식 64px 두 열에서 **한 열 42px** 로 합쳐졌다(16/16 조합 `labelStart=42`, 4K 는 루트 20px 기준 정규화). 그룹 헤더는 위치 신호를 갖지 않아 펼침과 선택이 갈린다. 같은 그림의 반복(`ticket` 4곳·`report` 4곳·`docs`/`policy`/`users` 3곳씩)이 자식 `icon:` 키 49개 삭제로 0이 됐다. 접힘 기본값과 메뉴 필터가 **역할이 아니라 콘솔**로 정해진다. 독립 검수가 이 위에서 회귀 하나를 잡았고(1366x768 에서 «내 정보» 랜드마크가 통째로 스크롤 밖으로 나갔다 — 펼침 기본을 1920 실측만 보고 전 뷰포트에 적용했다) 같은 Wave 에서 고쳤다: 셸이 전부 펼친 높이를 **재서** 정하고(`useLayoutEffect`, 뷰포트 상수 없음), 프로브가 «스크롤 없이 안 보이는 랜드마크 0건» 을 **18조합 전부**에서 확인한다. 랜드마크 경계 간격도 4px → 12px 로 올렸다. `nav-contract.test.jsx` 21 단언(선택 면 단언은 `.Mui-selected` 규칙 하나만 보도록 좁혔다 — 이웃 `:hover` 가 대신 답하던 구멍을 검수가 변이로 실증했다) · `--stage wave`(**W3** 범위) PASS · `ev:qa_run:w3-after`(664페이지, 번들 c03113861aaa5138 — W2 대비 셀 단위 5,312칸 대조에서 신규 fail 0 · fail→pass 2) · `ev:qa_run:w3-nav-e2e`(기하 16조합 + 팔레트 2테마) · `ev:review:wf_6fca7af0-570` + 재검증 `ev:review:wf_9c8f1032-de5` · `ev:note:D-184` · commit `ab2bc618`
- **Findings**: F-W1R-05 · F-W1R-18 · F-W2R-02 종결 (W3)
- **Depends on**: (없음)
...
- **Wave**: W5
- **Requirement**: Search와 Filter 영역은 Control을 한 Container 안에 넣었다는 이유만으로 완료된 것으로 판단하지 않는다. 검색, Category, Status, Project, Assignee, Sort, View 등 각 Control의 관계와 중요도를 기준으로 Layout을 구성한다. 현재 자유게시판처럼 Search Input과 Category는 위에 있고 Sort Select 하나만 아래 왼쪽에 홀로 떨어져 Container 대부분이 비는 Layout을 허용하지 않는다. 현재 Sprint처럼 여러 Filter는 첫 줄에 배치되고 담당자 하나만 다음 줄 왼쪽에 남아 전체 정렬과 리듬이 깨지는 Layout도 다시 설계한다. 상황에 따라 Primary Search, Primary Filters, Secondary Filters, Sort/View Control, Reset 또는 Saved View를 의미 단위로 Grouping한다. 서로 관련된 Control은 시각적으로 하나의 Group으로 읽혀야 한다. **W5 결과**: 축의 관계와 중요도로 Layout 을 구성한다 — 도구 줄(어떻게 볼지)과 필터 줄(무엇을 볼지)을 가르고, 되돌리기는 조건 칸이 아니라 흐름 끝의 텍스트 동작으로 둔다. 이 요구가 이름으로 지목한 두 화면(`/board` 정렬 select, `/sprint` 담당자)이 실제로 사라졌고, 그것을 지키라고 만든 검사가 **구조적으로 발화할 수 없던 상태**도 함께 고쳤다.
- **Affected**: ALL
- **Implementation**: `frontend/src/ui/FilterBar.jsx`, `frontend/src/ui/filters.jsx`, `frontend/src/ui/kit.jsx` (`FormField`·`DataTable` 카드 뷰·`cardFieldLabel`/`cardHeaderControls`), `frontend/src/ui/bulkSelect.jsx`, `frontend/src/ui/theme.js` (`CONTROL`·`MuiTabs`·`MuiIconButton`), `frontend/src/screens/TicketFilterBar.jsx`, `frontend/src/screens/DataScreen.jsx`, `frontend/src/screens/TeamDocs.jsx`, `frontend/src/screens/Board.jsx`, `frontend/src/screens/Users.jsx`, `frontend/src/screens/MyTickets.jsx`, `frontend/src/screens/Project.jsx`, `frontend/src/screens/SchedulerCalendar.jsx`, `frontend/src/screens/registry/governance.js`, `frontend/src/screens/registry/automation.js`, `scripts/ui_qa/assertions.py`, `scripts/ui_qa/probe_selftest.py`, `scripts/ui_qa/filter_e2e.py`
- **Verification**: `python -m scripts.ui_qa.run --fail-on isolated_control_row control_baseline_mismatch plain_dropdown_for_entity header_cell_alignment_mismatch numeric_alignment`, `python -m scripts.ui_qa.probe_selftest`, `python -m scripts.ui_qa.filter_e2e --base-url https://clovirassist.gooddi.lab`, `npx vitest run src/ui/filter-row-width.test.jsx src/ui/entity-combobox.test.jsx src/ui/department-filter.test.jsx src/ui/table-card-select.test.jsx src/ui/theme-contract.test.js`, `python scripts/check_ui_renewal_coverage.py --stage wave`
- **Status**: DONE
- **Evidence**: 축의 관계와 중요도로 Layout 을 구성한다 — 도구 줄(어떻게 볼지)과 필터 줄(무엇을 볼지)을 가르고, 되돌리기는 조건 칸이 아니라 흐름 끝의 텍스트 동작으로 둔다. 이 요구가 이름으로 지목한 두 화면(`/board` 정렬 select · `/sprint` 담당자)이 실제로 사라졌고, **그것을 지키라고 만든 검사가 구조적으로 발화할 수 없던 상태**도 함께 고쳤다(`isolated_control_row` 가 `fit-content` 컨테이너 때문에 «윗줄 여유» 를 항상 0 으로 재고 있었다). 실브라우저에서 고아 줄·높이 어긋남·되돌리기 표현·결과 줄 위치를 4화면 × 4뷰포트 × 2테마 = 32조합으로 직접 쟀다(`filter_e2e.py`). 프로브를 여섯 번 좁혔고 좁힌 자리마다 «그래도 진짜 결함은 잡힌다» 는 반례를 함께 넣었다 — 문서 밖으로 치운 서랍 · 흐름에 없는 것(absolute) · 두 줄로 접힌 글 · 키가 다른 둘의 중심선 · 격자에 놓인 판 · 검색 상자 자신. `ev:qa_run:w5-after`(번들 `08b5525cb2f52618` · 83 Route × 2테마 × **9뷰포트** = 1,494페이지 — 승격 다섯 중 넷이 fail 0, `control_baseline_mismatch` 는 **1,381 pass / 0 fail**) · `ev:qa_run:w5-filter-e2e`(면 계약 32/32 조합 + 키보드 사슬 PASS) · `ev:test:scripts/ui_qa/probe_selftest.py`(반례 19사례) · `ev:review:wf_e886b628-93e`(독립 3 렌즈 24건 → 적대적 검증 10건 → 확정 6 · 기각 4, 확정 전부 반영) · `ev:note:D-186` · commit `(2차 커밋에서 채운다)`
- **Findings**: (없음 — W5 조사 Finding 은 `ROUTE_COVERAGE.json` 의 `w5_discovery_findings` 에 164건 등록)
- **Depends on**: (없음)
...
- **Affected**: ALL
- **Implementation**: `frontend/src/app/navIcons.js`, `frontend/src/app/navConfig.js`, `frontend/src/app/AppShell.jsx`(사이드바부), `frontend/src/app/CommandPalette.jsx`, `frontend/src/ui/theme.js`(`ICON`·`remPx`), `scripts/check_icon_props.py`
- **Verification**: `frontend/src/app/nav-contract.test.jsx`, `python scripts/check_icon_props.py`, `bash scripts/static_checks.sh`, `python -m scripts.ui_qa.nav_e2e`
- **Status**: DONE
- **Evidence**: 계열은 이미 하나(`@mui/icons-material` `*Outlined`)였고 실제 결함은 **크기**였다 — `AppShell.jsx:437` 이 MUI `SvgIcon` 에 없는 `size={18} strokeWidth={1.8}` 를 넘겨 조용히 무시되고 기본 24px 로 그려져 **자식 아이콘이 부모 그룹(20px)보다 컸다**. 크기의 정본을 `ICON = { nav 20, inline 18, action 20, hero 24 }` + `remPx()` 로 만들고 전부 rem 으로 렌더한다(4K 레버를 함께 탄다). 실브라우저 실측 `dist/ui-qa/w3-nav-e2e/anatomy.json`: **16/16 조합**에서 그룹 글리프 20 · 펼침 화살표 18 · 자식 글리프 0개, 3840(루트 20px)에서도 정규화 후 같은 값 — F-W2R-02 의 "4K 에서 글리프가 30px 칸을 넘쳐 라벨과 맞붙는다"가 닫혔다. 채워진 실루엣 `AddRounded` 는 자식 아이콘 규칙 적용으로 소비처와 함께 사라졌다. PLAN 이 요구한 신규 static check ⓐ 를 `scripts/check_icon_props.py` 로 신설해 `static_checks.sh` 에 걸었다(사례 5개 자기 검사 · 변이 주입으로 검출 확인). 팔레트도 같은 언어를 쓴다 — 실브라우저 실측 6줄 전부 글리프 20px · 굵기 500(`palette-menu-light.png`), 최근 방문 4줄은 각자 자기 랜드마크 글리프라 4종류다(`palette-recent-light.png`). `ev:qa_run:w3-after`(번들 c03113861aaa5138) · `ev:qa_run:w3-nav-e2e`(기하 16조합 + 팔레트 2테마 = 18/18 PASS) · `ev:test:frontend/src/app/nav-contract.test.jsx` · `ev:review:wf_6fca7af0-570` + 재검증 `ev:review:wf_9c8f1032-de5` · `ev:note:D-184` · commit `ab2bc618`
- **Findings**: F-W1R-38 종결 (W3)
- **Depends on**: (없음)
...
- **Wave**: W5
- **Requirement**: Search, Filter, Category, Sort, View Control을 하나의 기능 영역이라는 이유만으로 항상 큰 Card, Panel 또는 흰색 Rectangle 안에 감싸지 않는다. 현재 자유게시판처럼 Control은 화면 좌측 일부만 사용하지만 Container는 Page 전체 폭을 차지하여 오른쪽 대부분이 비어 있는 구조를 사용하지 않는다. Filter Surface가 실제로 필요한지 먼저 판단한다. Search, Category, Status, Sort, View Toggle처럼 단순한 탐색 기능만 존재하는 경우에는 Page Toolbar 또는 Inline Control Group 형태를 우선 검토한다. 필터 수가 적다면 Page Content 위에 자연스럽게 배치하고 별도의 Elevated Surface를 만들지 않아도 된다. Search와 주요 Filter는 좌측 또는 Content Flow의 시작점에 배치한다. Sort, View Option처럼 결과 표현을 조정하는 Control은 필요하면 우측에 배치하여 Search/Filter와 역할을 구분한다. **W5 결과**: 필터를 기계적으로 큰 Box 에 넣지 않는다 — `c-toolbar-card` 를 **저장소에서 없앴다**(JSX 소비처 0, `FilterSurface` 를 쓰는 화면 여섯). 독립 검수가 잡은 것을 여기 적어 둔다: 이 문장은 처음에 「소비처 다섯 곳」이라고 적혀 있었는데 그때 `/projects` 는 그대로 판을 쓰고 있었고 실제 소비처는 넷이었다(실측 잉크 폭 비 1920 0.355 · 3840 0.234 — 이 항목이 「없앴다」고 자랑한 `/policies` 판 0.43 보다 나빴다). 검수 뒤 `/projects` 도 옮겼다. `/policies` 의 1610×108 판(잉크 폭 비 0.43)이 사라졌고, 유한집합 화면(정책·통합·기능 플래그·RBAC)은 `smallSet` 선언으로 저장된 뷰를 그리지 않는다. `oversized_empty_surface` 의 높이 문턱을 넓혀 «넓고 낮은 띠» 를 처음으로 볼 수 있게 했다.
- **Affected**: ALL
- **Implementation**: `frontend/src/ui/FilterBar.jsx`, `frontend/src/ui/filters.jsx`, `frontend/src/ui/kit.jsx` (`FormField`·`DataTable` 카드 뷰·`cardFieldLabel`/`cardHeaderControls`), `frontend/src/ui/bulkSelect.jsx`, `frontend/src/ui/theme.js` (`CONTROL`·`MuiTabs`·`MuiIconButton`), `frontend/src/screens/TicketFilterBar.jsx`, `frontend/src/screens/DataScreen.jsx`, `frontend/src/screens/TeamDocs.jsx`, `frontend/src/screens/Board.jsx`, `frontend/src/screens/Users.jsx`, `frontend/src/screens/MyTickets.jsx`, `frontend/src/screens/Project.jsx`, `frontend/src/screens/SchedulerCalendar.jsx`, `frontend/src/screens/registry/governance.js`, `frontend/src/screens/registry/automation.js`, `scripts/ui_qa/assertions.py`, `scripts/ui_qa/probe_selftest.py`, `scripts/ui_qa/filter_e2e.py`
- **Verification**: `python -m scripts.ui_qa.run --fail-on isolated_control_row control_baseline_mismatch plain_dropdown_for_entity header_cell_alignment_mismatch numeric_alignment`, `python -m scripts.ui_qa.probe_selftest`, `python -m scripts.ui_qa.filter_e2e --base-url https://clovirassist.gooddi.lab`, `npx vitest run src/ui/filter-row-width.test.jsx src/ui/entity-combobox.test.jsx src/ui/department-filter.test.jsx src/ui/table-card-select.test.jsx src/ui/theme-contract.test.js`, `python scripts/check_ui_renewal_coverage.py --stage wave`
- **Status**: DONE
- **Evidence**: 필터를 기계적으로 큰 Box 에 넣지 않는다 — `c-toolbar-card` 를 **저장소에서 없앴다**(JSX 소비처 0, `FilterSurface` 를 쓰는 화면 여섯). 독립 검수가 잡은 것을 여기 적어 둔다: 이 문장은 처음에 「소비처 다섯 곳」이라고 적혀 있었는데 그때 `/projects` 는 그대로 판을 쓰고 있었고 실제 소비처는 넷이었다(실측 잉크 폭 비 1920 0.355 · 3840 0.234 — 이 항목이 「없앴다」고 자랑한 `/policies` 판 0.43 보다 나빴다). 검수 뒤 `/projects` 도 옮겼다. `/policies` 의 1610×108 판(잉크 폭 비 0.43)이 사라졌고, 유한집합 화면(정책·통합·기능 플래그·RBAC)은 `smallSet` 선언으로 저장된 뷰를 그리지 않는다. `oversized_empty_surface` 의 높이 문턱을 넓혀 «넓고 낮은 띠» 를 처음으로 볼 수 있게 했다. 같은 규칙을 폼에도 적용했다 — `/settings?tab=ai` 의 균등 트랙(`repeat(2, minmax(0,1fr))`)이 값이 «1» 한 글자인 칸에 775px 를 주고 오른쪽 770×95px 를 백지로 남기고 있었다. 트랙을 내용이 요구하는 폭(`auto-fill · 17~26rem`)으로 바꾸고 컨트롤 폭은 종류가 정하게 하자 `equal_column_split` 이 1920·3840 양쪽에서 **fail→pass** 했다. 넓은 화면은 **열을 늘려서** 쓰고 컨트롤을 늘려서 쓰지 않는다. `ev:qa_run:w5-after`(번들 `08b5525cb2f52618` · 83 Route × 2테마 × **9뷰포트** = 1,494페이지 — 승격 다섯 중 넷이 fail 0, `control_baseline_mismatch` 는 **1,381 pass / 0 fail**) · `ev:qa_run:w5-filter-e2e`(면 계약 32/32 조합 + 키보드 사슬 PASS) · `ev:test:scripts/ui_qa/probe_selftest.py`(반례 19사례) · `ev:review:wf_e886b628-93e`(독립 3 렌즈 24건 → 적대적 검증 10건 → 확정 6 · 기각 4, 확정 전부 반영) · `ev:note:D-186` · commit `(2차 커밋에서 채운다)`
- **Findings**: (없음 — W5 조사 Finding 은 `ROUTE_COVERAGE.json` 의 `w5_discovery_findings` 에 164건 등록)
- **Depends on**: (없음)
