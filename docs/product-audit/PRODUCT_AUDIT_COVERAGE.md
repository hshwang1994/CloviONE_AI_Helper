...
**대비 측정 3차까지 갔고, 방법은 섰지만 결과는 아직 못 쓴다**(`PA-F-034`·`PA-F-035`).

| 시도 | 방법 | 결과 |
|---|---|---|
| v1 | 계산 스타일로 조상의 불투명 배경 찾기 | 실패 — 그라데이션(`background-image`)을 통과, 화면당 8건 오탐 |
| v2 | 요소 스크린샷의 최빈 색 2개를 fg/bg | 실패 — 그라데이션 위에선 최빈 2색이 **배경의 인접 두 명암**(한 채널 1 차이) |
| v3 | fg는 CSS `color`, bg만 픽셀에서(fg와 거리 60 초과 최빈) | **방법 성립** — 후보 91 중 85 판정 가능 |

v3 결과도 아직 Finding 이 아니다: ⓐ `/me` 20건은 **열린 모달 2개의 50% scrim** 아래에서 잰 오염 ⓑ `/dashboard`·`/users` 3건은 그라데이션에서 **글자 자리와 배경 픽셀의 공간 짝짓기**가 검증 안 됨.

→ `CTR` 축은 **여전히 미실측**. 다음 회차: 오버레이 닫고 + 글자 바로 아래 픽셀로 재기.
