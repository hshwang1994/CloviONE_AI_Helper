...
**계산 스타일 기반 WCAG 대비 측정은 이 제품에서 구조적으로 틀린다.** 상단바가 `background-image` 그라데이션이라 `backgroundColor`가 투명하고, 배경 탐색이 그것을 통과해 페이지 배경(`rgb(245,247,252)`)을 집는다 → 흰 글자가 '흰 배경 위'로 계산돼 화면당 8건이 오탐으로 나왔다. **결과 폐기**(`PA-F-034`).

그래서 `CTR` 축(대비)은 **아직 실측하지 못했다.** 제대로 하려면 계산 스타일이 아니라 **실제 렌더 픽셀 표본**이 필요하다. 저장소에는 토큰 값 기준 대비 검사(`tests/regression/test_css_says_what_it_does.py`)가 이미 있다 — 렌더 기준은 아니다.
