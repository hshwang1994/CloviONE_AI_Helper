...
**백엔드 2,903건 중 실행 완료가 확인된 것은 829건(29%)뿐이다.**

| 디렉터리 | 테스트 | 이번 Cycle |
|---|---:|---|
| `tests/integration` | 1,313 | 미완주(27%·49%에서 중단), **실패 1건 관측** |
| `tests/unit` | 761 | 미실행 |
| `tests/security` | 498 | **완주·전부 통과** |
| `tests/regression` | 331 | **완주·전부 통과** |

> Y축 칸을 EXECUTED로 적는 것은 '전부 통과했다'는 뜻이 아니라 **'실행으로 확인했다'**는 뜻이다. 실제 결과는 `PA-RC-0009`(전체 회귀가 실증된 적 없음)와 `PA-RC-0008`(race 약 40% 실패)이다.
>
> 프런트는 예외다 — `npm test`가 111초에 253파일/1,718건 완주·전부 통과로 **실증됐다.**
