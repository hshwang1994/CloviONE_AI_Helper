...
skills_applied: ui-ux-pro-max, redesign-existing-projects, impeccable
verdict: REFINE
current_state: /chat은 3분할이다 — 셸 레일 264px + 대화 목록 288px + 대화 영역. 빈 상태에 일러스트·「무엇을 도와드릴까요?」·시작 프롬프트 7개가 있고, 입력창 위에 「오늘 AI 사용량 0/200」 쿼터가 표시된다. 이 화면에서는 헤더 어시스턴트 칩과 우하단 FAB이 사라진다(맥락 인지 동작). 문제는 명명이다 — 같은 화면에서 breadcrumb 「도우미 › AI 도우미」, 사이드바 「AI 도우미」, h1 「채팅」, 패널 제목 「채팅」이 동시에 쓰인다. 제품 전체로는 「클로비」(헤더 칩·온보딩·사이드바 카드)가 더해져 **3종**이다. 관리자 서비스 상태의 「업무 도우미」도 처음에는 4번째로 셌으나, 추적해 보니 표시명이 아니라 `CHAT_WORKFLOW_NAME` **DB 조회 키**여서 제외했다(`PA-F-062`).
