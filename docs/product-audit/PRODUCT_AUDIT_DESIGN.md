...
verdict: REDESIGN
current_state: 헤더 64px + 좌측 고정 레일 264px + 본문 1656px(86.3%). 그 아래 전역 배너가 관리자 5장·331.5px, 사용자 4장·216px 쌓이고 26개 라우트 전부에서 동일하다. 페이지 h1은 관리자 y=447에 온다. 배너 중 닫을 수 있는 것은 관리자 2/5, 사용자 1/4. 첫 두 장은 같은 동기화 정지를 티켓/문서로 나눠 두 번 말한다.
...
두 가지가 나온다. 하나는 **175% 배율에서만 가로 스크롤이 생긴다**는 것 — CLAUDE.md가 지원 범위로
적은 배율 구간 안이라 결함이다. 다른 하나는 **폭이 커질수록 활용률이 올라간다**는 것(86.3 → 88.3
→ 91.1%)으로, 이는 상한(max-width)이 없어 4K에서 본문이 3500px까지 늘어난다는 뜻이다. 표에는
받아들일 만하지만 폼·본문 텍스트에는 한 줄이 지나치게 길어진다. 두 항목 모두 셸 레이아웃에서
나오므로 `PA-RC-0016`에 함께 넣는다.
