...
## A-3. 지금까지 확정한 Root Cause — 3건 (전부 Confirmed)

| RC | Sev | 요지 |
|---|---|---|
| `PA-RC-0027` | **High** | `/me`·`/my-stats`의 개수 타일이 *"모른다"* 를 `0`으로 그린다. 같은 응답의 비율·진척은 `null`로 옳게 답하고 형제 화면(`work.py`)은 가드가 있다. 문서(`DASHBOARD_METRICS.md` §3)와 **두 파일의 코드 주석**이 그 불변식을 명시하는데 호출부 2곳에만 가드가 없다. 프런트는 이미 옳다. TEST SERVER 23명 중 **10명**이 이 경로 |
