...
| 항목 | 값 |
|---|---|
| 신규 Finding | `PA-F-042` ~ `PA-F-054` |
| 신규 Root Cause | **`PA-RC-0012`**(Med) 제목 계층이 시각 API에 종속 · **`PA-RC-0013`**(Med) `/users`만 목록 상태를 URL에 안 싣는다 · **`PA-RC-0014`**(Med) 스키마 422가 영문으로 나오고 필드에 연결되지 않는다 |
| HANDOFF 블록 | **3건** (전부 27필드 자기검사 PASS, 미루는 표현 0건). `deferred_for_human_approval=0` |
| Coverage | 2340칸 · EXECUTED **183**(158→) · OBSERVED **218**(62→) · STATIC_ONLY **609**(784→) · UNSEEN **1330**(전부 사유 있음) |
...
3. **H축 잔여 — 입력 경계.** 없는 ID(`PA-F-052`)와 **과길이 제출**(`PA-F-054`)은 닫았다.
   **아직 안 한 것: 중복 값·특수문자/XSS 페이로드·빈 필수값의 실제 제출.**
4. **P/Q/R축 잔여.** `ux-writing`은 이번에 적용해 `PA-RC-0014`를 냈다. **아직 안 한 것:
   Q축(같은 개념이 화면마다 다른 용어인가)과 R축(`humanize-korean`). 그리고 P축의 핵심 질문 —
   존재하는 문구가 아니라 **없어서 문제인 문구**(프롬프트가 특별히 지목).**
5. **S축 — `PA-F-052`가 남긴 관측.** `/board/:id`·`/team-docs/:id`의 not-found 판정이 5~10초.
   Notion 동기화 정지라는 환경 요인과 코드 요인을 **분리하지 못했다.** 분리해서 재측정.
6. **서버 렌더 4화면**(`/login`·`/forgot-password`·`/reset-password`·인계 화면)의 heading/구조 —
   이번 M축 측정은 SPA 전용이었다. `PA-RC-0012`의 범위에 포함할지 판단 필요.
7. **`/users` 정렬 부재**(`PA-F-051`) — 의도 근거를 못 찾아 등급 보류. 요구사항·계약을 더 찾거나
   사용자 수가 늘었을 때 재평가.
8. **Blind Re-Audit 2회** — **아직 0/2.** 위 1~4가 어느 정도 닫힌 뒤에 할 것(지금 하면
