...
| Coverage | 2340칸 · EXECUTED **181**(158→) · OBSERVED **218**(62→) · STATIC_ONLY **611**(784→) · UNSEEN **1330**(전부 사유 있음) |
...
3. **H축 잔여 — 입력 경계.** 없는 ID는 닫았다(`PA-F-052`). **아직 안 한 것: 과길이·중복·
   특수문자·빈 필수값의 실제 제출과 그 오류 문구.**
4. **P/Q/R축 재측정.** `PA-RC-0002`가 100%로 닫혔으므로 **이번엔 다른 각도**로 — 존재하는
   문구가 아니라 **없어서 문제인 문구**(프롬프트 P축이 특별히 지목). `ux-writing`·
   `humanize-korean` Skill을 이 축에서 실제로 적용할 것(Gate E는 아직 `ui-ux-pro-max` 1건뿐).
