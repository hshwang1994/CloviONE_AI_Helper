...
## A-3. 지금까지 확정한 Root Cause 후보

- **`PA-RC-0027`(High)** — `/me`의 티켓 타일 4종이 *"모른다"* 를 **`0`으로 그린다.**
  같은 응답의 `sprint`는 `null`로 옳게 답하고, 형제 화면(`work.py`)은 가드를 제대로 걸어
  *"셀 수 없습니다"* 라고 말한다. 코드 주석이 **두 파일에서** 그 불변식을 명시하는데
  (`0건과 '모른다'는 다른 말이다`, `두 화면이 같은 상황에서 다른 말을 하면 안 된다`)
  정작 `service.py`의 버킷 5종에만 가드가 없다. 프런트는 이미 옳다(`null`→`-`).
  TEST SERVER 기준 **23명 중 10명**이 이 경로에 있다.
- **`PA-RC-0028`(Medium)** — `/dashboard`와 `/diagnostics`가 같은 것을 두 번 그린다.
  `build_diagnostic_bundle`이 **`build_dashboard()`를 통째로 품는다**(코드 주석이 명시).
  본문 줄 기준 **68%** 중복, `최근 주요 변경`은 16/16 문자열 동일, `백업`은 CTA까지 동일.

## A-4. 다음 조사 후보

1. L축 나머지 필수 표면 판정(18종 중 현재까지 깊게 본 것은 dashboard·home·admin-console·table-screens)
2. 사용자 콘솔의 빈 화면들 — `/my-tickets`(카드 0·버튼 1)·`/my-stats`(카드 1·버튼 1)
3. F축 RBAC — 역할 5종이 실제로 있으므로 화면/직접 URL/직접 API 3계층 대조
4. P·R축 — 문구와 한국어(이전 Cycle 이후 문구가 많이 바뀌었다)
5. N·O축 — 배율/뷰포트/다크
6. Blind Re-Audit 2회(진입점은 이전 Cycle이 쓴 「퇴사 처리」·「장애 대응」을 재사용하지 않는다)
