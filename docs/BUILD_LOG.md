...
## 2026-08-10 (Sonnet 구현 사이클 4, 배치 6) — Notion 다운 502 캐스케이드 + M4 두 건 더 + 휴지통 필터 불일치

**무엇을 했나**: UA-05(weekly-digest가 Notion 장애 시 502로 문서·게시판 집계까지 함께
죽던 것), UA-07·UA-08(스프린트/월간리포트 기본값이 UTC로 계산돼 KST 경계 9시간 동안
틀리던 M4 재발 두 건), UA-09(휴지통 문서가 주간 다이제스트 "변경됨" 집계에 남아 404
링크를 만들던 것) 총 4건 수정. UA-04/UA-06/UA-10은 프런트(React) 변경이 필요해 이번
배치(백엔드 전용) 범위 밖으로 명시적으로 보류(BACKLOG.md에 사유 기록, 방치 아님).
커밋 `40073c1` → 배포 `UPGRADE_OK` → 세 엔드포인트 실호출로 정상 경로 무회귀 확인
(버그 자체는 특정 시각/상태에서만 재현되는 종류라 지금 시점엔 실서버 재현 불가 —
상세는 WORK_STATE.md 배치 6 항목).
...
임퍼소네이션 쓰기 차단과 `request.state.actor` 배선을 빠뜨렸던 것을 고침(CORE-08,
지금은 이 의존성을 쓰는 라우트가 전부 GET이라 무해하지만 테스트 자체가 0건이었다).
기능 플래그가 JSON 값의 타입을 안 가려 문자열 `"false"`가 파이썬에서 참으로 읽혀
파일과 반대로 동작하던 것을 고침(CORE-10, 진짜 boolean만 인정). `SettingsCache.
current()`가 내부 dict를 참조로 돌려줘 부르는 쪽이 실수로 고치면 캐시 전체가
오염되던 것과, 기동 시 설정 캐시 로드 실패를 `except: pass`가 원인 구별 없이 삼켜
DB 잠금·손상 같은 진짜 장애도 로그 한 줄 없이 넘어가던 것을 고침(CORE-12 중 2건,
나머지 4건은 다음 배치로 미룸).

**검증 방법론**: 4건 전부 회귀 테스트를 새로 추가했고, 고치기 전 코드로 일부러 되돌려
전부 실패하는 것을 직접 확인한 뒤 복원했다. static_checks가 처음 쓴 CORE-12 경고
로그 문구의 금지 glyph(em dash)를 실제로 잡아내 배포 전에 고쳤다.

**검증 상태**: 백엔드 pytest 전체 green, `STATIC_CHECKS_OK`. 커밋 `bc650d7` 배포 →
`UPGRADE_OK`, 서비스 3종 active, healthz/readyz 정상. 이 배치의 네 항목은 전부 실서버
안전 재현이 불가능한 종류(잠복 코드 경로, 운영 설정 파일 손상, 파이썬 객체 참조,
DB 장애 주입)라 배포 후 관련 화면(`/api/admin/feature-flags`, `/admin`) 정상 로드만
확인 — 로컬 테스트로만 검증됨.
...
(529항목) 자체를 다시 훑어 다음 배치를 골랐다. 남은 Critical 2건(`AI-30` 러너 컨텍스트
누수, `FAIL-01` 빈 상태/오류 상태 혼용)은 각각 전용 사이클감이라 미루고, "이미 있는 가드를
새 자리에 안 걸었다" 류의 High 8건 + 인프라 2건을 골랐다.

**무엇을 했나**: 주간 다이제스트·스프린트/티켓 집계가 `visible_user_ids`를 안 넘기거나
`scope.is_dept`일 때만 걸러 org 범위 뷰어에게 전사 데이터가 새던 것(UA-01/UA-02, 이 서버
실측으로 이미 확인됐던 유출)을 고침. `notion_mapping` 쓰기 4종에 권한 상승 방지 검사
(`ensure_can_manage_target`) 추가(SEC-01). 공지에 스코프 가드가 전혀 없던 것을 고침(UB-01).
AI 쿼터 목록이 전역 상한의 "현재 사용"에 전 사용자 합계를 보여 실제 판정(사용자별)과
안 맞던 것을 최다 사용자 값으로 교체(UB-02). 로그아웃이 임퍼소네이션 세션을 안 끝내던 것을
고침(UB-03). 워커 락의 신규 생성 경로가 소유권 확인 없이 성공을 반환하던 경쟁 조건과, 락
파일 쓰기가 비원자적이라 갱신 중 다른 프로세스가 빈 파일을 볼 수 있던 것을 고침
(CORE-01/CORE-03, `secret_refs.write()`의 `mkstemp`+`os.replace` 패턴 재사용). 백업이 전체
DB 복사+검증 내내 SQLite 쓰기 락을 쥐고 있어 그동안 앱의 다른 모든 쓰기가 막히던 것을 고침
(UA-03, `trash/service.py`의 S7과 같은 구조로).

**검증 방법론**: 9건 전부 회귀 테스트를 새로 추가했고, 그 테스트가 실제로 버그를 잡는지
고치기 전 코드로 일부러 되돌려 실패를 직접 확인한 뒤 복원했다. 배포 후 실서버 검증 중
두 항목(UA-01/UA-02, SEC-01)에서 **자기 자신의 검증 시도가 애초에 그 항목을 검증하지
못했다는 것**을 발견했다 — 200/404가 나왔다고 검증 성공으로 착각하지 않고 정직하게
"미검증"으로 남겼다. 상세는 [WORK_STATE.md](WORK_STATE.md), 항목별 근거는
[BACKLOG.md](BACKLOG.md).

**검증 상태**: 백엔드 pytest 전체 green, 프런트 vitest 189파일/1272건 green,
`STATIC_CHECKS_OK`. 커밋 `aa5e346` 배포 → `UPGRADE_OK`, 서비스 3종 active, healthz/readyz
정상. 실서버 curl 검증: UB-01·UB-03 확실히 확인(403 응답 실측, 임퍼소네이션 세션 종료
실측), UA-03은 기능 자체만 확인(락 미보유는 로컬 테스트로만), 나머지 4건은 로컬 테스트로만
검증(이유는 WORK_STATE.md 참조).

---

## 2026-08-10 (Sonnet 구현 사이클 3, 9·10단계) — 게시판/채팅 규약 통일 + 채팅 파서 공용화 (`SONNET_HANDOFF.md §3` 9~10단계)

**무엇을 했나**: 게시판(`app/board/*`)을 티켓·문서 댓글과 같은 계약으로 맞췄다 —
수정은 작성자 본인만(`ensure_can_edit`), 삭제는 작성자·운영자군(`ensure_can_delete`,
새로 분리), 삭제된 댓글은 목록에서 사라지지 않고 본문 없는 툼스톤으로 남는다(답글이
고아가 되는 것 방지). 방장 오프보딩 인계 로직을 오프보딩 마법사 전용에서
`app/team_chat/service.py::transfer_owned_rooms`로 옮겨 `/users`의 직접
비활성화·보관 경로도 같은 인계를 타게 했다. 전체 채팅의 `you.display_name` 배선으로
자기 자신 @멘션이 강조되게 했다. 방장 나가기 확인 문구를 결과(방 소멸 vs 인계)별로
분기했다. `ManageRoomModal` 초기화 effect의 의존성 버그(이름 저장 시 초대 선택이
날아감, `[open, title]` → `[open]`)를 고쳤다 — 일부러 되돌려 새 회귀 테스트가 실제로
잡는 것까지 확인 후 재적용. 게임 명단에 "자리 비움" 배지(`is_present()` 신설)를
추가해 명단(active)과 추첨 풀(present) 어긋남을 화면에서 보이게 했다. AI 답변
링크화기가 팀 채팅과 같은 `trimUrlTail`을 쓰게 통일하고(검증 중 공백 없는 한글 조사
글자 붙은 URL은 두 링크화기 모두 여전히 못 뗀다는 기존 한계를 발견 —
`KNOWN_LIMITATIONS.md`에 기록), `RichText`의 머리글 블록에도 링크화 적용, "오전
9:30" 같은 접두 붙은 시각이 정의목록으로 오탐되던 것을 정정.

**검증 상태**: 백엔드 pytest 전체 green(기존 테스트 1건이 지금은 의도적으로 막힌 옛
동작을 정답으로 못박아 둔 것을 발견·수정), 프런트 vitest 189파일/1272건 green,
`STATIC_CHECKS_OK`. 커밋 `a756f41` 배포 → `UPGRADE_OK`, 서비스 3종 active,
`/healthz`·`/readyz` 정상. **Chrome으로 실서버를 직접 열어**: 게시판 댓글
수정→"(수정됨)" 표시, 삭제→툼스톤 렌더, 전체 채팅 자기멘션이 멘션 칩(`MuiBox-root`)
컴포넌트로 렌더되는 것(DOM 직접 조회로 확인), 방 이름 저장 후 초대 체크박스 선택
유지(9-6의 핵심 회귀 재현 시나리오)를 라이브로 확인. 게임/채팅방 화면 모두 콘솔 오류
0건. 생성한 테스트 게시글·댓글·채팅방·메시지는 전부 정리됨(잔여 데이터 없음).
방장 인계(9-2, 두 번째 계정 필요)·자리 비움 배지 실발현(9-7, 90초 이상 미폴링
필요)·나가기 네이티브 confirm 문구(9-5, `window.confirm()` 자동화 시 탭 블로킹
위험으로 의도적으로 미시도)·채팅 파서 3건(10-1~10-3, 콘솔 오류는 0건 확인했으나
트리거 문장을 직접 입력해 렌더 결과까지 보진 않음)은 로컬 테스트로만 검증됨 —
상세는 [WORK_STATE.md](WORK_STATE.md).

---

## 2026-08-10 (Sonnet 구현 사이클 3, 로컬 완료분) — 설정/Notion 쓰기/셸 스코프/위생 묶음 (`SONNET_HANDOFF.md §3` 5~8·11단계)

**무엇을 했나**: `FN-50`(H-1, Notion 본문 저장이 자식 있는 블록을 통째로 지움 — `has_children`
추적으로 제외), 게시판→티켓 전환 영구 실패(긴 한 줄 문단이 `TicketCreate` 줄 길이 제한에
결정론적으로 걸림 — `_fit_for_ticket_description` 신설), `WORKER-01`(워커가 최대 600초 낡은
설정 캐시로 돎 — 60초 재적재 틱 신설), `SET-11`(설정 문자열 정규화 없음 — 저장 전 strip),
셸 스코프 배선(`ScopeBar` 라우트 인지, 부서 관리자 표시 대상 정정, 세션 만료 감지 배선,
로그아웃 실패 처리), RBAC 매트릭스 누락 행 2개, `ConfigVersion` ORM 유니크 제약, `/healthz`
ticket_source 노출, team-chat ETag, Donut/BrandLogo/LineSeries/강조색/reduced-motion 프런트
버그 다수. 상세 근거·파일 목록은 [WORK_STATE.md](WORK_STATE.md)의 사이클 3 블록.

**위생 묶음(11단계) 10개 항목은 Workflow 도구로 병렬 서브에이전트 10개에 위임했다** —
파일이 서로 겹치지 않는 독립 항목이라 격리 없이 병렬 실행. **서브에이전트 결과를 그대로
믿지 않고 10개 diff를 전부 직접 리뷰했다** — 이 저장소의 검증 원칙(자작 프로브·서브에이전트
결과는 표본 확인 전엔 안 믿는다)을 서브에이전트 산출물에도 그대로 적용한 것. 그 리뷰에서
실제 결함 1건을 잡았다: 강조색 계정별 저장 구현이 `ThemeModeProvider`에 `userId` prop을
기대하도록 짜였는데, 그 컴포넌트는 실제 앱 트리에서 `AuthProvider` **바깥**에 마운트돼
있어 그 prop이 **영원히 채워질 수 없는 구조**였다 — 서브에이전트의 단위 테스트는 prop을
직접 주입해 통과했을 뿐, 실제 화면에서는 죽은 코드였다(이 프로젝트가 그동안 반복해서 찾아낸
"헬퍼는 있는데 부르는 쪽이 안 부른다" 결함 패턴을 새 코드가 스스로 재현할 뻔한 사례).
`identifyAccentUser()`를 컨텍스트로 노출해 계정을 아는 첫 지점(`UserMenu.jsx`)이 부르는
구조로 다시 짜고, 실제 트리 배선을 증명하는 테스트를 새로 추가했다.

**직접 재현·수정한 또 다른 버그**: 세션 만료 배선(다른 API의 401이 `["me"]` 쿼리를
무효화)을 처음 짰을 때 `/api/me` 자신의 401도 무효화를 트리거해 무효화→재요청→401→무효화…
무한 루프가 됐다. 전체 vitest 스위트 실행 중 이 테스트 파일에서 워커 프로세스가 힙 고갈로
죽는 것으로 실측했다 — 재현 후 `path !== "/api/me"` 가드로 고치고, 그 루프 자체를 고정하는
회귀 테스트를 추가했다.

**검증 상태**: 로컬 게이트 전부 green(백엔드 pytest 전체, 프런트 vitest 전체 184파일/1248건,
`STATIC_CHECKS_OK`, 번들 재빌드). **아직 배포·실환경검증 전** — 다음 세션/턴이 이어서 한다.
9·10단계(게시판·채팅 규약 통일 나머지, 채팅 파서 공용화)는 착수 전.
