...
## 2026-08-08 (사이클 0) — 지속 작업 기억 구축 + Opus 전수조사 착수

**무엇을 했나**: 대화 Context에 의존하던 작업 상태를 프로젝트 파일로 옮겼다. `docs/`에 이미 34개
문서가 있어 **역할이 겹치는 것을 새로 만들지 않고 배정**했다 — `WORK_PLAN_INDEX.md`(MASTER PLAN,
전면 개정)와 이 파일(HISTORY)은 재사용, `WORK_STATE.md`·`BACKLOG.md`·`QA_COVERAGE.md`·
`DECISIONS.md`는 신규. `CLAUDE.md`에 §0(진입점 안내)과 §9 색인 항목만 추가했다(덮어쓰지 않음).

**무엇을 새로 발견했나** — 4갈래 전수조사 결과를 `BACKLOG.md`에 **96항목**으로 등록:
- **디자인 시스템 23건**: 기본 버튼이 `outlined`라 평범한 버튼까지 테두리 상자 · `primary`와
  `danger`가 둘 다 `contained`(위험이 주요와 같은 무게) · Button에 `loading` 상태 없음 ·
  `fontWeight`가 65파일에 8종 값(≥700이 148회) · **registry 표 28개에 열 폭 지정 0건** ·
  토큰 4벌 · `screens.css` 240클래스 중 138(58%) 죽음 · `Drawer = Modal` 별칭 · 다크 대비 2.4:1
- **AI 도우미 50건**: LLM 챗이 아니라 키워드 규칙엔진 + 원샷 `claude -p --max-turns 1 --tools ""` ·
  **`/v1/assistant/summarize`가 러너에 없어 `assistant_narrative_enabled`를 켜도 문장이 0개** ·
  **타임아웃 역전**(플랫폼 180s < n8n 240s → 중복 쓰기 위험) · 스트리밍·중단버튼 없음 ·
  기억 8턴×1000자인데 규칙엔진 턴이 이력에서 빠져 **대화에 구멍** · 대화 삭제가 러너 사본에 전파
  안 됨(`clear_persisted_context` 호출 0회) · 모델이 보는 데이터가 Notion 티켓뿐 ·
  **프롬프트 콘솔이 채팅에 무영향**(죽은 조작판) · 마크다운이 아니라 정규식 5개짜리 줄 분류기라
  **코드블록이 망가짐** · 드로어가 스텁(단일행·IME 무방비·429에 영구 잠김·"현재 문맥"은 거짓말)
- **기능/권한 23건**: `notion_mapping` 쓰기 4종에 **`ensure_can_manage_target` 누락**(권한 경계) ·
  `allowed-services.json`에 **`api.anthropic.com:443` 없음**(LLM API 백엔드 영구 실패) ·
  **메일 UI 0개**(SMTP 오류 시 비밀번호 재설정이 조용히 실패) · 고아 엔드포인트 9종 ·
  러너 레지스트리가 죽은 조작판 · 죽은 스키마 4종
- **검증 인프라 6건**: **서버가 HEAD가 아니다**(라운드 9~14 미배포 → 조사 신뢰도 0) ·
  실브라우저 E2E 수행된 적 없음(`tests/smoke/`는 빈 디렉터리) · QA 하네스가 자체서명 HTTPS를
  못 타고 · 라우트 7개 누락(**화면 약 2,600줄이 시각 검사 밖**) · 역할 1종으로만 검사

**어떤 결정을 했나**: `DECISIONS.md` D-01~D-20. 핵심은 **공통화 ≠ 동일화**(같은 의미는 같은 표현,
다른 의미는 적절한 차이 — 양방향으로 합치고 분리한다) · 시각 위계를 제품 규칙으로 고정 ·
스트리밍은 sync 제너레이터 `StreamingResponse`로(불변규칙 #1 유지) · 대화 이력 소유권을 플랫폼으로 ·
**CSP는 되돌리지 않고 문서를 정정**(2026-08-04 사용자 지시로 의도적 완화) · 조사 전에 HEAD 배포.

**Coverage 변화**: `QA_COVERAGE.md` 신설. 73라우트 × 8검증축 전부 `-`(미검증)에서 출발.
이유를 명시했다 — 서버가 HEAD가 아니라 지금 검증하면 오염된다.
