...
| AI-15 | Med | 문맥이 250,000자를 넘으면 요약이 아니라 **`conversation_history`를 통째로 삭제** | `assistant.py:215-227` | ✅ **재확인 결과 서술이 낡음(2026-08-13) — 통째 삭제가 아니라 2단계 강등이다.** 현재 `persist_context_result()`(`assistant.py:210-238`)는 250,000자를 넘으면 먼저 **1단계**로 `conversation_history`를 최근 10턴(그 외 필드도 각자 최근 N개)만 남기는 완만한 절삭을 한다 — 이 시점에 대부분 다시 250,000자 밑으로 내려온다. 그래도 여전히 넘칠 때만 **2단계**로 `interaction_history`/`unhandled_messages`/`conversation_history`/`last_results`를 통째로 지운다(원 서술이 맞는 부분은 이 2단계뿐, 항상 일어나는 경로가 아니다). 이 1단계 절삭 자체엔 시험이 하나도 없어 실제로 도는지 코드만으로는 확정할 수 없었다 — `test_assistant.py::test_persist_context_truncates_history_instead_of_dropping_it_when_over_budget` 신규 추가(100턴·약 305,000자 컨텍스트를 저장해 최근 10턴만 남고 최신 턴이 보존되는 것을 직접 확인), green. 언제 이 완만한 절삭이 추가됐는지는(이 행 자체가 낡은 시점) 특정 커밋을 찾지 못함 — 현재 소스 기준으로만 정정 | 구현완료 |
