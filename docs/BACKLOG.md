...
**마지막 갱신**: 2026-08-08 (사이클 0) · **총 항목**: 331
...
| [RN 러너 `assistant.py`](#rn--러너-assistantpy-전수조사-사이클-0--재현으로-확인됨) | RN-01~20 (20) | **라우터를 실제 실행해 재현.** 질문·거절이 승인 없는 Notion 쓰기가 된다. High 3 |
| [VIS 실화면 판독](#vis--실화면-판독-사이클-0-서버-배포본) | VIS-01~66 (66) | **기계 검사 21종이 전부 통과한 화면**에서 눈으로 찾은 것. **11화면 판독** |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-53 | **High** | **화면 세 개를 겹쳐 보니 운영 사고가 드러난다**: 설정에서 `자동 백업 일정 = 꺼짐`, 대시보드에서 `마지막 백업 20일 전`(빨강), 그리고 `FN-09`대로 **백업 실패 알림이 없다**(예외를 삼킨다). 즉 **백업이 꺼진 채 20일이 지났는데 아무도 통보받지 않았고**, 각 화면은 자기 조각만 말해 아무도 전체 그림을 못 본다. 이것이 `IA-01`(운영 12메뉴 분산)이 실제로 만드는 피해다 |
