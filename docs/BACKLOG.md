...
| VIS-119 | Med | **KPI 6장이 4+2로 접혀 둘째 줄에 ~1,100px 빈 칸이 생긴다.** 카드 폭이 고정이라 6장이 4장 폭에 안 맞는다 ‖ 재확인만 하고 이번 배치에서는 손대지 않음 — 카드 그리드 열 수 조정은 `Home.jsx`의 "카드 수는 열 수의 약수여야 줄이 안 남는다"는 기존 규칙과 맞물려 있어(§46-53 주석), 이 화면 하나만 임의로 고치면 그 규칙과 다시 어긋날 수 있다. 다음 "구조 먼저" 스캔에서 KPI 그리드 클러스터로 함께 다룰 후보 ‖ ✅ **완결(2026-08-17, D-110)** — `DataScreen.jsx`의 요약 카드 그리드를 고정 `repeat(4,...)`에서 `repeat(auto-fit, minmax(14rem,18rem))`로 교체 — 카드 수와 무관하게 그 줄을 항상 채운다(빈 칸 자체가 안 생김). `Home.jsx`의 고정 그리드는 그 화면 자신의 "카드는 항상 6장" 계약이 있는 별개 경우라 안 건드림. 신규 시험 2건+revert-to-verify, 프런트 전체 회귀 green |
...
| VIS-151 | Med | **가장 중요한 구역이 가장 비어 있다.** 「확인이 필요한 항목」에 카드가 **딱 하나**(폭 250px)이고 그 줄의 나머지 **1,300px 가 빈다**. 아래 구역들은 4~5장씩 채워져 있어, 시선이 가장 먼저 닿는 자리가 가장 허전하다(`VIS-137` 카드 줄바꿈과 같은 뿌리) ‖ ✅ **완결(2026-08-17, D-110)** — `VIS-119`/`VIS-137`과 같은 수정(`DataScreen.jsx` 요약 카드 그리드 `auto-fit`)으로 닫힘. `unreadCountKey`(이 행이 말하는 "안 읽음" 단일 카드) 경로도 같은 상수를 쓰도록 통합해, 카드 1장이 최대 18rem까지만 커지고 나머지는 정상적인 여백으로 보인다(카드가 화면 전체로 늘어나 더 어색해 보이는 것도 방지) |
