...
**마지막 갱신**: 2026-08-08 (사이클 0) · **총 항목**: 309
...
| VIS-49 | Med | **FAB 겹침이 네 번째 화면에서 반복**(`/projects`·`/dashboard`·`/users`·`/team-docs`). 7화면 중 4화면이므로 **FAB 배치 규칙 자체를 고쳐야 한다** — 개별 화면 수정으로는 끝나지 않는다. `theme.js`가 `FAB_CLEARANCE`를 export해 두고 **소비자가 0**인 것(`DS-25`)이 정확히 이 문제를 위한 미사용 장치다 |
