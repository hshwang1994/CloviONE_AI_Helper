...
| PA-04 | Med | **입력 길이 정책이 서버에만 존재한다**(`PA-RC-0005`). 백엔드는 길이 제약을 452건 선언하는데 프런트 `maxLength`는 21건뿐이고 **전부 사용자 콘솔 수제 화면**이다. 관리자 화면 28개를 그리는 공용 폼 경로(`kit.jsx::FormField`·`DataScreen.jsx`·`registry/*.js`)에는 길이 개념이 **0건** — 관리자는 상한을 넘겨 입력하고 저장을 누른 뒤에야 거절당한다. `UX-40`(422 사유가 화면에 안 옴)과 겹치면 보이는 것은 영어 상수 `Invalid request data` 하나뿐이라 **입력을 잃는다** | `frontend/src/ui/kit.jsx:766,830`(`inputProps` 설정하면서 `maxLength`는 없음) · `var/product-audit/limits_scan.json` · 스캐너 `scan_limits.py` | **부분 구현완료** — 공용 메커니즘 완성 + 등록 화면 6/13(`create:` 있는 화면 기준) 배선. `app/core/field_limits.py`가 Pydantic `model_json_schema()`로 손으로 안 옮기고 직접 유도(`FORM_SCHEMAS`가 registry key → 스키마 매핑) → `scripts/generate_field_limits.py`가 `frontend/src/generated/fieldLimits.json`으로 빌드 시 내보냄(런타임 API 아님, `check_bundle_fresh.py`와 같은 관용) → `scripts/check_field_limits_fresh.py`가 `static_checks.sh`에서 드리프트를 잡음(revert-to-verify로 실제 확인) → `kit.jsx::FormField`가 `maxLength`를 `inputProps`로 렌더 + 긴 텍스트는 남은 글자 수 표시 + 붙여넣기 초과 시 토스트 경고(자르지만 조용히 안 넘어감, revert-to-verify 확인). 매핑 완료: `prompts`·`policies`·`templates`·`departments`·`job-titles`·`organizations`(전부 create/edit 필드명을 실제 스키마와 대조 확인함). **남은 화면**(`schedules`·`documents`·`approvals`·`integrations`·`runners`·`workflows`·`notifications`·`ai-quotas`·`feature-flags` 등)은 `app/core/field_limits.py::FORM_SCHEMAS`에 한 줄만 추가하면 된다(프런트는 이미 `config.key`로 자동 조회 — 화면별 코드 변경 불필요). `Users.jsx`는 이 공용 경로(`FormModal`/`FormField`)를 안 타는 손수 제작 화면이라 범위 밖(별도 처리 필요) |
