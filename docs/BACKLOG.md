...
| PA3-09 | Med | **기본 필터가 걸린 화면이 「없습니다」를 무조건형으로 말한다**(`PA-RC-0038`). `/approvals` 가 `?status=pending` 으로 열리는데 0건일 때 「승인 요청이 없습니다」 + **필터 지우기 버튼 없음**. 실제 상태는 「대기 중인 것이 없다」다. 같은 제품의 `/users` 는 「조건에 해당하는 사용자가 없습니다」 + 필터 지우기 2곳으로 정확히 구분한다. 판정 기준에 **「기본 필터도 활성 필터로 센다」** 를 명시하는 것이 핵심(`useQueryState` 가 기본값을 주소에 안 쓴다) | `PA-RC-0038` · `PA-F-097` · Blind pass 1 | ⬜ 미착수 |
