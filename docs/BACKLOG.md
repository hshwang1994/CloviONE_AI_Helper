...
| VIS-54 | Med | **설정 값 칸에 raw JSON이 그대로 잘려 나온다** — `smtp` 행이 `{"enabled":false,"host":"","port":587,"security":"starttls","from_addres…`. 읽을 수 없고 중간에서 잘린다. 동시에 그 안의 `enabled:false, host:""`가 **SMTP가 설정되지 않았다는 사실**인데(`FN-01`: 메일 UI가 0개라 비밀번호 재설정이 조용히 실패한다) 화면은 그것을 **JSON 파편으로만** 말한다 |
...
| SET-11 | Low | Notion DB id 설정에 **정규화가 없다** — 검증기는 `strip()` 해서 보고 저장은 원문. 읽는 쪽 6곳 중 **3곳만 strip** 해서, 공백이 낀 id 를 저장하면 **관리 화면과 연결 테스트는 통과하는데 실제 조회 URL 에만 `%20`** 이 붙는다(개행이면 `httpx.InvalidURL`). ‖ 검증자가 **High→Low** 로 내렸다 — 유일한 편집 화면(`NotionConsole.jsx:106`)이 이미 `.trim()` 해서 보내므로 **raw API PUT 으로만 도달**한다 | 발견 |
