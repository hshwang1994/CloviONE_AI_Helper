...
| SEM-02 | Low | **`/me` 의 제목 계층이 `h1 → h3` 로 건너뛴다.** `h1「오늘」` 다음에 `h3` 다섯 개(오늘 마감·AI 도우미·팀 채팅·이번 주 내 진척·최근 문서)이고 `h2` 가 없다. 일관되긴 하지만 스크린리더 목차에서 한 단계가 빈다 ‖ **범위 확장(Product Audit `PA-F-031`/`RD-4`, 2026-08-15)** — `/me` 한 화면의 문제가 아니다. 목록 화면 **11개 중 8개가 `h1` 하나뿐인데 버튼이 13~39개**다. 필터 영역·표·일괄 작업 영역이 시각적으로는 구분되는데 **문서 구조상으로는 한 덩어리**라, 스크린리더 사용자는 39개 버튼을 구획 없이 순회한다. 처방은 목록 화면에 섹션 제목(`h2`)을 주는 것이고 **시각 디자인은 그대로 둘 수 있다**(마크업 의미만 바꾼다). `SEM-03`과 같은 계열이므로 **함께** 처리할 것 | 발견 |
