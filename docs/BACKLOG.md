...
| PA3-12 | Low | **`/new-ticket` 담당자가 인원수만큼 길어지고 안내가 입력과 함께 사라진다**(`PA-RC-0035`). 체크박스 14개 평면 나열(검색·그룹 없음), `MuiFormHelperText` **0회** — 안내가 전부 placeholder 에 있어 입력 시작하면 사라진다. 정작 `제목` 필드는 placeholder 를 예시로만 쓰는 옳은 패턴을 이미 보여준다. (계측의 「무레이블 4건」은 MUI Select 숨은 input 으로 **결함 아님** 확인) | `PA-RC-0035` · `PA-F-093` · 입력 25개 전수 계측 | ⬜ 미착수 |
