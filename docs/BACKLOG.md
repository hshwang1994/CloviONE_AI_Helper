...
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| PA-01 | **High** | **글자 크기 스케일이 선언만 되고 소비되지 않는다**(`PA-RC-0001`). `tokens.css`가 6단계를 정의하는데 화면을 실제로 그리는 MUI `sx`/`styled` 층에서 이 토큰을 읽는 곳이 **0곳**이라, 비테스트 소스에 `fontSize` 리터럴이 **31종·278회** 쌓여 본문이 11~17px의 1px 연속체가 됐다. 같은 구조가 `RADIUS`(참조 2회 vs 표현 27종·108회)에서도 반복된다. **본문 크기 SSOT가 둘로 갈려 서로 다른 값을 주장한다** — `tokens.css:243` "body 15px" vs `theme.js:271` `body1: 14px`. 처방은 규율 요구가 아니라 **간격(`p: 2`)과 같은 수준의 일급 타이포 API를 주는 것**이다(PA-F-013 대조 실험: 같은 `sx` 층에서 간격 준수율 96% vs 글자 0%) | `frontend/src/styles/tokens.css:243-253` · `frontend/src/ui/theme.js:37,271` · `var/product-audit/design_scan.json` · 스캐너 `scan_design.py` | **부분 구현완료(재단장 기초)** — SSOT 충돌 해소: `tokens.css`의 `--font-size-base`(15px, 정본 어디에도 없던 값)를 지움, `--font-size-md`(14px)가 이미 정확한 본문 값이었다. `theme.js`에 RD-1~RD-3 확정값 그대로 `FONT_SIZE`(6단계)·`RADIUS.full`(4번째 의미 슬롯) 신설 + `sectionTitle`·`statValue` MUI typography variant 추가(`FONT_WEIGHT`는 DS-05가 이미 5단계로 신설해 둔 것을 재사용). `kit.jsx`(공용 키트, 23곳) 전체와 `SectionTitle` 컴포넌트를 이 토큰으로 마이그레이션(값 보존 확인, revert-to-verify로 단계 수 고정 검증 — `theme-baseline.test.js` 신규 3건). **DS-05와 같은 이유로 한꺼번에 안 옮긴다**: 새 토큰 자체는 완성됐지만 나머지 호출부(약 270여 곳)는 아직 리터럴이다 — `static_checks.sh`의 `fontSize:` 리터럴 금지 검사(acceptance_criteria 5)는 그 전면 마이그레이션이 끝난 뒤에나 추가할 수 있다(지금 걸면 270여 곳이 즉시 위반이 된다). 부수 발견: `QAH-07`(2026-08-15 이전 커밋)이 `ChatPane.jsx`에 JSX 주석 구문 오류를 남겨 전체 vitest 20개 파일이 조용히 깨져 있었다(개별 타겟 실행만으론 안 잡힘) — 이번에 전체 스위트를 처음 돌리며 발견해 정정, `jsx-comments.test.js`에 이 패턴(`(` 바로 뒤 `{/*`) 재발 방지 테스트 추가 |
