...
> 이것이 **왜 중요한가**: 이 저장소가 4K 지원을 위해 만든 유일한 장치가 "루트 폰트사이즈 하나로
> 글자·여백·간격이 같은 비율로 커진다"는 것이다(`styles/root.css` 주석). 절대 px이 하나라도 섞이면
> 그 요소만 상대적으로 작아지고, 커진 주변과 대비돼 **오히려 더 안 보인다**. 규칙을 세우는 것으로는
> 부족하고 **px 글자 크기를 금지하는 검사**가 있어야 다시 안 샌다.
