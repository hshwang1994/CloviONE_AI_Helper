...
| QAH-07 | Low | QAH-05 수정 중 발견 — `color: "X.main"`(X=primary/success/warning/error) 텍스트색 raw 사용이 game-room·Board 밖에서도 저장소 전체 최소 15개 파일에 더 있다: `AppShell.jsx`·`UserMenu.jsx`·`ui/kit.jsx`(**필수 입력 표시 `*` — 전역 공유, 폼마다 나온다**)·`ui/Mascot.jsx`·`DevReport.jsx`·`Games.jsx`·`Profile.jsx`·`SetupWizard.jsx`·`TeamChatWidget.jsx`·`TeamDocs.jsx`·`chat/MessageThread.jsx`·`chat/WelcomeStatus.jsx`·`ChatPane.jsx`·`ChatRooms.jsx`·`settings/AccentPicker.jsx`. **미착수, 미확정** — 일부는 `bgcolor:` 오탐(정규식이 `bgcolor:`도 `color:`로 잘못 잡음, 예: `AppShell.jsx:169,514`·`Mascot.jsx:230`은 배경색이라 대비 문제 아님), 나머지도 대부분 이 공유 컴포넌트들은 QAH-03의 519표본 하네스(68라우트 전수)에 이미 포함됐을 고빈도 자리라 실제로 미달인지 불확실하다(반면 game-room은 하네스 라우트 목록 자체에 없어 확실히 빠졌었다). QAH-05와 같은 방법(정확한 렌더 배경 확인 + 4프리셋×2모드 WCAG 계산)으로 파일별로 다시 확인해야 한다 — 특히 `kit.jsx`의 필수 표시 asterisk는 폭이 가장 넓어(모든 폼) 최우선으로 확인할 값어치가 있다 | 발견 |
