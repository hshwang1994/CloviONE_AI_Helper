...
| VIS-49 | Med | **마스코트 FAB이 카드를 덮는 것이 네 번째 화면에서 반복**(`/projects`·`/dashboard`·`/users`·`/team-docs`). `VIS-42`와 합쳐 **FAB 배치 규칙을 고쳐야 한다**는 결론이 확정된다 — 개별 화면 수정으로는 끝나지 않는다 |
...
| VIS-42 | Med | **마스코트 FAB이 "상세" 버튼을 덮는 것이 세 번째 화면에서 반복**된다(`/projects`·`/dashboard`·`/users`). 개별 화면 문제가 아니라 **FAB 배치 규칙 자체**가 우측 액션 열과 충돌한다는 확정 근거 |
