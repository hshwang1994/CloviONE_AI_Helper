...
| KBD-04 | Low | 탭 **역순 점프**가 `/users`·`/chat` 에서 2회, `/new-ticket` 에서 1회 — 시각 순서와 탭 순서가 어긋나는 지점이 있다(정확한 위치는 `dist/keyboard/keyboard.json` 의 정지점 좌표로 추적 가능) | **재확인 못 함(2026-08-16)** — 인용된 `dist/keyboard/keyboard.json`은 이번 세션 `SEC-11` 정리로 이미 없다(`dist/`는 gitignore 대상이라 애초에 일회성 측정 산출물이었다 — 지웠어도 유일한 원본은 아니었다, 재측정하면 다시 만들 수 있는 종류). 소스 읽기로는 실제 탭 순서를 안정적으로 재현할 수 없어(키보드 포커스 순서는 DOM 순서·CSS·조건부 렌더가 함께 결정) 실브라우저 재측정이 필요 — 이번엔 미룬다 |
| KBD-05 | Low | `/users` 에서 **본문에 닿는 탭 위치를 특정할 수 없었다**(내 판정 기준으로 `None`) — 상단바·사이드바 요소가 40개 넘게 이어진다. 「본문 바로가기」가 있으니 치명적이지 않지만, 그것을 모르는 사용자는 관리자 내비 37항목을 탭으로 지나야 한다 | **재확인 못 함(2026-08-16), `KBD-04`와 같은 이유** — 실브라우저 키보드 탐색 재측정이 필요해 이번엔 미룬다. `VIS-113`(보강)이 이미 기록한 "관리자 내비 37항목 중 16개가 화면 밖" 실측과 같은 근본 원인(내비가 너무 길다) 계열로 보인다 |
