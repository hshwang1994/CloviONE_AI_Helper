...
| VIS-133 | Med | **페이지 제목이 이름이 아니라 ID 다.** H1 이 `GIT-57`이고, 사람이 읽는 제목 「앱 변경 신청 시, YAML 사라지는 버그 수정」은 카드 안쪽의 작은 제목으로 들어가 있다. 브라우저 탭·북마크·뒤로가기 이력에 전부 `GIT-57`만 남는다 — ID 는 찾는 열쇠이지 의미가 아니다 ‖ **부분 구현완료(2026-08-13, SEM-03과 함께) — H1만.** `Ticket.jsx`의 `PageHeader`에 `ticketId(t)` 대신 `t.title`을 넘긴다 — 화면 H1은 이제 실제 이름이다. `ticketId`(GIT-57)는 사라지지 않고 메타 '티켓 번호' 행으로 이동. `ticket-detail.test.jsx` 신규 시험, revert-to-verify 확인. **브라우저 탭 제목은 이번에 손대지 않았다** — 재조사 결과 `documentTitle.js::useDocumentTitle`은 애초에 `pathname`만 보고 정적 표(`EXTRA_LABELS["/tickets"]="티켓"`)에서 라벨을 고른다(`AppShell.jsx`가 전역에서 한 번만 호출, 개별 화면이 override하는 경로가 아예 없다) — 즉 실제 탭 제목은 "GIT-57"이 아니라 **모든 티켓에서 똑같이 "티켓 | ClovirAssist"**였다(VIS-133 원문의 "GIT-57" 진단 자체가 이제는 부정확 — 아마 예전 구현 기준). 근본 불만("탭으로는 어느 티켓인지 구별 안 됨")은 여전히 유효하다 — 새 발견으로 별도 기록, `brandOverride`/`setBrand()`와 같은 모듈 전역 오버라이드 패턴으로 화면별 동적 제목을 추가하는 것이 다음 후보(TeamDoc/BoardPost/Ticket/Chat 전부 해당, SEM-03과 같은 4화면) |
