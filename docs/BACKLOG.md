...
| PA2-06 | **High** | **관리자 IA가 8그룹 39목적지 평면이라 내비가 839px 창으로 1976px를 보여준다(42%)**(`PA-RC-0017`). 설정이 6화면에 흩어져 `/settings` 안내 4문단이 그 사실을 설명한다. 사용자 콘솔은 23링크·접힘 0으로 정상 — **관리자만의 문제** | `PA-RC-0017` · `PA-F-059`·`064` · `verify_nav.json` | 🔶 **구현 진행 중(2026-08-16) — 8개 acceptance_criteria 중 6개 자동 시험으로 충족, 2개(browser_verification·RESP-04) 남음.** `NAV` 8그룹 39항목 → 5그룹 35항목(운영/사용자와 권한/자동화/연동/감사, 전부 정확히 7항목). 설정 6화면 중 4개(시스템 설정·유지보수·Notion 관리·AI 관리)를 `/settings` 1화면 + 4탭(시스템 정책/OS와 서비스 동작/연동/AI)으로 통합(`SettingsShell.jsx` 신설) — 초기 설정은 체크리스트 마법사라 탭 그릇이 안 맞아 독립 화면으로 남기고 감사 그룹으로 재분류(근거: `DECISIONS.md` D-92). 안내 4문단 **삭제 확인**(자동 시험). 옛 URL 4개(`/system`·`/notion-console`·`/llm-console`·`/maintenance`)는 `/settings?tab=*`로 리다이렉트, 나머지 35개는 경로 자체가 안 바뀜(리다이렉트 불필요, 자동 시험으로 확인). 레일 필터 입력 신설(2글자로 좁혀짐, 관리자 셸만 — 사용자 콘솔 미변경). breadcrumb 3단(`PageHeader`의 `tab` prop, 하위 호환). **RBAC**: 역할 4종(user·operator·auditor·system_admin) × 신규 탭 전체의 allow/deny를 자동 시험으로 확인(`settings-shell.test.jsx`), revert-to-verify로 실제 증명(게이트 무력화 시 operator가 OS 탭에 닿는 것을 시험이 정확히 잡음). 되돌릴 수 없는 동작(서비스 재시작 등)의 확인 단계는 로직을 안 건드려 그대로(기존 `system-ops-confirm.test.jsx` green). 회귀 중 이전 PA-RC-0016(`StatusNotices.jsx`)의 금지 문자(가운뎃점·em대시) 누락도 함께 발견해 고침. 신규 vitest 27건 + 기존 스위트 갱신 3파일, 전체 프런트 회귀 273파일 1862건 green, `static_checks.sh` green(git-secrets의 기존 SEC-20 인간 전담 항목 제외), 번들 재빌드 완료. **남은 것**: (a) `RESP-04`(1024~1200px 축소 레일, `PA-RC-0017` 자신의 acceptance_criteria에는 없고 `PA2-05`의 교차 참조뿐이라 후순위) (b) 실브라우저 검증(1920×1080 light/dark 스크린샷, 역할 4종 로그인 대조, 옛 URL 직접 입력) — 다음 통합 배포 + Chrome E2E 사이클로 이월. 상세: `DECISIONS.md` D-92 |
