...
**마지막 갱신**: 2026-08-15 · **총 항목**: **559행 / 42범주** · **Critical 9 · High 133 · Med 258 · Low 150 · 기타/재분류 9**

> 위 수치는 기계 집계다(`^\| ID \|` 행의 심각도 열). 이전 기록 *"529행 / 36범주 · Critical 5 · High 111"* 은
> 실제와 어긋나 있었다 — 갱신 시점(2026-08-09) 이후 늘어난 행이 헤더에 반영되지 않았다.
> Product Audit Cycle `PA-20260812` 승격분 7행(§PA)을 포함한 값이다.
...
| DS-05 | High | **`fontWeight`가 65개 파일에 8종 값으로 흩어져 있다** — 700(70회)·750(48)·800(30)·600(11)·400(8)·650(6)·500(3)·780(2). **≥700이 148회.** 토큰이 아니라 그때그때 고른 값이라 숫자·제목·라벨이 무차별로 굵어져 "중요한 것"이 안 보인다 | 전수 grep | **부분 구현완료**(MEGA CYCLE C) — `theme.js`에 `FONT_WEIGHT` 토큰 추가(regular/medium/semibold/bold/extrabold). 재검증한 실제 분포는 70개 파일·10종 값(원 조사보다 넓어짐, ≥700 약 156회)으로 계속 번지는 중임을 확인. 70여 호출부 일괄 치환은 이번 범위 밖 — 토큰만 만들어 새 코드가 쓸 수 있게 함 ‖ **아직 열려 있음 확인(Product Audit, 2026-08-15)** — 실브라우저 렌더에서도 토큰 밖 굵기가 **5종** 살아 있다(`PA-F-028`). 원인은 `PA-01`(`PA-RC-0001`)과 **같다** — 토큰을 정의하는 층과 소비하는 층(MUI `sx`)이 다른 언어를 써서, 토큰을 더 만들어도 소비되지 않는다. `PA-01`과 **같은 배치로** 처리할 것(둘은 같은 소비 경로 문제의 두 얼굴이다). 확정 재양자화 값은 Findings `RD-2`(400/500/600/700, 800은 워드마크 전용) ‖ ✅ **구현완료(교차 확인, 2026-08-16)** — `PA-01`이 2026-08-15(커밋 `b214102`)에 이 행이 지목한 바로 그 잔여(620/650→semibold, 720/750→bold, 780→extrabold, 29개 파일 4클러스터)를 이미 처리했으나 이 DS-05 행 자체는 그때 갱신되지 않았다(Whole-product 재감사가 잡은 "이미 해결됐는데 미완료 표시" 사례). 이번에 `grep -rn "fontWeight:\s*[0-9]"` 직접 재실행으로 독립 재확인 — 비테스트 소스 전체에서 raw 숫자 `fontWeight` 리터럴 **0건**(`FONT_WEIGHT.*` 토큰 참조만 남음). 상세: `PA-01` 행 자체, `DECISIONS.md` D-78 |
...
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| AI-01 | High | **`assistant_narrative_enabled`를 켜도 문장이 0개 나온다.** `assistant_runner_url` 기본값이 `/v1/assistant/summarize`인데 러너는 `/message`·`/context/sync`·`/quiz` **셋만** 받고 나머지는 404 ‖ **보류(MEGA CYCLE H, 2026-08-10)**: 단순 config 오타가 아니다 — `narrate.py`가 보내는 `{kind,locale,requester,facts}` 페이로드를 받는 러너 엔드포인트 자체가 없다(퀴즈처럼 스키마·프롬프트·정제기·라우팅을 새로 만들어야 함, 러너 별도 배포 필요). 이 기능은 기본 OFF(fail-closed)라 지금 당장 사용자 영향은 없다 — 다음 AI 도우미 심화 사이클로 넘긴다 | `app/core/config.py:132` vs `assistant.py:5707` (양쪽 직접 확인) | ✅ **구현완료+실측완료(2026-08-17, D-109)** — `/quiz`와 같은 격리 패턴으로 `assistant.py`에 `/v1/assistant/summarize` 신설(스키마·프롬프트·정제·동시성 관례 전부 `/quiz` 재사용, `tools=""` 유지로 `AI-19` 공격면 무관), 러너 배포(`3.60.0`, `dist/deploy-runner.sh`, n8n/플랫폼 무접촉). TEST SERVER 실측 중 별개 운영 공백 2개를 만나 정면으로 해결했다: (1) 플래그 토글 API가 409 — `install-clovirone-web-assistant.sh`의 의도된 하드닝(`chmod 0640`+`ProtectSystem=strict`, 서비스가 자기 보안설정을 스스로 못 고침)이 원인이라 권한을 풀지 않고 root sudo 직접 편집으로 켬. (2) `assistant_runner_token` secret 파일이 이 서버에 한 번도 없었다 — 러너 토큰이 전 엔드포인트 공유 단일값임을 확인 후 이미 동작 중인 `game_runner_token`과 동일 값으로 provisioning. 실측: `GET /api/assistant/briefing?narrate=true`가 실제 자연스러운 한국어 문장 반환 + 실브라우저로 `/#/me`의 "문장 요약 만들기" 버튼 클릭 → 렌더 확인(스크린샷, 콘솔 오류 0). **부수 발견**: 이 과정에서 `narrate.py`/`games/ai.py` 둘 다 `except FileNotFoundError`가 `secret_refs.py`의 실제 예외(`SecretMissingError`)와 안 맞아 "미설정" 상황이 죽은 코드로 빠지고 있던 결함을 발견, 같은 근본 원인으로 둘 다 수정 + 회귀 시험 신규 2건(revert-to-verify 확인). 러너 시험 323건(신규 9)+플랫폼 61건(신규 2) green |
| AI-02 | High | **타임아웃 역전**: 플랫폼 180s < n8n 240s → 플랫폼이 먼저 포기하고 **동일 본문을 재전송**하는데, 중복 쓰기 방지는 n8n **휘발성 `staticData`**에만 있다(n8n 재시작 시 소멸). 우리가 보내는 `idempotency_key`를 n8n은 **읽지도 않는다** → Notion 중복 티켓 위험 | `config.py:30`, n8n 노드 240000ms, `assistant.py:25` | **재확인(2026-08-15)** — **플랫폼 쪽은 이미 올바르고 시험으로 고정돼 있다**: `app/jobs/handlers/chat_message.py:188`이 재시도 전체에서 안정적인 `idempotency_key`(`chatmsg:{message_id}`, `job.idempotency_key`가 아니라 — 그건 재시도마다 값이 달라져 중복을 막지 못한다는 이유가 주석에 있음)를 이미 보내고, `tests/integration/test_chat_ticket_routing_contract.py`(17건, 전부 green)가 이 계약을 고정한다. 진짜 남은 갭(n8n 워크플로가 그 키를 읽고 dedupe하게 만드는 것)은 이 저장소가 못 고치는 게 맞다 — `docs/RUNNER_HANDOFF.md`("이 저장소에서는 고칠 수 없는 것") 자체가 이미 이 정확한 요청을 n8n 담당자에게 넘긴 핸드오프 문서다. **새로 발견**: 3번째 타임아웃 층이 있다 — `runner/claude-work-assistant/assistant.py:25`의 `ASSISTANT_TIMEOUT_SECONDS`(기본 180s)는 러너 자신이 Claude 응답을 기다리는 예산이라, 플랫폼(180s)·n8n 노드(240s)와 셋이 서로 얽혀 실제 경합 창이 정확히 어디서 열리는지는 세 시스템의 실제 타이밍 데이터 없이는 단정할 수 없다. 이 저장소만으로 안전하게 검증 못 할 다중 시스템 타이밍이라 `n8n_timeout_seconds`를 추측으로 올리지 않았다 — 확신 없는 설정값 변경보다, 실제로 확인한 것과 아직 확인 못 한 것을 정확히 남기는 게 다음에 이걸 다룰 사람(n8n 담당자 포함)에게 더 값어치 있다고 판단 |
...
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| AI-05 | High | **스트리밍이 없다.** `subprocess.run`이 전체 출력을 블록하고 전송은 202+1.5초 폴링. 답이 통째로 튀어나온다. 저장소 전체에 SSE/WebSocket 0건 | `assistant.py:2397`, `chat-helpers.js:102` | **중복 확인(2026-08-16)** — `AI-54`가 이 항목과 `AI-06`을 실측 응답시간(5초/17초/20초)까지 더해 다시 발견하고 2026-08-15에 이미 조사완료 상태로 분리해 뒀다(스트리밍은 여전히 큰 작업, 취소는 별도 위험 있어 보류). 이 항목 자체를 별도로 재조사하지 않고 `AI-54`로 합쳐 추적한다 ‖ **재분류(2026-08-17, architect 설계, D-118)** — 두 반쪽으로 갈린다는 것이 확정됐다: ⓐ **진짜 토큰 단위 스트리밍은 이 저장소 소관 밖이다** — n8n 워크플로 자체가 `responseMode: responseNode` + 완결형 `respondToWebhook`(`docs/n8n/ClovirONE_AI_Work_Assistant_v7.json`)이라 한 번에 완성된 응답만 낸다(러너도 `--output-format json`이지 `stream-json`이 아니다) — `docs/RUNNER_HANDOFF.md`의 n8n 쪽 미해결 항목으로 옮긴다. ⓑ **플랫폼이 실제로 할 수 있는 것은 정직한 단계 표시**(대기/클레임됨/n8n 전송됨/정리 중 + 대기열 앞 건수) — 지금 폴링 인프라 위에 얹을 수 있고 SSE가 전혀 필요 없다(`--workers 1` 고정 상태에서 SSE는 스레드풀 고갈 위험이 더 크다는 것도 D-118이 확인). D-118의 Phase 4로 예정 |
| AI-06 | High | **중단(Stop) 버튼이 없다.** 브라우저→잡→n8n→러너 어디에도 abort 경로가 없다. `lib/api.js`에 `AbortController` 0건 | 전수 grep | **중복 확인(2026-08-16)** — `AI-05`와 동일: `AI-54`로 합쳐 추적(취소 쪽 조사 상세는 `AI-54` 참고) |
| AI-07 | High | **모든 채팅이 단일 워커 한 슬롯에 직렬화.** `schedule_run` 잡이 최대 3600초 그 슬롯을 잡을 수 있고 그동안 전 사용자 채팅이 `pending` | `jobs/worker.py:62-123`, `worker_main.py:280-287`, `schedules/router.py:66` | **조사완료(2026-08-16), 보류** — 진단이 정확함을 소스로 직접 확인했다: `repository.claim_next`(`jobs/repository.py:123-154`)는 `job_type` 우선순위 없는 순수 FIFO(`ORDER BY created_at, id`)이고, `Worker.run_forever`(`worker.py:213-235`)는 `run_once`를 순차 호출하는 단일 스레드 폴링 루프라 **한 번에 잡 하나만** 처리한다(비동기·병렬 실행 없음, 불변규칙 §3-1과 일치). `schedules/router.py:66`의 `timeout_seconds: int = Field(default=180, ge=1, le=3600)`로 스케줄 하나가 최대 3600초까지 이 슬롯을 실제로 쥘 수 있다 — 기본값(180초)은 채팅 자체의 다른 타임아웃 층(`AI-02`)과 같은 자릿수지만, 관리자가 상한까지 늘리면 그 시간 동안 전 사용자 채팅이 정말로 멈춘다. **왜 빠른 패치가 아닌가**: `worker_main.py:289-291`이 이미 이 설계를 "스케일 심"으로 명시한다 — 싱글턴 리스(`WorkerLock`)로 워커를 의도적으로 하나만 허용하는데, 워커가 둘이면 ⓐ `claim_next`가 원자적 `UPDATE...RETURNING`이라 잡 중복 실행 자체는 막히지만 ⓑ **스케줄 발화 tick 콜백**(`tick_callbacks`, 같은 워커 루프 안에서 돈다)이 두 워커에서 각자 독립적으로 "지금이 발화 시각"을 판단해 같은 스케줄이 **두 번 큐에 들어갈 수 있다** — `claim_next`의 원자성이 막아 주는 범위 밖이다. `claim_next`의 정렬만 `chat_message` 우선으로 바꾸는 얕은 완화도 검토했으나 기각한다 — 대기 중인 잡의 순서만 바꿀 뿐, **이미 실행 중인** `schedule_run`을 선점하지 못해(동기 실행, 중간에 끊을 수 없음) 신고된 증상(진행 중인 스케줄이 슬롯을 붙들고 있는 동안의 정지)을 실제로는 해결하지 못한다 — 고친 척만 하는 반쪽 수정이라 하지 않는다. 제대로 고치려면 (a) `chat_message`류(대화형)와 `schedule_run`류(배치)를 별도 리스+별도 워커 프로세스/스레드로 분리하고 (b) 스케줄 tick의 "발화 여부 판단"을 그 분리와 무관하게 정확히 한 번만 일어나게(예: tick 자체에 별도 원자적 클레임) 다시 설계해야 한다 — 워커 동시성 모델 자체를 건드리는 변경이라 `AI-05`(스트리밍 없음)·`AI-06`(중단 버튼 없음)과 같은 "채팅 응답성" 축의 더 큰 아키텍처 작업으로 다음 AI/Runner 심화 사이클에 함께 넘긴다 ‖ **정정+설계 완료(2026-08-17, architect 재조사, D-118)** — 이 행 자신의 "(b) 스케줄 tick 이중 발화" 우려가 **틀렸다**: `app/schedules/scheduler.py`를 다시 읽으니 스케줄 발화 자체가 이미 `schedule_runs.idempotency_key = "{schedule_id}:{scheduled_at}"` UNIQUE 제약으로 insert-first 처리돼 있다 — 두 워커가 동시에 같은 스케줄을 발화 판단해도 먼저 insert하는 쪽만 성공하고 나머지는 `IntegrityError`로 조용히 진다(`create_run_and_enqueue`/`_record_skip` 확인, `scheduler.py` 자신의 모듈 docstring이 이미 이 설계를 명시). **진짜 이중 실행 위험이 있는 것은 다른 14개 tick**(백업 스케줄·보존 정리 — Notion 아웃바운드 삭제/보관 호출 포함·문서·티켓·프로젝트 동기화·승인 SLA 알림·검색 색인 등, 유니크 제약이 없다) — 그래서 해법 방향은 "대화형 잡을 별도 lease+워커로 뺀다"는 이 행의 원래 결론과 같지만, 안전장치는 "스케줄 tick도 안전하게"가 아니라 **"대화형(신규) 워커가 애초에 이 14개 tick을 등록하지 않는다"**(배선 자체를 안 하고, 혹시 등록되면 기동 시 즉시 실패하는 assertion까지 이중화)로 재설계됐다. 구체적 파일별 계획(신규 `app/jobs/lanes.py`, `claim_next`/`recover_stuck` 시그니처 확장, `worker_lock.py`/`worker_main.py`/신규 systemd 유닛)과 6단계 phasing은 D-118 전문 참고 — Phase 0(사전 검증 2건: 러너 `conversation_lock`이 대화별로 갈리는지, `SettingsCache.load`가 build-then-swap인지)은 이미 소스로 직접 재확인 완료(둘 다 참) ‖ **구현+실측완료(2026-08-17, D-118 Phase 1~3, D-125)** — 설계했던 레인 분리를 끝까지 구현·배포·TEST SERVER 실측했다: 대화형 잡(`chat_message`/`llm_connection_test`) 전용 워커 프로세스가 배치 워커(스케줄/백업/동기화 14개 tick)와 완전히 분리된 리스·프로세스로 동작하며, 배치 워커를 `systemctl stop`으로 완전히 내린 채로도 채팅이 독립적으로 처리되는 것을 직접 증명했다(Phase 2). 동시성을 1→3으로 올려 서로 다른 대화 간 실제 동시 처리도 확인했고, 그 과정에서 실제로 발견된 SQLite 이중 쓰기 실패 엣지 케이스(멈춘 잡이 배치용 65분 타임아웃을 물려받던 결함)를 전용 840초 타임아웃으로 고치고 자가치유까지 실제 잡으로 end-to-end 확인했다(Phase 3). 상세: `docs/DECISIONS.md` D-118/D-119/D-124/D-125 |
| AI-08 | Med | **진행 표시가 가짜.** 세 점 애니메이션이 "마지막 메시지가 사용자 것"이라는 불리언 하나에서 나온다. 러너가 주는 `timing.ai_ms`를 받아 저장하면서도 화면에 안 쓴다 ‖ **구현완료(2026-08-13)** — n8n이 돌려주는 `timing.{total_ms,ai_ms,...}`가 `chat_message.py::handle_chat_message`를 거쳐 어시스턴트 메시지의 `structured_payload_json`에 그대로 저장되고 `message_view()`가 이미 `structured`로 내보내는데(둘 다 기존 코드, 백엔드 변경 없음), 프런트 어디에도 이걸 읽는 코드가 없었다(전수 grep으로 확인). ①완료된 어시스턴트 메시지: 신설 `responseTimeLabel(m)`(`chat-helpers.js`)이 `timing.total_ms`(없으면 `timing.ai_ms`)를 "3.2초"로 바꿔 `MessageThread.jsx`의 시각 라벨 옆에 캡션으로 그린다. ②대기 중 타이핑 말풍선: 점 3개 무한 반복 애니메이션은 1초를 기다리나 1분을 기다리나 화면이 똑같았다 — `TypingBubble`이 `since`(대기를 시작한 사용자 메시지의 `created_at`) prop을 받으면 1초마다 다시 그려 실제 경과 초("12초")를 함께 보여준다(진행률 예측이 아니라 "아직 살아있다+지금까지 이만큼"이라는 정직한 사실). aria-live로 매초 재낭독하지는 않는다(이 파일이 이미 명시적으로 피해 온 폴링 낭독 과잉과 같은 실수를 반복하지 않음) — sr-only 안내는 처음 나타날 때 한 번만. 신규 시험 9건: `chat-helpers.test.js`에 `responseTimeLabel` 4건(total_ms 우선·ai_ms 대체·timing 없음→null·음수/비숫자 방어), `message-thread-response-time.test.jsx` 5건(완료 메시지 캡션 노출·timing 없는 메시지엔 캡션 없음·사용자 메시지엔 캡션 없음·since 없으면 경과 표시 없음·since 있으면 가짜 타이머로 0→3→11초 확인). revert-to-verify 확인함(소스만 되돌리면 새 시험 6건이 정확히 "함수 없음/캡션 없음/경과 표시 없음"으로 실패, 나머지(부재를 확인하는 시험 3건)는 되돌려도 여전히 통과 — 부재는 신·구 코드 모두에서 참이라 revert가 그 시험들까지 깨뜨리면 안 되는 게 맞고, 실제로 그랬다) | `chat-helpers.js`, `chat/MessageThread.jsx`, `Chat.jsx` | 구현완료 |
| AI-09 | Med | **답을 기다리는 동안 아무것도 못 한다** — 컴포저 잠기고 Enter가 토스트로 거부(최대 10분) | `useChat.js:414,475` | **재확인(2026-08-16), AI-54로 합침** — `useChat.js:483-485`의 `busy\|\|awaitingReply` 전송 차단 가드가 이 증상의 직접 원인이고, `AI-54`가 이미 이 가드를 실측(5초/17초/20초)하고 왜 섣불리 풀면 안 되는지(겹치는 러너 작업→맥락 뒤섞임 재발)까지 조사완료해 뒀다 — 같은 근본 원인이라 별도로 다시 조사하지 않고 `AI-54`로 추적 |
| AI-10 | Med | 러너 동시성 `BoundedSemaphore(2)`(퀴즈와 공유), 3번째 요청은 즉시 429. 소켓 backlog 5(stdlib 기본값 미변경) | `assistant.py:53,6032,6113`(원 인용 줄 42/5810/5888은 stale — 재확인 시 실제 세마포어 선언·acquire 두 곳은 이 줄들) | ✅ 구현완료(2026-08-13, 관측성 + 백로그만 — 상한값 2 자체는 미변경) — `ASSISTANT_MAX_CONCURRENCY`(기본 2, 세마포어 상한)는 이미 배포 시 환경변수로 조정 가능하고, 퀴즈 타임아웃(45s < 티켓/채팅 180s)이 이미 "버려진 퀴즈가 permit을 오래 쥐어 프로덕션 요청이 추가 429를 받는" 위험을 완화하도록 의도적으로 설계돼 있었다(주석에 "검수 확인 사항"으로 명시) — 즉 세마포어 공유 자체와 상한 2는 **이미 검토된 트레이드오프**이고, 실측 없이 숫자만 올리는 것은 이 저장소가 신중히 다루라고 명시한 자리(공유 프로세스)에 근거 없는 용량 가정을 박는 것이라 손대지 않았다. 대신 **실제로 비어 있던 두 가지**를 고쳤다: ① 429가 날 때 아무 로그도 안 남아 운영자가 상한을 올려야 할 때인지 판단할 근거가 없었다 — `_busy_response(endpoint)`를 신설해 기존 관례(`assistant_complete`/`quiz_error`와 같은 `print(json.dumps({"event":...}), flush=True)`)로 `assistant_busy` 이벤트를 남기고 퀴즈발/티켓·채팅발을 `endpoint`로 구분(두 호출부 중복 제거도 겸함). ② `ThreadingHTTPServer`가 stdlib 기본 백로그(5)를 그대로 써서, 순간적으로 6번째 TCP 연결이 오면 세마포어에 닿지도 못하고 OS 단계에서 조용히 거부됐다(n8n엔 원인 불명 연결 오류로만 보이고 로그도 안 남는다) — `_AssistantServer(ThreadingHTTPServer)`로 `request_queue_size=32`만 늘림(실제 동시 처리량은 여전히 세마포어가 정하므로 이 값은 순간적 폭주를 accept까지만 받아 깨끗한 429로 응답할 여유를 준다, 처리 능력 증설이 아님). 신규 시험 3개(`test_assistant.py`), revert-to-verify 확인함(되돌리면 `AttributeError: no attribute '_busy_response'`/`'_AssistantServer'`) |
| AI-11 | Med | 폴링이 5회 연속 실패하면 **자동 폴링을 포기**하고 수동 새로고침만 남는다 ‖ **구현완료(2026-08-13)** — 백오프 설계 자체(실패마다 간격 2배, 상한 5초)는 의도된 것이라 그대로 뒀다. 문제는 5회를 넘으면 `pollDelayMs()`가 `false`를 돌려줘 `refetchInterval`이 완전히 멈추고, 그 뒤로는 서버가 돌아와도 사람이 "새로고침"을 직접 눌러야만 재개됐다는 것. `false` 대신 신설 `POLL_RECOVERY_MS`(20초)를 돌려주도록 바꿨다 — 죽은 서버를 촘촘히 두드리지는 않으면서도 완전히 사라지지도 않는다. 회복 확인이 성공하면 `queryFn`이 이미 하던 대로 `pollFailRef.current`를 0으로 리셋하므로, 다음 간격은 기본값(1.5초)으로 **저절로** 돌아온다 — 이 경로엔 새 분기가 필요 없었다(`pollDelayMs` 반환값 하나만 바뀌면 백오프·회복 확인·기본 복귀가 전부 같은 함수·같은 로직으로 자연히 이어진다). 기존 시험 `chat-state.test.jsx`의 "5회째에 멈춘다"류 3건(직접 단위 시험 1건 + 인터벌 시험 2건)이 옛 동작(완전 정지)을 단언하고 있어 함께 갱신 — 인터벌 시험 하나는 "포기 후에도 20초마다 계속 확인"으로, 다른 하나는 신규로 "회복 성공 시 자동으로 기본 간격 복귀"를 추가 확인. revert-to-verify 확인함(되돌리면 갱신한 3건이 정확히 예측한 증상 — 직접 단위 시험은 `false`, 인터벌 시험은 "포기 후 추가 호출 0건"·"회복 호출 자체가 없어 6회에서 멈춤" — 으로 실패) | `chat-helpers.js`(`pollDelayMs`), `useChat.js` | 구현완료 |
| AI-12 | Low | n8n 캐시가 5분 TTL이라 그보다 긴 간격이면 **Notion 전체 프로젝트+전체 태스크를 `returnAll`로 통째로 덤프**한다 | n8n 워크플로 노드 | **재확인(2026-08-16), 이 저장소 범위 밖** — 근거가 "n8n 워크플로 노드"라고 이미 밝히듯, 이건 이 Git 저장소가 소유하지 않는 n8n 워크플로 정의 안의 설정이다(`CLAUDE.md` §3-9 "공유 서비스 보호" 대상, `docs/RUNNER_HANDOFF.md`에 이미 정리된 "이 저장소에서는 고칠 수 없는 것"과 같은 종류). 코드로 재현·수정할 수 없어 그대로 남긴다 |

### AI 기억·문맥 (사용자 체감 "대화가 안 이어진다"의 정체)
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| AI-13 | High | **플랫폼이 대화 이력을 0건 보낸다.** 연속성이 전적으로 러너의 별도 SQLite에 있다 | `app/jobs/handlers/chat_message.py:166-189`(재확인, `request_body`에 `context`/이력 필드 자체가 없음) | **재확인(2026-08-16), 아키텍처 묶음으로 이동** — 여전히 사실이다. 다만 "고친다"는 것이 곧 플랫폼의 `chat` 테이블 이력을 매 요청마다 러너로 실어 보낸다는 뜻이라, ⓐ 두 이력 저장소(플랫폼 DB · 러너 SQLite) 중 무엇이 정본인지 ⓑ 매 요청 페이로드 크기/지연 ⓒ 러너가 이미 갖고 있는(이제 `AI-14`로 구멍도 메운) 자체 이력과 어떻게 합칠지까지 걸린 설계 결정이다 — `AI-05`/`AI-06`/`AI-54`(채팅 응답성 아키텍처 묶음)와 같은 급의 전담 세션 대상으로 옮긴다 |
| AI-14 | High | **모델 기억 = 최근 8턴 × 각 1,000자.** 게다가 **규칙엔진 턴은 이력에 안 들어가** 모델이 보는 대화에 **구멍**이 난다 | `assistant.py:3030,3058-3066` | **구현완료(2026-08-16)** — 8턴×1,000자 상한 자체는 의도된 예산이라 유지. "규칙엔진 턴이 이력에 안 들어간다"는 실제 결함이었다: `query_tickets`/`update_ticket`/`comment_ticket`(합쳐 900줄+, 각각 여러 return 지점)는 전부 `conversation_history`를 안 쓰고, `claude_query`/`create_ticket` 둘만 스스로 기록하고 있었다 — 규칙엔진이 응답한 턴 다음에 자유형 질문이 오면 방금 무엇을 했는지가 모델이 보는 대화에서 사라졌다. 개별 함수의 (많고 흩어진) return 지점을 고치는 대신, 모든 응답이 반드시 지나는 단일 지점(`process_request`)에 `_ensure_turn_in_history` 게이트를 추가 — claude_query/create_ticket이 이미 기록한 턴(마지막 두 항목이 이번 턴과 일치)은 중복 추가하지 않고, 그 외 모든 경로는 여기서 채운다. `CANCELLED`(응답 문구 자체가 "대화 문맥을 취소했다"고 답하는 유일한 경로)는 명시적으로 제외해 그 문구를 거짓으로 만들지 않는다. 신규 회귀 2건(`test_rule_engine_query_turn_lands_in_history`: 규칙엔진 턴이 기록·누적되는지, `test_cancel_does_not_resurrect_history`: 취소 후 되살아나지 않는지), revert-to-verify(게이트 제거 시 정확히 `KeyError: conversation_history`로 재현 확인 후 복원). 러너 전체 스위트 308건 green(회귀 없음) |
| AI-15 | Med | 문맥이 250,000자를 넘으면 요약이 아니라 **`conversation_history`를 통째로 삭제** ‖ **재확인 결과 서술이 낡음(2026-08-13) — 통째 삭제가 아니라 2단계 강등이다.** 현재 `persist_context_result()`(`assistant.py:210-238`)는 250,000자를 넘으면 먼저 **1단계**로 `conversation_history`를 최근 10턴(그 외 필드도 각자 최근 N개)만 남기는 완만한 절삭을 한다 — 이 시점에 대부분 다시 250,000자 밑으로 내려온다. 그래도 여전히 넘칠 때만 **2단계**로 `interaction_history`/`unhandled_messages`/`conversation_history`/`last_results`를 통째로 지운다(원 서술이 맞는 부분은 이 2단계뿐, 항상 일어나는 경로가 아니다). 이 1단계 절삭 자체엔 시험이 하나도 없어 실제로 도는지 코드만으로는 확정할 수 없었다 — `test_assistant.py::test_persist_context_truncates_history_instead_of_dropping_it_when_over_budget` 신규 추가(100턴·약 305,000자 컨텍스트를 저장해 최근 10턴만 남고 최신 턴이 보존되는 것을 직접 확인), green. 언제 이 완만한 절삭이 추가됐는지는(이 행 자체가 낡은 시점) 특정 커밋을 찾지 못함 — 현재 소스 기준으로만 정정 | `assistant.py:210-238` | 구현완료 |
| AI-16 | High | **대화를 지워도 러너 사본은 안 지워진다** — `clear_persisted_context`가 정의만 되고 **호출 0회**. 대화 전문과 이미지 분석 노트가 `/var/lib/n8n/…state.sqlite3`에 무기한 잔존 (**프라이버시 결함**) ‖ **부분완화, 보류(MEGA CYCLE H)**: MEGA CYCLE A의 `CONTEXT_MODE_TTL_SECONDS` 스윕(`cleanup_stale_conversation_state`)이 이미 "무기한"을 "24시간 이하"로 좁혔다 — 노출 창은 남지만 무한하지 않다. `clear_persisted_context`를 즉시 호출하려면 플랫폼→러너로 "이 대화를 지워라"를 전달할 HTTP 경로가 새로 필요하다(지금 `/context/sync`는 저장만 하지 삭제를 못 받는다) — 러너·플랫폼 양쪽 배포가 걸린 실기능 추가라 다음 사이클로 넘긴다 | `assistant.py:267`, `chat/service.py:109-114` | ✅ **구현완료(2026-08-16)** — 기존 `game_runner_url`/`assistant_runner_url`(`app/games/ai.py`·`app/assistant/narrate.py`)과 같은 관문(n8n을 안 거치고 러너 HTTP 서버를 직접 호출, `OutboundClient(allowlist="runners")`, `secret_ref`로 시크릿 파일 참조)을 재사용했다 — 새 외부 호출 경로를 안 만듦. 러너: `POST /v1/assistant/context/delete` 신설, `/context/sync`와 같은 `conversation_lock`으로 감싸 진행 중인 턴의 재저장과 겹쳐 "지웠는데 곧바로 되살아나는" 경합을 막는다. 플랫폼: `app/chat/service.py::notify_runner_conversation_deleted` — `narrate.py`와 같은 계약("절대 예외를 던지지 않는다", 러너가 죽어도 삭제 자체는 그대로 성공, 실패 시 기존 TTL(24h) 안전망에 맡김). 대화 식별 키는 `_build_job_payload`와 같은 폴백(`backend_conversation_id` 우선, 없으면 플랫폼 id) — 다른 키를 쓰면 애초에 저장된 적 없는 키를 지우는 셈이라 무효하다. 신규 회귀: 러너 4건(`test_context_delete_*`, revert-to-verify로 삭제 경로 자체 확인) + 플랫폼 6건(`test_chat_runner_context_delete.py`, revert-to-verify로 "절대 예외 안 던짐" 계약 확인 — try/except를 임시로 re-raise로 바꿔 4개 실패 조합 전부 재현 후 복원). 러너 전체 스위트(284건) green |
| AI-17 | Med | 러너 상태가 유실되면 **승인 턴이 아무 티켓도 안 만드는데 플랫폼은 그것을 알 방법이 없다** | 계약 테스트 주석 | **처음부터 재조사 완료(2026-08-16), 우려했던 안전 문제는 재현 안 됨** — 인용 근거는 여전히 못 찾았지만(리팩터로 소실 추정), 실제로 `route_request`를 직접 호출해 최악의 경우를 재현했다: 플랫폼은 `context`를 러너에 절대 재전송하지 않으므로(`app/jobs/handlers/chat_message.py` 전수 grep, `context` 필드 0건 확인) 러너 자신의 SQLite 영속 상태가 사라지면 `choose_context`가 진짜 빈 context를 만든다 — 여기에 LLM CLI 실패까지 겹친 이중 최악 시나리오로 시험했다. 결과: `"등록해줘"`(빈 context)는 `NEED_INPUT`/`QUERY`로 정직하게 되묻고, `"네"`(빈 context)도 `fragment_clarify`의 안내 문구로 되묻는다 — **둘 다 `pending_action`/`write_request`가 전혀 안 생긴다.** 즉 상태가 유실돼도 티켓이 "조용히 실패"하지 않는다(만들 시도 자체가 없다) — 사용자가 다시 설명해야 하는 불편(UX 품질)은 남지만, 원 서술이 우려한 "성공한 것처럼 보이며 실은 아무 일도 안 일어나는" 위험한 침묵 실패는 이 경로에서 발생하지 않는다. 신규 회귀 2건(`test_lost_state_degrades_to_honest_clarify_not_silent_action`·`test_bare_yes_with_lost_state_asks_instead_of_guessing`), 러너 전체 스위트 310건 green |
| AI-18 | Med | 대화 목록이 **100개 하드캡**, 페이지네이션 없음. 자동 제목은 첫 메시지 `content[:60]`(AI-55로 이미 해결 — 잘렸을 때만 말줄임표) ‖ **구현완료(2026-08-13)** — 하드캡 자체를 고쳤다. `chat/service.py::list_conversations()`가 `limit`(기본 100, 상한 1000)을 받고 `total`(필터 기준 전체 개수, limit과 무관)을 함께 돌려주도록 변경, `GET /api/conversations`가 이 둘을 그대로 노출. 화면(`useChat.js`)은 `total > 받은 개수`일 때만 사이드바 맨 아래에 "대화 더 보기"를 그린다(그 이하인 절대다수 사용자는 지금과 완전히 동일) — 누르면 offset을 이어붙이는 대신 `limit`을 100씩 늘려 같은 목록을 처음부터 다시 받는다(그사이 새 대화가 생기거나 정렬이 바뀔 때 이어붙이기가 항목을 중복/누락시키는 문제를 원천적으로 피함). `keepPreviousData`(DataScreen 등과 같은 관용)로 그 재요청 동안 목록이 스켈레톤으로 깜빡이지 않는다. 신규 시험: 백엔드 `test_list_conversations_reports_total_and_respects_limit`(limit 적용·total 정확성·상한 초과 422), 프런트 `chat-conversation-load-more.test.jsx` 4건(버튼 노출 조건·클릭 시 loadMoreConvs 호출·isFetching 중 비활성화·빈 목록에서는 안 그림). revert-to-verify 확인함(되돌리면 `?limit=2`가 무시되고 3건이 그대로 옴 · 프런트 버튼 테스트 2건이 정확히 "버튼 없음"으로 실패). 조사 중 별도 버그 2건을 추가 발견해 AI-69(고쳤다)·AI-70(발견만, 별도 행)으로 분리 기록 | `chat/service.py:52-82`, `chat/router.py:86-98`, `useChat.js`, `ConversationSidebar.jsx` | 구현완료 |

### AI 능력
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| AI-19 | High | **에이전틱 도구 사용이 전무** (`--max-turns 1`, `--tools ""`). 유일한 도구는 비전용 `Read` | `assistant.py:2364,2374,2604` | **조사(2026-08-17, architect 에이전트)** — `_run_claude()`(러너 안, 이 저장소가 직접 고칠 수 있는 코드)가 단일 관문이라 기술적으로는 닿는 거리에 있다. 다만 `--tools`를 넓히는 것은 CLAUDE.md §3-8("Runner 코드 웹 편집, 임의 shell 실행... 제품 기능으로 추가하지 않는다")·§3-9(공유 서비스 보호)의 경계와 정면으로 부딪힌다 — 사용자가 도달 가능한 AI 채팅에 파일시스템/shell 도구를 열면 그 자체가 저장소가 명시적으로 금지한 성격의 기능이 된다. 코드 작업이 아니라 **먼저 사람이 판단할 보안 정책 결정**(어떤 도구를, 어떤 샌드박스로, 누구에게)이라 이번엔 손대지 않는다 |
| AI-20 | High | **모델이 보는 데이터가 Notion 티켓·프로젝트뿐.** 플랫폼의 통합 검색 인덱스(`app/search/`)·문서(`team_docs`)·게시판·승인·일정·조직/사람·알림에 **접근 불가**. ← 업무 도우미의 핵심 결손 | `chat_message.py` import 목록 | **조사(2026-08-17, architect 에이전트)** — `AI-13`(대화 이력)과 같은 처지: n8n이 티켓/프로젝트 페이로드를 조립해 러너에 넘기는 그 경로 자체가 이 저장소 밖이라, "검색·문서·게시판·승인도 같이 넘기자"는 변경은 n8n 워크플로 편집이 필요해 플랫폼만으로는 절반만 닿는다. 게다가 그 데이터들은 티켓과 달리 RBAC/조직 범위가 제각각이라(문서 열람 권한, 게시판 비공개 여부 등) "모델에게 무엇을 보여줘도 되는가"의 범위 설계부터 필요하다 — `AI-13`/`AI-05`/`AI-06`/`AI-07`과 같은 "채팅 아키텍처" 묶음으로 다음 심화 사이클로 넘긴다 |
| AI-21 | Med | 티켓 800 / 프로젝트 300 상한 초과 시 `tickets_truncated`로 **개수 질문을 거절**한다 | `assistant.py:3032-3033,2689` | **재확인(2026-08-16), 현재 코드와 안 맞음(스테일 추정)** — `tickets_truncated`(`len(tickets)>800`)는 `claude_query`(자유형) 안에서만 계산되고, 그 함수가 LLM에 넘기는 프롬프트 페이로드만 `tickets[:800]`로 자른다(모델 컨텍스트 예산 때문) — 이때는 거절이 아니라 "일부만 봤다"는 캐비엇을 답에 붙인다(`AI-62`가 이미 고정). "몇 개야?" 같은 **정형** 개수 질문은 별개 함수 `query_tickets`의 `COUNT` 분기(`:3696-3712`)가 처리하는데, 이 분기는 800으로 자르지 않은 전체 `tickets`(상한은 `MAX_TICKETS=10000`)를 그대로 세어 `TICKET_COUNT`를 답한다 — 거절 경로 자체가 없다. 원 서술이 가리키는 줄 번호(`:3032-3033`)도 지금 코드와 안 맞아(AI-14 작업 중 확인한 현재 `tickets_truncated` 위치는 `:3093` 부근) 오래된 스냅샷으로 보인다. 코드 변경 없음 |
| AI-22 | Med | **프롬프트 관리 콘솔이 채팅에 아무 영향이 없다**(죽은 조작판). `get_published` 소비자는 문서 생성뿐이고 채팅 프롬프트는 러너에 하드코딩 | `documents/service.py:86,91`; `assistant.py:2328,2509,2674,2734` | **재확인(2026-08-16), 설계 판단 필요해 보류** — 여전히 사실이다: `app/prompts/`·`app/documents/service.py`의 `get_published` 소비자는 문서 생성뿐이고(`grep` 전수 확인), `app/jobs/handlers/chat_message.py`가 러너로 보내는 `request_body`에는 프롬프트 관련 필드가 아예 없다(`AI-13`과 같은 자리에서 같이 확인) — 채팅 쪽 실제 프롬프트는 `assistant.py`의 `DRAFT_PROMPT`/`VISION_PROMPT`/`QUERY_PROMPT`/`QUIZ_PROMPT`/`UPDATE_EXTRACT_PROMPT` 5개 하드코딩 상수뿐이다. `screen_context`(`AI-30`)와 같은 방식으로 플랫폼이 published prompt를 `request_body`에 실어 보내는 배선 자체는 기술적으로 어렵지 않지만, **그 전에 제품 판단이 먼저 필요하다**: ⓐ `app/prompts` 모델의 `purpose`는 자유 텍스트라 이 5개 프롬프트 중 무엇에 대응하는지 구조적으로 알 방법이 없다 ⓑ 이 5개 프롬프트는 각각 매우 길고 세밀하게 튜닝돼 있다(파일 전체에 회귀 이력 주석이 반복됨) — admin 편집 텍스트가 **대체**해야 하는지 **추가**돼야 하는지에 따라 결과가 완전히 달라지고, 잘못 고르면 이미 검증된 동작을 admin 편집 하나로 깰 수 있다. 기계적 배선이 아니라 이 두 질문에 대한 답이 선행돼야 하는 항목이라 다음 설계 세션으로 넘긴다 ‖ **경미한 재평가(2026-08-16)** — `registry/authoring.js:22`의 화면 자체 안내문을 직접 확인: "발행하면 이 프롬프트 이름을 참조하는 **템플릿**이 다음 문서 생성부터 이 버전을 사용합니다"라고 이미 정직하게 범위를 밝히고 있다 — "죽은 조작판"은 과한 표현이었다, 문서 생성에는 실제로 작동한다. 남은 진짜 문제는 화면 제목이 그냥 "프롬프트"라 처음 보는 관리자가 채팅까지 다룬다고 오해할 여지뿐이다(작아서 이번엔 안 건드림, `area: "콘텐츠"`로 이미 AI/채팅과 다른 그룹에 있어 완화됨). 위 두 설계 질문(ⓐⓑ)은 여전히 유효 — 큰 배선 작업은 그대로 다음 설계 세션 대상 |
| AI-23 | Med | **모델이 고정 안 된 별칭** `ASSISTANT_MODEL=sonnet`(systemd env) — 관리 콘솔에서 못 바꾸고 버전 관리·감사도 안 된다 | `assistant.py:24`, systemd 확인 | **재확인(2026-08-16)** — 여전히 사실: `MODEL = os.environ.get("ASSISTANT_MODEL", "sonnet")`(`:24`)뿐이고, `app/` 전체에 `ASSISTANT_MODEL`/`assistant_model` 참조가 0건(관리 콘솔 노출 없음, grep 확인). 다른 AI 항목들과 달리 이건 순수 운영 설정 노출 문제라 설계 판단 부담은 작지만, 러너(별도 배포 단위)가 systemd env를 읽는 자리에 플랫폼 DB 설정을 실시간 반영하려면 러너 쪽에 새 설정 소스(파일 watch 또는 매 요청 조회)가 필요해 러너 코드 변경이 선행돼야 한다 — 이번 배치 범위 밖으로 남기고 다음 러너 작업 사이클 후보로 기록 |
| AI-24 | Low | 못 하는 것이 코드에 명시: 이메일·Teams·캘린더 발송 · 티켓 삭제 · 일괄 수정 · 실시간 정보 | `assistant.py:511-515,5365-5379,2992-2996,2679` | **재확인(2026-08-16), 결함 아님으로 종결** — 인용 줄 번호는 이후 편집으로 옮겨졌지만(`assistant.py:2756` 부근) 내용은 그대로다: "아직 못 하는 것: 여러 티켓을 한 번에 일괄/자동으로 수정·삭제, 티켓 삭제(대신 상태를 '취소'로), 이메일·Teams·캘린더 같은 외부 작업"을 모델이 사용자에게 사실대로 답하도록 시스템 프롬프트가 명시하고, 실시간 정보(날씨·뉴스 등) 접근 불가도 `:2739`에서 별도로 솔직히 밝히게 한다. `CLAUDE.md` §3-8("제품 기능 경계... 임의 shell 실행... 추가하지 않는다")과도 방향이 같은 **의도된 경계**를 정직하게 알리는 것이지, 숨겨진 결함이 아니다 |

### AI 프런트 — 드로어가 스텁 (클로비 버튼 3개 중 2개가 여기로 간다)
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| AI-25 | High | ✅ 구현완료 — 드로어가 자기 말풍선을 새로 만들지 않고 전체화면 `Chat.jsx`가 이미 쓰는 `Message`(`chat/MessageThread.jsx`)를 그대로 재사용하게 고쳤다. 리치 텍스트·티켓/프로젝트 카드·선택지 칩·실패 표시·재시도·복사·타임스탬프가 전부 공짜로 따라온다(렌더 로직이 두 벌이 되지 않는다). `hideCards`는 드로어에 xxl 결과 레일 개념이 없어 항상 `false`. 신규 시험(`assistant-drawer-parity.test.jsx`, AI-25 2건 — 카드 요소·복사 버튼 확인), revert-to-verify 확인 | `AssistantDrawer.jsx` | 구현완료 |
| AI-26 | High | ✅ 구현완료 — 헤더에 '새 대화' 버튼 추가(대화가 있을 때만 노출). `ConversationSidebar.jsx`의 '새 대화' 버튼과 동일한 세 호출(`clearDraft`·`setComposingNew(true)`·`setCid(null)`)을 재사용 — 실제 생성은 첫 메시지 전송까지 미뤄지므로 눌러만 보고 아무 말도 안 하면 빈 대화가 안 쌓인다. 전체 대화 목록까지는 드로어 폭(460px)에 안 들어가 새로 만들지 않았다 — '전체 화면으로 열기'가 이미 그 목록으로 가는 1클릭 경로다. 신규 시험 1건, revert-to-verify 확인 | `AssistantDrawer.jsx` | 구현완료 |
| AI-27 | High | ✅ **구현완료(2026-08-11)** — `InputBase`(단일행 `<input>`)를 네이티브 `<textarea>`로 교체하고 `useChat()`이 이미 내주던 `textareaRef`(자동 높이 `useLayoutEffect`)를 재사용, `Chat.jsx`와 동일한 `onKeyDown` IME 가드(`isComposing`\|`keyCode===229`) 이식. 신규 테스트(`assistant-drawer-composer.test.jsx`, 3건: Shift+Enter 줄바꿈만/Enter 전송/IME 조합중 Enter 무시), revert-to-verify 완료, 프런트 전체(211파일/1404건) + 정적검사 + 번들 재빌드 green | `AssistantDrawer.jsx:189-193` vs `Chat.jsx:346` | 구현완료 |
| AI-28 | Med | ✅ 구현완료 — 붙여넣은 이미지 미리보기(썸네일+파일명+이름 붙은 제거 버튼, `Chat.jsx`의 칩과 같은 모양)를 컴포저 위에 추가하고, 전송 버튼 조건을 `!text.trim() && !pending.length`로 고쳐 이미지만 있어도(글자 없이도) 보낼 수 있게 했다. 신규 시험 1건(`downscaleImage`는 실제 canvas/Image 디코딩을 써 jsdom엔 진짜 코덱이 없으므로 그 함수만 부분 모킹, 나머지 크기 검사·`pending` 갱신 로직은 실제 코드로 검증), revert-to-verify 확인 | `AssistantDrawer.jsx` | 구현완료 |
| AI-29 | High | ✅ 구현완료 — `maintenanceNotice`/`rateLimitNotice` 배너 + 해제 버튼을 `Chat.jsx`와 동일하게 컴포저 위에 추가했다. 드로어는 `keepMounted`라 페이지를 옮겨도 `useChat` 인스턴스가 안 바뀌므로(전체화면과 별도 인스턴스라 서로 안 풀림) 이 배너가 드로어 자체에 있어야만 새로고침 없이 풀린다. 신규 시험 1건(429 응답 → 배너+"닫기" 확인 → 눌러야 전송 버튼이 풀림), revert-to-verify 확인 | `AssistantDrawer.jsx` | 구현완료 |
...
| AI-32 | Med | 드로어가 **항상 마운트**돼 모든 화면에서 `/api/conversations` + 메시지 요청이 나간다 ‖ **구현완료(2026-08-13)** — 항상 마운트 자체는 의도(닫아도 대화를 잃지 않으려고, `AssistantDrawer.jsx` 자체 주석)라 안 건드림. 진짜 문제는 `useChat()`의 `ai-quota`/`conversations` 두 쿼리가 그 마운트 사실과 무관하게 `enabled` 게이트 없이 즉시 나가던 것 — `pasteEnabled`(같은 "항상 마운트" 문제를 붙여넣기 리스너에서 이미 막은 선례)와 같은 모양으로 `dataEnabled` 옵션을 추가했다. 드로어는 처음 열릴 때까지 `false`를 넘기고 한 번 열리면 계속 `true`(대화 유지가 목적이라 다시 안 꺼짐), `Chat.jsx`(전체 화면)는 옵션을 안 넘겨 기본값 `true`로 기존과 동일하게 즉시 fetch — 공유 훅의 다른 소비처에 영향 없음. 신규 시험 2건(`assistant-drawer-lazy-queries.test.jsx` — 열기 전엔 두 호출 다 없음 · 열면 즉시 나감), revert-to-verify 확인(되돌리면 첫 시험이 "열기 전에 이미 호출됨"으로 정확히 실패). 기존 `chat-ai-quota.test.jsx` 등 50건 재확인(전부 통과, `Chat.jsx` 쪽 즉시 fetch 그대로 보존됨을 재확인) | `AppShell.jsx:652-654`, `useChat.js:84-88,300-309` | 구현완료 |
...
| AI-34 | High | **펜스 코드블록이 지원 안 되는 정도가 아니라 망가진다** — 파서에 fence 상태가 없어 `- foo`는 불릿이 되고 `def f(x):`는 정의목록 행이 된다. 문법 강조·코드 복사 버튼도 없다 | `chat-helpers.js:218-246` | ✅ **구현완료(2026-08-16 문서 정정 — 실제 수정은 2026-08-11 22:55 커밋 `0d671b5`, BACKLOG 갱신만 누락돼 있었다)**: 재검증(2026-08-11) 직후 같은 날 저녁 `parseBlocks`에 `fence` 상태(`RE_FENCE`)를 추가해 펜스 안 줄이 `classifyLine`을 아예 안 거치게 했고(불릿/정의목록 오분류 근본 차단), 닫는 펜스 없이 응답이 잘려도 지금까지 모은 줄을 잃지 않고 `code` 블록으로 낸다. `RichText.jsx`에 `<pre><code>` 렌더 분기 추가(텍스트 노드만, `innerHTML` 없음). 신규 시험 5건(`chat-helpers.test.js`+`RichText.test.jsx`), revert-to-verify 확인, chat 관련 25파일/224건 무회귀 확인 — 지금 재실행해도 61건 green(회귀 없음). **문법 강조는 이번 수정에 없었고 여전히 없다**(라이브러리 도입 또는 자체 토크나이저가 필요해 원래도 이번 패치 범위 밖) — 다만 "코드 복사 버튼이 없다"는 부분은 그 사이 생긴 `MessageThread.jsx:154`의 메시지 전체 복사 버튼(`copyText(m.content)`)이 실질적으로 흡수한다(코드블록 전용 버튼은 아니지만 코드가 포함된 답변 전체를 클립보드로 복사할 수 있다). 자매 항목 `AI-33`(인라인 마크다운 전반)은 이 커밋이 처음부터 명시적으로 범위 밖으로 남긴 더 큰 아키텍처 결정이라 그대로 열려 있다 |
| AI-35 | Med(→ 재분류) | **Notion 외 링크는 클릭조차 안 된다** — `github.com`·사내 위키·Jira가 복사 버튼으로 격하 ‖ **버그 아님, 의도된 보안 경계로 재분류(2026-08-13)** — `chat/links.jsx::PlainUrl`의 툴팁 문구("외부 링크는 열 수 없습니다, 눌러서 주소를 복사합니다")와 `linkifyText` 바로 위 주석("허용 도메인은 실제 링크로, 그 외는 복사 버튼으로")이 이 제한을 사용자에게도 코드에도 명시적으로 드러낸다 — 빠뜨린 게 아니라 **임의의(모델이 생성했거나 대화 상대가 붙여넣은) 외부 URL을 클릭 한 번으로 열 수 있게 두지 않으려는** 의도적 판단이다(AI 응답이 프롬프트 인젝션으로 악성 링크를 포함하게 될 경우의 위험 축소). 화이트리스트를 넓히는 것(예: 사내 위키·Jira 같은 특정 신뢰 도메인 추가)은 코드 결함 수정이 아니라 **이 경계 자체를 다시 그리는 보안 판단**이라 기계적으로 고칠 항목이 아니다 — 필요하면 위협 모델을 다시 검토하는 별도 결정으로 다룬다 | `chat-helpers.js:429-433`, `chat/links.jsx:14-35` | 재분류(버그 아님) |
| AI-36 | Med | **재생성 없음**(재시도는 `failed`일 때만) · **내 메시지 수정 후 재전송 없음** · **메시지 삭제 없음** · **분기 없음** | `MessageThread.jsx:111`, `chat/service.py:240-241`, `conversations/models.py:33-47` | **재확인(2026-08-16), 채팅 UX 기능 묶음으로 이동** — 여전히 사실(`MessageThread.jsx` 전수 확인, `onRetry`는 실패 메시지 전용). `AI-42`(결과 직접 실행 없음)·`AI-68`(내보내기·공유·피드백 없음)과 함께 **버그가 아니라 아직 없는 기능**이라는 같은 성격이다 — 각각 "재생성이 기존 응답을 대체하는지 새로 추가하는지", "분기가 대화를 어떻게 나누는지(새 conversation? 트리?)" 같은 실제 제품 설계가 선행돼야 해 quick-fix로 만들지 않는다. 3건을 "채팅 UX 기능 완성도" 묶음으로 함께 다음 설계 사이클로 넘긴다 ‖ **부분 구현완료(2026-08-16)** — 설계 질문에 직접 답을 내려 구현했다: **재생성**은 기존 응답을 **대체**한다(soft-delete 후 재큐잉, 형제 함수 `retry_message`와 `_reenqueue` 헬퍼 공유) — 여러 버전을 나란히 보여주는 UI는 이번 범위 밖. **대화 마지막 턴에서만** 허용(`_is_last_turn`) — 중간 턴을 바꾸면 그 뒤 맥락과 어긋나고, 그건 분기(branch)의 영역이라 그대로 미룬다. **메시지 삭제**는 소유 대화 안이면 화자와 무관하게 가능(soft-delete, `deleted_at`), 짝 메시지는 자동으로 안 지운다(단순함 우선), 프런트는 team_chat의 같은 기능과 같은 이유(되돌릴 수 없음)로 확인 대화상자를 거친다. `alembic/versions/0059_message_delete_feedback.py`(`Message.deleted_at`+`feedback` 컬럼), `app/chat/service.py`(`regenerate_message`·`delete_message`·`set_message_feedback`), `app/chat/router.py`(`POST .../regenerate`·`DELETE /api/messages/{id}`·`PATCH .../feedback`), `frontend/src/screens/chat/MessageThread.jsx`(재생성·삭제·피드백 버튼)+`useChat.js`(뮤테이션 3종, `AssistantDrawer.jsx`도 같은 `useChat()`을 공유해 드로어도 동시에 기능 동등성 확보 — `assistant-drawer-parity.test.jsx`가 이미 이 원칙을 지키고 있어 그대로 맞췄다). 신규 회귀: 백엔드 `tests/integration/test_chat_message_actions.py` 11건(revert-to-verify 2건 — `_is_last_turn` 가드, soft-delete 필터), 프런트 `message-thread-actions.test.jsx` 13건. **여전히 미룸**: 내 메시지 수정 후 재전송(러너 자체 이력과의 상태 불일치 위험, `AI-13`/`AI-14`와 같은 뿌리라 그 아키텍처 사이클로) · 분기(새 conversation vs 트리 설계 필요) |
| AI-68 | Med | **대화 내보내기/전체 복사 없음**(메시지 단위 복사만) · **공유 링크 없음** · **피드백(👍/👎) 없음** | `MessageThread.jsx:149-155` | **재확인(2026-08-16), `AI-36` 묶음으로 이동** — 여전히 사실. `AI-36`/`AI-42`와 함께 "채팅 UX 기능 완성도" 묶음(상세는 `AI-36` 참고) ‖ **부분 구현완료(2026-08-16, AI-36과 같은 커밋)** — **피드백(👍/👎)**은 `AI-36`에서 함께 구현(`Message.feedback`, 같은 값을 다시 누르면 취소). **전체 복사**도 추가(`chat-helpers.js::formatConversationText` + `Chat.jsx` 헤더의 복사 버튼, 순수 텍스트만 담고 구조화 카드는 본문에 이미 목록으로 나온 내용이라 다시 펼치지 않음) — 내보내기 중 가장 값싼 조각만 골랐다. **여전히 미룸**: 파일 내보내기(형식 선택 등 UX 설계 필요) · **공유 링크**(누구에게·읽기전용 여부·다른 조직 티켓/문서 참조 시 RBAC 재유출 위험 — 이번 세션 SEC-34~37이 다룬 바로 그 계열의 새 위험 표면이라 설계 없이 만들지 않는다) |
| AI-69 | Low | `useChat.js`의 `archiveConv`/`deleteConv`/전송 실패 정리(3곳)의 낙관적 캐시 갱신이 `setQueryData(["conversations", showArchived], ...)`로 실제 쿼리 키(`["conversations", showArchived, debouncedQ, convLimit]`)보다 짧았다 — `setQueryData`는 정확히 일치하는 키만 찾으므로 이 갱신은 캐시 어디에도 안 닿아 조용히 no-op이었고(`NotificationBell.jsx`가 이미 같은 부류를 "접두어 불일치"로 겪고 고친 전례가 있음, `useChat.js`엔 안 옮겨져 있었음), 실제 반영은 뒤이은 `invalidateQueries`(비동기 재조회)에만 의존해 왔다. 그사이 자동 선택 이펙트(대화 복원)가 옛(방금 보관/삭제된 항목이 아직 남은) 데이터로 먼저 돌면 주석이 막으려던 "보이지 않는 대화가 다시 열림" 상황이 재현될 수 있었다 ‖ **구현완료(2026-08-13, AI-18 작업 중 발견)** — 3곳 전부 실제 키(`debouncedQ`·`convLimit` 포함)로 수정, `total` 필드도 함께 갱신. 신규 시험 `chat-conversation-cache-key.test.jsx` 2건 — 재조회 응답을 일부러 영영 안 끝나게 묶어 두고도(재조회가 아니라 낙관적 갱신만으로) 보관/삭제 직후 목록 캐시에서 항목이 즉시 빠지는지 확인. revert-to-verify 확인함(키를 되돌리면 두 시험 다 "항목이 아직 남아 있음"으로 정확히 예측한 증상으로 실패) | `useChat.js`(archiveConv·deleteConv·doSend catch) | 구현완료 |
| AI-70 | Low | `ConversationSidebar.jsx`의 "검색 결과가 없습니다" 빈 상태가 **죽은 코드다** — `const filtered = convItems`(AI-38로 서버 검색 전환 후 그냥 별칭)라서 `filtered.length`는 항상 `convItems.length`와 같다. 바깥 분기가 이미 `!convItems.length`로 먼저 걸러 "아직 대화가 없습니다"/"보관된 대화가 없습니다"를 보여주므로, 검색어가 있는데 결과가 0건이어도(= `convItems`가 빈 배열) 안쪽 `filtered.length` 삼항의 `false` 가지("검색 결과가 없습니다")는 도달할 방법이 없다. 사용자에게 보이는 문구가 실제로는 "검색 중이었다"는 맥락을 담지 못하고 "대화가 아예 없다"는 문구로 잘못 나온다(오류는 아니고 UX 정확도 문제) | `ConversationSidebar.jsx:119,182,213` | ✅ **구현완료(2026-08-15)** — 죽은 `filtered` 별칭을 지우고(`convItems`를 직접 씀), 바깥 빈 상태 분기가 `convFilter.trim()`을 먼저 봐서 검색 중이면 "검색 결과가 없습니다", 아니면 기존대로 보관/전체 빈 상태를 말하게 했다(보관 필터가 켜져 있어도 검색 쪽이 더 구체적인 최근 행동이라 우선). 신규 회귀 `chat-search-empty-state.test.jsx` 4건(검색 결과 없음/보관 중에도 검색이 우선/검색어 없을 때 회귀 없음/공백만인 검색어는 미검색 취급), revert-to-verify 확인(원본으로 되돌리면 정확히 예측한 2건이 실패) |
| AI-71 | **High** | **재생성이 최초 답변과 `message_id`가 충돌해 n8n이 실제로 만든 새 답변이 조용히 버려지고 있었다.** `app/jobs/handlers/chat_message.py::handle_chat_message`가 성공한 답변에 `f"a-{message.message_id}-{job.attempt_count}"`를 쓰는데, `regenerate_message`(AI-36)는 매번 **새 Job**을 큐에 넣어 `attempt_count`가 다시 1부터 시작한다. 최초 전송이 흔한 경우대로 attempt 1에 성공해 있으면 그 답변 행은 `regenerate_message`가 `deleted_at`으로 soft-delete만 하고(행은 남는다) `UNIQUE(conversation_id, message_id)`는 `deleted_at`을 모르므로, 재생성의 새 Job도 attempt 1에 성공하면 정확히 같은 message_id로 INSERT를 시도해 `IntegrityError`로 거부된다. DBTX-02(job 핸들러 5곳+웹 경로 7곳) 통합 배포 뒤 실제 Chrome E2E로 발견 — n8n은 실제로 좋은 답변("안녕하세요! 다시 반갑습니다 😊...")을 만들어 줬는데 저장이 거부되며 사용자에게는 "연결 문제" 안내만 남았다(재생성 버튼이 40초 안에 재활성화되지 않음, `verify_chat_features_e2e.py`가 결정적으로 잡음) | `app/jobs/handlers/chat_message.py:272`(수정 전) | ✅ **구현완료(2026-08-16)** — 성공 경로도 실패 경로(`-fail-{job.id}`)가 이미 쓰던 `job.id`(Job마다 전역 유일한 UUID) 기반으로 통일: `f"a-{message.message_id}-{job.id}"`. 접미사 형식을 파싱하는 코드는 어디에도 없음을 직접 확인(프런트는 불투명 문자열로만 사용, `_is_last_turn`/소프트삭제 질의는 접두사만 봄) — 안전한 교체. 예전 형식을 문자 그대로 검증하던 시험 2건(`test_chat_ticket_routing_contract.py`)도 `job.id` 기반으로 함께 수정. 신규 회귀 `test_chat_handler.py::test_regenerate_after_a_first_attempt_success_does_not_collide_with_the_old_reply`(실제 워커로 재생성까지 끝까지 돌려 새 답변이 저장되는지 확인 — 기존 재생성 시험들은 워커를 안 돌리고 enqueue만 확인해 이 결함을 못 잡고 있었다). Revert-to-verify 완료: `job.attempt_count`로 되돌리면 **TEST SERVER 로그와 글자 그대로 같은** `IntegrityError`로 재현 → 복구 → 재확인. 관련 스위트 전체 green. 상세: `DECISIONS.md` D-89 |
...
| AI-39 | Med | **첨부가 이미지 3종(PNG/JPEG/WebP)뿐** — PDF·CSV·DOCX·TXT·코드 파일 불가. 업무 도우미의 상한 | `app/chat/attachments.py:21`, `Chat.jsx:323` | **재확인(2026-08-15) — 이미 충분히 안내됨, 추가 조치 없음** — 최근 트리아지가 "아이콘만 보고는 제약이 안 드러난다"고 재지목했는데, 현재 소스(`Chat.jsx:346-348`)에는 이미 `Tooltip title="이미지 첨부(PNG, JPEG, WebP)"` + `aria-label="이미지 첨부"`가 붙어 있다(마우스 호버·스크린리더 둘 다 커버). 실제로 허용 안 되는 파일을 골라도 `attachments.py:52`가 명확한 한국어 오류를 준다("PNG, JPEG, WebP 이미지만 첨부할 수 있습니다."). 남는 갭은 터치 전용 모바일에서 호버가 없어 툴팁을 못 본다는 것뿐인데, 이건 이 앱 대부분의 툴팁이 공유하는 훨씬 넓은 범위의 문제라 이 항목 하나만 따로 고치지 않는다 |
| AI-40 | Med(→ 재분류) | 보낸 이미지가 복구 불가 — 서버는 파일명만 저장한다(스크롤 올려도 스크린샷을 다시 못 본다) ‖ **버그 아님, 의도된 정책으로 재분류(2026-08-13)** — `app/chat/service.py:227`가 명시적으로 "Names only — image bytes are never stored on the platform (§13 확장)"이라 적어 뒀고, 실제로 잡 payload에 있던 이미지 바이트는 그 잡이 **성공으로 끝난 뒤에만** 지워진다(`strip_attachment_bytes`) — 실패해서 재시도할 때는 `_recover_attachments_for_retry`가 아직 안 지워진 최신 잡 payload에서 그대로 되살린다. 즉 "저장 안 함"은 빠뜨린 게 아니라 성공 뒤 바이트를 남기지 않으려는 설계고, 재시도 안전성까지 별도로 챙겨 뒀다. 사용자 고지도 이미 있다 — `MessageThread.jsx:48`의 `attachCaveat`("전송 후 이미지 원본은 저장되지 않습니다, 파일 이름만 남습니다")를 hover 전용 `title`뿐 아니라 **항상 보이는 캡션**으로도 띄운다(모바일/터치에서 hover가 안 먹는다는 이유를 주석에 남겨 뒀다). 정책을 뒤집어 이미지를 영구 저장하는 것은 버그 수정이 아니라 별도 저장/보존 정책 결정이다. (참고: `docs/BACKLOG.md` 뒷부분 "④ AI-40" 절은 **다른 감사 회차의 별개 항목**(AI 답변 정확도)이 우연히 같은 ID를 재사용한 것이라 이 이미지 결함과 무관하다 — 혼동 방지로 남겨 둠) | `MessageThread.jsx:85-95` | 재분류(버그 아님) |
...
| AI-42 | Med | **결과에서 직접 실행이 없다**(배정·상태변경·댓글). "상세" 버튼조차 `"N번 상세 보여줘"` 텍스트 왕복이다 | `chat/TicketCard.jsx:114` | **재확인(2026-08-16), `AI-36` 묶음으로 이동** — 여전히 사실: `TicketCard.jsx`에 버튼이 "상세" 하나뿐이고 그마저 텍스트 명령을 채팅으로 되보낸다(`onChoose(index + "번 상세 보여줘")`). `AI-36`/`AI-68`과 함께 "채팅 UX 기능 완성도" 묶음(상세는 `AI-36` 참고) |
| AI-43 | Med | **결과 레일이 2200px 이상에서만 보인다** — 1080p/1440p 사용자는 존재 자체를 모른다 ‖ **보류(MEGA CYCLE H)**: `xxl(2200px)`는 앱 전역 브레이크포인트 시스템(`ui/theme.js` `BREAKPOINTS`)의 값이라 이 화면만 낮추려면 공유 상수를 안 건드리는 별도 임계값이 필요하고(`xl`~`xxl` 사이엔 기존 토큰이 없다), 그 폭에서 3열(사이드바+본문+레일) rem 폭이 실제로 맞는지 라이브 확인이 필요한 디자인 판단이라 이번 사이클(주로 백엔드·상태머신 수정)에서는 건드리지 않는다 | `Chat.jsx:78`, `chat/layout.js:9-14` | 보류 |
...
| AI-46 | Low | 슬래시 명령 없음 · 드래그앤드롭 없음 · 음성 없음 · 모델 선택/커스텀 지시 없음 | | **재확인(2026-08-16), `AI-36` 묶음으로 이동** — 여전히 사실, 전부 버그가 아니라 아직 없는 기능이라 `AI-36`/`AI-42`/`AI-68`(채팅 UX 기능 완성도)과 같은 묶음으로 다음 설계 사이클로 넘긴다 |
| AI-47 | Low | 제안 칩이 **두 벌로 갈렸다**(`chat-helpers.js:114-124` 7개 vs `AssistantDrawer.jsx:35-40` 4개) ‖ **재검토, 보류(MEGA CYCLE H)**: 실제로 읽어 보니 "두 벌"이 아니라 **목적이 다른 두 목록**이다 — `QUICK_PROMPTS`(전체화면 빈 화면 시작 예시, 일반 티켓/프로젝트 조회)와 `SUGGESTIONS`(드로어 전용, "현재 화면 요약"·"관련 문서 찾기" 등 `screen_context` 활용 전제의 맥락 인식형 문구)는 의도적으로 다르다. 겹치는 항목은 "이번 주 마감인 티켓 알려줘" 1개뿐 — 억지로 하나로 합치면 드로어의 맥락 인식 문구가 사라진다. 유지보수 중 드리프트 위험(둘이 우연히 갈라질 수 있음)은 남지만, 지금 통합은 기능 손실이라 보류한다 | | 보류 |
| AI-48 | Med | `processing` 상태로 멈춘 메시지는 **사용자가 복구할 수 없다** — 재시도는 `failed`만 허용, 스윕은 3900초 뒤 ‖ **보류(MEGA CYCLE H)**: `DEFAULT_RUNNING_TIMEOUT_SECONDS`(3900s)는 채팅 전용이 아니라 **모든 잡 타입이 공유하는** 스윕 임계값이고, 스케줄러 잡(최대 3600s 정당 실행)을 오탐 복구하지 않기 위해 일부러 높게 잡혀 있다 — 채팅만 낮추려면 잡 타입별 임계값 오버라이드가 필요해 공유 인프라(`app/jobs/repository.py`) 설계 변경이다. 정상 종료 시 실패 사유별 안내(`on_failure`)는 이미 잘 되어 있다 — 문제는 워커가 죽는 등 **비정상** 종료 시나리오뿐이라 우선순위를 낮춰 다음 사이클로 넘긴다 | `chat/service.py:240-241`, `jobs/repository.py:31` | 보류 |
...
| AI-50 | Med | **채팅 → 실제 Notion 티켓 생성이 한 번도 실물 검증된 적 없다** | `WORK_PLAN_INDEX.md` §7, 계약 테스트 하단 체크리스트 | **재확인(2026-08-16), 의도적으로 직접 수행하지 않음** — TEST SERVER의 `/projects` 화면을 실측하니(스크린샷) SK하이닉스·인천국제공항공사·현대모비스·KB국민카드·엔씨소프트·한국수력원자력 등 **실존 기업명이 22개 프로젝트 전부**에 쓰이고 있다 — "TEST"/"QA"/"샘플"로 분리된 프로젝트가 하나도 없다. 즉 이 서버가 연동한 Notion 워크스페이스는 앱 자체는 TEST SERVER여도 **데이터는 실제(또는 실제와 분리 안 된) 업무 추적 워크스페이스일 가능성이 높다.** 채팅으로 "티켓 만들어줘"를 실제로 실행하면 그 워크스페이스에 진짜 페이지가 하나 생기고, 이는 로컬 앱 상태와 달리 **되돌리기 쉽지 않고 실제 팀이 보는 공유 시스템에 흔적을 남기는** 행위라 되돌릴 수 없는 결정 앞에서 사람에게 확인받는다는 원칙에 따라 직접 실행하지 않았다. 원래 메모("사람 확인 필요", "에이전트가 증명할 수 없다")가 여전히 맞다 — 다만 이번에 그 이유를 스크린샷 근거로 구체화했다. 안전하게 검증하려면 ⓐ 격리된 테스트 전용 Notion 프로젝트를 먼저 만들거나 ⓑ 사람이 직접 실행하고 결과만 공유해야 한다 |
...
| FN-19 | Low | `restore_rehearsals`는 `scripts/restore_rehearsal.py`(cron/수동)만 쓴다 — 그게 안 걸려 있으면 "복구 리허설" 화면이 영구히 빈 화면인데 앱 안에 채울 방법이 없다 ‖ **재분류(2026-08-13) — 버그 아님, 이미 의도적으로 설계·문서화된 제약.** `backups/router.py::list_rehearsals`(현재 줄 88-111) docstring이 그대로 밝힌다: "앱은 리허설을 스스로 돌리지 않는다... 기록이 비어 있으면 한 번도 안 했다는 뜻이고, 화면은 그 사실과 함께 실행 방법을 그대로 보여 준다 — 하지 않은 일을 한 것처럼 꾸미지 않는다." 실제로 빈 응답에도 `command`(정확한 CLI: `.venv/Scripts/python.exe scripts/restore_rehearsal.py --record`)와 `note`(왜 워커가 아니라 스크립트인지)를 함께 돌려줘 "채울 방법이 없다"는 이 행의 전제와 달리 **이미 채우는 방법을 안내하고 있다.** 복구 리허설을 앱 안에서 원클릭으로 돌리게 만드는 것은 버그 수정이 아니라 "복구 검증은 실서비스와 분리된 별도 프로세스로만 한다"는 설계 결정을 다시 여는 것이라 기계적으로 고칠 항목이 아니다 | `backups/router.py:88-111` | 재분류(버그 아님) |
...
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| QA-01 | High | **테스트 서버가 HEAD가 아니다** — 라운드 9~14 미배포. 조사 신뢰도 0 | 해시 비교(직접 확인) | ✅ **중복 판정(2026-08-16) — `PA-05`가 같은 문제를 이미 닫았다**: `PA-05`(BACKLOG.md 하단 PA 표)가 이 항목이 지목한 것과 정확히 같은 증상(테스트 서버가 저장소 HEAD보다 여러 라운드·다수 커밋 뒤처져 E2E 신뢰도가 0에 가까움)을 2026-08-15 17:19 KST에 재빌드+통합 배포로 해소했다 — 배포 뒤 `healthz`/`readyz` OK, `BUILD_STAMP.json` source_hash 일치, 실제 서빙 자산 해시 일치까지 3중 확인함(PA-05 상세 참고). 이후 이 세션이 돈 추가 배포(AI-70 등, `converge-ai70` E2E)도 매번 같은 방식으로 배포 확인을 거쳤다 — "여러 라운드 동안 아무도 모르게 뒤처진다"는 이 항목의 핵심 위험이 재발하지 않았다. 이 행 자체가 `PA-05`보다 먼저 적힌 오래된 중복 기록이라 별도로 새로 할 일은 없다 |
...
| QA-03 | Med | **QA 하네스가 자체서명 HTTPS를 못 탄다**(`urlopen` + `new_context()`에 TLS 예외 없음) → 서버를 직접 못 겨눔. SSH 터널로 우회 | `scripts/ui_qa/run.py:52` | ✅ **이미 해결됨(재확인, 2026-08-17)** — `run.py`에 `--insecure` CLI 플래그가 있고(102행), `_probe_server`의 `urlopen`(74행: `ssl._create_unverified_context()`)·`ensure_session`(191행)·probe context(214-215행)·`capture.new_context`(251-254행, `capture.py:182`의 `ignore_https_errors=insecure`)까지 전부 이 플래그로 연결돼 있다. 자체서명 서버(TEST SERVER 포함)를 겨눌 때 `--insecure` 하나면 전 구간이 TLS 예외를 받는다 — 이번 세션의 RESP-01/FN-42 실측 스크립트도 같은 메커니즘(`ensure_session(..., insecure=True)`)으로 TEST SERVER HTTPS에 직접 접속했다. 이 행은 그 플래그 도입 이전 관측이 남은 stale duplicate |
| QA-04 | Med | **QA 하네스에 라우트 7개가 빠졌다** — `/projects`, `/projects/:id`, `/ideas`, `/notion-console`, `/llm-console`, `/system`, `/setup`. **화면 코드 약 2,600줄이 시각 검사 밖** | `scripts/ui_qa/routes.py` vs 실제 라우트 | ✅ **이미 해결됨(재확인, 2026-08-17)** — 7개 전부 `routes.py`에 이미 등록돼 있다: `/projects`(181행)·`/projects/:id`(182-183행, `hash_template`)·`/ideas`(185행)·`/system`(271행)·`/setup`(272행)·`/notion-console`(273행)·`/llm-console`(274행). stale duplicate |
| QA-05 | Med | **QA 하네스가 역할 1종(system_admin)으로만 돈다** — 역할별 메뉴 노출·데이터 범위·403 막다른 길을 실물에서 못 본다 | `scripts/ui_qa/auth.py:44` | ✅ **구현완료+실측완료(2026-08-17, D-108)** — `run.py`에 `--role`(routes.py `ROLE_RANK` 기반 choices 검증) 신설, `auth.py::ensure_session`에 `role` 파라미터 추가(기본 role은 기존 동작 그대로, 다른 role은 role별 계정+role별 세션 캐시). 판정 로직(`Route.visible_to`/`out_of_reach`, `QA-12`)은 이미 있었는데 system_admin만 돌아 한 번도 실제로 안 갈라졌던 것 — 로컬 서버 실측으로 새 role 프로비저닝·캐시 재사용·`run.py --role operator` 실행 시 관리자 전용 라우트 3개가 실제로 "미검사"로 제외되는 것까지 확인. 그 과정에서 `credentials.json`이 세션 캐시와 달리 role 불일치를 검사 안 해 다른 role 요청이 조용히 옛 계정으로 로그인되는 실제 버그를 발견·수정(재실측으로 고쳤음을 확인) |
...
| QA-08 | Med | QA-07의 **구조적 원인**: 프런트에 시계 주입 관례가 없다. 백엔드는 `tests/fakes/clock.py`를 두고 결정론을 강제하는데 프런트는 `vi.setSystemTime`을 **172파일 중 5개**만 쓴다. 절대 날짜 픽스처는 **55개 파일**에 있다. 상대 시각 헬퍼(`Dashboard.daysSince`, `lib/format.js:79`, `registry/automation.js:112,336,358`, `registry/integrations.js:144`, `chat-helpers.js:38,94`, `LoginHandoff.jsx:58`)와 만나는 조합만 위험하다 — 이번에 전수 대조해 **활성 rot는 1건뿐**임을 확인했고(`scheduler-calendar`의 "예정"은 서버 `kind` 파생이라 안전) 나머지는 잠복이다. **잠복을 잡을 가드가 없다** | 전수 대조 | **재확인(2026-08-16)** — 그 "1건"(`QA-07`)은 이미 구현완료로 닫혔다. 나머지 5개 위험 지점 중 이름으로 직접 확인되는 테스트는 `fmtRelative`/`daysSince`를 참조하는 파일이 `ops-service-status.test.jsx`(=QA-07) **하나뿐**이라, 나머지(`registry/automation.js:112,336,358`·`registry/integrations.js:144`·`chat-helpers.js:38,94`·`LoginHandoff.jsx:58`)는 지금은 활성 rot가 없다(=그 시간 의존 출력을 직접 assert하는 시험 자체가 없다)는 뜻이지 안전이 보장됐다는 뜻은 아니다 — 여전히 "잠복"이 정확한 표현이다. 신뢰할 만한 가드(절대 날짜 리터럴이 이 6개 특정 헬퍼로 실제로 흘러 들어가는 조합만 잡고, 무관한 55개 파일의 절대 날짜는 오탐 없이 넘겨야 함)는 이번 세션에 만든 `scan_errcopy.py`/`check_git_secrets.py`와 같은 급의 전용 정적분석 도구가 필요해 — 그 둘도 각각 v3·값-유출 재설계를 거치고서야 신뢰할 수 있었다 — 성급하게 붙이지 않고 다음 전용 세션으로 남긴다 |

---

## RN — 러너 `assistant.py` 전수조사 (사이클 0) — **재현으로 확인됨**

> **행 정정(2026-08-12)**: 아래 RN-01~14는 표에 해결 표시가 없어 미착수로 읽혔지만,
> 2026-08-10 `5db9fbf`("MEGA CYCLE A")가 5개 공유 Root Cause로 이미 15건(AI-30 Critical +
> RN-01~14 전부, RN-08만 예외)을 해결했다. 커밋 메시지가 각 RN이 어느 Root Cause(A~E)에
> 속하는지 정확히 밝히고 있고, 실제 소스(`assistant.py`)에서 `CONTEXT_MODE_TTL_SECONDS`·
> `cleanup_stale_conversation_state`(실제 호출부 있음)·`_READ_OR_QUESTION_RE`/`_NEGATION_RE`가
> 라우터의 여러 분기(pending 안쪽만이 아니라 최상위 포함)에서 쓰이는 것을 직접 grep으로
> 재확인했다. 신규 회귀 시험 16건(`test_mega_cycle_ai_assistant.py`)도 그 커밋에 포함. 각 행에
> 개별 표시 대신 여기 한 번에 남긴다 — RN-08만 그 커밋이 스스로 "n8n 쪽 협조 없이는 못 고친다"
> 고 명시한 진짜 예외라 아래에서 별도로 열어 둔다.
...
| ID | 심각 | 문제 |
|---|---|---|
| RN-01 | **High** | ✅ 구현완료(`5db9fbf`, Root Cause D) — **"완료했어?"라는 질문이 티켓을 완료로 바꾼다.** `norm()`이 `?`를 지우므로 의문문과 평서문이 같은 문자열이 되고, 그걸 막으라고 만든 `_READ_OR_QUESTION_RE` 가드는 **pending 분기 안에서만** 쓰인다 — 최상위 라우터(`:5653-5672`)엔 질문 가드가 **없다**. 재현: `'그거 완료했어?'`·`'로그인 버그 완료했어?'`·`'다 했어?'` → 전부 `WRITE_UPDATE {진행상태: 완료}`, **미리보기도 승인도 없이**. 티켓 하나를 열어 둔 상태에서 동료에게 하듯 물어보면 Notion이 바뀐다 |
| RN-02 | **High** | ✅ 구현완료(`5db9fbf`, Root Cause D) — **"완료로 바꾸지 마"가 완료로 바꾼다.** 부정 가드(`_NEGATION_RE`)가 `is_approval_message` 안에만 있는데, 분기 ③이 그보다 **먼저** `carries_change`를 돈다(`:5506-5510`). `detect_target_status`는 `바꾸` 어간만 보고 뒤에 오는 부정을 안 본다. 재현: `'아니 완료로 바꾸지 마'`·`'완료로 바꾸지 말아줘'`·`'아직 완료 아니야 완료로 바꾸지 마'` → 전부 `WRITE_UPDATE status=완료`. **이중 실패**다 — 거절이 실행되고, 원래 미리보기 중이던 우선순위 변경은 조용히 버려진다. `"…하지 마"`형이 안 터지는 건 `_DRYRUN_RE`에 우연히 걸려서일 뿐이라 `바꾸지 마/미루지 마/지우지 마`는 무방비 |
| RN-03 | **High** | ✅ 구현완료(`5db9fbf`) — **티켓 선택 대기 중엔 아무 말이나 해도 이전 변경이 엉뚱한 티켓에 쓰인다.** `pending_question=="ticket_selection"`이면 `is_update_intent`가 **무조건 True**(`:5184`)이고 탈출구는 `_READ_OR_QUESTION_RE`뿐이다. 재현: `'안녕하세요'`·`'그만할래'` → `WRITE_UPDATE page_id=t1 status=완료`. **중단 의사인 "그만할래"가 쓰기를 수행한다**(`CANCEL_COMMANDS`에 `"그만"`이 있지만 정확 일치만 본다) |
| RN-04 | Med/High | ✅ 구현완료(`5db9fbf`, Root Cause B) — **"진행 중인 작업 보여줘"가 "진행 중인 작업이 없습니다"로 답한다.** `:5382`의 `"진행중인작업"` 부분문자열 게이트가 정규화된 질의를 삼킨다. 재현: `route_request("진행 중인 작업 보여줘") → CONTEXT_STATUS`. 이 게이트는 pending 블록보다 위에 있어 어떤 경로로도 복구되지 않는다 |
| RN-05 | Med | ✅ 구현완료(`5db9fbf`, Root Cause B) — **`진단`·`스키마확인`이 맨 `in` 검사라 생성·조회를 가로챈다.** 재현: `'성능 진단 티켓 만들어줘'`·`'스키마 확인 티켓 만들어줘'` → `DIAGNOSTIC`. 같은 파일의 다른 게이트(`_CREATE_ACTION_RE`·`_STATUS_CHANGE_RE`)는 앵커·활용형을 요구하는데 여기만 아니다 |
| RN-06 | Med | ✅ 구현완료(`5db9fbf`, Root Cause B) — **못 하는 일을 말 순서에 따라 조용히 삼킨다.** `'티켓 만들어줘 그리고 메일 보내줘'` → 올바르게 거절. 그런데 `'담당자에게 메일 보내줘 그리고 티켓도 만들어줘'` → **거절 없이 티켓만 만들고 메일 얘기는 사라진다**. 캘린더·Teams는 억제 예외 목록에 아예 없어 티켓 낱말이 섞이면 **항상** 거절이 사라진다. 이걸 위해 만든 `PARTIALLY_SUPPORTED`(`:5359`)는 도달하지 않는다 |

### 승인·재시도 프로토콜
| ID | 심각 | 문제 |
|---|---|---|
| RN-07 | Med | ✅ 구현완료(`5db9fbf`, Root Cause A) — **CREATE 승인 대기에 만료도 내용 결속도 없다.** `pending={"kind":"CREATE"}`에 초안 id·해시가 없어 승인 시 `context["ticket_draft"]`를 그대로 쓴다 → 일주일 묵은 미리보기가 뒤늦은 "응" 한 마디에 그대로 생성된다(그 사이 프로젝트·담당자가 사라졌어도). UPDATE는 승인 시점에 대상 존재·소유를 재검증하는데 **CREATE·COMMENT는 안 한다** |
| RN-08 | Med | **"재시도"가 실패했는지 확인하지 않는다.** `:5416`이 `dispatched_at`만 보고 재전송한다. 필요한 정보(`last_action.success`)는 이미 context에 있고 다른 함수는 그걸 본다. 실제 시나리오: Notion 쓰기 성공 → `/context/sync` 실패 → 러너엔 dispatched pending이 남음 → 사용자가 확인을 못 봐 "재시도" → **두 번째 `WRITE_CREATE` → Notion 티켓 중복**. 재승인 가드가 이걸 막으려고 있는데 `재시도`는 명시적으로 그 가드를 우회한다. `5db9fbf`가 명시적으로 미해결 남김 — n8n 쪽에 write-outcome을 알려줄 신호가 이 파일 어디에도 없고, "재시도가 즉시 재전송"은 다른 교착을 막으려던 의도적 기존 동작이라 그대로 고치면 그 교착이 되살아난다. n8n 쪽 협조 필요, 계속 미해결 |
| RN-09 | Med | ✅ 구현완료(`5db9fbf`, Root Cause C) — **분기 ③이 재정의가 성공할지 알기 전에 pending을 먼저 지운다**(`:5506-5510`). `update_ticket`이 `NEED_INPUT`/`FORBIDDEN`/`NO_CHANGE`를 돌려주면 **앞서 미리보기한 변경이 사라지고 그 사실을 아무도 말해 주지 않는다** |
| RN-10 | Med | ✅ 구현완료(`5db9fbf`, Root Cause E) — **멱등 캐시가 쓰기를 전혀 보호하지 못한다.** 러너는 캐시 적중 시 `write_request`까지 그대로 담아 `duplicate: true`로 반환하는데, n8n의 `업무 해석 결과` 노드는 `needs_write: Boolean(write)`만 보고 **`duplicate`를 안 읽는다**. 중복 쓰기를 실제로 막는 건 n8n 자체의 휘발성 `processedMessages`뿐이다(`AI-02`와 같은 뿌리) |

### 동시성·상태 저장
| ID | 심각 | 문제 |
|---|---|---|
| RN-11 | Med | ✅ 구현완료(`5db9fbf`) — **`_CONV_LOCKS.clear()`가 사용 중인 락을 버린다**(`:260`). 주석은 "유휴 락만 버린다"고 하는데 **거짓**이다 — 512개 상한을 다른 대화가 밟으면 보유 중인 락도 사라지고, 같은 대화의 다음 요청이 새 락을 만들어 **두 턴이 동시에 돈다**(락이 막으려던 바로 그 경합) |
| RN-12 | Med | ✅ 구현완료(`5db9fbf`, Root Cause E) — **`/context/sync`가 세마포어도 대화 락도 안 잡는다**(`:5762`). 메시지 턴이 LLM 호출 동안(10~60초) 락을 쥐고 있는데 그 사이 들어온 sync는 턴의 `persist_context`에 덮어써진다 → **이미 만들어진 티켓에 대해 "승인 대기 중"이 되살아나고** `재시도`가 열린다(RN-08로 이어짐) |
| RN-13 | Med | ✅ 구현완료(`5db9fbf`, Root Cause E) — **`/message` 경로가 저장 실패를 삼킨다**(`:193`). DB가 잠기거나 가득 차면 CREATE 미리보기를 담은 응답이 **HTTP 200으로 나가는데 pending은 저장되지 않는다** → 다음 "등록해줘"가 빈 context를 읽어 잡담으로 답하고 **티켓은 영원히 안 만들어진다**(오류 표시 없음). `/context/sync`는 같은 교훈으로 실패를 보고하도록 이미 고쳐져 있다 |
| RN-14 | Med | ✅ 구현완료(`5db9fbf`, Root Cause A) — **`conversation_state`가 영원히 안 지워진다.** 삭제 경로 `clear_persisted_context`는 **호출 0건**(`AI-16`을 러너 쪽에서 재확인). (요청자 × 대화) 조합마다 최대 250,000자가 쌓인다 |
| RN-15 | Med/Low | ~~**CLI 타임아웃이 재시도 대상이 아니다** — 재시도는 비정상 종료만 덮는다. `TimeoutExpired`가 그대로 올라가 504가 되고 **턴 전체가 버려져 이미 성공한 비전 분석까지 사라진다**. 게다가 이미지 중복 방지 키가 *저장되지 않은* context에 있어 재시도가 이미지를 다시 쓴다~~ ‖ **구현완료(2026-08-13)**: 가장 값진 부분(비전 분석 유실)만 정확히 고쳤다 — `process_request()`가 `route_request()` 호출을 `try/except subprocess.TimeoutExpired`로 감싸, 이미 성공한 비전 분석(`new_context`, 이미지 중복 방지 키 포함)을 예외가 올라가기 **전에** `persist_context_result()`로 저장한다. 저장 후 예외는 그대로 다시 올려(`raise`) 기존 504 계약은 그대로 둔다 — 이 함수의 목적은 "부분 진행을 구하는 것"뿐, 타임아웃 자체를 성공으로 바꾸지 않는다. `_run_claude()`의 CLI 레벨 자동 재시도를 타임아웃까지 확장하지는 않았다 — 남은 예산이 이미 거의 없는 상태에서(deadline-aware 루프가 `remaining<10`이면 어차피 중단) 재시도해도 성공 가능성이 낮고, 이 결함의 핵심(데이터 유실)과는 별개 판단이라 범위를 좁혔다. 신규 시험 1건(`test_timeout_after_vision_still_persists_the_already_done_vision_work` — 비전 성공 뒤 route 단계 타임아웃을 재현, 예외는 그대로 올라가되 `load_persisted_context`로 image_notes가 저장됐는지 확인), revert-to-verify 확인. 러너 전체 270건 green |

### 보안·노출
| ID | 심각 | 문제 |
|---|---|---|
| RN-16 | Med | ✅ 구현완료 — `authorized()`(`Handler.authorized`)의 `hmac.compare_digest` 호출을 `try/except TypeError`로 감쌌다. 비ASCII 헤더는 애초에 올바른 토큰일 수 없으므로 `False`(미인증) 처리 — 보안 완화가 아니라 더 정확한 거부다. 신규 시험(`test_non_ascii_authorization_header_is_a_clean_401_not_a_crash`, 실제 `HTTPServer`를 띄워 비ASCII `Authorization` 헤더로 실제 요청, 401 확인). revert-to-verify: 되돌리니 정확히 `TypeError: comparing strings with non-ASCII characters is not supported`가 `socketserver`까지 새어 클라이언트가 `RemoteDisconnected`를 받는 것을 재현·확인 | `assistant.py:authorized()` | 구현완료 |
| RN-17 | Med | ~~**`diagnose`가 Notion 내부 정보와 서버 경로를 일반 채팅 사용자에게 준다**(`:5248-5286`): 요청자의 Notion user id, 작업 DB 담당자 속성 타입, **동명이인의 Notion id와 이메일**, 그리고 `/etc/claude-work-assistant/user-map.json` 경로.~~ `RN-05`(맨 `in` 게이트) 때문에 평범한 티켓 요청에서 실수로 도달할 수 있다는 전제는 2026-08-12에 이미 정정됨(`5db9fbf` Root Cause B가 그 오발동 경로를 닫음) — 그러나 `diagnose` 자체는 **의도적으로** 권한 게이트 없이 열려 있어("진단"이라는 말만으로 누구나 도달) 근본 노출은 그대로였음 ‖ **구현완료(2026-08-13)**: 이 함수에 권한 개념을 새로 만드는 대신(이 파일 전체에 role/permission 체크가 원래 없다 — `authorized()`는 n8n/플랫폼 공용 토큰만 본다), 응답에서 남의 정보를 빼는 쪽으로 고쳤다. 동명이인의 Notion id·이메일은 요청자 자신의 id(`cur_id`)와 일치할 때만 전체를 보여주고, 다른 사람 것은 건수만 남긴다("(다른 동명이인 후보) 티켓 N건"). Notion 스키마 내부(담당자 속성 타입)와 서버 파일 경로(`/etc/claude-work-assistant/user-map.json` 리터럴)는 일반 사용자가 알아도 할 수 있는 게 없는 순수 내부 정보라 응답에서 제거(조치 안내는 "관리자에게 문의"로 대체). 요청자 본인 정보(자기 이메일·이름·자기 Notion id)는 새로 드러나는 게 아니라 그대로 유지. 신규 시험 2건(`test_diagnose_hides_other_same_named_persons_id_and_email`·`test_diagnose_does_not_leak_schema_internals_or_server_path`), revert-to-verify 확인(되돌리니 동명이인 이메일이 그대로 노출되는 것 재현) |
| RN-18 | Low | ~~Notion 원문 오류 문자열이 사용자에게 그대로 표시된다(`:5060`). 같은 파일의 다른 곳은 전부 조심하는데 여기만 예외~~ ‖ **구현완료(2026-08-13)**: `pending_action_status_response()`의 `last_action.error`(n8n이 실제 Notion 쓰기를 하고 `/context/sync`로 돌려준 원문, 이 프로세스가 만든 값이 아니다)를 채팅 응답 문장에 더는 그대로 안 넣는다 — `_run_claude`·`persist_context_result`와 같은 원칙으로 원문은 서버 로그(`notion_write_failed` 이벤트)에만 남기고 사용자에게는 고정된 안전한 문장을 준다. 구조화 데이터(`last_action` extra kwarg)의 원문은 그대로 둔다 — n8n 자신이 쓴 값이라 n8n에게는 새로 드러나는 정보가 아니다. 신규 시험 1건(`test_pending_action_status_hides_raw_notion_error_from_user`), revert-to-verify 확인 |
| RN-19 | Low | ~~죽은 코드: `resolve_statuses`·`infer_project_keyword`·`clear_persisted_context` 전부 호출 0건. `_sanitize_quiz`의 `num_options` 인자가 **사용되지 않아** 4지선다를 요청해도 2지선다가 올 수 있다. 퀴즈 504가 `QUIZ_TIMEOUT_SECONDS`(45) 대신 `TIMEOUT_SECONDS`(180)를 보고한다. 퀴즈 실패 시 `200 {ok:true, ai_used:true, quiz:[]}`~~ ‖ **구현완료(2026-08-13), 단 죽은 코드 3건 중 1건은 재검증 결과 결함 아님으로 정정**: `resolve_statuses`(자기 docstring이 "callers 유지용"이라 주장했지만 실제 호출 0건 — 주장 자체가 거짓)와 `infer_project_keyword`는 삭제. **`clear_persisted_context`는 삭제하지 않았다** — `docs/DECISIONS.md` D-15가 이 함수의 "호출 0회"를 대화 이력 소유권을 플랫폼으로 옮기는(삭제가 러너까지 전파되게 하는) 아직 안 끝난 더 큰 아키텍처 결정의 증거로 명시적으로 인용하고 있다(오래된 감사가 이 함수를 "죽은 코드"로만 본 것은 D-15라는 더 새롭고 권위 있는 기록과 대조 없이 내린 판단이었다). `_sanitize_quiz(raw, num_options)`가 옵션 슬라이스에 하드코딩된 `6` 대신 실제 `num_options`를 쓰게 배선(`generate_quiz`가 이미 2~6으로 clamp해서 넘겨준다). 퀴즈 핸들러의 504가 `TIMEOUT_SECONDS`(180) 대신 실제 예산인 `QUIZ_TIMEOUT_SECONDS`(45)를 보고하게 수정. `generate_quiz()`가 `(questions, ai_ms)` 2-튜플 대신 `(questions, ai_ms, ok)` 3-튜플을 돌려주게 해 "CLI가 실패했다"와 "CLI는 성공했는데 검증 후 문제가 0개"를 구별 — 실패(`ok=False`) 시 `/context/sync`와 같은 원칙(HTTP 200 유지, 본문에 `ok:false`)으로 응답. 기존 테스트 3곳의 2-튜플 언패킹을 3-튜플로 갱신(기존 기대값 자체는 안 바뀜 — 두 테스트의 원본 fixture가 우연히 6개 상한에 안 걸리는 값이었다). 신규 시험 3건(`num_options` 실제 반영, 퀴즈 CLI 실패 시 `ok:false` HTTP 응답, 퀴즈 타임아웃 시 올바른 상수 보고), 러너 전체 회귀 green |
| RN-20 | Low | ~~`runner/README.md`가 `/context/sync`를 "저장이 터져도 200 ok:true로 격리"라고 적는데 코드는 그 동작을 **의도적으로 버렸다**(`ok:false` 반환). README가 `/healthz`도 빠뜨린다~~ ‖ **구현완료(2026-08-13, 문서만)**: 코드는 이미 올바르다(`test_a_failed_state_save_is_not_reported_as_ok`/`test_a_successful_state_save_still_reports_ok`가 이미 고정) — README만 옛 동작을 그대로 서술하고 있어 정정. `/context/sync` 설명을 "HTTP는 항상 200, 저장 실패는 본문의 `ok:false`로 신호"로 바꾸고, 엔드포인트 목록에 `/healthz`(GET, 응답 모양, 배포 게이트 용도) 항목 추가 |
...
| RG-03 | Med | **알림 화면에 사용자용 알림의 액션이 아예 없다** — `chat_room`/`chat_mention`/`ticket`/`board_post`가 `OBJ_ROUTE`에 없어 두 이동 액션의 `when`이 모두 false다. `muted`(직렬화기 주석: "목록에서 빼지 않고 표시만 한다")를 보여 주는 열도 없고, **`DELETE /api/notifications/{id}`에 대응하는 액션도 없다**(그 엔드포인트는 "알림이 무한정 쌓였다"를 고치려고 만든 것이다 — `FN-03`) | | ✅ **구현완료(2026-08-12) — 3개 중 2개는 이미 다른 항목에서 닫혀 있었다**: 이동 액션은 `RG-02`가 `related_route` 우선 처리로 `chat_room`/`chat_mention`/`ticket`/`board_post`를 포함한 서버 계산 대상 전부를 이미 해결했고(그 항목 자체 설명에 이 네 유형이 명시돼 있다), 삭제 액션은 `FN-03`이 이미 구현했다("삭제" 버튼, 소유권 기반, role 게이트 없음). **남은 것은 `muted` 열 하나뿐**이었다 — 서버는 이미 `row.type in muted`(`app/notifications/router.py::_view`)를 내려주고 있었는데 목록 열이 없어 뮤트해 놓고도 왜 계속 오는지 알 길이 없었다. `registry/notifications.js`의 `columns`에 배지 열 추가(뮤트면 "뮤트된 유형" 배지, 아니면 `-`). 신규 시험 2건(`notification-muted-column.test.jsx`), revert-to-verify로 열을 죽이면 실제로 실패 확인 |
...
| VIS-06 | Med | **떠 있는 마스코트가 기본 화면에서 표의 "상세" 버튼 위에 놓인다**(7행 NH손해보험 근처). ※ 자동 `fab_overlap`이 통과한 것은 **검사가 틀려서가 아니다** — 그 검사는 "스크롤 어느 지점에서도 못 누르는가"를 묻고(`assertions.py:399-401`에 그 판단이 적혀 있다) 스크롤하면 누를 수 있으므로 통과가 맞다. 내가 지적하는 것은 **기본 시야에서 콘텐츠를 가려 스크롤을 강요한다**는 UX 문제다 ‖ `VIS-122`(`/jobs`)와 같은 뿌리, 같은 완화 사실 — `Projects.jsx`의 `<DataTable ... onRow={open} .../>`도 행 전체가 클릭 가능해(`kit.jsx` `TableRow onClick`) 가려진 상세 버튼 대신 같은 행 아무 데나 눌러 열 수 있다. 진짜 수정(`DataTable` 공유 컴포넌트 또는 클로비 위치)은 `VIS-122`와 함께 전담 세션으로 ‖ ✅ **완결(교차 확인, 2026-08-16)** — 이 클러스터(`VIS-06`·`30`·`42`·`49`·`63`·`122`)가 요구하던 두 방향이 각각 다른 RC로 이미 이뤄졌다: `PA-RC-0023`이 `DataTable`의 `상세` 버튼 열 자체를 삭제(행 클릭+키보드로 대체), `PA-RC-0019`+`0020`이 떠 있는 클로비(FAB)를 컴포넌트째 삭제(헤더 칩+단축키로 대체) — 겹칠 버튼도, 덮을 FAB도 이제 없다. 독립 재확인: `AppShell.jsx`에 `MascotButton` 참조 0건, `position:"fixed"`가 좌상단(스킵 링크류) 1곳만 남고 우하단 고정 요소 없음 |
...
| VIS-09 | Low | KPI를 한정하는 주석("평균 진행률은 계산이 끝난 20건만 셌습니다…")이 **타일과 분리된 회색 작은 글씨**로 아래에 떨어져 있다. 정직한 문구인데 그것이 수식하는 숫자와 연결돼 보이지 않는다 ‖ ✅ **구현완료(2026-08-15)** — `/projects/:id`의 기존 "정답" 패턴(숫자 바로 아래 계산식)을 공용화했다: `kit.jsx::StatCard`에 선택적 `note` prop을 신설해 카드 안에서 직접 그리게 하고(72곳 소비처는 안 쓰면 그대로), `Projects.jsx`의 합쳐진 각주 한 문단을 "평균 진행률"·"Health 하위" 두 타일 각각의 `note`로 갈랐다. 신규 시험 1건이 각주가 해당 타일의 `.k-stat` 안에 있고 다른 타일로 안 새는지 DOM으로 확인, revert-to-verify로 실제 실패 확인. `VIS-27`(대시보드의 같은 패턴)과 같은 커밋 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-11 | Med | **행에서 가장 눈에 띄는 것이 raw UUID다.** `대상 ID` 열의 `e037ecad-255b-…`가 **브라우저 기본 파란 밑줄 링크**로 그려져(DS-23의 실물 확인) 정작 사람이 찾는 스케줄 이름보다 강조된다. 사람에게 의미 없는 값이 화면에서 가장 강하다 ‖ ✅ **구현완료(2026-08-16)** — `DS-23`(구현완료)이 공용 헬퍼(`columnHelpers.jsx`의 `linkCol` 등)는 이미 MUI `Link`로 고쳤지만, registry 화면 자신이 손으로 `React.createElement("a", {href...})`를 쓰던 자리는 그 수정이 안 닿았다 — 저장소 전체를 같은 패턴으로 검색해 **13곳**(`automation.js` 6·`authoring.js` 5·`integrations.js` 1 — `워크플로 ID`·`템플릿 ID`·`스케줄 ID`·`문서 생성 ID`·`러너 ID`·`프롬프트 ID`·`정책 ID`·`연동 ID`·대상 유형별 딥링크 등)을 한 번에 MUI `Link`(`underline="hover"`, 내부 해시 라우트라 `target="_blank"` 없음 — 새 탭 여는 것은 외부 URL 전용 `linkCol`과 다른 잘못된 동작이라 안 붙임)로 교체했다. `registry-identifiers.test.jsx`에 신규 회귀 2건(템플릿 상세 `prompt_id`/`policy_id` 링크가 `role=link`+`MuiLink` 클래스+올바른 `href`로 렌더되는지, `target` 속성이 없는지 확인), revert-to-verify 확인(bare `<a>`로 되돌리면 `MuiLink` 클래스 부재로 정확히 실패). 관련 registry/DataScreen 스위트(7+7 파일, 84건) green |
| VIS-12 | Med | **10열 중 4열이 `-`**(다음 실행·마지막 실행·유효 기간)인데 전폭을 차지한다. 행이 1개인 표가 전폭 카드를 쓰고 그 아래로 **약 500px가 빈 흰 공간**이다. `FN-13`이 "워크플로 실행 이력을 역추적할 방법이 없다"고 한 바로 그 정보가 들어갈 자리가 비어 있다 ‖ ⚠️ **재확인(2026-08-17, D-117 뒤) — 코드 변경 없음, 결함 아님으로 재분류.** `-` 열들은 실제로 아직 없는 값(비활성 일정의 다음 실행, 한 번도 안 돈 일정의 마지막 실행)을 정직하게 `-`로 말하고 있다 — "0과 없음을 구별 못 한다"는 부류가 아니라 반대로 그 구별을 이미 하고 있다. 열 개수도 원 서술의 10개가 아니라 `PA-RC-0023`(2026-08-16, `상세` 버튼 열 제거) 이후 9개다. 표 아래 공백은 VIS-35와 같은 이유로 안 채운다(내용 없는 영역을 억지로 늘리면 그 자체가 새 어색함) — 제안된 "실행 이력 인라인 노출"은 이미 각 행의 `실행 이력` 서브리스트 드로어로 온디맨드 제공 중이라 상시 노출은 표 전체 밀도를 낮추는 트레이드오프다. 상세: `DECISIONS.md` D-117 하단 |
| VIS-13 | Med | **이 설치의 유일한 자동화가 꺼져 있다**(`활성: 아니오`)는 것이 화면에서 가장 중요한 사실인데, 중립 회색 아웃라인 칩이라 **아무 강조가 없다**. 켜짐/꺼짐이 같은 무게다 ‖ ✅ **완결(2026-08-17, D-112)** — `templates`(authoring.js) 화면은 이미 같은 문제를 고쳐 둔 상태였다(`활성`/`비활성` 어휘 + 비활성=주의 톤). 그 패턴을 `enabledCol` 공용 헬퍼로 승격(`activeCol`과 같은 자리, `data-screen/columnHelpers.jsx`)해 `schedules`(이 행)·`integrations`·`runners`·`workflows` 4곳의 `badgeCol("enabled","활성")`을 전부 교체 — 같은 결함이 반복되지 않게 배선 자체를 통일했다. 신규 시험 8건(`registry-active-badge.test.jsx`에 4화면 × 2건 추가), 프런트 전체 회귀 green |
...
| VIS-33 | Med | **KPI 타일 6개가 전부 `0`이다**(오늘 마감·지연·진행 중·7일 내 마감·안 읽은 알림·안 읽은 채팅). 첫 화면에서 사용자가 얻는 정보가 0이고, `VIS-01`(프로젝트 8개 중 4개 0)보다 심하다. "값이 없다"와 "재보니 0"이 여전히 구분되지 않는다 ‖ ⚠️ **재확인(2026-08-17) — "구분이 안 된다"는 이제(어쩌면 그때도) 사실이 아니다.** `Home.jsx:306-324`(`/me`, 이 행이 말하는 6개 타일이 있는 그 화면)를 직접 확인: 티켓 기반 4개 타일은 전부 `tickets.due_today ? tickets.due_today.count : null` 패턴으로 버킷 자체가 없으면(티켓 소스 미연결 등) `null`을 넘기고, `StatCard`(`kit.jsx:276`)는 `value == null ? "-" : value`로 렌더한다 — "0"과 "-"가 이미 코드 레벨에서 분리돼 있다. 안 읽은 알림/채팅 2개는 자체 DB 카운터라 애초에 "미연결" 상태가 없어 null 가드가 필요 없다(주석 확인: "채팅이 꺼진 설치에서는 chat_unread가 null... 카드 자리를 '막힘'이 채운다"는 별도 분기로 이미 처리). "전부 0"으로 관측됐다면 그 계정이 실제로 티켓·알림·채팅이 다 0건이었을 가능성이 높다 — 그 자체는 버그가 아니다 |
| VIS-34 | Med | **같은 실패 문구가 한 화면에 두 번** — "티켓 소스를 읽지 못해 이번 주 진척을 계산할 수 없습니다."가 '이번 주 내 진척' 카드와 'AI 도우미' 카드에 각각. 그리고 AI 카드의 "오늘 마감 0건, 지연 0건, 진행 중 0건, 막힘 0건"은 **위 KPI 타일과 같은 숫자의 세 번째 표현**이다(`VIS-25`와 같은 패턴이 사용자 콘솔에서도 반복) ‖ ✅ **부분 구현완료(2026-08-17, Explore agent 재발견 → 직접 구현)** — 두 증상을 다르게 다뤘다. **실패 문구 중복**은 진짜 순수 중복(두 카드가 같은 사실을 각자 완결형 문장으로 반복)이라 `AssistantPanel.jsx`의 `Briefing`을 "위 스프린트 카드와 같은 이유로 이번 주 진척도 계산할 수 없습니다"로 짧게 참조하게 바꿨다(전체 문장은 `SprintProgress`에만 남는다). **KPI 숫자 중복**은 `VIS-25`와 정확히 같은 검토를 거쳤지만 결론이 갈렸다 — `VIS-25`(관리자 대시보드)는 이미 "요약 줄 + 상세 구역, 클릭하면 이동" 구조와 `<Note>` 신호가 있어 반복을 그대로 뒀는데, `Briefing`의 숫자는 이 컴포넌트 자신의 설계 계약("숫자가 먼저, 문장은 나중" — 주석 `AssistantPanel.jsx:17`)에서 '문장 요약 만들기' 버튼이 무엇을 문장으로 바꾸는지 보여주는 근거 역할까지 한다 — 완전히 장식적인 중복이 아니다. 숫자를 지우는 대신 `VIS-25`가 쓴 것과 같은 신호(관계 명시)를 캡션에 추가했다: "위 숫자는 이 화면 상단 카드와 같은 값입니다." 신규 시험 2건(`grouped-tickets-collapsible.test.jsx`류가 아니라 `home.test.jsx`에 추가 — `AssistantPanel`은 자체 비동기 쿼리라 `findByText`로 별도로 기다려야 실제로 뭘 렌더하는지 보인다는 것 자체가 이번에 확인한 사실), revert-to-verify 확인함. 프런트 전체 회귀(286파일/1,983건) green |
| VIS-35 | Med | **오른쪽 열이 y≈900에서 끝나는데 왼쪽 열은 y≈1500까지 이어져** 우하단 약 600px가 빈 흰 공간이다. 2열 그리드가 높이를 조율하지 않는다 ‖ ✅ **완결(2026-08-17, D-117)** — 로컬 실측(빈 데이터 QA 계정)으로는 격차가 51px뿐이었지만, 그건 이 계정에 티켓이 0건이라 왼쪽 티켓 카드가 EmptyState 삽화로만 채워졌기 때문이다 — 원 서술의 실데이터 상태(행이 많은 티켓 표)에서는 왼쪽이 행 수만큼 계속 길어지는데 오른쪽(스프린트 도넛+최근 문서 5개, 고정 크기)은 안 늘어나 격차가 재현될 구조임을 코드로 확인했다. 높이를 강제로 맞추는 대신(빈 카드를 늘리면 그 자체가 더 어색하다) 격차의 원인이던 조합 — 왼쪽 열 안에 티켓 카드와 함께 쌓여 있던 `AssistantPanel`("AI 도우미") — 을 격자 밖 전체 폭으로 옮겨 없앴다. 신규 시험 1건(`home.test.jsx`, `data-testid="home-body-grid"` 밖에 있는지 직접 확인), 로딩 스켈레톤도 같은 3번째 구역을 추가해 레이아웃 점프 방지, 프런트 전체 회귀 green |
| VIS-36 | Med | **팀 채팅 카드가 "아직 메시지가 없습니다"를 말하는 데 약 500px를 쓴다**(빈 영역 + 컴포저). 정보량 대비 공간이 화면에서 가장 크다 ‖ ✅ **완결(교차 확인, 2026-08-16)** — `PA-RC-0018` direction 6이 이 카드 자체를 `/me`(`Home.jsx` SideRail)에서 제거했다(`Home.jsx:191-196`·`333-338` 주석이 근거) — 팀 채팅은 사이드바 "채팅방"(`/chat-rooms`)으로 이동, 카드가 더 이상 존재하지 않아 크기 문제도 함께 소멸 |
| VIS-37 | Low | 게시판 카드가 `0 내 글 / 0 받은 댓글 / 0 조회` 세 숫자 아래에 "기능개선 · 서운경, 댓글 0"이라는 **다른 모양의 데이터**를 같은 카드에 밀어 넣는다 ‖ ✅ **완결(교차 확인, 2026-08-16)** — `VIS-36`과 동일 원인, `PA-RC-0018` direction 6이 이 카드를 `/me`에서 제거했다(사이드바 "자유게시판" `/board`로 이동) |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-107 | **High** | **실데이터에 `등록되지 않은 job_type: notion_mapping_sync` 오류가 남아 있다**(2026-07-16). 그 핸들러는 `jobs/handlers/notion_mapping_sync.py`에 **실재한다** → 잡을 넣은 코드와 처리하는 워커의 버전이 어긋났을 때(배포 순서) 생기는 오류로 보인다. `AI-02`(타임아웃 역전 → 재전송)와 함께 **배포·큐 정합성**을 따로 봐야 한다는 신호 ‖ **재검증(2026-08-15) — 재현 안 됨, 코드 변경 없음.** `worker.py::run_once`가 이 오류를 내는 조건은 정확히 하나 — 그 잡을 **claim한 워커 프로세스 자신의** `_handlers`(`worker_main.py::build_handlers()`) 딕셔너리에 그 타입이 없을 때뿐이다. `notion_mapping_sync`는 지금 그 딕셔너리에 영구히 등록돼 있어(직접 확인), 오늘 이 오류가 나려면 **그 타입이 처음 배포되던 바로 그 순간**의 배포 순서 경합만 남는다 — 2026-07-16 기록이 정확히 그 최초 도입 시점과 맞아떨어진다(그 뒤로는 같은 재현 조건이 없다). `permanent=True`(77-81행)라 재시도는 안 되지만, `jobs` 화면 자체가 `status==='failed'`에 **재시도 버튼**을 이미 제공해 복구 경로는 있다 — 새 job_type을 또 도입할 때 같은 좁은 창이 재발할 수 있다는 일반론은 맞지만, 지금 재현 가능한 결함은 없다 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-104 | **High** | **도킹된 클로비 카드가 사이드바 메뉴 끝을 덮는다.** 1920×**1080** 캡처에서 `내 정보` 그룹의 마지막 항목들(`내 업무량`·`내 활동`)이 y≈990~1030에 오는데 카드가 y≈1018에 고정돼 있어 **가려진다.** 내 Chrome 세션(높이 1305)에서는 셋 다 보였다 → **뷰포트 높이에 따라 메뉴가 잘린다.** 1080은 가장 흔한 노트북 높이다. 스크롤하면 닿긴 하지만, 사용자는 **메뉴가 거기 있다는 것을 모른다**. `VIS-42`·`VIS-49`(FAB이 표 액션을 덮음)와 **같은 가족** — 떠 있는 클로비 요소가 콘텐츠를 덮는 네 번째·다섯 번째에 이어 **여섯 번째** ‖ ✅ **구현완료(2026-08-15)** — 근본 원인은 겹침이 아니라 **발견성**이었다: `VIS-113`이 이미 이 목록을 자체 스크롤(`flex:1, overflowY:auto`)로 만들어 뒀고 마스코트 카드는 `flexShrink:0`로 그 아래 자기 자리를 갖는 형제 요소라(겹치는 게 아니라 순서대로 배치), 스크롤하면 실제로 닿는다 — 감사문 자신도 "스크롤하면 닿긴 하지만"이라고 적었다. 남는 진짜 문제는 스크롤 가능하다는 신호가 화면에 없다는 것. `AppShell.jsx::SidebarNav`에 스크롤 위치 감지(`scroll`/`resize` 리스너, `ResizeObserver`는 안 씀 — jsdom에 없어 테스트가 전부 깨진다)를 추가해 아래에 더 볼 항목이 있으면 목록 하단에 안쪽 그림자를 얹고, 끝까지 스크롤하거나 애초에 안 넘치면 사라지게 했다. 신규 회귀 `sidebar-scroll-affordance.test.jsx` 3건(넘칠 때 그림자/안 넘칠 때 없음/끝까지 스크롤하면 사라짐), revert-to-verify 확인, 관련 사이드바 스위트 12건 green. 실측 대신 코드 근거로 판단했으므로 실제 1080p 렌더링 확인은 다음 Chrome E2E에서 |
| VIS-105 | — | **역할 게이팅은 여기서도 정확하다**: 일반 사용자에게 상단 `사용자/관리자` 세그먼트 탭이 **아예 없고**, 관리자 메뉴도 없다. `operator` 캡처와 함께 **RBAC 화면 층이 3역할에서 일관되게 맞는다**는 확인 |
| VIS-106 | Med | 일반 사용자의 홈도 **KPI 6개가 전부 `0`**이고 **온보딩 모달이 화면을 덮는다**(`VIS-32`·`VIS-33`이 역할과 무관하게 재현). 즉 **신규 사용자가 처음 보는 화면이 "가려진 0의 벽"**이다 — 제품 첫인상으로서 가장 나쁜 조합 ‖ ✅ **완결(교차 확인, 2026-08-17)** — 이 행이 근거로 든 두 행이 둘 다 더 이상 이 서술을 지지하지 않는다: `VIS-32`(온보딩 모달이 QA 하네스 캡처를 영원히 덮던 것)는 이미 `_dismiss_tour()`로 완결됐고(2026-08-11), `VIS-33`(0 타일)은 오늘 재확인 결과 코드가 이미 0-vs-null을 정확히 구분하고 있어 사실이 아니었다(위 `VIS-33` 참고). 이 행 자체는 그 둘을 "역할 무관 재현"으로 묶어 부른 것뿐이라 독립된 새 증거가 없다 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-99 | Med | **업무량 분석 표 8열 중 3열이 전 행 비어 있다** — `실제 WD` · `평균 실제WD/건` · `예상 정확도`가 **모든 개발자에게 `-`** 다. 실제 WD가 어디서도 기록되지 않는다는 뜻인데, 화면은 그것을 "아직 없음"이라 말하지 않고 빈 칸으로 둔다. `VIS-02`(전 행 동일값 열)의 세 번째 사례이고, **"예상 정확도"는 이 리포트의 존재 이유에 가까운 지표**라 비어 있는 것이 특히 아프다 ‖ ⚠️ **부분 해결(2026-08-17)** — 재확인 결과 원 서술이 부정확했다: `act_wd`는 "어디서도" 안 기록되는 게 아니라 **실제로는 드물게 기록된다**(TEST SERVER 실측: `ticket_cache` 1,116건 중 27건=2.4%가 0이 아닌 값을 가짐). 특정 개발자·기간 조합이 우연히 그 27건을 못 만나면 합계가 0이 되어 `-`로 보이는 것— `VIS-02`류의 "전 행 항상 빈 열"이 아니라 데이터 자체가 희소해서 생기는 현상이다. 그런데 조사 중 **더 구체적인 실제 버그**를 찾았다: `DevReport.jsx`의 "실제 WD" 열 tooltip이 "아직 입력 전이라 지금은 추정치로 채워져 있습니다"라고 적어 뒀는데, 백엔드(`app/reports/service.py::_bump`)는 `ticket.act_wd or 0`만 쓸 뿐 예상치(`est_wd`)로 대체하는 로직이 전혀 없다 — **tooltip이 실제로 없는 동작을 있다고 말하고 있었다.** 그 거짓 설명을 고쳤다(사실대로: 담당자가 직접 안 적으면 0으로 집계된다는 것을 명시). **아직 안 고친 부분**: 세 열 모두 "0(진짜 실제 WD 0)"과 "0(아무도 안 적음)"을 구별 못 한다 — `FN-42`와 같은 모양의 문제(신뢰도가 숫자 하나에 안 드러남)라 제대로 고치려면 백엔드가 "실제 집계된 티켓 수"를 별도로 내려야 한다. 프리밸런스가 2.4%로 낮아 이 화면 전체에 영향이 크므로 다음 배치 후보로 남긴다 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-95 | **High** | **`RG-04` 확인** — 상세 모달이 `대화 ID: 112347f0-6510-4aec-9e89-cfe670a0fa0a`를 **평문으로만** 보여 준다. 그 대화는 `#/chat?c=<id>`로 바로 열 수 있고, 서버는 `schedule_id`·`schedule_run_id`·`generation_id`를 **"idempotency_key 문자열 파싱 말고는 갈 길이 없었다"는 이유로 추가**했는데 **화면에 링크가 하나도 없다.** 유일한 이동 액션은 `감사 로그에서 보기` 하나다 → `FN-13`(워크플로 실행 이력 역추적 불가)이 **서버는 이미 고쳐졌는데 화면이 안 받은** 상태임이 실물로 확인됐다 ‖ **재검증(2026-08-15) — 두 갈래로 나뉜다.** ① `schedule_id`/`schedule_run_id`/`generation_id`(`RG-04`의 원래 범위)는 이미 해결돼 있다 — `registry/automation.js:445-449`에 `schedule_id`→`#/schedules?id=`, `generation_id`→`#/documents?id=` 링크가 실재한다(`schedule_run_id`는 여는 화면이 따로 없어 의도적으로 참조값만). ② `conversation_id`는 `RG-04` 원래 범위가 아니라 이 항목이 새로 추가한 관찰인데, **링크를 걸지 않는 편이 맞다** — `#/chat?c=<id>` 딥링크는 실제로 존재하지 않는다(`Chat.jsx`/`useChat.js` 어디도 URL에서 대화 ID를 읽지 않는다, 코드 직접 확인). 설령 구현해도 `app/chat/router.py::get_messages`가 `get_owned_conversation`으로 **호출자 본인 소유 대화만** 허용해 다른 사용자의 대화면 403/404가 난다 — 관리자가 남의 대화를 열람하는 관리자 우회 경로 자체가 백엔드에 없다. 게다가 바로 위 줄의 `user_id` 필드 주석이 이미 **"이 큐 화면이 대화 내용 열람의 우회로가 되지 않게" 서버가 요청자 이름을 일부러 뺐다**고 명시한다 — `conversation_id`를 클릭 한 번으로 여는 링크로 만들면 그 설계 의도와 정면으로 부딪힌다. **결론: 코드 변경 없음** — RG-04 범위는 이미 닫혔고, conversation_id는 의도적 비링크가 맞는 상태다 |
| VIS-96 | Med | **한 모달에 raw UUID가 6개**다 — 작업 ID · 요청자 계정 ID · 메시지 ID · 멱등키(`chatmsg:a516ba24-…`) · 대화 ID. 전부 링크도 이름 해석도 없다. 16개 필드 중 6개가 사람이 못 읽는 값이라 **모달의 정보 밀도가 실제보다 훨씬 낮다** ‖ ⚠️ **재확인(2026-08-17) — "이름 해석 누락"이 아니라 대부분 이름이 없는 값이거나, 하나는 의도된 차단이다.** `registry/automation.js:436-439` 주석이 명시: `user_id`(요청자 계정 ID)는 백엔드가 **일부러** 이름/이메일을 안 준다(`app/jobs/router.py::_job_view` — 작업 큐 화면이 대화 내용 열람의 우회로가 되지 않게 하는 의도된 프라이버시 경계, "여기에 이름을 그리려면 그 원칙부터 다시 정해야 한다"). 나머지 5개(작업 ID·메시지 ID·멱등키·대화 ID·`schedule_run_id`)는 애초에 "사람 이름"이 없는 상관관계 ID라 해석할 대상 자체가 없다(`schedule_id`는 이미 `#/schedules?id=` 링크로 처리돼 있음, line 446-447). 이 행을 "패턴 재적용으로 빨리 닫을 수 있다"고 다시 판단하지 말 것 — 실제로 액션 가능한 부분이 없다 |
...
| VIS-91 | Med | 그 한 줄마저 **약 450px 빈 공간 아래, 입력창 바로 위에 붙어 있다** — `ChatPane`이 `justifyContent: flex-end`라 위쪽이 통째로 비고 안내는 바닥에 깔린다. 시선이 먼저 닿는 자리가 빈 곳이다 ‖ ⚠️ **재확인(2026-08-17) — 의도된 설계, 되돌리면 예전에 실제로 났던 버그가 재현된다.** `ChatPane.jsx:292-295` 주석: "기본값(flex-start)이면 내용이 위에 쌓이고 아래가 빈다 — 운영에서 재현된 증상(대화가 참여자 표시 바로 아래에 붙고 입력창까지 큰 빈 공간이 남음)." 즉 지금의 `flex-end`는 그 실제 결함을 고친 결과다. 메시지가 적을 때 위가 비는 것은 iMessage/Slack/Discord 등 채팅 UI의 표준 동작(항상 최신 메시지·입력창이 바닥에 붙는다)이라 안 고친다 — "고치면" 예전 버그로 되돌아간다 |
| VIS-92 | Med | **게시판만 필터 관용구가 또 다르다** — 카테고리를 **알약 칩 6개**(전체/자유/질문/정보 공유/맛집/공지)로 그리고, **검색을 오른쪽 위**에, 정렬을 그 아래에 둔다. 다른 화면은 전부 "왼쪽 검색 + 오른쪽 셀렉트"라 **좌우가 뒤집혀 있다.** `DS-12`(툴바 9벌)·`DS-13`(탭 관용구 3종)에 이어 **네 번째 관용구** ‖ ✅ **완결(2026-08-17, D-116)** — 카테고리 칩 묶음은 값 개수만큼 자유롭게 줄바꿈해야 해서(`TicketFilterBar`의 select처럼 고정 폭 칸에 못 넣는다) `FilterBarGrid` 공유 그리드를 그대로 가져다 쓰지 않고, `Board.jsx` 자신의 2열 그리드에서 두 열의 순서(검색+정렬을 먼저)와 폭 배분(가변 열이 이제 카테고리 쪽)만 뒤집었다. 상태 칩 줄은 이미 카테고리 아래에 있어 손대지 않음. 신규 시험 1건(`board.test.jsx`, DOM 순서 확인, revert-to-verify 완료), `idea-board.test.jsx`(같은 컴포넌트를 kind="idea"로 씀) 회귀 없음 확인 |
...
| VIS-86 | Low | ~~본문 편집기의 이모지 버튼 8개가 `aria-label="이모지 ✅"` 처럼 **이모지를 그대로 되읽는다** — 스크린리더 사용자에게 그 버튼이 무엇을 하는지 아무 정보도 주지 않는다("완료 표시 넣기" 같은 뜻을 말해야 한다)~~ ‖ **구현완료(2026-08-13)**: `ui/BodyEditor.jsx`의 `BODY_EMOJIS`를 `{emoji, label}` 쌍으로 바꿔 각 버튼이 실제로 삽입하는 내용의 뜻을 말하게 했다("완료 표시 넣기" 등). 같은 뿌리(같은 되읽기 패턴)를 찾아 팀 채팅 이모지 피커(`ChatPane.jsx`, `chat-compose.js::EMOJI_GROUPS` 60종)도 같이 고쳤다 — 이쪽은 `EMOJI_GROUPS`의 "유니코드 문자만 담는다" 계약을 기존 시험이 고정하고 있어 데이터 모양은 안 바꾸고, 별도 `EMOJI_LABELS`(emoji→표준 한국어 명칭) 맵을 신설해 조회하게 했다. 신규 시험 4건(`body-editor-emoji-label.test.jsx`, `chat-compose.test.js`에 2건 추가 — 모든 이모지가 이름표를 갖는지, 이름표에 이모지 자신이 반복되지 않는지) + 기존 `chatpane.test.jsx`·`body-editor-toolbar-contrast.test.jsx`의 옛 `aria-label="이모지 X"` 참조 3곳을 새 이름표로 갱신. 프런트 전체 회귀(243파일/1608건) green |
...
| VIS-82 | Med | 알림 상세 모달의 **`내용`이 `-`** 다. 제목 말고는 본문이 비어 있어, 목록에서 얻는 정보와 상세에서 얻는 정보가 같다 ‖ ✅ **이미 해결됨(교차연결, 2026-08-17)** — `ADM-03R`이 이 행이 관측한 시점의 지배적 유형(계정 잠금 알림, 당시 103건 중 56건)에 실제 본문을 달았다(`app/auth/router.py:427-435`, ETA+안내 포함). 재확인 결과 러너 불가·잡 실패·비밀번호 변경 필요·티켓 배정·백업 실패·승인 요청/결정 등 나머지 유형도 전부 실제 `body`를 갖고 있다 |
| VIS-83 | Med | 알림 목록의 **`읽음` 열이 전 행 "읽음"** 이라 정보가 0이다(`VIS-02`와 같은 죽은 열). 그리고 `RG-03`대로 **삭제 액션이 없다** — `DELETE /api/notifications/{id}`는 "알림이 무한정 쌓였다"를 고치려고 만든 것인데 부를 방법이 없다 ‖ ✅ **양쪽 다 낡은 기록(2026-08-16 재확인)** — 삭제 액션은 이 항목이 참조하는 `RG-03` 자신이 이미 "✅ 구현완료"로 닫혀 있다(`FN-03`이 구현, `registry/notifications.js:102` `{label:"삭제", variant:"danger", method:"DELETE", path:(r)=>"/api/notifications/"+r.id, confirm:"..."}`로 실존 확인) — 이 행만 그 닫힘을 반영 안 하고 있었다. `읽음` 열도 `readCol("read_at","읽음")`(공용 헬퍼, `columnHelpers.jsx`)이라 행마다 실제 `read_at` 값에 따라 "읽음"/"안읽음" 배지를 정확히 갈라 그린다(죽은 열 아님) — 원 감사가 본 "전 행 읽음"은 이 화면이 열람 시 자동으로 읽음 처리하는 동작(`registry/notifications.js:94-96` 드로어를 열면 그 알림의 `read_at`을 즉시 채움)과 겹친 표본이 우연히 전부 읽음 상태였을 가능성이 높다 — 코드 자체는 안읽음 케이스를 정확히 렌더한다(안읽음 배지 렌더 로직 존재, 새 알림이 오면 재현 가능). 코드 수정 없음 |
...
| ID | 심각 | 확인 결과 |
|---|---|---|
| VIS-76 | — | ✅ `AI-25` 구현완료로 함께 해결 — 어시스턴트 답변이 **평문 그대로** 그려진다. 러너가 보낸 `1. [진행] [Infra] 넥서스 캡쳐 설정 작업` / `프로젝트: … / 담당자: 황형섭` / `마감일: 2026-08-31 / 우선순위: 없음` 이 전체화면 채팅에서라면 번호 목록과 정의목록이 될 텐데, 드로어에서는 들여쓴 생짜 텍스트다. 복사·재시도·타임스탬프도 없다 |
| VIS-77 | — | ✅ `AI-26` 구현완료로 함께 해결 — 헤더에 확대(⤢)와 닫기(✕)뿐, **"새 대화" 버튼이 없다.** 열자마자 예전 대화가 자동 복원돼 그 스레드에 갇힌다 |
| VIS-78 | — | ✅ `AI-27` 구현완료(2026-08-11)로 함께 해결 — 하단 입력이 **한 줄짜리**다("클로비에게 질문하세요") |
| VIS-79 | — | **`AI-30` 확인**(당시 번호 — 이후 `AI-66`으로 재번호됨, 진짜 Critical `AI-30`은 별개) — 헤더가 "현재 화면을 기준으로 도와드려요", 아래에 "현재 문맥: 문서". `/team-docs`에 있으니 라벨은 맞는데 **그 문맥은 전송되지 않는다** — 실제로 열린 대화 내용은 문서와 무관한 티켓 얘기다 ‖ 부분구현 — `AI-66` 행 참고(배선은 이 저장소 끝까지 확인됨, n8n 쪽 마지막 홉만 남음) |
| VIS-80 | Med | ~~어시스턴트가 같은 되묻기를 반복한다. "이번주 완료된 작업 정리해서 티켓 하나 만들어줘" → *"티켓을 생성할 프로젝트를 알려주세요…"*, 사용자가 `??` 라고 답하자 **똑같은 문장을 그대로 한 번 더** 낸다. 자기가 이미 물었다는 것을 모른다 — `RN-03`(선택 대기 중 아무 말이나 처리)과 같은 뿌리로 보이는 대화 품질 결함이고, **실제 대화 기록에 남아 있다**~~ ‖ **구현완료(2026-08-13)**: 재조사 결과 `RN-03`과 뿌리가 **다름을 확인**(추정은 틀렸음) — `RN-03`은 `update_ticket()`의 "느슨한 키워드 일치가 무관한 답을 정답으로 잘못 채택"(오채택) 문제, VIS-80은 `create_ticket()`의 프로젝트 미언급 되묻기가 애초에 `pending_question`을 안 남겨(같은 함수의 후보 다수 경로는 이미 `project_selection`을 남기는 것과 비대칭) 다음 턴이 "지금 이 질문에 답하는 중"인지 스스로 알 방법이 없던 문제(무기억) — `git blame`으로 해당 블록이 `RN-03`의 수정 커밋(`5db9fbf`)에 닿지 않은 원본 그대로임도 확인. `pending_question: "create_project"`를 남기고, 재진입 시(`already_asked`) 문구를 "프로젝트를 이해하지 못했어요..."로 바꿔 최소한 반복 인식은 하게 했다(반복 자체를 영구히 막을 수는 없다 — 다음 대답도 여전히 애매할 수 있다). 부수 결함도 함께 확인·수정: 되묻는 문맥의 `pending_original_message`를 `message`(그 턴의 원문, 예: "??")가 아니라 `semantic_message`(원본+지금까지의 대답 누적)로 넘겨, 되묻기 루프가 길어져도 원래 티켓 요청 내용이 사라지지 않게 했다(실측: 예전 코드는 두 바퀴 만에 원본이 지워짐). `resolve_project(message, ...)` 자체는 손대지 않음(범위 밖). 신규 시험 1건(`test_create_project_ask_varies_on_repeat_and_keeps_original_request` — 두 물음이 달라지는지 + 원본 보존 + 최종 응답 후 `pending_question` 정리까지 확인), revert-to-verify 확인. 러너 전체 회귀 301건(전체 디렉터리) green |
...
| VIS-74 | **High** | **기존 검사 행렬이 그 구간을 한 번도 샘플링하지 않았다.** 목록이 768 다음 바로 1366이라, 표가 카드로 접히는 폭(≤899.95px)과 열이 넉넉해지는 폭(1366) **사이**가 통째로 빈다. **노트북과 반쪽 창에서 가장 흔한 폭**인데 2026-08-04 실행이 "992페이지 fail 0"이었던 이유가 여기에 있다 → `1200x900`을 기본 목록에 넣고, 임의 `WxH`를 받도록 하네스를 고쳤다 ‖ ✅ **구현완료 확인(2026-08-15) — 상태 미표기였을 뿐 이미 반영돼 있었다.** `scripts/ui_qa/capture.py:69`에 `Viewport("1200x900", 1200, 900)`이 기본 목록에 실재하고("나중에 결함을 실제로 잡아서 추가했다" 주석 포함), `run.py --viewports`(`nargs="*"`)로 임의 WxH도 받는다 — 둘 다 소스 직접 확인. 코드 변경 없음, 문서만 정정. 이 1200×900 축을 포함한 실제 하네스 실행 결과는 다음 통합 Chrome E2E에서 함께 확인 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-64 | **High** | **한 페이지가 11,558px다** — 감사 로그(6,014px)의 거의 2배이고 제품 최장이다. **페이지네이션이 아예 없어** 담당자별로 묶인 티켓 200행 이상이 통째로 한 화면에 들어간다(감사 로그는 그래도 100행/페이지다). 스크롤 끝까지 가는 데만 화면 열 번 분량이고, 고정 헤더가 없어 어느 담당자 구역인지도 중간에 잃는다 ‖ ✅ **구현완료(2026-08-15) — 페이지네이션이 아니라 접이식 그룹으로.** 백엔드가 "그 주 전량"을 의도적으로 주는 설계(서버가 자르면 화면이 거는 조건과 총 건수가 어긋난다, 코드 주석에 명시)라 진짜 페이지네이션은 API 계약을 흔든다 — 대신 이미 있던 "담당자 카드 클릭→그 사람만 필터"(회의 흐름 3단계 재설계의 일부)와 같은 정신으로, `MyTickets.jsx::GroupedTickets`에 `collapsible` prop을 신설해 그룹을 접은 채 시작하고 회의에서 한 사람씩 펼치게 했다(`SidebarNav` 그룹 토글과 같은 패턴, `<th scope="colgroup">`/`<h3>` 의미는 유지한 채 안에 진짜 `<button>`을 얹음). `Sprint.jsx`의 "담당자별 티켓" 목록에만 켰다 — `collapsible` 기본값은 false라 나머지 세 소비처(내 티켓·미할당·팀 티켓)와 `Sprint.jsx`의 "계획 논의" 목록은 기존 동작 그대로. 신규 회귀 `grouped-tickets-collapsible.test.jsx` 4건(기본값 무변화·접힌 채 시작·그룹별 독립 토글·좁은 화면 카드 목록도 동일) + `sprint-redesign.test.jsx` 기존 3건을 접힌 상태를 펴는 단계 추가로 갱신(PersonCard와 그룹 토글 버튼의 접근성 이름이 겹치는 것도 같이 잡음), revert-to-verify 확인, 관련 티켓 화면 스위트 56건 green. 실제 11,558px가 얼마나 줄었는지는 다음 Chrome E2E에서 실측 |
...
| VIS-66 | **High** | **`UA-02`의 시각적 확인** — 이 화면이 바로 전사 **담당자별 생산성**을 차트와 표로 그리는 곳이고, API 실측에서 `role=user`가 이 데이터를 **200으로 받는다**는 것을 확인했다(17명 전원의 완료율·지연·배정). 즉 유출은 이론이 아니라 **화면으로 이미 그려지고 있다** ‖ ✅ **`UA-02`(2026-08-10) 수정으로 해소 — 코드 직접 재확인.** 이 화면이 의존하는 `sprints/service.py::_visible_ids`를 다시 읽으면 `scope.is_dept` 특수취급이 없고 `visible_user_ids(db, build_scope(db, viewer))` 하나만 쓴다(UA-02가 고친 그대로) — org 범위 관리자·평범한 `role=user` 둘 다 이제 필터링된다. 남는 예외는 **부서가 없는 계정**의 `GLOBAL_SCOPE` 폴백뿐인데, 이는 UA-02 자신이 "신규입사자 온보딩" 의도된 설계로 명시하고 그대로 둔 부분이다 — 이 화면의 유출은 그 예외가 아니라 dept 범위 게이트 누락이 원인이었으므로 해당 안 됨. 실서버 재검증은 UA-02와 같은 이유로 격리된 음성 사례를 못 만들어 `test_org_axis.py`(합성 two-org)로만 확인됨 — 다음 Chrome E2E 때 이 화면도 함께 훑는다 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-58 | **High** | **6,014px 길이의 표에 고정 헤더가 없다.** 한 페이지가 100행(`MAX_PAGE_SIZE`)인데 스크롤하면 열 제목이 사라져 **30행쯤부터 자기가 무슨 열을 보는지 알 수 없다**. 감사 로그는 "언제·누가·무엇을·결과"를 대조하는 화면인데 그 대조가 불가능해진다 ‖ ✅ **구현완료(2026-08-15, 커밋 `2b0aa45`)** — `kit.jsx::DataTable`에 선택적 `stickyHeader` prop 신설(MUI `Table stickyHeader` 위임 + 헤더 셀 불투명 배경 명시), `DataScreen.jsx`가 `config.stickyHeader`를 전달, `registry/governance.js`의 `audit` 설정에서 켬. 이 앱 상단바(`AppBar position="fixed"`)와 `#main-content`가 분리된 스크롤 컨테이너라 `top:0`가 상단바와 안 겹치는 것을 코드로 확인. `ui-ux-pro-max` 스킬로 offset 보정·virtualize-vs-paginate 판단 확인(이미 pageSize 100 페이지네이션이 있어 virtualize 불필요). 신규 시험 2건, revert-to-verify 확인, 프런트 전체 261파일/1,785건 green. **실브라우저 스크롤 확인은 다음 배포 사이클의 Chrome E2E에서**(VIS-64/PA-01 폰트 배치와 함께 통합 재배포 예정) |
| VIS-59 | Med | **로그인/로그아웃 잡음이 화면을 지배한다** — 같은 `대상 ID`의 "사용자, 로그인"·"사용자, 로그아웃"이 수십 행 연속으로 이어진다. 묶음·접기·중복 축약이 없어, 실제로 봐야 할 사건(빨강 `실패` 2건, `설정, 수정`, `team_docs.body 수정`)이 그 사이에 묻힌다 ‖ ✅ **부분 구현완료(2026-08-17)** — 원 서술이 제안한 "묶음·접기"(연속 로그인/로그아웃 행을 하나로 접는 UI)는 표 자체의 그룹핑 기능을 새로 만드는 큰 작업이라 이번 스코프가 아니다. 대신 **더 직접적인 해법**을 택했다: `exclude_actions` 서버 필터(`app/audit/router.py::_filtered_stmt`, `action`과 반대 방향 — 하나만 골라 좁히는 대신 지정한 것만 뺀다) + 화면의 새 "표시 범위" 드롭다운("로그인/로그아웃 제외")으로, 접어서 숨기는 대신 아예 안 불러온다(표를 더 짧고 가볍게 만든다는 점에서 목적은 같다). 목록·CSV 내보내기 둘 다 같은 조건을 쓴다(0033 계약). 필터를 안 켜면 예전과 100% 동일 — 새 사건 유형이 잡음이 되면 화면 쪽 옵션 값(쉼표로 구분된 action 목록)만 늘리면 된다. 신규 시험 5건(백엔드 2 + 프런트 3), revert-to-verify 확인함. 표 자체의 그룹핑·접기 UI는 여전히 다음 후보로 남는다 |
| VIS-60 | Med | **100행 전부 `대상 ID`가 잘린 UUID**(`aa683445-5fd3-4c25-bb54-…`)다. `VIS-11`·`VIS-29`와 같은 문제인데 **100배 규모**라, 넓은 열 하나가 통째로 사람이 못 읽는 값으로 채워진다 ‖ ⚠️ **재확인(2026-08-17) — `VIS-11`/`RG-07`과 "같은 패턴"이 아니다, 훨씬 큰 작업이다.** `VIS-11`은 이미 링크였던 원소의 **스타일**만 고쳤고(hand-rolled `<a>` → MUI `Link`), `RG-07`은 **백엔드가 이미 이름을 내려주는데** 프런트가 안 쓰던 자리만 고쳤다(`documents.requested_by_name` 등). 이 행의 `object_id`(감사 로그 `governance.js:229`)는 **다르다** — `object_type`에 따라 가리키는 엔티티 종류가 20개 이상(user/department/workflow/prompt/policy/integration/schedule/…, `OBJTYPE_OPTS`)으로 갈리는 **다형적** id다. `app/audit/router.py::_serialize_row`(147행)를 확인한 결과 `object_id`에 대응하는 `object_name` 같은 필드 자체가 없다 — `actor_name`(행위자, 항상 user 하나)처럼 단일 타입 조회 하나로 되는 게 아니라, `object_type`별로 다른 테이블을 조회하는 백엔드 작업이 새로 필요하다. 프런트 전용 세션 하나로 끝날 일이 아니다. 다만 프런트만으로 하는 **부분 완화**는 가능하다 — 이미 "관련 항목 보기" 액션이 쓰는 `OBJ_ROUTE`/`OBJ_ID_PARAM`/`objRouteHref`(governance.js:356-370)로 `object_id`를 텍스트 대신 그 엔티티 화면으로 가는 링크로 바꿀 수 있다(이름 해석은 아니지만 최소한 클릭 가능해진다) — 이 부분 완화조차 이번 배치에서는 손대지 않았다 |
| VIS-61 | Med | **"상세" 버튼이 100번 반복**되며 6,000px 페이지에서 가장 무거운 요소다. `DS-03`(표 내부 액션 표현 없음)이 밀도가 높아질수록 선형으로 나빠진다는 확인 ‖ ✅ **완결(교차 확인, 2026-08-16)** — `PA-RC-0023`(D-96)이 `kit.jsx`의 `DataTable`에서 `상세` 버튼 열(`__open` 합성 열 + `openButton()`)을 완전히 제거, 행 클릭/Enter·Space로 대체 — 반복되던 버튼 자체가 더 이상 없다 |
| VIS-62 | Low | 빨강 `실패` 2건이 초록 `성공` 98건 사이에 있어 **색만으로는 못 찾는다**. 필터에 `결과`가 있지만, 기본 화면에서 이상을 눈에 띄게 하는 장치(요약 줄·앵커)가 없다 |
| VIS-63 | Low | 마스코트 FAB 겹침 **5번째 화면**(`/audit`) ‖ ✅ **완결(교차 확인, 2026-08-16)** — `VIS-06`에 정리한 대로 `PA-RC-0019`+`0020`이 FAB 자체를 삭제, `PA-RC-0023`이 겹칠 대상이던 `상세` 버튼도 삭제 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-53 | **High**(정정) | **백업이 꺼진 채 20일이 지났는데 아무도 통보받지 않았다.** 설정 `자동 백업 일정 = 꺼짐` + 마지막 백업 20일 전 + `FN-09`대로 **백업 실패 알림 없음**(예외를 삼킨다). ※ **정정**: 처음엔 "각 화면이 조각만 말해 아무도 전체 그림을 못 본다"고 적었는데 **틀렸다** — `/diagnostics` 최상단 오류 상자가 `위험 마지막 백업이 오래됨(20일 전)`을 **한 자리에 모아서 말한다.** 남는 진짜 문제는 **아무도 그 화면을 열어 보라는 신호를 못 받는다**는 것이다(알림 없음). 즉 "정보가 없다"가 아니라 "**밀어 주는 채널이 없다**" ‖ ✅ **정정(2026-08-15): 이 항목을 쓸 때 `RSTR-03`(2026-08-11, 커밋 `6d591e9`)이 이미 같은 Root Cause를 해결한 것을 놓치고 중복 조사했다.** `app/backups/service.py::backup_health_alert_reason()`(꺼짐/한 번도 성공 못함/`BACKUP_STALE_ALERT_DAYS`=7일 정체 셋 다 판정) + `check_backup_health()`가 `worker_main.py`의 기존 10분 백업 틱에 이미 배선돼 있고, `_recently_alerted_backup_health()`가 `BACKUP_ALERT_COOLDOWN_HOURS`(24시간) dedup까지 이미 구현한다 — 상세는 `RSTR-03` 행 참조. **이 재조사가 실제로 찾은 새 결함 하나**: `AppShell.jsx`의 `BADGE_TYPES`에 `backup_failed`가 등록돼 있지 않아 알림함·메일은 가는데 사이드바 배지는 안 떴다(RSTR-03도 이 등록을 놓쳤다 — 정확히 RSTR-03 자신이 경고한 "형제 함수 누락" 패턴이 배지 등록에서 재발). `jobFailed`/`approvalPending`과 같은 패턴으로 `backupFailed: ["backup_failed"]` + `BADGE_ROUTES["/backup"]`(`AppShell.jsx`) + `/backup` 항목의 `badge` 키(`navConfig.js`) 신설. 신규 회귀 `nav-badge-admin.test.jsx` 2건(작업 큐/승인/백업 배지 합산 확인 + 백업 정상 시 무배지), revert-to-verify 확인, 관련 배지 스위트(`nav-badge.test.jsx` 4건) green |
...
| VIS-55 | Med | **`설명` 열의 첫 문장이 `설정` 열과 글자 그대로 같다** — "대화 보존 기간(일)" / "대화 보존 기간(일). 초과 시…". 10행 전부 이 중복을 반복해 가로 공간을 먹는다 ‖ ✅ **완결(2026-08-17, D-115)** — `SettingsMain.jsx`의 `description` 열에 render를 추가: 설명이 실제 렌더된 라벨(`r.label`)로 시작하면 그 접두부 + 뒤따르는 마침표/콜론을 벗겨내고 나머지만 보여준다(라벨과 설명 소스가 프런트/백엔드로 갈려 있어, 대조 대상은 라벨 텍스트를 다시 계산하지 않고 실제 렌더값을 쓴다 — 둘이 어긋나도 안전하게 원문을 보여주는 쪽으로 fallback). 행 수는 원 서술(10행)이 아니라 현재 11행이지만 중복 자체는 대다수 행에서 재현 — 마침표(대화·알림·휴지통 보존 기간)·콜론(비밀번호·세션·잠금 정책) 두 계열 다 확인. 신규 시험 3건(`settings-description-dedup.test.jsx`: 마침표 계열, 콜론 계열, 안 겹치는 설명은 원문 유지) |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-50 | **High** | **4K에서 앱은 글자만 키우고 레이아웃 구성은 그대로다.** 3840 화면에서 빈 상태 블록(마스코트 + 안내 + 3단계)이 **가로 약 15%**만 쓰고 아래로 약 1,600px가 비어 있다. 1920 레이아웃을 그대로 확대한 모습이라 "4K를 지원한다"기보다 "4K에서도 안 깨진다"에 가깝다. `narrow_main` 검사가 통과한 것은 **main 컨테이너**가 전폭이기 때문이고, 그 안의 실제 콘텐츠 밀도는 검사 대상이 아니다 — 기계 검사와 사람 눈이 갈리는 또 하나의 자리 ‖ **조사완료(2026-08-15), 구현은 실측 확인 후로 — `VIS-52`(Med)와 같은 Root Cause.** `kit.jsx::EmptyState`(31개 파일 공유)의 `art` 이미지가 `{sm:160, xxl:200, uhd:240}`로 멈춰 있다(`BREAKPOINTS.uhd`=3000px, `CONTENT_MAX_WIDTH.uhd`=3040px인데 그림은 240px) — 600→3000px 5배 뷰포트 구간에서 삽화는 1.5배만 큰다. 텍스트(`maxWidth:"60ch"`)는 가독성 때문에 일부러 안 늘려야 맞지만, 삽화 크기·수직 padding(`py`, 지금은 breakpoint 무관 고정값)은 그래야 할 이유가 없다. **여기서 멈춘 이유**: 정확한 목표 px/비율은 "얼마나 커야 자연스러운가"라는 취향 판단이라 코드만으로 확정하기 위험하다 — `VIS-24`/`VIS-25`와 같은 이유로 실제 렌더링 없이는 손대지 않았다. **다음 세션 제안**: `EmptyState`/`ErrorState`(같은 패턴 공유) 양쪽의 `art` 폭에 `uhd` 다음 구간을 추가하거나 배수를 키우고, `py`도 `uhd`에서 breakpoint 객체로 늘려 다음 Chrome E2E(4K 포함)에서 직접 보고 조정한다 ‖ ✅ **구현완료(2026-08-15) — 실제 스크린샷으로 직접 확인 후 조정.** 이 세션 자체의 E2E 결과물(`dist/ui-qa/converge-pa15-4k/light/3840x2160/user_my-tickets.png`)을 `Read` 도구로 직접 열어 봤다 — 정확히 이 항목이 말한 그대로, 마스코트+안내 블록이 캔버스 왼쪽 위에 작게 몰려 있고 아래로 광활한 빈 공간이 실측됐다. `EmptyState`/`ErrorState` 둘 다 `art` 폭 `uhd:240`→`320`(기존 `sm:160`/`xxl:200`이 만드는 뷰포트 대비 8~9% 비율에 240이 못 미쳤다 — 그 비율에 맞춘 값), 세로 `py`도 `uhd`에서만 `{xs:6, uhd:14}`로 키움. **전체 뷰포트 높이를 채우는 실제 세로 가운데 정렬은 하지 않았다** — 이 컴포넌트가 31개 파일에 공유돼 모달·좁은 카드 등 고정 높이가 아닌 맥락도 많아, 그건 각 소비처의 레이아웃 문제지 이 컴포넌트 하나로 안전하게 못 고친다(코드에 이유를 남겨 둠). 관련 `kit.test.jsx` 33건 + 전체 `npm test`(264개 파일·1,797건) green |
| VIS-51 | Med | 사이드바가 4K에서 **상대적으로 더 좁아진다**(1920에서 화면의 13.8% → 3840에서 8.6%). rem 기반이라 루트 폰트(16→20px) 비율로만 커지는데 뷰포트는 2배가 되기 때문이다. 의도된 동작이지만 **결과적으로 4K에서 내비게이션이 잔글씨 띠처럼 보인다** — `DS-32`(절대 px 두 곳)와 겹쳐 체감이 더 나빠진다 ‖ ✅ **실측 확인 — 실제로는 문제가 아니다(2026-08-17, TEST SERVER 3840×2160 실측)** — 수치(340px=8.9%) 자체는 원 서술과 일치하지만, "잔글씨 띠처럼 보인다"는 주관적 우려를 실제 스크린샷으로 직접 검증하니 **사실이 아니었다**: 아이콘·라벨·여백 모두 넉넉하고 또렷하게 읽힌다(`dist/vis51_check/dashboard_4k.png`, 로컬 전용). 이 항목이 함께 지목한 `DS-32`(절대 px 글자 크기)는 이미 별도로 완결됐다(MEGA CYCLE C). 코드 주석(`AppShell.jsx:55-56`)이 우려한 "화면의 7%" 시절(고정 284px 하나만 쓰던 옛 버전)의 문제가 이미 단계적 확대(264→300→340px)로 실제로 해소됐다는 뜻 — "화면 대비 %가 줄어든다"는 수학적으로는 맞지만, 내비게이션 사이드바는 VS Code·Slack·Notion 등 실제 전문 소프트웨어 대부분이 그렇듯 **뷰포트에 비례해 커질 필요가 없다**(메뉴 라벨 길이가 화면 커진다고 늘어나지 않는다) — 오히려 percentage 고정을 강행하면 4K에서 사이드바가 530px까지 부풀어 본문 콘텐츠 공간을 부당하게 뺏는다. 코드 변경 없이 종결 |
| VIS-52 | Med | 빈 상태가 **남는 공간을 전혀 쓰지 않는다** — 삽화도 안내도 1920과 같은 크기다(`EmptyState` art가 `uhd:240px`에서 멈춘다). 4K에서 정보 밀도를 올릴 기회가 규칙으로 설계돼 있지 않다 ‖ ✅ **이미 해결됨(교차연결, 2026-08-17)** — `VIS-50`과 같은 근본 원인·같은 커밋으로 함께 고쳐졌다(`kit.jsx`의 `EmptyState` art 폭 `uhd:240→320px`, 코드 주석이 `// VIS-50\VIS-52`로 둘을 명시). `VIS-50` 행만 ✅ 표시가 붙어 있었다 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-44 | Med | **`DS-09`의 실물 확인** — 문서 유형 배지에서 `매뉴얼`(초록)·`회의록`(파랑)·`작업 계획서`(주황)는 색이 나오는데 **`참고자료`가 회색으로 떨어져 `기타`와 구분되지 않는다**. 이 화면 20장 중 13장이 `참고자료`라 **대부분의 카드가 유형을 색으로 말하지 못한다**. 원인은 `lib/badges.js`가 내는 `pink`가 `kit.Badge`의 `TONE_COLOR`에 없어서다 ‖ ✅ **이미 해결됨(교차연결, 2026-08-17)** — `DS-09`가 이 실물 확인 그대로 고쳤다: `kit.jsx`의 `EXTRA_TONE_VARS.pink`가 `--badge-pink-bg/fg`에 연결됐고 `lib/badges.js::DOC_TYPE_KIND["참고자료"]="pink"`가 이제 회색이 아니라 구분되는 색으로 렌더된다 |
| VIS-45 | Med | **20장 중 14장이 "작성자 없음"이다.** Notion 미러가 작성자를 못 채우고 있다는 뜻인데, 화면은 그것을 정상 상태처럼 조용히 표시한다("동기화가 작성자를 못 가져왔다"와 "정말 작성자가 없다"가 구분되지 않는다). `안내` 줄은 "마지막 동기화 성공"만 말한다 ‖ ⚠️ **원 서술이 부정확했다 — 실측 결과 "동기화 실패"가 아니라 "Notion 원본에 실제로 없음"(2026-08-17, TEST SERVER DB 직접 조회)** — `app/team_docs/notion_docs.py::_person_names`의 주석이 이미 폴백 구조를 밝히고 있었다: People 속성을 못 읽으면 빈 목록, 그러면 `owner`(수동 텍스트 필드)로 폴백한다 — 그리고 그 폴백은 **이미 프런트에 구현돼 있다**(`TeamDocs.jsx:359,552`: `(d.author_names||[]).join(", ") || d.owner || "-"`). "동기화가 작성자를 못 가져왔다"는 가설을 검증하려고 실제 DB를 조회했다: `document_cache` 107건 중 `author_names` 채워짐 37건, `owner` 채워짐 5건, **둘 다 빈 것 70건(65%)** — 원 서술의 70%(20장 중 14장)와 근접해 같은 현상. 37건이 `author_names`를 정상적으로 갖고 있다는 것 자체가 **통합 전체의 People 읽기 권한이 막힌 게 아님**을 보여준다(전체가 막혔다면 0건이어야 한다). 빈 70건을 샘플로 직접 확인하니 `last_edited`가 최근(2026-08-03)이고 제목 등 다른 필드는 정상 최신값 — **동기화 자체는 그 문서들의 현재 Notion 상태를 정확히 반영하고 있다**(오래된 캐시가 아니다). 결론: 이 70%는 "동기화가 놓친 것"이 아니라 **Notion 원본 문서 다수가 애초에 People도 owner 텍스트도 채운 적이 없다**는 데이터 희소성 문제 — `VIS-99`(실제 WD 2.4%만 기록)와 같은 패턴이다. 코드가 이미 할 수 있는 폴백(owner)을 다 하고 있고, 그 이상 구분할 신호가 Notion 응답 자체에 없다(People 빈 배열이 "권한 없음"과 "정말 0명"을 구별 안 해 준다) — 새 코드로 만들어낼 수 있는 구분이 아니다. "작성자 없음"이라는 화면 문구는 이 상태를 정직하게 말하고 있다고 판단해 코드 변경 없이 종결 |
...
| VIS-49 | Med | **FAB 겹침이 네 번째 화면에서 반복**(`/projects`·`/dashboard`·`/users`·`/team-docs`). 7화면 중 4화면이므로 **FAB 배치 규칙 자체를 고쳐야 한다** — 개별 화면 수정으로는 끝나지 않는다. **저장소가 이미 같은 함정을 세 번 밟았다** — `MyTickets.jsx:982-987` 주석이 그 기록이다: *"우하단 마스코트 FAB이 이 버튼을 덮는다… 같은 함정을 이 저장소가 이미 두 번 밟았다(놀이방 '보내기', AI 채팅 '전송')"*. 세 곳 모두 `pr: FAB_CLEARANCE`로 **개별 대응**했다. 내가 찾은 표 우측 액션 열은 **네 번째 이후**이고 아직 안 고쳐졌다 — 개별 패치가 아니라 규칙이 필요하다는 증거 ‖ ✅ **완결(교차 확인, 2026-08-16)** — `PA-RC-0019`+`0020`이 FAB 자체를 완전히 삭제해 배치 규칙 문제가 근본적으로 해소됐다(개별 `FAB_CLEARANCE` 패치가 아니라 FAB 삭제로 해결, `DECISIONS.md` D-98) |

### `/users` 사용자 관리 (1920×1080, light)
| ID | 심각 | 문제 |
|---|---|---|
| VIS-39 | **High** | **역할 배지 색이 의미를 나르지 못한다.** `감사자`와 `운영자`가 **같은 파랑**이라 색만으로 구분되지 않고, `시스템 관리자`는 **빨강**인데 이 제품에서 빨강은 위험·실패를 뜻한다(대시보드 `4 실패 작업 위험`, `20일 전`) → **최고 권한 계정이 오류처럼 보인다**. `DS-08`(색이 장식으로 쓰인다)의 가장 뚜렷한 실물 ‖ **부분 구현완료(2026-08-15)** — `system_admin` 배지를 `danger`(빨강)에서 `purple`로 바꿨다: `EXTRA_TONE_VARS`(kit.jsx)에 이미 다크모드·대비 검증까지 끝난 톤이라 새 작업 없이 상태 팔레트(성공/실패/경고/정보)와 안 겹치는 색을 얻었다. `감사자`/`운영자` 동일 파랑은 재검토 결과 **의도적으로 남겨 둠** — 배지 옆에 실제 역할 텍스트(`감사자`/`운영자`)가 항상 같이 보이므로 색만으로 구분해야 하는 상황이 아니고(색은 "관리자보다 아래인 권한 있는 역할"이라는 **등급**을 나타내고 텍스트가 **구체적 역할**을 나타낸다), WCAG "색만으로 정보 전달 금지" 원칙도 이미 지키고 있다. `Profile.jsx`·`registry/governance.js`도 역할을 보여주지만 배지가 아니라 텍스트/감사로그 값이라 이 배지-색 문제의 대상이 아님을 확인 |
| VIS-40 | Med | **관리자 12명 전원이 `관리자` + `전체 관리자` 배지 2개를 단다** — 24개 배지가 사실상 같은 말을 반복한다. 범위 배지는 org/dept로 좁혀진 사람이 있을 때만 정보가 되는데 **그런 사람이 0명**이라(`admin_scope` 전원 global) 지금은 순수 잡음이다 ‖ ⚠️ **재확인(2026-08-17) — 의도된 설계, "0명"은 오늘의 데이터 상태일 뿐이다.** `Users.jsx:67-86`(`adminConcept()`)의 주석이 이 배지의 존재 이유를 스스로 밝힌다: role=admin+admin_scope 조합(조직관리자/부서관리자/전체 관리자)이 백엔드엔 있었는데 화면 어디에도 이름이 없어 "관리자가 이 기능 자체를 찾지 못했다"는 RBAC 발견성 문제를 고치려고 만든 배지다. `전체 관리자`가 `warn` 톤인 것도 의도적이다("조직/부서로 좁혀지지 않은 관리자"라는 보안 관련 신호). `admin_scope`는 사용자 편집 폼에서 실제로 org/dept로 바꿀 수 있는 필드라(F2), 오늘 0명인 것은 이 서버 데이터의 우연이지 구조적 결함이 아니다 — 배지를 지우면 정확히 이 기능이 고치려던 발견성 문제가 되돌아온다. 안 고친다 |
| VIS-41 | Med | **첫 화면 최상단을 5줄짜리 안내 블록이 차지한다**(비활성화 vs 보관 차이, 승인 흐름, 임시 비밀번호 규칙). 매번 보는 사람에게는 영구 잡음이고, 처음 보는 사람에게도 표보다 먼저 읽히기엔 길다. 데이터가 화면 아래로 밀린다 ‖ ✅ **이미 해결됨(교차연결, 2026-08-17)** — `PA-RC-0022`(D-93)가 이 영구 블록을 `PageHeader`의 접이식 `help` 토글로 바꿨다(`Users.jsx:567-581`) — 기본은 접혀 있고 필요할 때만 펼친다 |
| VIS-42 | Med | **FAB이 우측 액션 열 위에 놓이는 것이 세 번째 화면에서 반복**(`/projects`·`/dashboard`·`/users`). 개별 화면 문제가 아니라 **FAB 배치 규칙**과 "액션을 우측 끝 열에 두는 표 규칙"이 구조적으로 충돌한다 ‖ ✅ **완결(교차 확인, 2026-08-16)** — `PA-RC-0019`+`0020`의 FAB 삭제로 충돌 자체가 사라졌다 |
| VIS-43 | Low | "상세" 버튼이 18행 반복되며 각 행에서 가장 무거운 요소다 — `DS-03`이 **손으로 쓴 화면(`Users`)과 registry 화면 양쪽에서** 동일하게 나타남을 확인. 즉 표 내부 액션 표현 부재는 전 제품 공통이다 ‖ ✅ **완결(교차 확인, 2026-08-16)** — `VIS-61`과 동일 원인, `PA-RC-0023`이 `DataTable` 공유 컴포넌트에서 `상세` 버튼 열 자체를 제거해 `Users`·registry 화면 전체에서 함께 해소 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-24 | **High** | **21개 타일이 전부 같은 카드다.** 6개 구역(확인 필요 1 · 지금 상태 5 · 작업 지표 4 · 현재 큐 2 · 인벤토리 3 · 시스템 리소스 4 · 내 업무 2)이 전부 흰 카드 + 큰 숫자 + 작은 라벨 + 꺾쇠다. **"4 실패 작업 위험"(지금 조치해야 함)과 "3 등록된 러너"(단순 재고)가 완전히 같은 무게**로 그려진다 — `DS-16`(카드 유형이 둘뿐)이 실제로 이렇게 보인다 ‖ **재검증(2026-08-13) — 절반은 이미 해소, 나머지는 시각 검증 없이 단독 착수 보류.** `headlineStats`(요약 5지표) 자체가 이 감사 하루 전(2026-08-07, `0a082f3`)에 신설된 재설계라 WF1 감사(2026-08-08)는 이미 이 구조를 보고도 문제로 봤다는 뜻 — 오래된 기록이 아니다. 실제로 `StatCard`의 `kind`(danger/warn/ok/미지정)로 심각도별 색 차등은 이미 있고("4 실패 작업"은 danger 빨강, "3 등록된 러너"는 인벤토리 섹션에서 항상 무채색), **섹션 자체도 이미 위계순**(경보→지금 상태→...→인벤토리, "급한 일이 먼저" 원칙이 주석에 명시)이라 "완전히 같은 무게"는 색·배치 축에서는 과장이었다. 남은 것은 **타일 형태(카드 크기·테두리·타이포)가 6개 구역에서 시각적으로 균일**하다는 지적인데, 이건 실제 렌더링을 봐야 판단할 수 있는 시각 디자인 문제라(코드만으로는 "충분히 다른가"를 판정할 수 없다) dev 서버+브라우저 없이 이번엔 손대지 않았다 — 다음 세션에서 실제 화면을 보고 판단 ‖ ✅ **실측 확인(2026-08-15) — 재검증(2026-08-13)의 결론이 맞았다, 코드 변경 없음.** 이 세션의 E2E 스크린샷(`dist/ui-qa/converge-pa15-4k/light/3840x2160/admin_dashboard.png`)을 `Read` 도구로 직접 열어 확인 — "확인이 필요한 항목"·"지금 상태"의 위험 타일(4건)은 빨간 숫자 + `위험`/`주의` 배지가 실제로 보이고, "인벤토리"의 "3 추가된 러너"는 무채색 검정 숫자로 배지가 아예 없다 — 색·배지 차등이 코드 주석뿐 아니라 실제 화면에서도 뚜렷했다. 남은 지적(카드 형태 자체는 6개 구역이 동일)도 스크린샷으로 확인되지만, 색·배지·섹션 위계가 이미 우선순위를 전달하고 있어 형태까지 바꿔야 할 만큼 급하지 않다고 판단 — 코드 변경 없이 종결 |
| VIS-25 | **High** | **같은 숫자가 한 화면에 세 번 나온다.** `4 미해결 실패 작업`이 "확인이 필요한 항목"·"지금 상태"·"현재 큐 상태"에 각각. `8.5% 디스크 사용`은 "지금 상태"와 "시스템 리소스"에 두 번. `2 활성 워크플로`도 "지금 상태"와 "인벤토리"에 두 번. **`7/7 서비스 정상`은 타일 + 서비스 7행 + 도넛 차트로 세 번** 표현된다. 화면을 훑는 사람은 이것이 서로 다른 지표인지 같은 것인지 알 수 없다 ‖ **재검증(2026-08-13) — 반복 자체는 사실이지만 의도된 설계다, "구별이 안 된다"는 우려는 이미 문구로 대응돼 있다.** 코드 직접 확인: 4건의 반복 예시가 전부 현재도 재현된다. 그런데 `headlineStats` 함수 자신의 주석이 이 반복을 **명시적으로 의도했다고** 밝힌다 — "예전에는 같은 값들이 여섯 구역에 흩어져 있어... 여섯 번 찾아야 했다... 이 줄은 요약이고 그쪽이 상세다." 그리고 "지금 상태" 섹션 바로 아래에 이미 `<Note>이 줄은 요약입니다. 값을 누르면 그 화면으로 내려가고, 자세한 항목은 아래 구역에 있습니다.</Note>`가 있어 WF1이 우려한 "이게 다른 지표인지 모른다"는 혼란을 문구로 직접 막고 있다. 대부분의 실제 대시보드(Datadog·Grafana류)도 "요약 KPI 줄 + 상세 구역" 구조를 쓴다 — 이 자체를 없애면 그 주석이 경고하는 "여섯 번 찾아야 했다" 상태로 되돌아갈 위험이 있어, 반복을 없애는 방향으로는 손대지 않았다. 코드만으로 판단하기 애매한 "그래도 시각적으로 헷갈리는가"는 VIS-24와 같은 이유로 실제 렌더링 확인이 필요해 보류 ‖ ✅ **실측 확인(2026-08-15) — 재검증(2026-08-13)의 결론이 맞았다, 코드 변경 없음.** VIS-24와 같은 스크린샷에서 직접 확인 — "이 줄은 요약입니다. 값을 누르면 그 화면으로 내려가고, 자세한 항목은 아래 구역에 있습니다." 문구가 실제로 "지금 상태" 타일 바로 아래에 렌더돼 있다(코드 주석·`<Note>` 존재만이 아니라 화면에 실제로 보임). 반복된 숫자(4/8.5%/2건/7-7)를 훑을 때 이 안내문이 바로 시야에 들어와 "다른 지표인가" 하는 혼란을 실제로 완화한다고 판단 — 코드 변경 없이 종결 |
...
| VIS-27 | Med | **구역마다 회색 각주가 따로 붙는다**(3개: "이 줄은 요약입니다…", "성공률은 최근 24시간에…", "티켓 소스를 읽지 못해…"). 타일에 붙지 않고 구역 아래에 떨어져 있어 어느 숫자를 한정하는지 시선으로 연결되지 않는다(`VIS-09`와 같은 패턴이 대시보드에서 3번 반복) ‖ ✅ **2/3 구현완료, 1/3 재확인 후 그대로 둠(2026-08-15)** — "성공률은 최근 24시간에…"는 네 타일 중 성공률 **하나에만** 해당하는데 전체 아래 떨어져 있어 진짜 오독 위험이 있었다 — `StatCard`의 새 `note` prop으로 성공률 타일 안에 옮겼다(`VIS-09`와 같은 커밋/같은 메커니즘). "티켓 소스를 읽지 못해…"는 `VIS-28`(자기모순)과 같은 자리라 그쪽에서 문구를 고쳤다. "이 줄은 요약입니다…"는 재확인 결과 **문구 자체가 이미 "이 줄"이라고 자기 지시** 하고 있어(대상이 특정 타일이 아니라 바로 위 줄 전체) 어느 숫자를 한정하는지 모호하지 않다 — 시각적 근접성(그리드 바로 아래, 사이 다른 요소 없음)도 이미 확보돼 있다. 이 인스턴스는 "타일별 note"로 옮길 대상이 아니라고 판단해 그대로 뒀다 |
| VIS-28 | Med | **"내 업무" 구역이 자기모순이다** — `3 차질 프로젝트 위험`·`0 지연 마일스톤` 타일을 보여 준 **바로 아래**에 "티켓 소스를 읽지 못해 내 업무를 셀 수 없습니다."라고 적는다. 셀 수 있다는 건지 없다는 건지 화면이 답하지 못한다 ‖ ✅ **구현완료(2026-08-15)** — 코드로 확인한 결과 모순은 표현의 문제였지 데이터 문제가 아니었다: `차질 프로젝트`·`지연 마일스톤`은 티켓이 아니라 프로젝트/마일스톤 소스라(`app/home/work.py`의 "장애 격리(§17.4)" 설계 — 주석에 이미 명시) 티켓 소스 장애와 무관하게 정상 집계된다. `WORK_UNKNOWN` 문구를 "내 업무를 셀 수 없습니다"(전체를 가리키는 것처럼 읽힘)에서 "내 미완료·이번 주 마감·지연 티켓은 셀 수 없습니다. 위 차질 프로젝트·지연 마일스톤은 다른 소스라 정상 집계됩니다."로 바꿔 무엇을 못 세고 무엇은 정상인지 명시했다. `dashboard-work.test.jsx`가 이제 `WORK_UNKNOWN` 상수를 직접 import해 정확한 문구를 고정(문구 재작성 시 테스트가 따로 안 갱신되는 드리프트 방지) |
| VIS-29 | Low | **"최근 주요 변경" 3행이 전부 같은 액션**("사용자, 비활성화")이고, 각 행의 **오른쪽 끝이 잘린 raw UUID**(`23681bfc…`)다. 가장 오른쪽 = 가장 늦게 읽는 자리에 사람이 못 쓰는 값이 있다 |
| VIS-30 | Low | 마스코트 FAB이 "현재 큐 상태" 카드를 덮는다 — `VIS-06`과 같은 문제가 **다른 화면에서 반복**된다(개별 화면이 아니라 FAB 배치 규칙의 문제라는 증거) ‖ ✅ **완결(교차 확인, 2026-08-16)** — `VIS-06`과 동일 원인, `PA-RC-0019`+`0020`의 FAB 삭제로 해소 |
...
| VIS-20 | Med | **한국어 단어 중간에서 줄이 끊긴다** — 사이드바 문구가 "지금 보 / 고 있는 화면을 기준으로 물어볼 수 있습니 / 다."로 쪼개진다. `KO_WORD_BREAK`가 이 자리에 적용돼 있지 않다 ‖ ✅ **이미 해결됨(재확인, 2026-08-17)** — 이 문구는 `ConversationSidebar.jsx:195`의 `EmptyState help`다. `kit.jsx:341`(`const bodySx = {..., ...KO_WORD_BREAK}`)가 이미 `help`(362행)·`title`(360행) 둘 다에 적용 중 — 코드 주석이 "31개 파일 전체의 빈 상태 안내문에 KO_WORD_BREAK가 빠져 있었다"고 스스로 기록한 바로 그 수정(§R1, line 2216 "EmptyState에 KO_WORD_BREAK 한 줄이면 31개 파일이 동시에 좋아진다")이 이미 반영된 뒤였다. 이 행은 그 수정 이전 관측이 남은 stale duplicate |
...
| UB-24 | Low | 공지 화면의 검색 상자가 **설정돼 있는데 안 그려진다** — `DataScreen.jsx:467 showSearch = config.searchable \|\| !config.paginated`이고 공지는 `paginated:true`에 `searchable` 미설정 → `searchFields`·`searchPlaceholder`가 죽은 설정. 제목으로 배너를 찾을 방법이 없다(백엔드에도 `q`가 없다) ‖ **구현완료(2026-08-13)** — 프런트는 `registry/platform.js`의 `announcements`에 `searchable: true` 추가(이미 있던 `searchFields`/`searchPlaceholder`가 그제서야 살아남). 백엔드는 지적대로 `q` 자체가 없어 검색창만 켜면 조용히 아무것도 안 거르는 상태가 됐을 것 — `app/announcements/router.py::list_announcements`에 `q` 쿼리 파라미터 추가, `notion_mapping/router.py::list_mappings`의 `q` 처리(대소문자 무시 부분일치)와 같은 모양으로 title/body를 검색. 신규 시험 1건, revert-to-verify 확인(되돌리면 `q`가 조용히 무시되고 전체 목록이 그대로 옴). **부수 발견·수정**: 기존 `announcement-window-kst.test.jsx`가 느슨한 `/제목/` 정규식을 썼는데 새 검색창의 `aria-label`("제목, 내용으로 검색")도 그 정규식에 걸려 다중 일치로 깨짐 — 폼의 실제 "제목" 입력칸만 가리키도록 앵커를 좁혀 정정(이 정정이 없었다면 이번 변경이 기존 시험을 회귀시켰을 것) | | 구현완료 |
...
| UB-26 | Low | 한 번도 성공한 적 없고 `error`도 아닌 미러는 **아무 안내도 안 낸다**(`router.py:69-78`) — 사용자가 빈 티켓 목록을 이유 없이 본다. 모듈 docstring이 깨겠다고 한 바로 그 상태이고, 판단에 쓸 `last_run_at`은 이미 로드돼 있는데 안 쓴다 | | ✅ **구현완료(2026-08-16)** — 정확히 원인대로였다: `app/tickets/router.py`의 `my_tickets`(139행)·`unassigned_tickets`(170행) 둘 다 이미 `_with_sync(...)`로 `sync` 블록(`last_run_at`/`last_success_at`/`ticket_count`/`error`)을 응답에 얹고 있었는데, **프런트가 그 데이터를 아예 안 그렸다** — 팀 티켓 화면(`TeamTickets.jsx`)은 이미 `TicketSyncBanner`로 "마지막 동기화: ..."를 보여주는데, 개인 범위(`MyTickets`/`Unassigned`, 둘 다 `frontend/src/screens/MyTickets.jsx`)만 "트리거 버튼이 필요 없다"는 이유로 배너 자체를 통째로 뺐었다(주석에 그 판단이 그대로 적혀 있었다) — 트리거 버튼 불필요와 상태 정보 불필요를 같은 것으로 잘못 묶은 것이었다. 이미 있는 `TicketSyncBanner`를 `canSync={data.can_sync}`(이 두 엔드포인트엔 `can_sync` 자체가 없어 자동으로 `undefined`→버튼 안 뜸)로 재사용해 두 화면 모두에 배선 — 새 컴포넌트도 새 백엔드 필드도 필요 없었다. 신규 회귀 2건(`tickets-list.test.jsx`, `team-tickets-sync.test.jsx`의 "can_sync:false면 안내는 뜨지만 버튼은 없다"와 같은 결), revert-to-verify 확인(배너의 `sync` prop을 `null`로 되돌리면 두 시험 다 정확히 예측대로 실패). 관련 스위트(그룹 티켓·필터·모달·밀도·대비 포함 11파일·196건) green. 원래 참조하던 `VIS-83`(같은 화면군의 "읽음 열이 죽었다"는 별개 주장)은 재확인 결과 이미 다른 이유로 낡은 기록이라 별도로 정정함(그 행 참고) |
...
| UB-28 | Low | `visible_user_ids`를 `IN (…)`로 인라인(`impersonation/router.py:164-166`) → 문서화된 ~1000 사용자 규모에서 SQLite 변수 상한(999) 초과. UA-24와 같은 부류 ‖ **구현완료(UA-24와 함께 처리)** — `IN`은 `NOT IN`과 달리 청크를 AND로 못 잇는다(교집합이 비어 아무 것도 안 남는다) — `or_(*(target_user_id.in_(batch) for batch in batched(ids)))`로 OR-이어붙이기를 써서 "합집합 중 하나"라는 원래 의미를 보존. **직접 짜다 잡은 회귀**: `visible`이 빈 집합일 때 `or_()`를 인자 없이 부르면(모든 배치가 없어 아무 조건도 안 생김) SQLAlchemy가 WHERE 절 자체를 빼 버려 "아무도 안 보임"이 "전체 노출"로 뒤집힌다(`or_()` 무인자 호출은 SQLAlchemy 자체가 지금도 deprecation 경고를 낸다) — 빈 집합을 먼저 걸러 `.where(False)`로 명시적으로 처리해 원래의 "빈 범위 = 아무 것도 안 보임" 의미를 지킴. 신규 시험(`test_org_axis.py`에 `two_orgs.org_b`에 551명을 넣고도 스코프가 그대로 지켜지는 것 확인), revert-to-verify 확인 | | 구현완료 |
...
| UB-30 | Low | `PATCH {"max_calls": null}`이 조용히 no-op(200 + 변화 없는 감사 행). `RollbackRequest.name`만 `max_length` 없음 ‖ **구현완료(2026-08-13)** — (1) `max_calls`는 DB에서 NOT NULL이고 null로 둘 수 있는 상태가 없다(0="차단", 무제한은 행을 지운다) — 명시적 null을 이제 `ValidationAppError`로 거부(note는 nullable이라 그대로 둠). (2) `RollbackRequest.name`에 형제 엔드포인트 `diff()`의 `Query(max_length=120)`과 같은 `Field(max_length=120)` 추가(Prompt/Policy.name의 DB 컬럼과 같은 값). 신규 시험 2건, 되돌린 코드에서 각각 재현 확인(null이 200으로 조용히 통과·121자 name이 검증 없이 404까지 흘러감) | | 구현완료 |
...
| UA-04 | Med/High | **문서 "재시도"가 만들어 둔 수정 경로를 안 쓰고 옛 막다른 길을 그대로 쓴다.** `documents/router.py:148`의 `/{id}/retry`는 프런트 호출자가 **0건**이고, `registry/automation.js:283`은 여전히 `/documents/generate`를 부른다. 백엔드 docstring이 "생성 폼을 다시 여는 방식은 idempotency 중복으로 막다른 길이었다(round30 감사 E High)"라고 적어 둔 바로 그 방식이다. 운영자가 같은 기간을 다시 넣으면 409 "이미 생성된 문서입니다" | FN-07과 동일 뿌리, 근거 보강 | **발견, 배치6에서 의도적으로 보류(2026-08-10)** — 프런트(`registry/automation.js`) 변경 + vitest + 번들 재빌드가 필요해 이번 배치(백엔드 전용)와 범위가 다르다. 코드 확인: 프런트의 "재시도" 버튼은 실은 새 기간으로 폼을 다시 여는 우회(주석에 의도적이라 적혀 있음)이지 진짜 재시도가 아니다 — 같은 기간을 다시 재시도해야 하는 경우(예약 실행 실패 재시도)는 여전히 막혀 있다. 별도 프런트 배치로 미룸 ‖ **재검증 완료·구현완료(2026-08-16)** — 배경 agent의 whole-BACKLOG 재감사가 "FN-07과 실제로 같은 결함인지, 특히 '같은 기간 재시도' 시나리오까지 커버하는지 확인 안 됨"으로 지목한 것을 직접 확인했다. `registry/automation.js:329-330` 재확인: `path: (r) => "/api/admin/documents/" + r.id + "/retry"` — 여전히 `/retry`를 부른다(옛 `/generate` 우회 없음). `app/documents/service.py::retry_generation` docstring이 UA-04가 우려한 바로 그 시나리오("같은 기간 재시도")를 정확히 겨냥해 설명한다 — 새 레코드를 안 만들고 **같은 레코드·같은 `idempotency_key`(`docgen:{generation.id}`)를 재사용**해 다시 큐에 넣으므로 구조적으로 409 idempotency 충돌이 날 수 없다(새 생성일 때만 나는 충돌이지 재시도 경로엔 애초에 그 코드 경로 자체가 없다). `tests/security/test_document_generation_scope.py::test_a_scoped_admin_can_still_retry_their_own_team`가 실제로 202+`pending`+잡 큐잉까지 확인하는 성공 경로 시험으로 이미 있다. **UA-04는 FN-07의 수정으로 완전히 닫혔다** — 별도 조치 불필요 |
...
| UA-10 | Med | **`GET /api/trash`가 무제한**이다. `list_items`가 전 행을 가져와 파이썬에서 거르고(사용자 전 행 스캔 추가), `Trash.jsx:41`이 **15초마다 폴링**한다. 양쪽 다 페이지네이션이 없다 | | ✅ 구현완료(2026-08-13) — 배치6이 보류했던 바로 그 반쪽짜리 위험(백엔드만 자르거나 프런트만 파라미터를 붙이면 항목이 말없이 사라지거나 아무 효과가 없다)을 피해 백엔드·프런트를 한 배치로 같이 배선했다. `repository.list_visible(db, scope, *, limit=100)`이 범위 필터링을 **먼저** 끝낸 뒤 자르고 `(항목, 전체 개수)` 쌍을 돌려주며(순서를 바꾸면 SQL 단계에서 먼저 잘라 실제로는 더 있는데 상한보다 적게 도는 조용한 손실이 생긴다), 라우터가 `?limit=`(기본 100·최대 1000, `Query(ge=1, le=...)`)을 받아 응답에 `total`을 싣는다. `Trash.jsx`는 AI-18(대화 목록)과 같은 "더 보기가 상한을 늘려 처음부터 다시 받기 + `keepPreviousData`" 패턴으로 `hasMore`/`loadMore`를 배선(오프셋 이어붙이기는 그 사이 복원·영구삭제로 항목이 바뀌면 중복·누락이 생겨 채택 안 함). 15초 폴링·ETag/304는 그대로 살아있고 `limit`은 URL 쿼리라 브라우저 조건부 캐시 키에도 자연히 포함돼 충돌 없음. `total`이 범위(scope) 판정 **이후** 값이라 남의 팀 개수가 새지 않는지까지 보안 테스트로 확인(`test_trash_scope.py`). 신규 시험 8개(백엔드 `test_trash_api.py` 3개 + 보안 1개, 프런트 `trash-load-more.test.jsx` 3개 + 기존 계약 테스트 URL 패턴 갱신), revert-to-verify 확인함(되돌리면 백엔드는 `KeyError: total`, 프런트는 목업이 새 `?limit=` URL을 못 잡아 빈 상태로 렌더) |
...
| UA-17 | Med | **감사 이상징후가 창 전체를 파이썬으로 끌어온다**(`audit/anomalies.py:127-135`, `limit` 없음). `MAX_WINDOW_HOURS=720`이고 화면에 "최근 30일" 선택지가 있어, 30일치 전 행을 `before_json`/`after_json` 포함 ORM 객체로 적재 + 기준선 질의로 30일 더 ‖ **구현완료(2026-08-13)** — 행 자체를 자르는 것(`.limit()`)은 다섯 규칙이 창 안 **전체**를 봐야 정확한 카운트/임계값 판정이 나오므로 정확성을 깬다(UA-24/UB-28의 "SQL IN 청크" 패턴과 달리 여기서는 자체가 아니라 자체 크기가 문제). 다섯 규칙이 실제로 읽는 컬럼은 `user_id`/`action`/`result`/`created_at`/`object_type`/`object_id` 여섯 개뿐인데(diff 본문은 어느 규칙도 안 읽는다) `select(AuditLog)`가 `before_json`/`after_json`(설정·사용자 변경 diff를 통째로 담는 무제한 Text)까지 매번 30일치 전량 적재했다 — 그 여섯 컬럼만 고르는 `select(AuditLog.col, ...)`로 교체(기준선 질의는 원래도 2컬럼만 썼음). SQLAlchemy 2.0의 `Row`도 ORM 인스턴스처럼 `.user_id`/`.action` 속성 접근이 되므로 다섯 규칙의 기존 코드는 한 줄도 안 바뀌었다. 신규 시험 2건(`tests/unit/test_audit_anomalies_query_shape.py`) — 나가는 실제 SQL 문자열에 `before_json`/`after_json`이 없고 여섯 컬럼은 있는지 직접 확인(파이썬 쪽 판단이 아니라 wire-level SQL로 증명) + 컬럼을 줄여도 판정 결과(예: `failure_burst`)가 그대로인지. revert-to-verify 확인함(되돌리면 캡처한 실제 SQL에 `before_json`/`after_json`이 그대로 들어 있어 첫 시험이 정확히 그 이유로 실패) | `audit/anomalies.py:120-131` | 구현완료 |
...
| UA-19 | Low | `POST /backups/{id}/verify`가 `running` 행을 막지 않는다(막는 것은 프런트 `when`뿐). 백업 도중 호출하면 `file_missing`으로 행이 잠깐 `failed`가 된다. `reap_stuck_running` docstring은 API가 막는다고 단언한다 ‖ **구현완료(2026-08-13)** — `router.py::verify()`에 `row.status == STATUS_RUNNING` 검사 추가, 409로 거부(`ConflictError`). 신규 시험으로 되돌린 코드에서 실제 재현 확인 — `running` 행을 검증 호출하니 200과 함께 `status`가 `failed`로 격하됐다(파일이 `/tmp`의 가짜 경로라 `file_missing`이지만, 실제로도 아직 다 안 쓰인 파일이면 같은 실패 모드). 복구 후 409 + 상태 불변 재확인 | | 구현완료 |
| UA-20 | Low | 부모 부서를 지울 때 **하위 부서 경고가 없다**(사용자 수만 본다). FK가 `SET NULL`이라 자식이 루트로 올라온다 — 3단 트리가 클릭 한 번에 평탄해진다 ‖ **구현완료 — `UA-20R`(§2332, High로 상향돼 같은 문제를 더 정확히 재서술)이 이미 이 문제 자체를 고쳤다.** `app/org/service.py::bulk_child_department_count()` + `frontend/src/screens/registry/org.js:101,104`의 `child_department_count` 확인 문구 재확인(행 정정, 2026-08-13) — 이 행은 UA-20R과 중복이라 별도 작업 불필요 | 구현완료 |
| UA-21 | Low | 깊이 상한(32) 절단을 **순환으로 보고**한다(`org/tree.py:78,105-107`) → 순환이 없는 트리에 `상위 관계 오류` 빨간 배지가 뜨고 관리자에게 부모를 고치라고 안내한다 ‖ **구현완료(2026-08-13)** — 신설 `_is_in_cycle(row_id, by_id)`가 `parent_id`를 따라 올라가며 **실제로 자기 자신에게 돌아오는지**만 독립 판정(깊이 상한과 무관). `build_rows()`는 emit 루프 전에 전체 행에 대해 `in_cycle = {row.id: _is_in_cycle(...) for row in rows}`를 미리 계산해 두고 `_emit()`은 이 dict만 읽는다 — 처음엔 `cycle` 여부를 재귀 파라미터로 넘기는 설계로 시도했으나, 그러면 순환의 첫 방문 노드만 `cycle:true`가 되고 같은 순환의 나머지 노드는 `_children_map()`이 순환을 모른 채 평범한 자식으로 재귀 진입해 `cycle:false`로 남는(다중 노드 순환의 절반만 표시) 버그가 자체 시험에서 드러나 사전 계산 dict 방식으로 재설계함. 신규 시험 3건(`tests/unit/test_org_tree_depth_vs_cycle.py`) — 상한보다 깊은 37단 정상 사슬은 `cycle` 없음, `parent_id` FK 제약 때문에 insert 후 UPDATE로 만든 진짜 2노드 순환은 둘 다 `cycle:true`, 두 시나리오가 섞여도 서로 다르게 판정. revert-to-verify로 되돌린 원본 코드가 신고된 버그(깊은 트리를 순환으로 오판)와 자체 발견한 버그(다중 노드 순환의 절반만 표시) 둘 다 실제로 갖고 있었음을 확인 후 복원(원래 코드에도 두 번째 버그가 이미 있었다는 뜻) | | 구현완료 |
| UA-22 | Low | 죽은 값들: `home/aggregate.py:96 done_total`(소비자 0, 유일하게 창이 없음) · `blocked` 버킷(팀채팅이 켜져 있으면 도달 불가 UI인데 매 요청 계산) · `work.py`의 `in_scope`/`today`/실패 사유(화면이 일반 문구로 덮어써 "토큰 미설정"과 "조회 실패"가 구분 안 됨) · `llm_console/service.py:197 verified`(항상 False, 화면은 하드코딩 배지) · `documents/router.py:78-87`이 N+1 피하려 붙인 `requested_by_name`을 화면이 **안 쓰고 UUID를 그린다** ‖ **재확인(2026-08-13) — 5개 중 2개는 이 행이 낡았다, 3개는 재확인해도 여전히 유효**: ①`requested_by_name`은 이제 `registry/automation.js:290`의 `personField(...)`가 실제로 소비한다(`RG-07`에서 이미 해결) — 이 행이 그 정정을 안 받았을 뿐. ②`llm_console/service.py:197`의 `verified:False`는 `LlmConsole.jsx:202`가 **정직한 하드코딩 배지**("확인 안 함")로 그린다 — 항상 False인 게 숨겨진 결함이 아니라 "아직 이 값을 검증하는 기능이 없다"는 사실을 그대로 보여주는 의도된 표시다, 버그 아님. ③`done_total`(`home.test.jsx`의 fixture에만 있고 실제 렌더/단언 소비처는 여전히 0건 확인) · `blocked` 버킷 · `work.py` 문구 3개는 재확인 결과 아직 그대로다 — 별도 조사·수정 필요(미착수) | | 부분구현완료 |
| UA-23 | Low | `home/aggregate.py:126`이 `t not in cancelled`로 dict 값 비교를 O(n²) 수행(500티켓·50취소 = 2.5만 회 깊은 비교), `work.py`가 같은 리스트로 4회 더 호출 ‖ **구현완료(2026-08-13)** — `cancelled`는 애초에 `status == 취소`로 걸러 만든 목록이라, "그 목록에 없다"의 부정은 별도 집합 없이 그냥 같은 조건(`status != 취소`)을 뒤집으면 된다(O(n²)→O(n), 새 자료구조도 안 만든다). `work.py`의 4개 호출은 이 함수(`sprint_progress`) 하나를 공유해서 부르는 것뿐이라(`assigned`/`done`만 읽고 `est_wd_total`은 안 씀에도 매번 전체가 계산됨) 별도 수정 불필요 — 공유 함수 한 곳만 고치면 전 호출부가 함께 빨라진다. 순수 성능 리팩터라 출력이 안 바뀌었는지가 핵심 — 신규 시험 `test_sprint_progress_excludes_cancelled_est_wd_from_total`(취소 티켓의 큰 견적치가 여전히 제외되는지 고정) 통과 확인. revert-to-verify 대신(출력이 old/new 동일해 그 방식으론 차이가 안 드러남) 500티켓·50취소 규모에서 old/new 구현을 같은 프로세스 안에서 직접 시간 비교 — 2.0배 빨라짐(실측) | `home/aggregate.py:126` | 구현완료 |
| UA-24 | Low(→ 정확성 문제로 `PERF-01`이 재분류, 규모 무관하게 남김) | `readers.py:81` `trashed_page_ids`를 `notin_()`에 통째로 넣어 휴지통이 커지면 SQLite 변수 상한(999)으로 **홈 전체가 500** ‖ **구현완료** — `app/home/readers.py::recent_documents()`가 `trashed`를 `app/core/db.py::batched()`(신설, `ID_BATCH_SIZE=500`)로 청크 내 `NOT IN`을 반복 AND(`x NOT IN A AND x NOT IN B` ≡ `x NOT IN (A∪B)`)로 이어 붙이게 수정 — 의미 그대로 보존, 청크 수만 늘어난다. 같은 청크 헬퍼가 `app/core/retention.py`에 이미 사설(`_batched`)로 있던 것을 확인해 공용으로 승격(3번째 소비처가 생겨 중복 제거 문턱을 넘음), `retention.py`는 그 공용 함수를 재사용하도록 갱신. 신규 시험 `test_home_recent_documents_large_trash.py`(휴지통 550건 — `ID_BATCH_SIZE` 초과 — 크래시 없이 정확히 걸러짐 확인), revert-to-verify(4개 소스 파일 되돌리면 두 신규 시험 다 `ImportError`로 기대대로 실패 → 복구 후 재확인). 같은 뿌리인 `UB-28`(임퍼소네이션 이력의 `IN`)도 같이 고침 | 코드 직접 확인(2026-08-13, 배경 조사 에이전트가 재발견) | 구현완료 |
| UA-25 | Low | 휴지통 일괄 실패 토스트가 원인을 **전부 "권한이 없어"로 뭉갠다**(실제로는 이미 처리됨·권한·**Notion 보관 실패** 3종). Notion 장애 때 관리자가 다른 계정으로 재시도한다 | | ✅ 구현완료(2026-08-13) — WORK_STATE.md는 "API 계약 변경 필요"로 적어 뒀지만 실제로는 **불필요했다**: `trash/service.py`의 `restore_bulk`/`purge_bulk`는 이미 건별 실제 사유(`failed[].error` — "이미 처리된 항목입니다."·`AppError.message`·"노션 보관처리에 실패했습니다...")를 돌려주고 있었고, `Trash.jsx`의 `bulkMsg`만 그 값을 안 보고 "권한이 없어"를 하드코딩했다. 같은 뿌리를 저장소 전체에서 찾으니(CLAUDE.md §4) `MyTickets.jsx::useBulkTrash`·`TeamDocs.jsx`의 문서 선택삭제도 **글자 하나까지 똑같이** 하드코딩돼 있었다 — 셋 다 같은 실수의 독립 발생. `lib/format.js`에 `bulkFailureNote(failed)`(실패 건수 + 첫 사유, 사유가 섞여도 첫 번째만 — `UsersBulk.jsx`가 이미 쓰던 관용과 통일)를 신설해 세 곳 모두 교체. 신규 시험 9개(헬퍼 단위 6개, 휴지통 일괄 복원/영구삭제 각 1개 — 이미 처리됨·Notion 실패 두 사유가 실제로 다르게 뜨는지, 문서 선택삭제 1개), revert-to-verify 확인함(되돌리면 헬퍼는 `TypeError: not a function`, 화면은 실제 사유 텍스트가 끝내 안 나타나 타임아웃). `MyTickets.jsx`는 무거운 전체 페이지 하네스(useAuth 등)가 필요해 전용 회귀는 안 만들고 헬퍼 단위 시험 + 기존 인접 스위트(무회귀 확인)로 대신함 |
| UA-26 | Low | 오프보딩 방 소유권 인계가 **비활성/보관 계정도 후보로 받는다**(`service.py:573-586`) → 이미 퇴사 처리된 계정이 방장이 되어 그 함수가 막으려던 상태가 된다 ‖ **구현완료(2026-08-13)** — "가장 오래된 다른 멤버"를 고르는 조회가 `room_id`/`user_id`만 보고 대상 계정 자체의 로그인 가능 여부는 안 봤다. `User`와 조인해 `active.is_(True)`·`archived_at.is_(None)`인 멤버만 후보로 남기게 수정 — `successor`가 비활성이어도 같은 필터로 자동 제외돼 별도 검사 없이 다음 활성 멤버로 넘어간다. 신규 시험 2건(가장 오래된 멤버가 비활성이면 건너뜀 · 유일한 다른 멤버가 비활성이면 "받을 사람 없음"과 같이 그대로 둠), revert-to-verify 확인(되돌리면 비활성 계정이 실제로 방장이 되는 것을 재현) | `app/team_chat/service.py::transfer_owned_rooms` | 구현완료 |
| UA-27 | Low | 이상징후 `new_actor_action`의 `count`가 **이벤트 수가 아니라 액션 종류 수**인데 화면은 한 "건수" 열로 그린다 ‖ **구현완료(2026-08-13)** — 형제 소견 4종(`failure_burst`/`volume_spike`/`off_hours`/`critical_action`)은 전부 원시 이벤트 목록의 길이를 쓰는데 `new_actor_action`만 `sensitive_fresh`(집합, 중복 제거된 액션 종류)의 길이를 썼다 — 같은 새 동작을 5번 해도 "건수" 열엔 1. `first_at`/`last_at`용으로 이미 계산해 두던 실제 이벤트 목록 `matching`으로 `count`를 맞춤(`evidence`는 "어떤 새 동작들이었나"를 보여줘야 하므로 종류 목록 그대로 둠). 신규 시험 1건(같은 새 동작 3회 → count=3 확인), 되돌린 코드에서 count=1로 재현 확인 | | 구현완료 |
| UA-28 | Low | `llm_console/service.py:143-151` `_source_of`가 `0`·bool 저장값을 `env`로 오분류. `provider.py:219-221`의 backend 오타는 화면에 "꺼짐"으로만 보여 "설정값이 잘못됨"과 구분 안 됨 ‖ **구현완료(2026-08-13)** — 재확인 결과 실제 결함은 원 서술보다 다르고(더 확실히) 재현됐다: `EDITABLE_KEYS` 6개는 전부 `string` 아니면 `int`(bool 아님, `_llm_concurrency`가 bool을 명시적으로 거절) 타입이라 "bool 저장값" 자체는 도달 불가능한 경로였다. 진짜 살아 있는 결함은 `llm_max_concurrency`(레지스트리 기본값 1 — **0이 아니라 참인 정수**)였다 — 예전 판정("bool 아닌 정수이고 0이 아니면 settings")이 **아무도 안 건드린 새 설치에서도 기본값 1이 그 조건을 통과해 항상 "settings"로 잘못 나옴**(엣지 케이스가 아니라 상시 오탐). 값의 타입/참값을 다시 추측하는 대신 `effective_settings()`가 이미 계산해 둔 `is_default`(저장 행이 있고 그 값이 레지스트리 기본값과 다른가)를 그대로 재사용 — `SettingEditor.jsx` 등 다른 설정 화면과 같은 신호를 쓰게 됐고, `llm_timeout_seconds`의 "0=env를 따름" 센티널 규약(`tenant_config.py::_override_is_set`)과도 저절로 맞아떨어진다(그 값 자체가 기본값이라 `is_default`가 이미 참). 백엔드 오타 쪽은 그대로 유효 — `resolve_config`가 `backend not in {cli,api}`면 안전하게 `enabled=False`로 접는데(의도된 안전장치) 화면은 그 결과만 보여 "사용 여부를 껐다"와 구분이 안 됐다. `overview()`에 `backend_invalid` 필드를 추가하고 `LlmConsole.jsx`가 그 값이 참일 때 "백엔드 값이 올바르지 않아 꺼진 상태" 경고를 따로 그리도록 배선. 신규 시험 7건(백엔드 5·프런트 2) — 미저장 설치의 `llm_max_concurrency`가 env로 나오는지·명시적으로 저장한 값이 settings로 나오는지·"끔" 저장이 settings로 유지되는지·잘못된/정상 backend 값의 `backend_invalid` 플래그·프런트 경고 노출 조건. revert-to-verify 확인함(소스 2파일만 되돌리면 새 시험 3건이 정확히 예측한 증상 — 미저장 설치가 "settings"로 오탐, `backend_invalid` 키 자체가 없어 `KeyError` — 로 실패, 나머지 4건은 이미 old 코드에서도 우연히 맞던 경우라 그대로 통과) | `llm_console/service.py:143-151,166-202`, `llm/provider.py:218-221`, `LlmConsole.jsx` | 구현완료 |
...
| DOC-03 | Low | `OPERATIONS.md:36`이 "웹에는 서비스 재시작 API가 없다"고 하는데 `KNOWN_LIMITATIONS.md` §6은 `#/system`에서 privhelper로 5개 유닛을 재시작할 수 있다고 정정했다 ‖ **구현완료(2026-08-13)** — `OPERATIONS.md:36-37`을 정정. SSH 명령은 그 5유닛(`clovirone-web-assistant`·`clovirone-web-worker`·`nginx`·`systemd-timesyncd`·`systemd-resolved`, `app/sysops/actions_service.py::MANAGED_UNITS` 확인) 밖의 서비스나 직접 접속이 필요할 때 쓰는 경로라고 명시하고, 그 다섯 개는 `#/system`(system_admin 전용)에서 특권 헬퍼로 재시작 가능하다고 `KNOWN_LIMITATIONS.md` §6을 가리키게 함 — 코드 변경 없음, 문서만 | `OPERATIONS.md:36-37`, `app/sysops/actions_service.py:32-38` | 구현완료 |
| DOC-04 | Low | `NEXT_SESSION_PLAN.md`(2026-07-29)의 A·E 항목은 이미 배송됐다 ‖ **구현완료 — 이미 반영돼 있었다(재확인, 2026-08-13)**: `docs/NEXT_SESSION_PLAN.md`가 이제 첫 줄부터 "(사용 중지) — WORK_STATE.md로 옮겼다"로 시작하고 A·E 항목이 배송됨·나머지는 BACKLOG.md로 이관됨을 명시한다 — 이 행이 요구한 정정이 이미 다른 시점에 끝나 있었다 | `docs/NEXT_SESSION_PLAN.md` | 구현완료 |
| DOC-05 | Low | `WORK_PLAN_INDEX.md`(2026-08-03)가 5일치 작업만큼 낡았다 | | 작업예정 |
| DOC-07 | Med | **`ARCHITECTURE.md`·`KNOWN_LIMITATIONS.md`·`RUNBOOK.md`가 D-118(대화형 워커 레인 분리) 이전 "worker는 항상 하나"인 세상을 그대로 서술한다.** 특히 `RUNBOOK.md` §1("Worker가 멈춤 — 채팅 응답이 계속 처리 중")은 채팅이 멈췄을 때 확인/재시작할 유닛으로 `clovirone-web-worker`(배치)만 안내하는데, 대화형 레인이 켜진 설치(TEST SERVER 포함)에서는 채팅이 `clovirone-web-worker-conversational`에서 돈다 — 이 문서를 그대로 따르면 정상인 배치 워커를 보며 정작 죽은 유닛은 못 찾는다. 부수적으로 같은 절이 stuck-job 타임아웃을 "10분(600s)"라 적고 있는데 실제 기본값은 `jobs/repository.DEFAULT_RUNNING_TIMEOUT_SECONDS=3900`(65분)이다 — 레인 분리와 무관한 별도의 기존 오기 | whole-product 재감사(Explore 에이전트, 2026-08-17) 발견, `app/jobs/repository.py:31`·`app/core/config.py:126`·`deploy/systemd/clovirone-web-worker-conversational.service` 교차 확인 | ✅ **구현완료(2026-08-17)** — 세 문서 모두 정정: `ARCHITECTURE.md`는 프로세스 표에 대화형 레인 행 추가 + 왜 분리했는지 설명, `KNOWN_LIMITATIONS.md` §2는 "기본값은 여전히 단일 worker, 옵션으로 켜면 아니다"로 재서술(둘 다 동시 writer라는 것과 짧은 전용 stuck-job 타임아웃 명시), `RUNBOOK.md` §1은 대시보드/설정으로 어느 유닛인지 먼저 판별하는 단계를 추가하고 레인별 실제 타임아웃(배치 3900초/대화형 840초)으로 정정, §6(DB locked)도 대화형 레인이 켜진 경우 동시 writer가 정상이라는 전제 보강. 코드 변경 없음, 문서만 |
| DOC-06 | Low | `~/.claude/plans/flickering-percolating-clover.md` §E(~90항목)가 **부분적으로 이미 해소됐다** — 예: `PF11 nginx gzip 없음`은 서버에 gzip이 이미 켜져 있고(`Content-Encoding: gzip` 확인), `Z5 MIN_FTS_CHARS`는 LIKE 폴백이 있다. **항목별 실측 없이 신뢰하면 안 된다** | 직접 확인 | **재확인(2026-08-16), 이 행 자체가 결론** — 파일은 지금도 존재한다(저장소 밖, `~/.claude/plans/`, 이 프로젝트의 tracked 문서가 아니다). ~90항목을 이번에 전부 재실측하는 것은 이 Low 항목 하나에 들일 노력 대비 과하다고 판단했다 — 그리고 이 행이 이미 내리고 있는 결론("항목별 실측 없이 신뢰하면 안 된다")이 바로 이 세션 내내 실제로 지켜 온 방법론이다(오늘도 BACKLOG.md의 수십 건을 문서만 보고 넘기지 않고 소스·TEST SERVER 실측으로 재확인했다). 새로 할 일이 남은 게 아니라 이미 실천 중인 지침이라 종결한다 |
...
| SYS-10 | Low | **`/notion-console`이 같은 경고를 한 화면에서 두 번 한다.** 스프린트 DB 필드 밑의 "포털과 팀이 서로 다른 것을 스프린트라고 부르는지 확인할 수 없습니다"와, 화면 맨 아래 별도 "스프린트 진단" 카드의 같은 내용. 아래 카드는 위 카드의 버튼("연결 테스트를 눌러 보세요")을 앵커 없이 가리킨다 ‖ **이미 구현완료(행 정정, 2026-08-13)** — `frontend/src/screens/NotionConsole.jsx`에 `SYS-10:` 코드 주석과 함께 이미 고쳐져 있었다: 행에는 짧은 이정표("아래 "스프린트 진단" 카드를 확인하세요")만 남기고 실제 설명은 진단 카드에만, 진단 카드의 안내는 실제 `href="#notion-test-button"` + `scrollIntoView`/`focus` 클릭 핸들러로 그 버튼에 스크롤+포커스한다. `notion-console.test.jsx`의 전용 시험 2건(중복 미노출 확인 + 클릭 시 스크롤/포커스 확인) 재실행으로 재확인(green, 15/15 파일 전체도 green) — 이 행 자체만 갱신 안 돼 있었다(WF8-3·SEC-12/13과 같은 자기모순 패턴) | 코드 재확인(2026-08-13) | 구현완료 |
| SYS-11 | Low | **`/notion-console`의 칩이 상태와 출처를 같은 시각 언어로 섞는다.** `서버 환경변수`(초록, = 값의 **출처**)와 `설정 안 함`(주황, = **상태**), `설정됨`(초록, = 상태)이 같은 칩 슬롯에 온다. 초록 "서버 환경변수"는 건강 판정처럼 읽힌다 ‖ **이미 구현완료(행 정정, 2026-08-13)** — 같은 파일에 `SYS-11:` 코드 주석과 함께 이미 고쳐져 있었다: 상태 칩(`설정됨`/`설정 안 함`, `ok`/`warn`)과 출처 칩(`화면에서 설정함`/`서버 환경변수`, 항상 `neutral`)을 완전히 분리. 전용 시험 1건("설정 여부 칩과 출처 칩을 따로 그린다") 재실행으로 재확인(green) — SYS-10과 같은 자기모순 패턴 | 코드 재확인(2026-08-13) | 구현완료 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-113 | High | **관리자 내비가 5그룹 37항목**(운영 14 · 사용자 7 · 연동 5 · 콘텐츠 4 · 자동화 7). 1080 높이에서 **약 절반만 보인다**(캡처 4장 모두 `사용자 > 직책 관리`에서 잘림). `/llm-console`·`/notion-console`에 있는 동안 **활성 항목이 화면 밖이라 "내가 어디 있는지"를 보여 주는 표시가 하나도 없다** — 브레드크럼이 "관리자 › 연동"이라고만 한다. `/system`·`/setup`은 운영 그룹이라 하이라이트가 보이고, 그 대비가 문제를 더 뚜렷하게 만든다 ‖ ✅ **부분 구현완료(2026-08-12)** — "표시가 하나도 없다"를 고쳤다: `AppShell.jsx::SidebarNav`가 라우트 변경 시 활성 항목을 목록 안으로 자동 스크롤한다(`scrollIntoView({block:"nearest"})`, `prefers-reduced-motion` 존중 — `kit.jsx`의 폼 검증 스크롤과 같은 패턴 재사용). 하이라이트(`aria-current`/`selected`)는 이미 있었다 — 스크롤이 안 따라가 안 보였을 뿐. **"37항목이 1080에 다 안 들어간다"(그룹 재편)는 이번 범위 밖** — `WORK_PLAN_INDEX.md` §3이 이미 "관리자 IA" 자체를 디자인 시스템 확정 이후의 별도 사이클로 잡아 뒀다(`VIS-114`/`VIS-115`와 같은 클러스터, 화면을 옮기는 결정이라 먼저 서야 하는 것이 있다). 신규 시험 2건(`sidebar-active-item-scroll.test.jsx`) — 스크롤 호출 확인 + 동작 최소화 시 `behavior:auto`, revert-to-verify로 ref 제거 시 두 시험 모두 정확히 그 증상으로 실패 확인. `AppShell.jsx`가 앱 전체 셸이라 프런트 전체(222파일/1514건) 재실행 green ‖ ✅ **남은 부분(그룹 재편) 완결(`PA-RC-0017`, 2026-08-16)** — 그때 범위 밖으로 미룬 바로 그 일이 나중에 별도 사이클로 실행됐다: `NAV` 8그룹 39항목 → 5그룹 35항목(운영/사용자와 권한/자동화/연동/감사, 전부 정확히 7항목)로 재편, TEST SERVER 실배포+실브라우저 16/16 확인. 독립 재확인(`navConfig.js` 직접 파싱): 현재 `NAV`가 정확히 5그룹·35항목. 상세: `BACKLOG.md` `PA2-06`, `DECISIONS.md` D-92 |
| VIS-114 | Med | **"운영" 그룹 14항목이 덤핑 그라운드다.** 대시보드·알림·작업 큐·설정·감사 로그·백업이라는 일상 운영 항목과, 진단·시스템 설정·초기 설정·유지보수·감사 이상 징후·복구 리허설이라는 **설치/인프라 항목**이 한 그룹에 섞여 있다. 뒤 6개는 `system_admin`·감사 성격이라 일상 운영과 사용 빈도가 자릿수로 다르다 | ✅ **완결(`PA-RC-0017`, 2026-08-16, 교차 확인)** — 8그룹 39항목을 5그룹 35항목(운영/사용자와 권한/자동화/연동/감사)으로 다시 짜면서 "운영" 그룹도 정확히 7항목으로 줄었다 — 진단·감사 이상 징후 등 설치/인프라 성격 항목은 "감사" 그룹으로, 나머지는 성격별로 재배치됐다(분류 근거는 `DECISIONS.md` D-92). 이 행이 지목한 "일상 운영과 설치/인프라가 섞인 덤핑 그라운드"라는 구조적 문제 자체가 재편으로 해소됨. 독립 재확인: `navConfig.js`의 "운영" 그룹이 현재 7항목. 상세: `BACKLOG.md` `PA2-06` |
...
| VIS-116 | Low | **클로비 진입점 2개가 동시에 보인다** — 좌하단 도킹 카드("클로비에게 물어보기", 내비 꼬리를 덮음 = VIS-104)와 우하단 플로팅 마스코트. 캡처한 `system_admin` 4화면 전부에서 동시 노출 ‖ 아래 "성격 변경"판(1269행)이 최신 판정이고 그쪽에서 구현완료 처리했다 |

### QA 하네스 자체의 결함 (조사 도구를 못 믿으면 조사 결과도 못 믿는다)

| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| QA-10 | High | **구현완료(2026-08-08, 행 정정 2026-08-13)** — 요약표가 `통과 0 / 실패 0 / 건너뜀 N` 인 검사에 **`← 한 번도 돌지 않음`** 표시와 `[주의]` 블록을 붙였다(`run.py`). ‖ 원래 문제: **`tiny_text` 검사가 실제 사용 뷰포트에서 한 번도 돌지 않는다.** `assertions.py:748`이 뷰포트 폭 `< 2200`이면 **skip**한다. 내가 이번 사이클에 돌린 캡처는 전부 1920 이하 → 67라우트 감사자 실행의 요약표가 `tiny_text 통과 0 / 실패 0 / **건너뜀 67**`로 나왔고, 이건 요약만 보면 "문제 없음"으로 읽힌다. 3840으로 6페이지를 다시 돌리자 **6/6 전부 실패**했다. 검사는 멀쩡하고 게이트가 문제다. `scripts/ui_qa/run.py:322`의 마커 문자열 재확인 | `dist/ui-qa-4k` 실행 | 구현완료 |
| QA-11 | Med | **구현완료(2026-08-08, 행 정정 2026-08-13)** — `discover_detail_hash` 가 401/403 을 **"이 계정 권한으로는 조회할 수 없습니다"** 로 구분해 적고 상태 코드를 남긴다(`capture.py`). 회귀 테스트 `tests/regression/test_ui_qa_harness_honesty.py`. ‖ 원래 문제: **하네스가 권한 거부를 "데이터 없음"으로 잘못 보고한다.** `c1-auditor` 실행 메모: `admin_job-detail (작업 상세) 건너뜀 — 표시할 데이터가 없어 상세 id를 찾지 못했습니다 (/api/admin/jobs)`. 실제로는 `auditor`가 `CONSOLE_OPS_ROLES`(operator/admin/system_admin)에 없어 **403**이다(`app/jobs/router.py:24-31`, `app/core/authz.py:67`). 역할 매트릭스 실행에서 이 오분류는 "이 역할이 못 본다"와 "기능에 행이 없다"를 구분 불가능하게 만든다 — **역할 조사에서 가장 알고 싶은 차이가 바로 그것이다.** `scripts/ui_qa/capture.py:248`의 마커 문자열 재확인 | | 구현완료 |
| QA-12 | Med | **구현완료(2026-08-08, 행 정정 2026-08-13)** — `Route.visible_to(role)` 를 추가하고, 실행 계정이 볼 수 없는 라우트는 **찍지 않고** `권한부족(미검사)` 메모 + `results.json` 의 `run.routes_out_of_reach` 로 남긴다. `qa-admin`(admin) 실행에서 system_admin 전용 4개가 정확히 제외됨을 확인. ‖ 원래 문제: **역할 커버리지 없는 라우트 커버리지는 허수다.** `qa-admin`(role=admin)으로 돌린 실행에서 `system_admin` 전용 4화면은 전부 권한 거부 화면으로 찍혔는데 하네스는 `ok`로 집계했다 — 21개 검사 전부 그 거부 화면 기준으로 통과한다. 화면이 아니라 **거부 배너**를 검사한 것이다. `route_inventory`에 `min_role`/`allowed_roles`가 이미 있으므로, 실행 계정의 역할로 볼 수 없는 라우트는 `ok`가 아니라 **`권한부족(미검사)`**로 표시해야 한다. `scripts/ui_qa/run.py:185,289`의 마커 문자열 재확인 | `results.json`의 `route_inventory` | 구현완료 |
| QA-13 | Low | `narrow_main`은 폭 `< 3840`에서 skip이라 3840 실행에서만 돈다(6/6 통과 확인). 게이트 자체는 의도대로지만 QA-10과 같은 요약표 착시를 만든다 — **skip과 pass가 요약에서 시각적으로 구분되지 않는다** | | ✅ 구현완료(2026-08-13) — `QA-10`의 `never_ran` 조건(통과 0·실패 0·건너뜀 N)은 narrow_main처럼 **일부만** 도는 검사는 안 잡는다: `통과 6 / 실패 0 / 건너뜀 60`은 통과·실패가 둘 다 0이 아니라 마커가 안 붙었다. `scripts/ui_qa/run.py`에 `check_marker(passed, failed, skipped)`를 신설(기존 인라인 로직을 그대로 함수로 추출) — `건너뜀 > 통과+실패`면 "← 대부분 건너뜀" 마커 + 별도 `[주의]` 블록을 붙인다(`never_ran`과 우선순위가 겹치지 않게 `never_ran`을 먼저 본다). 신규 시험 5개(`tests/regression/test_ui_qa_harness_honesty.py`— 이미 QA-10/11/12를 지키던 그 파일에 같은 "하네스 정직성" 절로 추가), revert-to-verify 확인함(되돌리면 `check_marker` import 자체가 없어 모듈 수집이 통째로 실패) |
...
| AI-59 | Med | **플랫폼은 러너 문맥을 못 보고 못 지운다.** `app/conversations/models.py`가 갖는 것은 `backend_conversation_id` 문자열뿐이고 실제 `mode`/`ticket_draft`는 n8n·러너 쪽에 있다. 그래서 AI-30 같은 상태를 **관리자도 사용자도 제품 안에서 초기화할 수 없다**(이미 기록된 "대화 삭제가 러너 사본에 전파 안 됨"과 같은 뿌리) ‖ **부분완화(MEGA CYCLE A의 TTL로 창이 24h로 제한됨), 초기화 기능 자체는 보류** — AI-16과 같은 근본 원인(플랫폼→러너 삭제/초기화 HTTP 경로 부재), 그 항목의 보류 사유를 그대로 공유한다 | | 보류 |
| AI-53 | High | **마크다운·코드블록 요청은 렌더 문제가 아니라 도달 자체를 못 한다.** 이전 기록은 "정규식 5개짜리 줄 분류기라 코드블록이 망가진다"였는데, 실측하니 **그 이전 단계에서 끝난다** — 규칙엔진이 질문을 티켓 검색으로 분류해 LLM 에 가지도 않는다. `pre`·`code`·`table` DOM 요소 **0개**. 즉 "마크다운 렌더러를 고친다"는 이 문제를 해결하지 못한다 | DOM 카운트 | ✅ **구현완료(2026-08-16)** — 직접 재현해 정확한 트리거를 확정했다: `is_query_intent`의 `query_markers`에 있는 "보여"(그 외 어떤 업무 낱말과도 무관)가 "파이썬 코드블록 예시를 보여줘"·"코드블록으로 예시 하나 보여줘" 같은 문장을 티켓 조회로 분류시켰다(`assistant.py`에서 직접 실행해 `is_query_intent(...)==True` 확인, 업무 낱말이 전혀 없는데도). `query_markers` 자체는 여전히 손대지 않는다(`AI-31` — 직접 좁히는 시도가 이미 세 번 회귀한 기록) — `AI-60`/`AI-61`이 쓴 것과 같은 검증된 패턴(교차 판정을 `route_request` 앞쪽에서 먼저 걸러 자유형 대화로 보냄)으로 새 가드 `is_code_example_request`를 신설했다: 코드 요청 낱말(`코드블록`/`코드예시`/`코드스니펫`/`스니펫`/`codeblock`)이 있고 `_WORK_NOUNS`(티켓/작업/할일/업무/이슈/일감)가 전혀 없을 때만 `claude_query`(LLM)로 직행한다 — 업무 낱말이 함께 있으면(예: "이 티켓에 코드 스니펫 첨부해줘") 기존 분류에 그대로 맡긴다. 신규 회귀 `test_code_example_requests_reach_the_llm_not_ticket_query`(`test_assistant.py`) — 가드 단위 판정 4건 + `subprocess.run` mock으로 실제 Claude CLI 호출 여부(`run.called`)까지 end-to-end 확인, revert-to-verify 확인(가드를 비활성화하면 `run.called`가 `False`로 정확히 예측대로 실패). 이 라우터가 과거 여러 번 회귀한 민감한 영역이라 `runner/claude-work-assistant`의 **전체 회귀(291건)**를 조기 실행해 전부 green 확인(CLAUDE.md §6 고위험 기반 변경 예외 적용). **문법 강조가 없다는 것과 마크다운 렌더러 자체의 한계(`AI-33`)는 이 수정 범위 밖** — 이번엔 "도달 자체를 못 한다"는 이 항목의 핵심 증상만 닫았다. 프런트 `RichText.jsx`의 `code` 블록 렌더는 이미 `AI-34`가 갖춰 뒀으므로, 이제 도달만 하면 그대로 렌더된다 |
| AI-54 | High | **스트리밍도 중단도 없다.** `aria-live="polite"` 영역이 **"답변이 도착했습니다."를 한 번** 알린다 — 진행 중 표시가 아니라 완료 통보다. 측정한 응답 시간 **5초 / 17초 / 20초**, 그동안 화면은 아무 진척을 보여 주지 않고 취소할 방법도 없다. 컴포저는 `rows=1` 단행, `maxLength=5000` | 실측 | **조사완료(2026-08-15), 두 갈래로 분리 — 스트리밍은 여전히 큰 작업, 취소는 생각보다 위험한 지름길이 있어 이번엔 보류.** ① **스트리밍**: `app/jobs/handlers/chat_message.py`가 잡 큐로 처리되는 구조를 확인(`AI-08`/`AI-11`이 고친 그 폴링 아키텍처와 동일) — SSE/WebSocket 같은 실시간 전송 계층 자체가 없다. LLM 러너 호출 방식(`app/core/http_client.py::OutboundClient` 경유, 청크 단위 응답을 프런트로 중계하는 경로 없음)까지 바꿔야 해 이 항목 하나로 처리할 범위가 아니다 — 원래 평가가 맞았다. ② **취소**: 처음엔 "job_id는 이미 `POST` 응답에 실려 온다(`app/chat/router.py:255`)니 새 사용자용 취소 엔드포인트 하나만 있으면 된다"고 생각했는데, 더 깊이 보니 **진짜 위험은 백엔드가 아니라 프런트**에 있다 — `useChat.js:483-485`의 `busy\|\|awaitingReply` 전송 차단 가드 자체가 "안 막으면 겹치는 러너 작업이 생겨 맥락이 뒤섞인 답이 나온다"는 이미 알려진 결함을 막으려고 있는 것이다. 사용자가 "취소"를 누르고 바로 새 메시지를 보내게 해 주려면 **이 가드를 풀어야** 하는데, 그러면 방금 취소한(그런데 서버에서는 여전히 실행 중일 수 있는, `cancel_queued`는 `STATUS_QUEUED`만 취소하고 `STATUS_RUNNING`은 취소 못 한다) 잡의 응답이 나중에 같은 대화에 "뒤섞인 맥락"으로 도착할 수 있다 — 이 가드가 막던 바로 그 버그가 취소 기능을 통해 재발한다. **다음 세션 제안**: (a) 러너가 실행 중인 잡을 실제로 중단할 방법이 있는지 먼저 확인(있으면 진짜 취소가 가능해지고 이 문제 자체가 없어진다), (b) 없다면 "취소"를 "이 응답은 무시하고 대화에서 숨긴다"는 뜻으로 좁혀 설계 — 전송 차단은 그대로 두되, 나중에 도착한 응답에 `dismissed` 같은 표시를 남겨 화면에 안 보이게만 하는 편이 안전하다 ‖ **(b) 설계 완료(2026-08-17, architect, D-118)** — 제안대로 "중단"이 아니라 "더 안 기다림(detach)"으로 확정: 아직 큐에 있으면 기존 `cancel_queued`의 CAS로 즉시 취소, 이미 러너에 나가 있으면 그 응답이 도착했을 때 **soft-delete로 남기고**(완전 폐기하면 n8n이 실제로 만든 부작용의 흔적이 사라진다) 컴포저는 "잡이 큐를 벗어난 적이 없다"가 증명될 때만 즉시 풀고 아니면 잠금 유지 — `useChat.js:540`(원 기록의 483-485는 그 사이 줄 번호가 밀렸다)의 가드 자체는 그대로 둔다. 상세 계약(어느 필드로 두 경우를 구분하는지, `handle_chat_message`의 어느 지점에서 취소를 다시 확인하는지)은 D-118 §4 참고. `job.status`가 `running`이면 못 막는다는 이 행의 핵심 진단은 그대로 유효 — 그걸 우회하지 않고 정직하게 설계했다 ‖ **(a) 확정(2026-08-16), (b)는 여전히 다음 사이클로 남김** — `app/jobs/handlers/chat_message.py::handle_chat_message`를 직접 읽어 확인: 워커가 `ctx.outbound_client.request(...)`로 n8n webhook을 **동기·블로킹**으로 호출하고(`app/jobs/worker.py`가 핸들러를 워커 프로세스 안에서 직접 실행, 불변규칙 §3-1 sync 일관성과 일치), 이 호출에 별도 중단 채널이 없다. `app/jobs/repository.py::cancel_queued`도 `WHERE status='queued'`로 CAS돼 있어 **의도적으로 `running` 잡은 취소 대상이 아니다**(설계 확인, 결함 아님) — job이 `running`으로 넘어가면 n8n이 응답할 때까지 플랫폼은 그 실행을 멈출 방법이 없다. **AI-08도 재확인**: `TypingBubble`의 경과시간 표시(`since` prop, 1초마다 재계산)는 이미 정확히 이 문제(진행 표시가 가짜)를 해결한 상태로 존재한다(2026-08-13 구현완료) — "진행 표시가 없다"는 이제 사실이 아니고, 남은 문제는 순수하게 "중단·동시 전송이 안 된다"뿐이다. **(b)를 이번에 구현하지 않은 이유**: 대화별 러너 상태(`assistant.py`의 `context`/`mode`/`pending_question`, `AI-30`이 다룬 그 상태)가 동시 요청에 안전한지 확인된 적이 없다 — "겹치는 러너 작업→맥락 뒤섞임"이 실제로 재발할 위험을 이번 세션에서 새로 검증하지 않은 채 가드를 푸는 것은 half-fix가 될 위험이 크다고 판단해, 이번엔 손대지 않았다. 진짜 (b)를 하려면 러너 쪽 동시성 안전성부터 확인해야 한다 — `AI-05`/`AI-06`/`AI-07`과 함께 "채팅 응답성" 아키텍처 사이클로 계속 묶어 둔다 |
...
| USE-05 | Low | **불리언 칩의 색이 "값"을 따르지 뜻을 따르지 않는다.** `예`=초록 채움 / `아니오`=흰 테두리로 고정이라, `활성: 예`(좋음)와 `승인 필요: 예`(제약)가 같은 초록이 된다. 열마다 초록의 의미가 뒤집힌다 | **재확인(2026-08-16), 근본 원인 확정 — 고치는 길은 이미 있음, 열별 판단이 먼저 필요** — `kit.jsx`의 공유 `Badge({value, kind})`가 원인: `kind`를 안 주면 `STATUS_KIND["true"]="ok"`(항상 초록)·`["false"]="neutral"`(항상 회색)로 무조건 떨어진다(`:88,100`). **고치는 메커니즘 자체는 이미 컴포넌트에 있다** — `Badge`는 호출부가 `kind`를 명시하면 그 값을 그대로 쓴다(`k = kind \|\| STATUS_KIND[raw] \|\| "neutral"`, `:146`). 즉 새 컴포넌트나 새 prop이 필요한 게 아니라, registry 각 파일의 불리언 열 정의마다 "이 열의 예/아니오 중 무엇이 좋은 뜻인가"(`활성:예`=ok, `승인 필요:예`=경고/제약일 가능성 등)를 하나씩 판단해 `kind`를 명시해야 한다 — 열 개수만큼의 개별 의미 판단이라 `PA-RC-0001`의 장꼬리 토큰 결정과 같은 성격(잘못 넘겨짚으면 오히려 잘못된 색을 붙인다), 이번엔 전수 판단을 내리지 않고 원인만 정확히 남긴다 |
| USE-06 | Low | **`web.sqlite3-wal`이 4.7MB로 본체 6.0MB에 육박한다.** WAL 체크포인트가 밀리고 있다는 신호이고, 이 상태에서의 백업·복구 동작은 확인된 바 없다(`backups` 1건, `restore_rehearsals` 0건) | **재확인(2026-08-16), 여전히 사실 — TEST SERVER 실측 갱신** — 방금 확인한 실제 값은 `web.sqlite3` 6,692,864바이트(6.4MB) 대비 `web.sqlite3-wal` 4,799,832바이트(4.6MB) — 원 관찰과 거의 동일한 비율로, **한 시점의 우연이 아니라 지속되는 패턴**임을 확인했다. `backups`/`restore_rehearsals`는 그새 각각 2건/1건으로 늘었다(`RSTR-02` 참고 — 완전히 0은 아니게 됨). **코드 조사로 원인 재평가(2026-08-16, 라이브 DB에 아무 것도 실행하지 않음, `app/core/db.py` 읽기만)** — `EngineOptions.pragmas()`(`:51-58`)를 확인한 결과 `journal_mode=WAL`·`busy_timeout`·`foreign_keys`·`synchronous=NORMAL` 넷뿐이고 **`wal_autocheckpoint`를 건드리는 코드가 없다** — 즉 SQLite 기본값(1000페이지 ≈ 4MB, 페이지 크기 4096바이트 기준)을 그대로 쓴다. 실측 WAL 4.6MB는 이 기본 임계값을 살짝 넘는 정도라 "체크포인트가 밀린다"기보다 **정상적인 자동(passive) 체크포인트 임계값 부근에서의 정상 진동**일 가능성이 높다. 더 중요한 것은 — **SQLite의 기본 자동 체크포인트는 TRUNCATE를 하지 않는다**(체크포인트된 프레임을 재사용 가능하게 표시할 뿐, 파일을 물리적으로 줄이지는 않음). 즉 WAL **파일 크기**만으로 "체크포인트가 안 되고 있다"고 단정하는 것 자체가 흔한 오해일 수 있다 — 실제로 체크포인트가 정상 작동해도 파일 크기는 과거 최고 수위에 머문다. 이 가설이 맞다면 원 지적은 결함이 아니라 SQLite WAL 모드의 정상 동작을 오독한 것이다. **다만 라이브 트렌드(시간에 따라 커지기만 하는지, 진동하는지) 없이 한 시점 스냅샷만으로는 완전히 확정할 수 없다** — 필요하면 `PRAGMA wal_checkpoint(TRUNCATE)`(안전한 표준 유지보수 동작, 다른 커넥션을 막지 않음)를 배포 파이프라인에 주기적으로 추가하는 것으로 파일 크기 문제 자체는 해소 가능하다(체크포인트 "정체"가 실재하든 안 하든 유효한 조치) |
| USE-07 | Low | 데이터 디렉터리에 `web.sqlite3.pre-seed-20260726-221510` / `…221530` 두 개가 7/26 이후 방치돼 있다. 백업 디렉터리가 아니라 **활성 데이터 디렉터리**에 있다 | **재확인(2026-08-16), 여전히 사실** — TEST SERVER `/var/lib/clovirone-web-assistant/`에 두 파일(각 1,142,784바이트, 7/26 22:15)이 지금도 그대로 있다. 3주 넘게 방치된 상태 — 삭제 자체는 위험이 낮지만(정식 백업이 아니라 pre-seed 스냅샷) 이 세션에서 확인 없이 지우지 않는다. 그대로 남김 |
...
| VIS-119 | Med | **KPI 6장이 4+2로 접혀 둘째 줄에 ~1,100px 빈 칸이 생긴다.** 카드 폭이 고정이라 6장이 4장 폭에 안 맞는다 ‖ 재확인만 하고 이번 배치에서는 손대지 않음 — 카드 그리드 열 수 조정은 `Home.jsx`의 "카드 수는 열 수의 약수여야 줄이 안 남는다"는 기존 규칙과 맞물려 있어(§46-53 주석), 이 화면 하나만 임의로 고치면 그 규칙과 다시 어긋날 수 있다. 다음 "구조 먼저" 스캔에서 KPI 그리드 클러스터로 함께 다룰 후보 ‖ ✅ **완결(2026-08-17, D-110)** — `DataScreen.jsx`의 요약 카드 그리드를 고정 `repeat(4,...)`에서 `repeat(auto-fit, minmax(14rem,1fr))`로 교체 — 카드 수와 무관하게 그 줄을 항상 채운다(빈 칸 자체가 안 생김). **첫 배포는 max를 18rem으로 고정했다가 TEST SERVER 실측(1920폭, /jobs 6장)에서 5+1로 접혀 오히려 더 나빠지는 것을 직접 보고 즉시 `1fr`로 정정**(D-110 정정 문단) — max가 가변이면 auto-fit의 열 수 계산이 min을 기준으로 삼아 6장 전부가 한 줄에 맞는다. `Home.jsx`의 고정 그리드는 그 화면 자신의 "카드는 항상 6장" 계약이 있는 별개 경우라 안 건드림. 신규 시험 2건+revert-to-verify, 프런트 전체 회귀 green, 재배포 후 재실측으로 빈 칸 0px 확인 |
| VIS-120 | Med | **6장 중 3장이 큐 내부 개념의 0이다** — `대기 0` · `실행 중 0` · **`실행 가능(ready) 0`**. `available_at <= now` 라는 워커 내부 상태를 관리자 화면의 최상단 지표 자리에 올려 놓았다. 정작 "평균 처리 시간"·"최근 24시간 실패" 같은 운영 지표는 카드가 없다 ‖ 재확인만 함 — 어떤 지표를 KPI로 남길지는 코드 버그가 아니라 제품 판단(운영자가 실제로 이 6장 중 무엇을 보고 판단하는지)이 필요해 다음 사이클로 미룸 ‖ ✅ **완결(2026-08-17, D-111)** — 큐 내부 개념 3장(대기·실행 중·실행 가능)은 그대로 둔다(다른 화면·워커 동작과 맞물린 계약이라 이번 범위 밖) — 대신 실제로 없던 운영 신호 2장을 추가했다: `queue_stats()`(`app/jobs/repository.py`)에 `recent_failed_24h`(최근 24시간 내 종료된 실패 건수)·`avg_processing_seconds_24h`(같은 창에서 성공한 건의 처리시간 평균, 최근 200건 상한 — 4초 폴링 화면이라 무제한 집계는 과함)를 더하고, `automation.js`의 `jobs.summary.cards`에 두 장을 이어붙였다. "0 vs 데이터 없음" 원칙을 지켜 평균은 집계할 성공 이력이 없으면 `0`이 아니라 `null`을 낸다(카드가 "-"로 그린다). 실패 카드는 `recent_failed_24h > 0`일 때만 danger 톤(0인데 빨갛게 잡음 내지 않음). 신규 백엔드 시험 3건(`test_jobs_repository.py`)+프런트 시험 5건(`registry-jobs-summary-cards.test.js`) 전부 green, TEST SERVER 배포 후 실측 확인. `VIS-121`(표 열 구성)은 이 카드 신호와 무관한 별개 판단이라 아직 열어 둔다 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-124 | Med | **720px 짜리 빈 달력**이 "이 구간 실행 0건, 예정 0건"과 함께 그려진다. 6×7 빈 상자 42개가 화면 대부분을 차지한다. 빈 상태가 "달력을 그리되 아무것도 없음"이지 "왜 없는지"가 아니다 ‖ ✅ **완결(2026-08-17, D-113, VIS-125와 함께)** — 아래 참고 |
| VIS-125 | Med | **왜 비었는지를 말해 주지 않는다.** 스케줄은 **1개 등록돼 있고 비활성**이다(`/schedules` 실화면). "예정 0건"이 아니라 "등록된 스케줄 1개가 꺼져 있어 예정이 없습니다 → 스케줄 화면 열기"여야 한다. **`/setup`이 이미 그 형식(상태+영향+행동)을 갖고 있다**(D-22) ‖ ✅ **완결(2026-08-17, D-113)** — `schedules.length>0 && counts.run===0 && counts.planned===0 && allDisabled`(등록된 일정 전부가 비활성)일 때 빈 격자 대신 `schedules.length===0`(이 화면에 이미 있던 "일정이 아예 없음" 분기)과 같은 `EmptyState` 관용으로 원인+행동(`situation`+`relatedLink`)을 보여준다 — D-22가 말한 "제품 안의 정답을 먼저 찾는다"를 그대로 따라 새 컴포넌트를 안 만들었다. "일정은 있지만 이번 달만 우연히 0건"(활성 일정이 하나라도 있는 경우)은 그대로 둔다 — 조용한 달일 뿐 결함이 아니다. 신규 시험 2건(`scheduler-calendar.test.jsx`: 전부 비활성일 때 안내+격자 없음, 활성 일정이 하나라도 있으면 격자 유지), revert-to-verify 확인 |
...
| VIS-122 확증 | High | **떠 있는 클로비가 마지막 행 「최초 실행 셋업 체크리스트 조회」의 `상세` 버튼을 실제로 덮고 있는 것이 이 스크린샷에 그대로 찍혔다.** `/jobs`에서 추론했던 것의 직접 증거 — 표 화면 전반에 적용된다 ‖ ✅ **완결(교차 확인, 2026-08-16)** — `VIS-122`에 정리한 대로 `PA-RC-0023`+`PA-RC-0019`/`0020`으로 겹칠 대상과 FAB 둘 다 제거 |
...
| ID | 심각 | 실측된 문제 |
|---|---|---|
| VIS-113 (**보강**) | High | **관리자 내비 37항목 중 16개(43%)가 1920×1080에서 화면 밖이다.** 900 높이에서 **20/37**, 768에서 **23/37**. 내비 콘텐츠 1,800px 에 보이는 높이는 846px 뿐 — 나머지는 스크롤해야 나온다. `/llm-console`·`/notion-console`처럼 **연동 그룹에 있는 화면은 활성 항목 자체가 처음부터 화면 밖**이라 "내가 어디 있는지"를 알 수 없다(실측: `외부 연동`·`Notion 관리`·`AI 관리`가 `top≈1144~1228`) ‖ ✅ **완결(교차 확인, 2026-08-17)** — 이 행이 지목한 것과 같은 근본 원인(37항목이 화면에 안 들어감)을 `VIS-113`(기본 행)이 이미 `PA-RC-0017`로 닫았다: `NAV` 8그룹 39항목 → 5그룹 35항목 재편, TEST SERVER 실측 16/16 확인. `navConfig.js` 직접 파싱으로 현재 5그룹·35항목 재확인 |
| VIS-116 (**성격 변경**) | Low | 가림 문제가 아니라 **중복 문제**다 — `/chat` 한 화면에 클로비 진입점이 3개(상단바 버튼 · 패널 우상단 마스코트 · 좌하단 도킹 카드). 이미 AI 도우미 화면 안인데 AI 를 부르는 입구가 셋이다 ‖ ✅ **재확인 후 구현완료(2026-08-15)** — 코드로 다시 확인한 결과 "상단바 버튼"과 "우하단 FAB"은 이미 `AppShell.jsx::onAssistant`(`pathname === "/chat"`)로 그 화면에서 숨어 있었다(주석: "그 화면에서는 아무 일도 하지 않으면서... 덮는다" — 이전 사이클에서 이미 고쳐진 것으로 보인다). **사이드바 도킹 카드(`MascotSidebarCard`)만 그 게이트가 빠져 있었다** — 클릭하면 이미 열려 있는 `/chat` 위에 같은 대화를 여는 드로어가 또 뜨는 막다른 진입점이었다. 형제 둘과 같은 `!onAssistant` 조건을 붙였다. 나머지(패널 헤더의 `MascotStatus`, 빈 스레드의 `Welcome` 마스코트, 결과 레일의 빈 상태 아이콘)는 재확인 결과 **"진입점"이 아니라 클릭 불가 상태 표시/빈 상태 아이콘**이라 그대로 둠 — 특히 `MascotStatus`는 과거에 "아무 상태도 안 전하는 장식"이었다가 실제 대화 단계(듣는 중/생각 중/답하는 중)를 전하도록 이미 고쳐진 것이라 되돌릴 이유가 없다. `VIS-155`(같은 화면의 "빈 상태 문구 3개 동시 노출")는 이미지 개수가 아니라 문구 중복이라 성격이 달라 이번 배치에서 안 건드리고 남겨 둠. 신규 시험 2건(`sidebar-mascot-hidden-on-chat.test.jsx` — /chat 에서 숨음 확인 + 다른 화면에서는 그대로 보임), revert-to-verify로 실패 재현 확인. `AppShell.jsx`가 전체 셸이라 프런트 전체(260파일/1,774건) 재실행 green |
...
| SRCH-02 | Med(→ 재분류) | **상단바가 입력처럼 생긴 버튼이다.** 폭 660px 의 둥근 상자에 placeholder 같은 회색 문구와 `Ctrl K` 배지가 있어 **모든 화면에서 텍스트 필드로 읽힌다.** 실제로는 `<button>`이고 클릭해야 진짜 입력이 있는 팔레트가 열린다. **클릭 후 타이핑은 정상이다**(실측: 클릭 → `회의` 입력 → 결과 9건). 즉 동작은 맞고 **어포던스만 거짓말**이다 — 입력처럼 생긴 것은 입력이어야 한다는 규칙을 어긴 유일한 자리 ‖ **재분류(2026-08-13) — 버그 아님, 의도된 기준선 설계.** `frontend/src/app/TopSearch.jsx:5-10`에 그대로 적혀 있다: "이미 그 일(키보드 이동, 그룹, 최근 항목)을 한다... 같은 기능의 **더 큰 표적**이다. 그래서 커서도 text가 아니라 pointer다(눌리는 것이다)." — 입력처럼 보이지만 버튼인 것이 실수가 아니라, 팔레트(더 강력한 그룹·키보드 이동·최근 항목)로 유도하려는 의도된 어포던스다. 사실관계(생김새·동작)는 이 행이 맞지만 "규칙을 어겼다"는 평가가 기준선 자체의 설계 판단과 배치된다 — 코드 변경 없음 | 재분류(버그 아님) |
...
| SRCH-04 | Low | **팔레트의 빈 질의 상태가 사이드바를 통째로 복제한다.** 아무것도 안 쳤을 때 현재 콘솔의 메뉴 전부(18개 또는 37개)를 경로와 함께 나열한다. 사이드바가 바로 옆에 열려 있는 상태에서 같은 목록을 모달로 한 번 더 보여 주는 셈이다. 최근 방문·자주 쓰는 것·추천 동작이 들어갈 자리다 ‖ **구현완료(2026-08-13)** — "자주 쓰는 것"까지는 과했다(집계·랭킹이 필요한 별도 기능) — 스코프를 "최근 방문"(VSCode Cmd+P·Slack Cmd+K와 같은 관용)으로 좁혔다. `lib/recentNav.js`(신설, localStorage, 서버 없음, 상한 6개)가 `AppShell.jsx`의 `loc.pathname` 변화마다 방문을 기록하고, `CommandPalette.jsx`의 빈 질의 분기가 그 목록을 현재 `groups`(이미 역할로 필터링됨)와 교차해 **지금도 유효한 메뉴만**, **지금 보고 있는 화면은 빼고** 보여준다 — 권한이 바뀌어 더는 못 보는 메뉴나 상세 경로(`/tickets/:id` 등, 메뉴 항목이 아님)는 교차 과정에서 자연히 걸러진다. "메뉴 › " 접두어(서버 검색 결과와 구분하려던 장치)는 애초에 메뉴 그룹이 아닌 "최근 방문"에는 안 붙인다. 신규 시험 7개(`recent-nav.test.js` 6개 + `command-palette.test.jsx`에 3개 추가·기존 2개는 새 계약에 맞게 재작성 — 빈 질의가 더 이상 전체 메뉴를 보여주지 않으므로), 프런트 전체 회귀(253파일·1718건) 무회귀 확인(공유 컴포넌트인 `AppShell.jsx`를 건드려 렌더하는 테스트가 13개 파일에 걸쳐 있음) | 구현완료 |
| SRCH-05 | Low | **콘텐츠 검색에 관리자 자산이 하나도 없다.** `notion`으로 검색하면 게시판 글 1건이 나오는데, 이름이 `notion-user-mapping`인 **워크플로**도 `Notion 관리` 화면도 `Notion 사용자 연결`도 `/search` 결과에 없다. `/search`는 티켓·문서·게시판·채팅만 색인한다 — 관리자에게 "이 설정이 어디 있더라"는 검색으로 답이 안 나온다 ‖ **LARGE-ARCHITECTURAL로 재분류(2026-08-13, `DECISIONS.md` D-69)** — 새 kind 추가 배선 자체는 이미 잘 닦여 있지만(`KIND_USER`가 이미 쓰는 `KIND_ROLE_GATE` 패턴 재사용 가능), `app/search/scoping.py`의 범위 판정(전역/조직/부서 세 갈래, 부서는 `owner_user_ids` fail-closed)에 "주인 없는 전역 자산"을 위한 네 번째 갈래가 없어 그대로 인덱싱하면 부서 범위 관리자에게만 결과가 안 보이는 새 결함을 만든다. 워크플로 목록도 프런트 전용 JS 레지스트리(`registry/automation.js`)라 백엔드가 읽을 원본 자체가 없다 — 스코프 모델 확장 + 소스 오브 트루스 결정이 먼저 필요한 설계 항목이라 이번 QUICK 수렴에서는 미루고 서술만 유지한다 | 보류 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-132 | **High** | **액션 우선순위가 뒤집혀 있다.** 헤더에 `목록`(무장식 텍스트) · `편집`(outlined) · **`원본 열기`(contained 파랑)** · **`삭제`(contained 빨강)** 네 개가 있고, **가장 크게 보이는 둘이 "앱을 떠나 Notion 으로 나가기"와 "지우기"**다. 정작 여기서 가장 자주 하는 일인 `편집`은 조용한 테두리 버튼이고, `목록`은 버튼처럼 보이지도 않는다. `DS`에 적어 둔 "primary 와 danger 가 둘 다 contained" 문제가 **제품에서 가장 많이 열리는 화면에서 그대로 드러난 사례** ‖ **구현완료(2026-08-15)** — `Ticket.jsx`: `수정`을 `primary`로, `원본 열기`를 `default`(outlined)로 맞바꿨다(`목록`=ghost·`삭제`=danger는 그대로). **같은 패턴을 쓰는 `TeamDoc.jsx`는 일부러 안 건드림** — 확인해 보니 그 화면은 인앱 `수정` 버튼이 아예 없어(Notion 소스 문서라 편집 자체가 Notion에서만 됨) `원본 열기`가 사실상 그 화면의 유일한 주 동작이다. Ticket.jsx는 인앱 `수정`이 진짜 있어서 그것과 경쟁하는 게 문제였다 — 두 화면이 겉보기엔 같은 코드 모양이어도 실제 상황이 달라 같은 수정을 강요하면 TeamDoc.jsx가 오히려 나빠졌을 것이다 |
| VIS-133 | Med | **페이지 제목이 이름이 아니라 ID 다.** H1 이 `GIT-57`이고, 사람이 읽는 제목 「앱 변경 신청 시, YAML 사라지는 버그 수정」은 카드 안쪽의 작은 제목으로 들어가 있다. 브라우저 탭·북마크·뒤로가기 이력에 전부 `GIT-57`만 남는다 — ID 는 찾는 열쇠이지 의미가 아니다 ‖ **부분 구현완료(2026-08-13, SEM-03과 함께) — H1만.** `Ticket.jsx`의 `PageHeader`에 `ticketId(t)` 대신 `t.title`을 넘긴다 — 화면 H1은 이제 실제 이름이다. `ticketId`(GIT-57)는 사라지지 않고 메타 '티켓 번호' 행으로 이동. `ticket-detail.test.jsx` 신규 시험, revert-to-verify 확인. **브라우저 탭 제목도 구현완료(2026-08-15)** — 그때 적어 둔 그대로 `setBrand()`와 같은 모듈 전역+구독자 패턴으로 `documentTitle.js`에 `setItemTitle(pathname, text)`/`itemTitleFor()` 신설, `titleForPath()`가 등록된 항목 제목을 나브 라벨보다 우선한다. `Ticket.jsx`(`t.title`)·`TeamDoc.jsx`(`doc.title`)·`BoardPost.jsx`(`post.title`)·`Chat.jsx`(기존 `activeTitle`, `PageHeader`와 동일 조건 재사용) 4화면 전부 배선. pathname이 등록 당시와 다르면 조용히 무시하도록 해서, 사용자가 이미 다른 화면으로 옮긴 뒤 도착하는 이전 화면의 지연 응답이 지금 탭 제목을 덮지 않는다(느린 API 경합 방어). 신규 시험 `document-title-item-override.test.jsx`(2건, 이 경합 케이스 포함) + 기존 `document-title-brand-async.test.jsx`(1건)와 나란히 유지, revert-to-verify로 지연 응답 무시 로직이 실제로 막는지 확인(그 한 줄을 되돌리면 정확히 예측한 대로 떠난 화면의 옛 제목이 새 화면을 덮음). 프런트 전체 259파일/1,771건 green |
| VIS-134 | Med | **속성이 두 군데로 갈라져 있고 규칙이 없다.** 오른쪽 `속성` 카드에는 프로젝트·담당자·마감 3개만 있고, **상태(`완료`)와 우선순위(`높음`)는 왼쪽 본문 카드 맨 위에 라벨 없는 칩 두 개**로 있다. `높음`이라는 칩만 보고 그것이 우선순위인지 난이도인지 알 수 없다 ‖ ✅ **완결(2026-08-17, D-114)** — 상태·우선순위를 `meta` 배열 맨 앞(가장 먼저 훑는 값)으로 옮겨 "속성" 카드로 합쳤다(`TicketEditModal`도 이 둘을 project/assignee/due와 같은 한 폼에서 다뤄 애초에 화면만 갈라 둘 이유가 없었다). 본문 카드 맨 위의 라벨 없는 칩 쌍은 제거. 신규 시험 1건(`ticket-detail.test.jsx`) |
| VIS-135 | Med | **편집 입구가 둘이고 차이를 알 수 없다.** 헤더의 `편집`과 본문 카드 안의 `본문 편집`. 전자가 전체를 여는지, 후자와 무엇이 다른지 화면에 설명이 없다 ‖ ✅ **완결(2026-08-17, D-114)** — 원 서술의 버튼 문구(`편집`/`본문 편집`)는 `VIS-132`/`SEM-03`으로 이미 `수정`/`본문 수정`으로 바뀌어 있었지만, 두 입구가 다른 범위를 편집한다는 사실 자체는 여전히 설명이 없었다. 헤더 `수정`·`본문 수정` 둘 다에 서로를 가리키는 툴팁을 붙였다 — `EditableBody`는 `TeamDoc.jsx`와 공유하는데 그쪽엔 경쟁하는 헤더 편집 버튼이 없으므로, 새 `editButtonHint` prop을 옵트인으로 둬 `TicketBody.jsx`만 채운다. **부수 발견**: MUI `Tooltip`의 기본값(`describeChild=false`)은 툴팁 문구를 자식의 `aria-label`로 덮어써 버튼의 접근 가능한 이름 자체를 "수정" 대신 긴 문장으로 바꿔 버린다(전체 회귀에서 9건이 이 이유로 실패, 원인 파악 후 `describeChild` 추가로 이름은 보존하고 설명만 붙는 `aria-describedby`로 정정) — 그 과정에서 `ui/kit.jsx`의 `Button`이 `forwardRef`가 아니라 MUI가 자식에 ref를 못 거는(포지셔닝에 필요) 콘솔 경고도 같이 발견해 `React.forwardRef`로 승격(이 저장소 최초의 `Tooltip`+`Button` 조합이 처음 드러낸 기존 결함). 신규 시험 2건(제목/설명 그대로가 아니라 열리기 전부터 붙는 `title` 속성으로 확인 — MUI 호버 팝오버는 jsdom에서 불안정), 프런트 전체 회귀(286파일/1980건) green |
...
| RN-12 (두 번째, `§424`의 다른 RN-12와 ID 충돌 — 서로 다른 감사 라운드가 같은 ID를 재사용함, 문서 정리는 별도) | Low | 헬스체크 시각이 러너 3개 모두 **초 단위까지 동일**(`2026-08-08 06:51:21.174136`)하다. 한 틱에서 순차 호출하며 같은 `now` 를 쓰기 때문으로 보이는데, 그러면 "이 러너가 언제 응답했는가"를 개별로 알 수 없다 ‖ **구현완료** — 러너·연동 둘 다 같은 결함(같은 스윕 함수 쌍, 같은 뿌리)이었다: `app/runners/service.py::run_runner_health_check`/`run_all_runner_health_checks`, `app/integrations/service.py::run_health_check`가 전부 스윕 바닥 `now`를 그대로 썼다. 이미 각 함수가 `time.perf_counter()`로 `latency_ms`를 재고 있었으므로 새 측정 없이 `now + timedelta(milliseconds=latency_ms)`로 실제 응답 도착 시각을 반영(연동 스윕 `run_all_integration_health_checks`는 단건 함수를 재사용하므로 자동으로 같이 고쳐짐). 기존 시험 2개(정확히 `now`와 같음을 단정하던 것)를 "now 이상, 1초 미만"으로 정정 + 신규 시험 2개(`time.perf_counter`를 결정적으로 모킹해 서로 다른 지연이 서로 다른 시각을 만드는 것을 직접 증명) 추가, revert-to-verify 확인(신규 2건이 기대대로 실패 → 복구 후 재통과). `record_runner_result`의 회로차단기 `now`는 건드리지 않음(신고된 증상은 `last_health_at`뿐이라 범위를 그대로 좁게 둠) | 구현완료 |
...
| ID | 심각 | 문제 |
|---|---|---|
| GM-01 | Med | **유휴 정리가 `status` 를 안 고쳐서 `status` 와 `closed_at` 이 영구히 어긋난다.** `cleanup_idle_rooms`(`service.py:228`)는 폴링이 180초 끊긴 방의 `closed_at` 만 찍고 `status` 는 그대로 둔다. 반면 명시적 파방(`disband_room`)은 **둘 다** 바꾼다(`status = ROOM_FINISHED` + `closed_at`). 결과: 실측에서 `status='playing'` 이면서 2026-07-28 에 닫힌 방, `status='waiting'` 이면서 닫힌 방 2개가 남아 있다. **`status` 로 세는 미래의 질의·리포트는 유령 3건을 영원히 센다** ‖ ✅ **구현완료(2026-08-12)** — `cleanup_idle_rooms`가 `closed_at`과 함께 `room.status = ROOM_FINISHED`도 찍도록 고쳤다(`disband_room`과 동일 종결 상태로 통일 — 이 경로 전용의 별도 상태를 새로 만들 근거는 없었다). `list_open_rooms`/`get_room`은 `status`가 아니라 `closed_at IS NULL`로 거르므로 이 수정이 그 조회 흐름을 바꾸지 않는다(직접 확인). 신규 시험 1건(`test_games_api.py`), revert-to-verify로 되돌리면 `'waiting' == 'finished'`로 정확히 원 증상 그대로 실패 확인. **원 발견의 유령 3건(과거 데이터)은 코드 수정과 별개** — 이 로컬 DB에 그 특정 3건이 있다는 근거가 없고, 있었다 해도 일회성 데이터 보정이라 스키마 마이그레이션 대상이 아니다. TEST SERVER 배포 후 실측으로 남아 있는 유령 행이 있으면 그때 직접 `UPDATE`로 보정할 것(운영 후속) |
...
| GM-04 | Low | **끝난 방 15개가 어디에서도 안 보인다.** `disband_room` 주석이 "히스토리를 남기지 않는다(§16.1)"라고 명시하므로 **의도된 것**이지만, 그러면 `game_rooms`·`game_events` 행은 읽는 사람 없이 계속 쌓인다. 보존 정책이 없다 ‖ **구현완료 — `RET-03`(§2441)이 이미 이 문제 자체를 고쳤다.** `app/core/retention.py::purge_old_game_rooms()`가 끝난 지 보존기간(기본 7일)을 넘긴 방과 그 멤버·이벤트를 지운다(행 정정, 2026-08-13) — 이 행은 RET-03과 중복이라 별도 작업 불필요 |
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| NOTI-01 | Med | **20종 중 15종이 한 번도 발생하지 않았다.** 대부분은 `USE-01`(그 기능 자체가 안 돌았다)로 설명되지만, 이 집계는 **알림이 실제로 도달하는지 검증된 적이 없다**는 뜻이기도 하다. 알림은 "실패가 조용히 묻히는" 문제(`VIS-108R`)의 유일한 해독제인데 그것이 미검증이다 | **재확인(2026-08-16), TEST SERVER 실측치로 갱신** — `USE-01`이 9/12 자동화를 실행 확인한 뒤라 발생 유형도 늘었다. 위 스냅샷(6종: account_locked 56·runner_unavailable 39·job_failed 3·password_change_required 2·chat_invited 2·ticket_assigned 1) 대비 **지금은 최소 9종**: `account_locked 56·runner_unavailable 39·backup_failed 16·approval_requested 14·password_change_required 9·ticket_assigned 7·job_failed 3·chat_invited 2·approval_decided 1`(서버 DB `SELECT type, COUNT(*) FROM notifications GROUP BY type` 직접 실측, 읽기 전용) — `backup_failed`·`approval_requested`·`approval_decided` 3종이 새로 발생했고 기존 2종(`password_change_required`·`ticket_assigned`)도 늘었다. 코드에 정의된 전체 유형 수는 `type_="..."` 리터럴 grep으로 21종을 확인했으나 `chat_invited`처럼 이름 있는 상수(`NOTI_CHAT_INVITED` 등)로 정의된 것은 이 grep이 놓친다는 것도 이번에 직접 확인했다(정확한 "몇 종 중 몇 종"은 상수 참조까지 포함한 재조사가 필요) — 그래도 "대부분 미발생"이라는 원래 결론은 지금도 사실이다(known 21종 중 최소 12종은 여전히 0건: `ai_quota_exhausted`·`approval_cancelled`·`approval_delegated`·`approval_expired`·`board_comment`·`document_comment`·`document_ready`·`idea_status_changed`·`maintenance_announcement`·`offboarding_handover`·`schedule_failed`·`ticket_comment`) |
...
| ADM-04 | Low | 감사 42종 중 **`impersonation.*`·`approval.*`·`schedule.*`·`document.generate` 가 전무**하다 — `USE-01` 과 정확히 일치한다. 감사 로그는 이미 "무엇이 실제로 쓰이는가"의 정답표인데, 제품 안에서 그렇게 읽히지 않는다(`/audit` 은 시간순 목록일 뿐 액션별 집계가 없다) ‖ **재확인(2026-08-16), 부분 진전** — TEST SERVER `audit_logs` 직접 실측(읽기 전용): `impersonation.start`·`impersonation.stop`이 이제 각 1건씩 있다(`USE-01`이 임퍼소네이션을 실행 확인한 것과 일치, 더는 전무가 아님). `approval.*`·`schedule.*`·`document.generate*`는 지금도 **0건**이라 여전히 사실이다. **부수 발견**: 승인 자체는 실제로 일어나고 있다(`NOTI-01` 재실측에서 `approval_requested` 14건·`approval_decided` 1건 확인) — 그런데 `approval.*` 감사 로그는 0건이라, 승인 흐름이 알림은 남기면서 감사 로그는 안 남기는 갭일 가능성이 있다(별도 확인 필요, 이번엔 기록만) | 발견 |
...
| ID | 심각 | 결론 | 상태 |
|---|---|---|---|
| PERF-01 | Low | **기록된 N+1 항목들(`UA-16` org 목록 3종, `UA-17` 감사 이상징후 전 창 적재, `UA-10` `/api/trash` 무제한, `UA-23` O(n²) 비교, `UA-24` SQLite 변수 상한)은 "지금 느리다"가 아니라 "규모가 커지면 터진다"로 재분류한다.** 실측에서 전부 20ms 이하다. **다만 `UA-24`(변수 상한 999 초과 시 홈 전체 500)는 성능이 아니라 정확성 문제**라 규모와 무관하게 남는다 ‖ ✅ 재확인(2026-08-13) — 이 행이 가리키던 다섯 건(`UA-16`·`UA-17`·`UA-10`·`UA-23`·`UA-24`) 전부 각자 행에서 구현완료로 닫혔다. 재분류 자체(성능이 아니라 확장성/정확성 문제)는 여전히 맞는 판단이었다 | 구현완료 |
| PERF-02 | Low | `GET /api/tickets` 는 **405** 다 — 루트에 `POST`(생성)만 있고 목록은 `/mine`·`/unassigned`·`/team` 으로 갈라져 있다. 정상 동작이지만, 자원 루트가 목록을 주지 않는 것은 REST 관례와 어긋나 외부에서 API 를 쓸 때 첫 시도가 반드시 실패한다 | **재확인(2026-08-16), 결함 아님 — 굳이 안 고침** — `app/tickets/router.py` 재확인 결과 지금도 `POST("")`만 있고 `GET("")`는 없어 405가 맞다(사실관계 그대로). 다만 "고친다"는 것 자체가 새 의미를 발명하는 일이다 — `/mine`·`/unassigned`·`/team`은 서로 다른 스코프/권한 규칙을 갖고 있어(개인 배정·미할당·팀 전체), 루트 `GET`이 그중 무엇을 대표해야 하는지 정답이 없다. 세 스코프를 합친 새 뷰를 만드는 것은 API 계약 확장이지 405 수정이 아니라 이번 quick-fix 범위 밖으로 남긴다 |
...
| DGEN-02 | Med | **이 설치에는 문서 생성용 워크플로가 아예 없다.** 등록된 워크플로는 2개(`ClovirONE AI 업무 도우미`=채팅 웹훅, `notion-user-mapping`)뿐이고 둘 다 문서 생성용이 아니다. 그래서 `document_generations` 0행은 "아무도 안 썼다"가 아니라 **"쓸 수 없었다"**이다. 빈 상태가 전제조건을 말해 주는 것은 훌륭하지만, **"지금 이 설치에 그 워크플로가 없다"**는 사실 자체는 말하지 않는다 — 링크를 눌러 `/workflows` 로 가서 2개를 보고 스스로 판단해야 한다 ‖ **구현완료(2026-08-13)** — 어떤 워크플로가 "문서 생성용"인지 판별할 스키마 상 표식이 없어(`Workflow`에 종류 필드가 없다) 동적으로 자동 감지하는 것은 이 행의 범위를 넘는 더 큰 기능이다. 대신 `registry/automation.js`의 `documents.emptyPrerequisite` 문구 자체를 강화 — "대상 워크플로가 필요하다"에서 그치지 않고 **"이 워크플로는 채팅(AI 업무 도우미)이나 Notion 매핑용 워크플로와는 별개"**라고 명시해, 이미 워크플로 2개(채팅·매핑용)가 등록된 설치의 관리자가 "워크플로는 있는데 왜 안 되지"로 헤매지 않게 한다 — `/workflows`로 가는 안내 링크는 그대로 유지. 신규 시험 2건(`registry-documents-empty-help.test.js`) — 문구가 "채팅"·"Notion 매핑"·"별개"를 포함하는지, 안내 링크가 그대로인지. revert-to-verify 확인함(문구를 되돌리면 새 시험이 정확히 "채팅"이 없다는 이유로 실패) | 구현완료 |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-137 | Med | **지표 카드 줄이 4장이 아닐 때 항상 어색하게 남는다.** 카드 폭이 고정이라 — `/jobs` 는 6장이 **4+2** 로 접혀 둘째 줄에 ~1,100px 공백(`VIS-119`), `/notifications` 는 **1장(680px)** 뒤로 ~1,200px 공백. 개수에 맞춰 폭이 늘거나(1장이면 전폭 요약 줄) 개수를 4의 배수로 맞추는 규칙이 없다 ‖ ✅ **완결(2026-08-17, D-110)** — `VIS-119`와 같은 수정으로 닫힘(`DataScreen.jsx`의 요약 카드 그리드를 `auto-fit`으로 교체). 이 행이 지목한 대로 `/jobs`·`/notifications` 둘 다 확인, 전수 조사 중 3번째·4번째 소비처(`/restore-drills`·`/ai-quotas`)도 같은 결함이었음을 추가로 발견해 함께 닫음 |
...
| RSTR-02 | Low | `backups` 가 **1행**뿐이다. 백업을 만드는 주기적 수단도 없다(`jobs` 130건에 backup 잡 0건, `schedule_runs` 0). 복원이 된다는 것을 증명해도 **복원할 백업이 하루치도 없으면** 의미가 반감된다 | **재확인(2026-08-16), 부분 진전 — 핵심은 여전히 유효** — TEST SERVER 실측(읽기 전용): `backups` **2건**·`restore_rehearsals` **1건**으로 늘어 "복원 리허설이 한 번도 없다"는 이제 사실이 아니다. 다만 원 지적의 핵심(**주기적/자동 백업 수단이 없다**)은 그대로다 — `jobs` 테이블에 `job_type LIKE '%backup%'`인 행이 지금도 **0건**이라, 지금 있는 백업 2건은 잡 큐를 거치지 않은 수동/별도 경로로 만들어진 것이다. `USE-06`(WAL 체크포인트 지연)과 같은 자리에서 함께 볼 문제 |
...
| APPR-04 | High | **승인 취소가 요청자에게 통보되지 않는다.** 같은 파일의 형제 종결 함수 `decide()`(승인/거절)·`expire_pending()`(만료)은 이미 `notify_user`를 부르는데 `cancel()`만 빠져 있다. 취소는 요청자 본인뿐 아니라 admin/system_admin도 남의 대기 요청을 대신 끝낼 수 있는 경로인데(`registry/governance.js:151`이 그렇게 게이트 — 의도된 권한), 그 경우 요청자는 벨도 `/notifications`도 아무 신호가 없다. 알림 어휘(`approval_cancelled`) 자체가 `app/profiles/prefs.py`·`frontend/src/lib/format.js::TYPE_KO` 어디에도 없어 배관이 안 깔려 있었다 | ✅ **구현완료** — `cancel()`에 `notify_user(type_="approval_cancelled", related=("approval", row.id))` 추가하되 `decide()`를 그대로 복사하지 않음: `decide()`는 자기 승인이 금지라 호출자가 요청자 자신인 경우가 없지만 `cancel()`은 본인 취소가 흔한 정상 경로라 `actor.id != row.requested_by`(남이 대신 끝낸 경우)일 때만 보내 자기-알림 낭비를 막았다. `related_object_type="approval"`은 기존과 동일해 딥링크(`destinations.py`)·프런트 role 게이트(`registry/notifications.js`) 둘 다 자동 적용(추가 배선 불필요, 직접 확인). `NOTIFICATION_TYPES`/`TYPE_KO` 등록. 신규 시험 2건(남이 취소→알림 생성, 본인 취소→알림 미생성) + 기존 전수 스캔 가드(`test_every_notify_call_site_type_is_registered_somewhere`) 재확인. revert-to-verify 완료(서비스 파일만 stash → "남이 취소" 시험 기대대로 실패 → 복구 → 48/48 재확인) |
...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-141 | Med | **목록이 참여 신호를 안 보여 준다.** 열은 제목·카테고리·작성자·**조회**·작성뿐이고 **댓글 수도 반응 수도 없다.** 제품은 두 표(`board_comments`·`board_reactions`)를 만들어 두고 목록에서는 **읽은 사람 수(수동적 지표)만** 보여 준다 — 게시판에서 "볼 만한 글"을 고르는 신호는 조회수가 아니라 댓글이다 ‖ ✅ **절반은 낡은 기록, 절반은 실제 수정(2026-08-16)** — **댓글 수는 이미 있었다**: `Board.jsx` 제목 옆에 `[N]` 배지로 이미 그려지고 있었다(원 감사가 놓침, `app/board/router.py::list_posts`가 `repository.comment_counts()`로 배선해 둠). **반응 수는 절반만 있었다**: 백엔드는 이미 두 종류(자유게시판·아이디어) 전부에 `repository.like_counts()`로 `like_count`를 채워 주는데(`Reactions` 컴포넌트 `onSuccess`의 기존 주석이 이미 그렇게 적어 뒀다), 프런트 `columns` 정의가 그 열을 `isIdea`일 때만 그려 **자유게시판만 반응 수를 못 봤다**. `idea_status`(진짜 아이디어 전용 값)와 `like_count`(두 종류 공통 값)가 같은 조건부 블록에 묶여 있었던 것이 원인 — 분리해서 `like_count`(공감) 열을 무조건 그리도록 이전. 새 백엔드 필드·새 쿼리 불필요(이미 응답에 있던 값). 신규 회귀 1건(`board.test.jsx`), revert-to-verify 확인. 관련 스위트(board 계열 12파일·186건) green |
...
| ID | 심각 | 문제 |
|---|---|---|
| RN-11 확증 | Med | **같은 세 서비스가 `/runners` 와 `/integrations` 두 화면에 다른 이름으로 등록돼 있다** — 러너 화면은 한국어(`티켓 러너`·`요청 해석기`·`업무 도우미`), 연동 화면은 영문 슬러그. 같은 것을 두 화면에서 따로 켜고 끌 수 있고, 어느 쪽이 이기는지는 화면에 없다(안내문이 *"러너 화면에서 이 연동을 선택한 경우에 한함"* 이라고 조건을 밝히지만, 그 선택 여부는 이 화면에서 보이지 않는다) ‖ ✅ **상당 부분 해결됨(교차연결, 2026-08-17)** — `RN-10`/`RN-11` 수정이 이름 불일치를 닫았다: `/integrations`의 "이름" 열이 이제 `serviceLabel()`(`registry/integrations.js:59`)을 써 `/runners`와 같은 한국어 이름을 보여준다. "어느 쪽이 이기는가" 질문도 두 화면의 도움말 문구에 이미 답이 있다(실제 dispatch를 결정하는 것은 이 두 registry 어느 쪽도 아니라 Workflow/n8n registry라는 것) |
| VIS-145 | Med | **「버전」 열이 화면마다 다른 것을 가리킨다.** `/integrations` 의 `버전` = `config_version`(설정 리비전 카운터), `/runners` 의 같은 필드는 **`설정 버전`** 이라고 더 정확히 쓰고, `/workflows` 의 `버전` 은 **다른 필드(`version`)** 다(`registry/integrations.js:49,146,151`). 연동 화면에서 `2` 는 **서비스 버전으로 읽힌다** — 실제로는 "설정을 두 번 고쳤다"는 뜻이다 ‖ **구현완료(2026-08-15)** — 재확인 결과 실제 위치는 원 서술과 조금 달랐다: `/integrations`·`/workflows` 둘 다 `config_version`이고 둘 다 이미 정상 텍스트를 쓰며, `/runners` **목록**에는 `config_version`("설정 버전", 이미 정확)과 **완전히 다른 필드** `version`(러너 소프트웨어 자체의 릴리스 버전, 예: "3.57.0")이 다섯 칸 간격으로 나란히 있는데 후자만 맨 "버전"이었다 — 상세/편집 폼(`:189,216`)은 이미 "러너 버전"이라고 정확히 쓰고 있어서(폼 `help`에 "설정 버전과 별개"라고 명시까지 돼 있었다), 목록 열 하나만 그 기존 정확한 이름을 안 따라가고 있던 것. 목록 열도 같은 이름("러너 버전")으로 맞췄다 — 새 용어를 만들지 않고 이미 있던 정확한 이름을 전파했을 뿐이다 |
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| RESP-01 | **High** | **1024×768 에서 관리자 표 화면이 사용 불가다.** 흔한 노트북·프로젝터 해상도이고, 1280 화면에서 브라우저를 반만 써도, 1280에서 125% 확대해도 이 구간에 들어온다. **카드 레이아웃은 이미 있고 아름답게 동작한다 — 켜지는 지점만 너무 낮다**(900px). `VIS-73`(1200 근처 붕괴)·`VIS-74`(768↔1366 사이를 한 번도 안 봄)의 정확한 원인이 이것이다 | ⚠️ **재현됨 — "구현완료"는 시기상조였다(Product Audit 실측, 2026-08-15).** 1024px에서 `/users`가 여전히 **가로로 넘친다**(scrollWidth **1123** vs client 1024). 다만 원 서술의 *"관리자 표 화면"* 전부가 아니라 **열이 많은 표에서만**이다 — `/jobs`·`/dashboard`는 1024에서 멀쩡하다(**범위를 좁히는 증거**). 게다가 **1024는 뷰포트 폭만의 문제가 아니다** — 1920 물리 해상도에서 Windows 배율 **175%** 를 주면 CSS 폭이 1097로 줄어 **26px 넘친다**(100/125/150%는 정상). 즉 `/users`가 요구하는 최소 폭이 **1123px**라는 것이 진짜 수치다. 노트북 사용자의 흔한 배율 설정에서 재현된다는 뜻이므로 원 서술보다 발현 조건이 넓다. 근거: `PA-F-030`·`PA-F-033` ‖ 원 기록: 아래 §RESP 정밀화가 이미 "카드 브레이크포인트를 올리는 것은 답이 아니다"(전역 상수로는 4열/9열 표를 동시에 못 맞춘다, RESP-02)라고 결론 냈으므로, 브레이크포인트가 아니라 `DataTable` 자체의 열 폭 계산을 고쳤다 — 아래 VIS-73/HOST-01/HOST-02 구현완료 참고. 같은 수정 ‖ **재확인(2026-08-16, 실제 Chrome E2E)** — HOST-01/02/03(`DataTable` 열 폭 기본값 + 텍스트 열 기본 말줄임) 배포 이후에도 `/users`가 1024px에서 여전히 넘친다: `scrollWidth=1123 clientWidth=1024 overflow=99px` — 원 실측(1123 vs 1024)과 **완전히 같은 수치**. 즉 HOST 배치는 "값 하나가 셀을 2,353px로 찢는 것"은 막았지만(그건 실제로 고쳐졌다, HOST-01/02/03 자체는 유효), **열이 여러 개일 때 각 열의 자연스러운 최소 폭(헤더 한글 텍스트 + 패딩)의 합이 1024px를 넘는 문제**는 별개이고 아직 안 고쳐졌다 — `DEFAULT_COL_MIN_WIDTH`(4.5rem=72px)는 바닥값일 뿐이라 `tableLayout:"auto"`에서 실제 렌더 폭은 내용이 더 넓으면 그 이상으로 벌어진다. 8개 컬럼(이메일/이름/역할/상태/부서/직책/Notion 연결/최근 로그인) + 264px 사이드바(RESP-04)가 합쳐져 1123px가 필요한 것으로 보인다 — 열 폭 바닥값 조정이 아니라 **열 우선순위/반응형 숨김 같은 별도 설계**가 필요하다는 뜻이라, 이번 채팅 기능 배포 사이클에서는 손대지 않고 그대로 열어 둔다(다음 전담 UI 사이클 후보, RESP-04와 같은 성격) ‖ ✅ **완결(재진단+구현+실측, 2026-08-16, D-106)** — 이 기록의 "1123px" 실측은 `PA-RC-0023`(같은 날 나중에 배포, `DataTable`의 「상세」 버튼 열 제거) **이전** 시점이었다. 재측정하니 페이지 자체(`html`/`body`)는 이미 `scrollWidth===clientWidth===1024`로 안 넘쳤고, 남은 건 `TableContainer` 내부의 64px 스크롤(734px 필요 vs 670px 가용, 컨테이너 안에 갇혀 있어 "사용 불가"는 아니었다)뿐이었다. 그 방향대로 `DataTable`에 opt-in `c.hideNarrow`(900~1200 전용, `TABLE_COMPACT_QUERY`)를 추가하고 `/users`의 `Notion 연결`·`최근 로그인` 두 열에만 달아 1024px에서 TableContainer 내부 스크롤까지 0으로(670=670) 닫았다. 카드 뷰(≤899.95px)는 안 건드림 — 700px 실측에서 두 필드 라벨이 행 수만큼 그대로 확인됨. 1920px 실측에서 9열 전부 복원 확인. 배포 `UPGRADE_OK`+`verify_deploy.sh` 전부 OK, TEST SERVER 실측 스크린샷(`dist/resp01_verify/`) 육안 확인, focused 회귀(kit.test.jsx 42건+users 관련 10파일 37건) green |
...
| CTR-03 | **High** | **고를 수 있는 강조색 4개가 다크에서 전부 미달이다**(정적 계산, 다크 표면 `#161C33` 기준): `기본 파랑` 3.59 · **`진한 파랑` 2.68** · `보라` 3.16 · `청록` 3.58. **"진한 파랑"이 다크에서 가장 나쁘다** — 사용자 직관과 반대다. 강조색 선택에 **대비 검증이 없다** | 재검증(2026-08-11): 텍스트 용도 실패는 CTR-01 수정으로 해소(모든 프리셋이 primaryStrong 경유 시 4.5 통과). 남은 것은 **재발 방지 게이트**(향후 프리셋 추가 시 검증 없이 다시 뚫릴 수 있음) — CTR-05와 함께 처리 예정, 아직 미착수 ‖ ✅ **정정(2026-08-15) — "미착수"가 stale, CTR-05가 같은 날(2026-08-11) 완료돼 게이트가 이미 있다.** `scripts/ui_qa/assertions.py::CLASSES`에 `"contrast"`가 등록돼 있음을 직접 확인 — QA 하네스의 기존 캡처 루프가 매 라우트마다 대비를 자동 판정하고 `report.py` 요약/실패표에 제네릭하게 반영된다(CTR-05 자신의 구현완료 노트 참고). 즉 향후 새 강조색 프리셋이 이 하네스가 도는 배포 사이클을 거치면 대비 위반이 자동으로 리포트에 잡힌다 — CTR-03이 요구한 재발 방지 게이트가 이미 충족됨. 코드 변경 없음, 문서만 정정 |
...
| KBD-04 | Low | 탭 **역순 점프**가 `/users`·`/chat` 에서 2회, `/new-ticket` 에서 1회 — 시각 순서와 탭 순서가 어긋나는 지점이 있다(정확한 위치는 `dist/keyboard/keyboard.json` 의 정지점 좌표로 추적 가능) | **재측정 완료(2026-08-16), 정확히 재현됨** — 이전엔 캐시된 인증 세션(`dist/ui-qa-admin-2`)이 `SEC-11` 정리로 사라져 재확인을 미뤘으나, 이번 세션이 만든 다른 세션 캐시(`dist/ui-qa/storage_state.json`, 138페이지 E2E가 남긴 것)를 `--auth-dir`로 재사용해 `scripts/ui_qa/keyboard.py`를 실제로 돌렸다. 결과가 원 서술과 **정확히 일치**: `admin_users`(=/users) 역순=2, `user_chat`(=/chat) 역순=2, `user_new-ticket` 역순=1 — 스테일이 아니라 지금도 그대로 재현되는 결함이다. 좌표 상세는 `dist/keyboard/keyboard.json`(재생성됨, gitignore 대상) |
| KBD-06 | Low | **`OrgTree.jsx`(조직도 트리)의 모든 `treeitem`이 `tabIndex=0`이라 roving tabindex가 아니다.** WAI-ARIA APG treeview 패턴은 트리 전체에서 Tab 정지점이 하나(선택된/활성 노드)뿐이길 요구하는데, 이 컴포넌트는 화살표 키 이동(이미 정확히 구현돼 있음, `org-tree-arrow-keys.test.jsx`)과 별개로 모든 노드가 각자 Tab 정지점이라 — 정확히 그 화살표 키 이동을 신설했던 이유였던 "Tab으로 노드를 하나씩(접기 버튼 있는 노드는 둘씩) 거쳐야 한다"는 원래 불편이 Tab 자체에는 그대로 남아 있었다 | whole-product 재감사(Explore 에이전트, 2026-08-17), `OrgTree.jsx:131`(수정 전) | ✅ **구현완료(2026-08-17)** — `OrgTree`가 `tabbableId`(선택된 노드, 없으면 첫 루트 노드)를 계산해 재귀 `TreeNode`에 전달, 각 노드는 `tabIndex={tabbableId === row.id ? 0 : -1}`. 화살표 키 이동은 DOM에 직접 `.focus()`하므로 `tabIndex=-1`이어도 그대로 동작(기존 `org-tree-arrow-keys.test.jsx` 3건 회귀 없음, 재실행 확인). 접기 버튼(`IconButton`)의 별도 Tab 정지점은 그대로 둠(마우스/스크린리더 전용 접근 경로로 의도된 것이지 이번 결함의 대상이 아님). 신규 시험 3건(`org-tree-roving-tabindex.test.jsx`: 미선택 시 첫 루트만 0·선택 시 그 노드만 0·접힌 자식이 선택된 경우도 정확히 하나만 0), revert-to-verify 확인(되돌리면 3건 전부 "모든 노드가 tabIndex=0"으로 실패) |
| KBD-05 | Low | `/users` 에서 **본문에 닿는 탭 위치를 특정할 수 없었다**(내 판정 기준으로 `None`) — 상단바·사이드바 요소가 40개 넘게 이어진다. 「본문 바로가기」가 있으니 치명적이지 않지만, 그것을 모르는 사용자는 관리자 내비 37항목을 탭으로 지나야 한다 | **재측정 완료(2026-08-16) — 부수적으로 측정 도구 자체의 버그 1건 발견·수정.** `admin_users`를 45스텝까지 늘려 재측정하니 이번엔 28번째에서 본문에 닿아 `None`이 아니었다(정확한 45스텝 재현은 아니지만 방향은 일관 — 여전히 매우 많은 탭이 필요하다는 원 지적이 유효). **그 과정에서 `keyboard.py`의 "건너뛰기 링크 감지" 정규식(`/건너\|본문으로\|skip/i`)이 실제 문구 "본문 바로가기"(`AppShell.jsx:572`, `component="a"`로 정상 구현된 진짜 링크)를 못 잡아 있는 링크를 "없음"으로 오탐하던 것을 발견** — `바로가기`를 정규식에 추가해 즉시 수정, 수정 전/후 재실행으로 되돌리기 없이 직접 대조 확인(오탐 없음). 즉 스킵 링크는 실재하고 정상 작동한다(원 서술의 "「본문 바로가기」가 있으니"가 맞았다) — 남은 진짜 문제는 그것을 모르는 사용자가 여전히 28번 이상 탭해야 한다는 것뿐이다. `VIS-113`(관리자 내비 37항목 중 16개가 화면 밖)과 같은 근본 원인(내비가 너무 길다) |
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| SEM-01 | Med | ✅ 구현완료(범위 확장) — 원 발견은 `/jobs`(100개)·`/users`(18개) 표본만 들었지만, 원인은 화면 2개의 버그가 아니라 **표식 자체가 저장소 어디에도 없던 상태**였다. `kit.jsx`의 `rowOpenLabel()`은 이제 `openLabel`보다 먼저 열 정의의 `rowName`(`ui/rowName.js`, 위치 무관 — Board/Trash/TeamDocs/MyTickets/Projects가 이미 이 방식으로 고쳐져 있었다)을 확인하는데, `frontend/src/screens/registry/*.js`(관리자 등록 화면 28개의 실제 설정)에는 `rowName`도 `openLabel`도 **단 한 곳도 없었다**. `DataScreen.jsx`가 `onRow`를 모든 등록 화면에 예외 없이 붙이므로, 첫 열이 `render()`를 쓰는 등록 화면은 전부 같은 결함을 안고 있었다 — 실사한 결과 11개(governance.js: `approvals`·`approval-delegations`·`audit`·`audit-anomalies`·`impersonation`, platform.js: `backup`·`restore-drills`·`ai-quotas`, automation.js: `documents`·`jobs`, org.js: `org-tree`) + registry 밖 2개(`Users.jsx`— 선택 체크박스 라벨도 같은 결함, `Offboarding.jsx`의 "실행 이력" 표)로 총 13개 화면. 각 화면에 이미 표시 중인 값(요청자/행위자/대상/파일명/제목 등, 새 정보 노출 없음)으로 `rowName`을 채웠다 — `jobs`는 요청자 이름/이메일이 서버가 일부러 감추는 값이라(`app/jobs/router.py _job_view`) 후보에서 제외하고 유형+생성 시각을 썼다. 새 시험 `registry-row-name.test.jsx`(11건, REGISTRY 설정 객체 직접 검증)·`users-row-open-label.test.jsx`(2건, 실제 렌더링) 추가, `offboarding.test.jsx`의 기존 3건은 옛 결함을 "정확히 '상세 보기'다"라고 정상으로 못박은 주석·단정이었어서 함께 정정. 프런트 전체 회귀(225 파일/1534건) green, 정적 검사 green(가운뎃점 금지 규칙에 걸려 분리자를 `/`로 교체). **추가 발견(2026-08-13) — 같은 결함의 두 번째 소비처.** `rowOpenLabel()`(행의 "상세 보기" 버튼/체크박스)만 `declaredRowName()`을 봤고, 같은 행을 여는 상세 모달·수정 드로어의 **제목**(`data-screen/detailFields.js::detailTitle()`, `DataScreen.jsx:755,794` 2곳에서 소비)은 여전히 `columns[0]` 원시값만 봤다. 첫 열이 `render()`인 화면에서 제목이 배지 원시 enum(`audit-anomalies`: "high"/"medium"/"low")이나 구별 안 되는 값(`jobs`: 같은 날 여러 건이면 전부 같은 시각)으로 샜다 — 행 버튼은 SEM-01로 이미 고쳐졌는데 같은 화면의 드로어 제목만 안 고쳐진 것. `detailTitle()`이 `columns[0]` 폴백보다 `declaredRowName(columns,row)`을 먼저 보게 수정 — SEM-01이 이미 11개 registry 화면에 채워 둔 `rowName` 선언을 그대로 재사용하므로 새 정보 노출 없음. 새 시험 `data-screen/detailFields.test.js`(5건: `audit-anomalies`/`jobs`/`org-tree` 3화면의 배지·원시값 누출 재현 + `notifications`의 무표식 폴백 유지 + 빈 columns 경계) 추가, revert-to-verify로 수정 없이 3건이 정확히 기대한 이유로 실패함을 확인. DataScreen 계열 회귀(`datascreen*.test.*`·`data-screen-*.test.jsx`, 33건) green. `detailFields.js:28`(`declaredRowName`) 재확인(행 정정, 2026-08-13) | 구현완료 |
| SEM-02 | Low | **`/me` 의 제목 계층이 `h1 → h3` 로 건너뛴다.** `h1「오늘」` 다음에 `h3` 다섯 개(오늘 마감·AI 도우미·팀 채팅·이번 주 내 진척·최근 문서)이고 `h2` 가 없다. 일관되긴 하지만 스크린리더 목차에서 한 단계가 빈다 ‖ **범위 확장(Product Audit `PA-F-031`/`RD-4`, 2026-08-15)** — `/me` 한 화면의 문제가 아니다. 목록 화면 **11개 중 8개가 `h1` 하나뿐인데 버튼이 13~39개**다. 필터 영역·표·일괄 작업 영역이 시각적으로는 구분되는데 **문서 구조상으로는 한 덩어리**라, 스크린리더 사용자는 39개 버튼을 구획 없이 순회한다. 처방은 목록 화면에 섹션 제목(`h2`)을 주는 것이고 **시각 디자인은 그대로 둘 수 있다**(마크업 의미만 바꾼다). `SEM-03`과 같은 계열이므로 **함께** 처리할 것 ‖ ✅ **PA-F-031의 8개 화면 구현완료(2026-08-15), `/me` 원래 사례는 별도로 남음** — 8개(`/my-tickets`·`/team-docs`·`/board`·`/notifications`·`/users`·`/prompts`·`/jobs`·`/audit`) 전부에 "필터"·"목록" `h2`(선택 시 "일괄 작업" 포함)를 `.sr-only`로 추가했다. `/jobs`·`/audit`·`/prompts`·`/notifications`는 전부 `registry.js` 기반 `DataScreen.jsx` 공유 셸이라 **거기 한 번**으로 넷 다 해결(`config.compact` 하위 패널에서는 `h3`로 자동 강등 — `PageHeader` 자신이 이미 그 자리에서 `h2`이기 때문). `/my-tickets`·`/team-docs`는 필터·목록 각 화면에서, `/board`도 같은 패턴. `/users`는 필터·목록에 더해 `UsersBulk.jsx`의 `BulkBar`(선택 시에만 나타나는 독립 카드)에 "일괄 작업" `h2`를 추가 — `MyTickets`/`TeamDocs`가 쓰는, 헤더에 내장된 `BulkActions`(선택 시만 렌더되는 인라인 툴바)는 카드 하나를 통째로 차지하는 것과 다른 모양이라 대신 `role="toolbar"`+`aria-label`로 처리(`bulkSelect.jsx` 공유 컴포넌트, `MyTickets`/`TeamDocs`/`Trash` 3화면에 한 번에 적용). **`/me`(`Home.jsx`)의 원래 사례는 아직 남아 있다** — 재확인 결과 여전히 `h1`(`PageHeader`) 다음 `SectionTitle`(`kit.jsx`, 기본 `component="h3"`) 4개가 직결돼 있고 구조적인 `h2`는 없다(PA-F-031 표의 "h2 1"은 측정 시점에 우연히 뜬 `EmptyState` 제목 하나였을 뿐 — `EmptyState` 자체도 `role="heading" aria-level={2}` 고정이라 이번 8개 화면과 같은 함정이다). `SectionTitle`은 `Home.jsx`·`MyStats.jsx`·`Profile.jsx`·`AssistantPanel.jsx` 4곳이 공유해 기본값을 h2로 올리면 이미 h2 아래 h3로 올바르게 있는 다른 소비처가 깨질 수 있어 각 소비처를 개별 확인해야 한다 — 이번 배치 범위 밖으로 남기고 다음 후보로 넘김. 신규 시험: `datascreen.test.jsx`(2건, compact 포함) · `board.test.jsx`·`teamdocs-view.test.jsx`·`tickets-list.test.jsx`(각 1건) · `users-bulk.test.jsx`(2건) · `bulk-actions-toolbar-role.test.jsx`(신규 파일, 2건). 전부 revert-to-verify 확인. 부수 발견: `datascreen.test.jsx`에 실제 결함 하나 더 — `DataScreen.jsx`가 딥링크 지원을 위해 마운트 시 `window.location.hash`를 라우터 없이 직접 읽는데, 이 시험 파일 어디도 그걸 초기화하지 않아 한 시험의 검색어가 다음 시험의 "초기 필터"로 새고 있었다(전체 파일 실행 시에만 재현, 격리 실행하면 통과 — 순서 의존 결함). 파일 전역 `beforeEach`에 `window.location.hash = ""`를 추가해 근본 수정 ‖ ✅ **`/me` 잔여도 구현완료(2026-08-15)** — 4개 소비처를 개별 확인한 결과: `Profile.jsx`는 이미 5곳 전부 `component="h2"`를 명시하고 있어 손댈 것이 없었다(재확인만). `Home.jsx`(단독 라우트, 자체 `PageHeader`가 h1)의 최상위 구역 4개(티켓 칸 카드·이번 주 내 진척·최근 문서·게시판)에 `component="h2"`를 추가. `AssistantPanel.jsx`·`TeamChatWidget.jsx`는 **항상 `Home.jsx` 안에만 박혀 있다**(단독 라우트 없음, `grep`으로 다른 소비처 0건 확인)는 사실이 이번에 새로 확정한 근거다 — 그래서 이 둘의 최상위 제목("AI 도우미"/"팀 채팅")도 Home의 다른 최상위 구역과 같은 무게(h2)를 받는다. `AssistantPanel.jsx`는 h2로 올린 결과 안쪽 소제목(`TicketLines`의 "내 몫"/"팀 전체"/"바뀐 것"/"최근 끝낸 일" 등, 기존 `h4`)이 h2→h4로 건너뛰게 돼 h3로 함께 낮췄다. `MyStats.jsx`(단독 라우트 `/my-stats`, 자체 `PageHeader`가 h1)의 5개 `SectionTitle`에도 `component="h2"` 추가. 신규 회귀: `home.test.jsx`(1건, h1 1개+h2 6개 정확히 확인) · `assistant-panel.test.jsx`(1건, 패널 제목 h2+`TicketLines` 소제목 h3) · `my-stats.test.jsx`(1건, h1 1개+h2 5개 정확히 확인), 전부 revert-to-verify 확인(4개 소스 파일을 한 번에 되돌려 3개 시험이 정확히 예측대로 실패하는 것 확인 후 복원) | 구현완료(8/8+`/me` 4개 소비처 전부) |
...
| AI-64 | Low | **응답 시간이 5초와 28초로 갈린다.** 규칙엔진 경로 5.1초, LLM 경로 20~28초. 사용자에겐 같은 채팅창인데 대기 시간이 5배 차이 나고 **진행 표시가 없다**(`AI-54`) | **중복 확인(2026-08-16), `AI-54`로 합침** — 이 행이 스스로 `AI-54`를 지목하고 있고, `AI-54`가 이미 같은 증상을 실측치(5초/17초/20초)로 조사완료해 뒀다. 별도 재조사 없이 `AI-54`로 추적 |
...
| `admin_offboarding` | 제목·내비가 **온보딩을 약속하는데 화면에 온보딩 기능이 없다**(`Offboarding.jsx` 503줄에 온보딩 경로 0건) ‖ **구현완료(2026-08-13)**: Explore 조사 결과 순수 명칭 오류였다 — 실제 신규 계정 생성(진짜 온보딩)은 이미 `/users`의 "+ 사용자 추가"로 존재하고 완전히 배선돼 있다(`org.js:141-146`이 이미 "/users"를 온보딩 체인의 종착점으로 문서화). `preview.onboarding`(체크리스트)은 `_onboarding_checklist()`가 "일할 준비가 됐는가"를 **뒤집어 오프보딩 점검에 재사용**하는 읽기 전용 표시일 뿐 능동적 워크플로가 아니다. 사이드바 라벨 "온보딩과 오프보딩"→"오프보딩", 화면 제목 "온보딩, 오프보딩"→"오프보딩"으로 정정, 새 기능은 만들지 않음(이미 다른 곳에 있다). `offboarding.test.jsx`에 회귀 시험 추가, revert-to-verify 확인 |
...
| `admin_integration-detail` | **25일 전 점검 결과를 초록 「정상」으로 현재처럼 표시**. 연동·러너 헬스 스윕이 워커 주기 작업에 없다(버튼으로만 돈다) ‖ **구현완료(2026-08-13)**: Explore 조사로 절반만 사실임을 확인 — 러너 헬스 스윕은 이미 `worker_main.py`의 `runner_health_tick`(90초)로 존재했다(감사 이후 이미 고쳐졌고 이 표만 미반영). **연동(integration) 헬스 스윕은 실제로 없었다** — `run_health_check()`(`app/integrations/service.py`)의 유일한 호출자가 수동 "헬스체크" 버튼뿐이었다(grep으로 `worker_main.py`에 "integration" 매칭 0건 확인). `run_all_integration_health_checks()` 신설 + `worker_main.py`에 90초 틱으로 배선(러너와 같은 간격, 별도 틱). 러너 스윕과 달리 스윕 전용 부수효과(회로차단기 등)가 없어 판정 기준을 복제하지 않고 기존 단건 함수를 그대로 재사용 — 러너 쪽이 기준 복제로 겪은 실제 오탐 사고(round36 감사)를 구조적으로 피함. `tests/unit/test_integration_health_sweep.py` 신규 5건(도달 가능/불가능, 엄격 2xx, health_url 우선, 비활성 건너뜀, 예외 격리), revert-to-verify 확인. 관련 회귀(`tests/integration` 전체 + `tests/unit` 전체, 수백 건) green |
| `user_my-stats` | 상단 경고("Notion 미연결")와 빈 상태("담당 티켓이 없어서")가 **서로 다른 원인**을 말하고, 빈 상태 CTA 는 실제 원인을 못 고친다 ‖ **구현완료(2026-08-13)**: `MyStats.jsx`가 `source.mapped===false`일 때 배너와 같은 원인("계정이 Notion 사용자와 아직 연결되지 않아...")을 말하는 EmptyState로 분기하고, 어차피 같은 이유로 비어 있을 `/my-tickets` CTA는 그 경우 주지 않는다(연결됨/0건 조합은 기존 문구 그대로). `my-stats.test.jsx`에 두 조건이 실제로 함께 오는(재현되지 않던) 경우를 재현하는 신규 시험 추가, revert-to-verify 확인 |
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| SEC-10 | **High** | **Notion 문서 1건에 평문 자격증명이 있고, 미러가 그것을 인증된 사용자 전원에게 보여 준다.** 앱 버그가 아니라 **원본 콘텐츠 문제**이고, 임의 본문을 앱이 마스킹하는 것은 현실적 해법이 아니다. 앱이 할 수 있는 것은 ① **문서 단위 열람 범위/민감 표시** ② 원본에서 제거·회전이다. **사용자에게 알려야 할 항목** — 실고객 워크스페이스라 내가 원본을 고치지 않는다 ‖ **부분 구현완료(2026-08-12, ①만)**: `document_cache.restricted`(마이그레이션 0057) + `doc_in_scope`가 최우선으로 보는 열람 제한 게이트 — 켜면 운영자군/작성자 본인 외에는 목록·상세·최근 열람 어디서도 그 문서가 안 보인다(같은 부서 동료도 예외 없음). `POST /api/team-docs/{id}/restrict`(운영자만, 범위 밖은 404) + `TeamDoc.jsx`의 배지·토글 버튼 + `TeamDocs.jsx` 목록의 🔒 표시. 부수 발견: `service.recent_documents()`("최근 열람")가 애초에 `doc_in_scope`를 전혀 안 거쳐 부서가 바뀌거나 문서가 나중에 restricted가 돼도 예전에 본 사람에게 계속 보이는 별도 유출 경로였다 — 같이 닫음. **② 원본 제거·회전은 여전히 사용자 몫이다 — 미확인.** 이 기능은 어떤 문서가 새는지 자동으로 찾지 않는다(그러려면 본문 스캔이 필요하고 범위 밖); 문서를 특정한 뒤 관리자가 상세 화면에서 수동으로 제한을 켜야 한다. 신규 테스트 `tests/security/test_document_restricted_scope.py`(11건) + `teamdoc.test.jsx`/`teamdocs-view.test.jsx` 확장, revert-to-verify 확인 ‖ **① 실제 적용 + 재확인(2026-08-16)** — TEST SERVER 실측: `document_cache` 107건 중 `restricted=1`이 **0건**이었다(①이 만들어진 뒤 한 번도 적용된 적 없었다는 뜻). 본문(`body_markdown`)은 절대 직접 읽지 않고, **값 없이 카테고리만 돌려주는 SQL**(`body_markdown LIKE '%키워드%'`의 CASE 식, 매칭된 카테고리명만 SELECT — 실제 값·주변 문맥은 한 글자도 보지 않음)로 1차(비밀번호/패스워드/password/계정정보/api key/secret/token)·2차(private key 헤더/root@/sshpass/액세스키/credential/pw:/아이디+비번 쌍) 두 라운드를 훑었다. **문서 1건이 걸림**: `72aeb79f-70fc-495c-b0b3-0e86b7b5bc86`("포스코DX 배포과정") — "비밀번호"·"패스워드" 카테고리 매칭, 배포 절차 문서라는 성격상 실계정 정보를 담았을 개연성이 높다고 판단해 **TEST SERVER `document_cache`에 직접 `restricted=1` 적용**(원본 Notion은 손대지 않음 — SEC-10 원 기록의 "원본은 안 고친다"와 일치, 이 앱의 로컬 미러만 바꿈) + `audit_logs`에 `team_docs.restrict` 행 기록(`request_id='autonomous-agent-SEC-10-2026-08-16'`로 출처 명시, HTTP 엔드포인트가 아니라 직접 SQL 트랜잭션으로 적용했다는 사실을 숨기지 않음). **⚠️ 사람 확인 필요 — 이 스캔은 완결이 아니다**: 키워드 기반이라 라벨 없이 붙은 값(고엔트로피 문자열만 단독으로 있는 경우)은 못 잡는다. 이 1건 외에 다른 문서에 같은 문제가 없다고 보장하지 않는다. **다음 사람이 할 일**: ⓐ `72aeb79f-...` 문서를 직접 열어 실제로 자격증명이 있는지 확인 ⓑ 있다면 Notion 원본에서 제거 + 그 자격증명 회전(이 저장소 밖 운영 행위, AI 권한 밖) ⓒ 없다면(위양성이면) `TeamDoc.jsx`에서 제한 해제 ⓓ 필요하면 이 스캔을 더 넓은 키워드/실제 워크스페이스 지식으로 사람이 직접 반복 ‖ **자기 버그 발견+수정(2026-08-16)** — 이 항목이 직접 SQL로 남긴 `audit_logs` 행의 `before_json`/`after_json`(`{restricted: false}`)이 따옴표 없는 키라 JSON 문법이 아니었다. Whole-product UI QA 스윕이 `admin_audit`(감사 로그) 화면 전체가 이 한 행 때문에 관리자 전원에게 500으로 막혀 있던 것을 잡았다 — `json.dumps()`로 값은 그대로 두고 문법만 정정, 837행 전수 재스캔으로 다른 깨진 행 없음 확인, 재검증(콘솔 오류 0건) 완료. 상세: `DECISIONS.md` D-90 | **부분 완화(①만 적용됨, ②는 사람 확인 대기) + ①이 남긴 자기 버그도 해소** |
| SEC-11 | Low | 조사 산출물 정리: `dist/ui-qa-*/…/user_team-doc-detail.png` 가 로컬에 남아 있다. git 에는 없으나 조사 종료 시 지운다 | **구현완료(2026-08-16)** — 원 서술이 가리키던 파일 1개 수준이 아니라 `dist/`(전부 gitignore 대상, 저장소·배포에 영향 없음) 전체가 7/18~8/5 사이의 지난 조사 산출물로 5.6GB 누적돼 있던 것을 확인 — 이번 세션의 활성 산출물(`stage/`·번들·`ui-qa/post-cachefix-full`)만 남기고 나머지(ui-qa-* 60여 개·baseline-* 30여 개·기타 스크린샷/로그)를 정리해 3.5GB로 줄임(git 이력·제품 코드에는 영향 없음, 로컬 디스크 정리) |
...
|---|---|---|---|
| `KO_WORD_BREAK` | `theme.js` (주석: "사용자 지적 #11") | **`EmptyState`**(`kit.jsx:295-311` — `maxWidth:"60ch"` 뿐, 토큰 없음). `Project.jsx`·`ProjectMetrics.jsx` 는 쓴다 | **31개 파일**이 `EmptyState` 사용 → 한글이 단어 중간에서 잘림. 이번 판독에서 3화면 재발 ‖ **구현완료(2026-08-13)**: `EmptyState`의 제목(`role="heading"`)·`bodySx`(situation/help/prerequisite/expected 공용)·`stepList` 세 자리 전부에 `KO_WORD_BREAK`를 추가 — help만 고치면 title·steps는 다음에 또 새는 재발 패턴이라 컴포넌트의 Korean 산문 자리 전부를 잠갔다. `ko-wordbreak.test.jsx`에 EmptyState 렌더 시험 추가, revert-to-verify 확인 |
...
| `primary:true` 헤더 액션 | DataScreen | `DataScreen.jsx:486-489` 가 `showCreate` 만 primary | `admin_notion-mapping` 주요 동작이 안 두드러짐 ‖ **재확인(2026-08-13) — 이미 해결돼 있었다.** 현재 소스(`DataScreen.jsx:531-538`)는 `showCreate` 없으면 `visibleHeaderActions.find(a=>a.primary)`도 CTA로 승격한다. `registry/org.js`의 notion-mapping "자동 동기화" 액션도 이미 `primary:true`(주석: "백업·문서 화면과 동일하게")가 붙어 있다 — WF1 감사(2026-08-08) 이후 다른 세션이 먼저 고쳤고 이 표만 갱신이 안 됐던 사례. 코드 변경 없음 |
| `Callout tone="warn"` | 같은 파일에서 이미 씀 | `DataScreen.jsx:524` 가 `config.help` 를 **무조건 info** | 경고가 안내처럼 보임 ‖ ~~**재확인(2026-08-13) — 구체적 소비처 없음, 보류.** registry 전체에서 `help` 텍스트가 실제로 경고(주의/위험/되돌릴 수 없음 등)로 읽히면서 info 톤 때문에 오해를 사는 화면을 못 찾았다(가장 근접한 `approvals.help`도 정책 설명이지 경고문이 아니다). 이 상태로 `config.helpTone` 같은 새 필드를 만드는 것은 확인된 문제 없이 설계 표면만 늘리는 것이라 보류 — 구체적 소비처가 나오면 그때 착수~~ **구현완료(같은 날 2026-08-13, WF44) — 이 보류 판단이 쓰인 바로 그날 소비처가 확인됐다.** `admin_policies` 자신의 배너 문구가 "발행하면 이 정책 이름을 참조하는 모든 템플릿이 즉시 영향받는다, 프롬프트보다 실제 파급력이 크다"고 스스로 경고하면서도 배너 자체는 항상 info 톤이었다 — 정확히 이 행이 찾던 "경고인데 info 톤" 소비처. `config.helpTone`(기본값 `"info"`, 값 없으면 기존 15개+ 화면 전부 그대로) 신설, `DataScreen.jsx`가 이를 읽어 `Callout tone`에 반영, `policies`에 `helpTone:"warn"` 배선. `datascreen-help-tone.test.jsx` 신규 4건, revert-to-verify 확인. 상세는 `admin_policies` WF1 단독 결함 행(위 §admin_policies 목록) 참고 |
| `activeCol`(warn 톤) | `columnHelpers.jsx:11-15` | `admin_templates` 는 `badgeCol` | 비활성이 회색 중립 — "전부 적용 안 됨" 경고와 톤 불일치 ‖ **구현완료(2026-08-13)**: `activeCol`은 `key:"active"`로 고정이라 이 화면의 실제 필드명(`enabled`)엔 그대로 재사용할 수 없었고, 그 라벨("사용 중"/"미사용")도 이 화면 자신의 필터("활성"/"비활성")와 어긋나 공유 헬퍼를 고치는 대신 인라인 `Badge` render로 이 화면의 기존 어휘 + warn 톤을 맞췄다. 이전엔 원시 불리언이 `statusText`를 타 "예"/"아니오"로 떴다는 것도 확인. `registry-active-badge.test.jsx` 신규, revert-to-verify 확인 |
| `shortUA` + Tooltip | `Users.jsx:42,:633` | `Profile.jsx:383-386` | UA 원문 그대로 노출 ‖ **구현완료(2026-08-13)**: `shortUA`가 `Users.jsx`에 갇힌 지역 함수였던 것을 `diffFields`와 같은 이유로 `lib/format.js`로 옮기고 양쪽이 그걸 참조. `Profile.jsx`에 `Tooltip`+`shortUA` 배선. `profile.test.jsx`에 hover→`role="tooltip"` 전체 문구 확인 시험 추가, Users.jsx 전체 회귀(8파일/32건) green, revert-to-verify 확인 |
...
| 수치 열 `align:"right"` | `governance.js:341` 등 | `platform.js:67` 백업 크기 열 | 숫자 비교가 어려움 ‖ **구현완료(2026-08-13)**: `size_bytes` 열에 `align:"right"` 추가. 이 기회에 `align` 배선 자체(8곳이 이미 씀)를 직접 검증하는 시험이 저장소에 하나도 없었다는 것도 확인해 `registry-active-badge.test.jsx`에 실제 `DataTable` 렌더 시험 추가, revert-to-verify 확인 |

> **R1 8건 중 6건 구현완료, 1건 재확인 결과 이미 해결(문서만 정정), 1건(`Callout tone="warn"`)은
> 구체적 소비처가 없어 보류** — WF1 감사가 짚은 "이미 만든 공용 규칙이 옵트인이라 조용히
> 빠진다" 근본 원인 클러스터가 수렴했다.
...
- `admin_integration-detail`: 설명이 **내부 메모**(`기존 서비스 — 존재 여부 사전조사로 확인`),
  ...
  **2026-08-13 부분구현완료** — 슬러그 쪽은 시드 값을 바꾸지 않고도 표시 계층에서 고쳤다
  (`ops/opsHelpers.js::serviceLabel()`을 목록 열 + 상세 제목에 배선, 이미 다른 화면이 쓰던
  매핑을 재사용). **내부 메모(설명)는 여전히 시드 데이터 문제로 남아 있다** — 상세는 위
  `admin_integration-detail` 행 참고.
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| SEM-03 | Med | `h1` 중복 4화면(`BoardPost`·`TeamDoc`·`Ticket`·`Chat`). `VIS-133`·`SEM`(h1=2)과 동일 원인 | ⚠️ **2026-08-12 "구현완료" 기록이 틀렸다 — 2026-08-13 실제 렌더로 재확인, 이번에 진짜로 구현완료.** 그 기록의 근거는 `grep -c 'component="h1"'`로 **각 파일 자체의 리터럴만** 센 것이었다 — `PageHeader`(`kit.jsx`)가 내부적으로 만드는 h1은 다른 파일 소스라 그 grep에 안 걸린다. 4화면 전부 실제 React 렌더 + `getAllByRole("heading",{level:1})`로 다시 확인한 결과 **전부 h1이 2개였다**(`TeamDoc.jsx`: PageHeader "문서" + 카드 안 `doc.title`, `BoardPost.jsx`: PageHeader "게시글" + 카드 안 `post.title`, `Ticket.jsx`: PageHeader `ticketId(t)`(예: "GIT-57", `VIS-133`이 지목한 바로 그 값) + 카드 안 `t.title`, `Chat.jsx`: PageHeader "AI 도우미" + 대화 제목 막대). SEM-03 자신의 근본 원인 진단("PageHeader에 제목을 넘길 수 있게 하면 세 결함이 한 번에 사라진다")대로 고쳤다 — `PageHeader`에 실제 제목을 넘기고, 카드 안 중복 요소는 `TeamDoc`/`BoardPost`/`Ticket`은 완전히 없앴다(같은 글자를 화면 맨 위와 카드 맨 위에서 두 번 읽지 않는다), `Chat.jsx`만 좁은 화면에서 서랍이 닫혀 있어도 "지금 보는 대화" 신호가 필요해 h2로 남겼다(완전 제거 대신 격하). `Ticket.jsx`에서 사라지는 `ticketId`(지원 문의 등에서 여전히 참조되는 값)는 메타 행 '티켓 번호'로 이동해 화면에서 사라지지 않는다. 새 시험 4건(`teamdoc.test.jsx`·`board-post-kind-crumb.test.jsx`·`ticket-detail.test.jsx`에 추가, `chat-page-heading.test.jsx` 신규 — `Chat.jsx`는 이 시험 전까지 전체 렌더 시험이 하나도 없었다), 각각 revert-to-verify 확인. 관련 화면 전체 회귀(15개 시험 파일, 77건) + 프런트 전체 회귀(235파일/1575건) green |
...
| `admin_departments` | 1 | 하위에 12명이 딸린 부서가 인원 0으로 보이고, 그 0이 삭제 버튼의 노출 조건이다 ‖ **재확인(2026-08-13) — 이미 해결돼 있었다(`UA-20R`).** `org.js` 삭제 확인 문구가 `child_department_count`를 보고 "하위 부서 N개가 최상위 부서로 올라갑니다(하위 부서와 소속 인원은 지워지지 않습니다)"를 이미 명시적으로 경고한다(`registry/org.js` 삭제 액션 `confirm`, 상세 `_delete_note`). 서버(`bulk_child_department_count`)도 실제로 값을 채워 보낸다. 코드 변경 없음, 문서만 정정 |
...
| `admin_feature-flags` | 2 | 설명 열이 운영자용 문구가 아니라 백엔드 개발 레지스트리 문자열을 그대로 되쓴다(R2, 미해결 — `admin_description` 신설 등 스키마 작업 필요) · ~~목록에 '기본값' 열이 없어, 위험 플래그가 기본값과 반대로 켜져 있다는 사실이 행에서 보이지 않는다~~ ‖ **구현완료(2026-08-13)**: `platform.js`에 '기본값' 열 추가 + '현재' 열이 기본값과 어긋나면 warn 톤(API `app/admin/feature_flags.py`가 이미 매 행에 `default`를 내려주고 있었다 — 순수 프런트 변경). `feature-flags-default-column.test.jsx` 신규 3건, revert-to-verify 확인 |
...
| `admin_integration-detail` | 3 | ~~모달 제목이 사람이 읽는 이름이 아니라 슬러그이고, 바로 아래 '이름' 행이 같은 값을 반복한다~~ ‖ **부분구현완료(2026-08-13)** — 슬러그 노출만 고쳤다, "반복" 자체은 남는다(아래). 원 감사 노트가 정확히 짚었듯 `name`은 `discovery.py`의 idempotency 조회 키 겸 systemd 유닛 이름이라(예: `claude-request-interpreter`) 슬러그 그대로 저장된다(저장값은 안 바꿈 — 다른 로직이 그 값으로 조회한다). `ops/opsHelpers.js::serviceLabel()`(Diagnostics.jsx 등이 이미 쓰는, 알려진 4종 슬러그→한국어 이름 매핑 + 그 밖은 kebab/snake 자동 정리)를 `registry/integrations.js`의 "이름" 열에 배선(`render`+`rowName`) — 목록도, `declaredRowName`이 쓰는 상세 드로어 제목도 이제 "요청 해석기" 등 사람이 읽는 이름을 보인다. **"제목이 첫 행과 같은 값을 반복"은 여전히 남는다** — `mergeDetailFields`(`data-screen/detailFields.js`)가 모든 `columns`를 상세 드로어 필드로도 합치는 구조라(제외 수단 없음), rowName으로 지정한 열은 제목**과** 필드 둘 다에 나온다 — `admin_job-detail`도 같은 구조적 결과다. 이건 이 화면만의 결함이 아니라 `rowName` 패턴을 쓰는 다른 화면들도 공유하는 아키텍처 특성이라 이번엔 안 건드림(새 "상세에서 숨김" 열 옵션을 신설하는 것은 더 큰 범위). 신규 시험 4건(`registry-integration-name-label.test.js`), 프런트 전체 회귀(242파일/1605건) green · ~~'설명' 필드에 운영자용 설명이 아니라 개발 중 남긴 내부 메모가 그대로 나온다~~ ‖ **재확인(2026-08-13) — 코드가 아니라 데이터 문제, 이번엔 손대지 않음.** `description`은 자유 입력 필드(`integrations.js`의 `edit`/`create` 폼에 이미 있다 — 관리자가 지금 바로 고칠 수 있다)이고 현재 값(`discovery.py`의 시드 문구, "존재 여부 사전조사로 확인" 등)은 설치 스크립트가 남긴 내부 메모다. 올바른 운영자용 설명 문구를 짓는 것은 이 서비스가 실제로 무엇을 하는지에 대한 도메인 판단이라 임의로 지어내지 않는다 — 관리자가 화면에서 직접 고치거나, 다음에 다룰 때 실제 서비스 담당자 확인 후 진행 · ~~같은 동작 버튼의 라벨이 화면마다 다르고, 화면 본문은 또 다른 용어를 쓴다~~ ‖ **재확인(2026-08-13) — `R3`(제품 전반 용어 사전) 범주로 재분류, 이번엔 손대지 않음.** 재확인해 보니 정확히 4개 지점이 같은 `/health` 계열을 각각 다른 말로 부른다: `integrations.js` 헤더 액션 "헬스체크", 러너 상세 액션 "헬스"/"테스트"(2개로 분리), 필드 라벨 "상태 확인", 도움말 문구 "헬스 체크"(띄어쓰기 다름). 연동·러너 두 화면 이상에 걸쳐 있고, 옳은 표준 용어를 하나 정해 전체를 통일하는 것은 `R3`(용어 사전)이 이미 "화면별로 쪼개 착수하지 않는다"고 못박은 범주와 정확히 같은 성격이라 별도 세션에서 `R3` 전체와 함께 판단 |
...
| `user_team-doc-detail` | 3 | ~~파괴적 동작이 화면에서 가장 무거운 버튼이고, 가장 자주 하는 동작인 본문 편집은 카드 안쪽 외곽선 버튼으로~~ ‖ **재확인(2026-08-13) — TeamDoc.jsx만의 결함이 아니라 화면 전반의 의도된 관례임을 확인, 이 화면만 손대지 않음.** `TeamDoc.jsx`의 헤더 "삭제"는 `variant="danger"`(`BUTTON_VARIANT.danger`→`contained`+`color:error`, 채워진 버튼)이고 카드 안 "편집"은 무표기(`BUTTON_VARIANT.default`→`outlined`) — 서술은 정확하다. 하지만 같은 패턴(상세 화면 헤더 액션의 파괴적 버튼=`variant="danger"`)이 `BoardPost.jsx:493`의 게시글 삭제에도 그대로 있다 — 이 화면 하나만 고치면 "삭제=채워진 빨강"이라는, 사용자가 앱 전체에서 이미 학습한 안전 신호와 어긋나는 화면이 하나 생긴다. 파괴적 액션의 시각적 무게를 재정의하는 것은 `R3`/`R6`/`R7`과 같은 부류의 **제품 전반 디자인 결정**이라 화면별로 쪼개 착수하지 않는다 — 별도 세션에서 한 번에 판단 · ~~페이지 헤더가 브레드크럼과 같은 단어를 제목으로 반복하고, 실제 문서 제목은 카드 안 두 번째 위치로 밀린다~~ ‖ **재확인(2026-08-13) — 이미 해결돼 있었다(`SEM-03`, WF42).** 현재 소스(`TeamDoc.jsx:317`)는 로드 완료 상태에서 `PageHeader`에 실제 문서 제목(없으면 "제목 없음" 폴백)을 넘겨 보여주고, 카드 안 중복 `<Typography component="h1">`은 이미 제거됐다(로딩/오류 상태의 `title="문서"`는 아직 제목을 모르는 상태이므로 그대로 둔 것, 다른 화면들과 같은 관례). `teamdoc.test.jsx`의 기존 h1 시험이 이미 이 상태를 고정하고 있음을 재실행으로 확인. 코드 변경 없음, 문서만 정정 · ~~본문 안 URL 이 본문과 같은 색·굵기의 평문이라 클릭할 수 없다(링크가 아니다)~~ ‖ **구현완료(2026-08-13)**: 팀 채팅 말풍선이 이미 쓰는 `linkifyText`(`chat/links.jsx` — 허용 도메인은 실제 링크, 그 외는 복사 버튼 폴백, 텍스트 노드만 써 innerHTML 아님)를 `TeamDoc.jsx::DocBlock`의 문단뿐 아니라 헤딩·목록·인용·콜아웃·토글·이미지 캡션까지 텍스트를 그리는 모든 자리에 적용(코드 블록은 원문 유지 — `RichText.jsx`의 같은 판단과 동일). `DocBody`는 `TicketBody.jsx`가 그대로 재사용하는 공용 렌더러라 **티켓 상세 본문도 같은 수정으로 함께 고쳐진다**(한 곳 수정, 두 화면 해결). 신규 시험 4건(`teamdoc.test.jsx` — Notion URL 실제 링크·비허용 도메인은 복사 버튼·코드 블록은 미적용·목록 항목도 적용), 관련 회귀(`teamdoc`·`ticket-detail`) + 프런트 전체 회귀(241파일/1601건) green |
...
| `CACHE-03` | Low | ~~부서/직책 개명이 `["tickets","assignees"]`(staleTime 60s)를 무효화 안 해, 다른 탭에 열린 티켓 생성/수정 모달의 담당자 후보 이름이 최대 60초 stale~~ ‖ **구현완료(2026-08-13)**: `app/tickets/service.py::list_assignees`가 담당자 후보에 이름과 함께 부서·직책·조직을 그대로 싣는다(사용자 지시 2026-08-04) — 세 화면 중 어느 하나만 개명해도 대상이었다. `data-screen/crossScreenKeys.js`의 `CROSS_SCREEN_KEYS`(X10, DataScreen 전체가 공유하는 지도)에서 `departments`/`organizations`에 `["tickets"]` 추가(기존 `["org-tree"]`와 병기), `job-titles`는 이 지도에 항목 자체가 없었어서 신설. 신규 시험 1건(`cross-screen-invalidation.test.jsx`, 기존 알림/작업큐/공지 패턴과 동일 스타일) — 값을 실제로 바꾸지 않으면 `DataScreen.jsx`의 PATCH 경로(diffFields, CONC-01)가 `api()`조차 안 부르고 조용히 끝나는 것을 처음엔 놓쳐 재현 실패 → 원인 파악 후 수정. 프런트 전체 회귀(241파일/1597건) green | 구현완료 |

WF44의 캐시 무효화 공백 3건(`CACHE-01`/`02`/`03`) 전부 구현완료.
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| RET-01R | Med | ✅ 구현완료(행 정정 + 실제 결함 수정) — 이 행의 "`UserSession` 정리 코드 0건" 전제는 **작성 시점에 이미 stale** 했다: `retention.py::purge_old_sessions()`(CORE-02)가 이미 있었고 `run_retention()`에도 연결돼 있었다(`"sessions": purge_old_sessions(...)`). 다만 그 함수는 `revoked_at IS NOT NULL` 행만 지웠는데, `sessions.py::validate()`의 만료 판정은 **그 토큰이 다시 제시될 때만** 도는 지연 판정이라 만료 후 아무도 다시 찾지 않은 세션(재로그인해 새 세션을 만들고 예전 탭은 버리는 흔한 경우)은 `revoked_at`이 영원히 안 찍혀 정리 대상에서 빠졌다 — "만료 353"과 "폐기 266"이 합이 378을 넘는 것(중복 카운트)도 이 사실과 일치한다. `purge_old_sessions`가 `revoked_at IS NULL AND expires_at < cutoff`도 함께 지우도록 수정, 새 테스트 `test_purge_old_sessions_removes_aged_expired_never_revoked` 추가(수정 전 실패 → 수정 후 통과 확인). 기존 `test_purge_old_sessions_keeps_active_sessions_forever`는 이름과 달리 `expires_at`이 200일 지난(=이미 죽은) 세션을 "안 지워짐"으로 고정하던 오탐 테스트였음을 확인, `expires_at`을 미래로 바꿔 `test_purge_old_sessions_keeps_unexpired_sessions_forever`로 정정. `retention.py:106,122` 재확인(행 정정, 2026-08-13) | 구현완료 |
| RET-03 | Low | `game_rooms`·`game_events` 도 보존 대상에 없다(`retention.py` 에 `game` 0건). 다만 `disband_room` 주석이 **"히스토리를 남기지 않는다(§16.1)"** 라고 의도를 밝히므로 세션과 성격이 다르다 — 정책이 "안 남긴다"인데 행은 남는 불일치다(`GM-04`) ‖ **구현완료(2026-08-13)** — `app/games/models.py` 모듈 docstring이 만들어질 때부터 이미 "게임 히스토리는 남기지 않는다(§16.1): 방이 끝나고 정리 시간이 지나면 방·이벤트를 지운다(retention에서 처리, 여기선 스키마만)"라고 명시했는데 그 처리 자체가 없었다. 신설 `purge_old_game_rooms()`가 `closed_at IS NOT NULL`(끝난 방)만, 보존기간(기본 7일, `game_room_retention_days` 설정으로 override 가능 — 다른 보존값들과 같은 관례) 지난 것만 지운다. 열려 있는 방은 나이와 무관하게 절대 안 지운다(`purge_old_sessions`와 같은 판단) — GM-01이 고친 `cleanup_idle_rooms`가 유휴 방을 이미 `closed_at`으로 닫아 두므로 여기 도달 못하는 "영원히 열린 채 방치된 방"은 없다. FK에 `ON DELETE CASCADE`가 없어(0019 마이그레이션 확인) 자식(`game_room_members`·`game_events`)을 먼저 지운다(`purge_old_conversations`와 같은 순서), `batched()` 재사용. `run_retention()`의 결과 딕셔너리에도 `game_rooms` 키로 배선. 신규 시험 4건(`test_retention_purge.py`) — 끝난 지 오래된 방+멤버+이벤트가 함께 지워짐·열린 방은 나이 무관 보존·보존기간 안의 방은 아직 보존·`run_retention()` 배선 확인. revert-to-verify 확인함(되돌리면 `purge_old_game_rooms` 자체가 없어 테스트 모듈이 `ImportError`로 아예 수집 실패). `core/retention.py`(신설 함수)·`games/models.py`(모듈 docstring, 코드 변경 없음) 재확인(2026-08-13) | 구현완료 |
| UA-10 확증 | Med | **`/api/trash` 폴링 15초 간격을 실측했다** — 40초 동안 3회, 간격 정확히 `15.0`·`15.0`. 응답 키는 `items`·`retention_days` 뿐이고 **`total` 도 페이지 필드도 없다.** `?limit=1`·`?page=1&page_size=1` 을 줘도 **무시**한다(둘 다 같은 응답). 기존 `UA-10`(무제한 + 15초 폴링) **확인 완료** ‖ **구현완료(2026-08-13)** — `UA-10` 행에서 실제 수정(백엔드 `limit`/`total` + 프런트 "더 보기") 완료. `?limit=1`은 이제 실제로 1건만 돌려주고 `total`이 응답에 실린다 — 이 행이 실측한 두 증상(무시됨·필드 없음) 모두 해소 | 구현완료 |
...
| VIS-151 | Med | **가장 중요한 구역이 가장 비어 있다.** 「확인이 필요한 항목」에 카드가 **딱 하나**(폭 250px)이고 그 줄의 나머지 **1,300px 가 빈다**. 아래 구역들은 4~5장씩 채워져 있어, 시선이 가장 먼저 닿는 자리가 가장 허전하다(`VIS-137` 카드 줄바꿈과 같은 뿌리) ‖ ✅ **완결(2026-08-17, D-110)** — `VIS-119`/`VIS-137`과 같은 수정(`DataScreen.jsx` 요약 카드 그리드 `auto-fit`, `minmax(14rem,1fr)`)으로 닫힘. `unreadCountKey`(이 행이 말하는 "안 읽음" 단일 카드) 경로도 같은 상수를 쓰도록 통합해, 카드 1장이 줄 전체 폭으로 늘어나 "확인이 필요한 항목" 강조 카드다운 무게를 갖는다(TEST SERVER 실측: 카드 우측 끝이 컨테이너 우측 끝과 정확히 일치, 빈 칸 0px) |
...
| VIS-156 | Med | **클로비 이미지가 한 화면에 5개** — 상단바 「클로비」 버튼 · 채팅 패널 헤더 마스코트 · 가운데 큰 마스코트 · 결과 패널 마스코트 · 좌하단 도킹 카드. `VIS-116`(3개, 1920)이 4K 에서 **5개**가 된다 ‖ ✅ **`VIS-116`과 같은 커밋으로 부분 구현완료(2026-08-15)** — 상단바 버튼은 이미 `/chat`에서 안 그려지고(재확인), 좌하단 도킹 카드는 이번에 그렇게 고쳤다 — 실제 동시 노출 가능한 개수는 5가 아니라 최대 3(패널 헤더 상태 · 빈 스레드 환영 · 빈 결과 레일, 뒤 둘은 각자 자기 조건에서만). 남은 3개는 클릭 가능한 진입점이 아니라 상태/빈 상태 표시라 `VIS-116`에서 각각 남긴 이유와 같다 |
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| VIS-158R | Med | ⚠️ 오탐 정정(2026-08-13) — ~~"이 제품에서 AI 답변을 가장 잘 보여 주는 방식이 화면 폭 2200px 이상에서만 존재한다. 1920 이하(대다수 사용자)는 같은 질문에 **말풍선 안 텍스트 목록**을 받는다."~~ **핵심 전제가 틀렸다**: `Chat.jsx`를 직접 읽고 `Message`(`chat/MessageThread.jsx`)를 렌더링해 재확인함 — `hideCards`는 `!!railMsg && railMsg.id === m.id`로만 계산되고 `railMsg`는 `railOpen`(`useMediaQuery(xxl 이상)`)이 거짓이면 항상 null이다. 즉 xxl(2200px) 미만에서는 **모든** 메시지의 `hideCards`가 항상 false라, `Message`가 말풍선 **안에** `CardStack`(제목·상태 배지·담당자·마감이 있는 진짜 카드, 레일과 동일한 컴포넌트)을 그대로 그린다 — "텍스트 목록"이 아니다. 신규 시험(`chat/message-thread-inline-cards.test.jsx`)이 `hideCards=false`일 때 카드가 실제로 보이는 것과 `hideCards=true`(레일이 대신 보여주는 그 메시지)일 때만 중복 방지로 숨는 것을 둘 다 확인. **실제로 남는 차이**(그래서 완전한 오탐은 아니라 등급을 Med로 낮춤)는 접근성이 아니라 편의성이다 — ≥xxl은 마지막 카드가 스크롤과 무관하게 오른쪽에 계속 남지만, xxl 미만은 다음 메시지를 치려면 그 카드가 있는 자리로 스크롤을 되돌려야 한다 | 발견(등급 하향) |
...
| DEPLOY-05 | **Critical** | **`build-bundle.sh`가 낡은 프런트 산출물을 조용히 배포할 수 있었다.** 이 스크립트는 프런트를 다시 빌드하지 않고 그 시점의 `app/static/react`를 그대로 패키징만 한다. 신선도 확인 단계로 쓰던 `check_bundle_fresh.py --write`는 **지금 `frontend/src`의 해시를 무조건 다시 적을 뿐**, `app/static/react`의 실제 컴파일 바이트가 그 해시와 맞는지는 검증하지 않는다 — `npm run build`를 빼먹고 `--write`만 돌리면 도구가 스스로 "최신"이라고 착각한다. 실제로 VIS-141(Board.jsx 공감 열) 배포 후 이 경로로 낡은 번들이 나갔다: 로컬에 새로 패키징한 스테이지의 `UserRoutes.js` 해시가 배포된 서버 파일과 정확히 일치했다(=재빌드가 반영 안 됨). `check_bundle_fresh.py` 자신의 docstring이 "이전 세션에서 이미 한 번 같은 사고가 났다"고 적어 뒀을 정도로 재발성 있는 실패 유형인데, `static_checks.sh`/`final_verify.sh`/서버측 install·update 스크립트에는 이 검사가 배선돼 있었지만 정작 "무엇을 배포할지 결정하는" `build-bundle.sh`에는 한 번도 연결되지 않아 안전망을 완전히 우회할 수 있었다 | **구현완료**(2026-08-16) — `build-bundle.sh` 맨 앞에 `check_bundle_fresh.py` plain 모드(검증만) 게이트 추가, 실패하면 비싼 패키징(rsync/wheelhouse/manifest) 이전에 즉시 종료(스스로 빌드하지 않음 — 무엇을 소스로 삼을지는 운영자가 정함). revert-to-verify: 게이트 추가 전 상태(스탬프 없음)를 그대로 재현해 `[FAIL]`·exit 1 확인 → `npm run build`+`--write`로 복구 후 통과·끝까지 패키징 확인. 그 번들을 실제 TEST SERVER에 재배포하고 서버의 `BUILD_STAMP.json`·`UserRoutes.*.js` 해시·`grep 공감`으로 배포본이 최신 소스를 반영함을 직접 확인, 71라우트×light/dark(138페이지) 전체 재검증 green(`auth_ok`·`horizontal_overflow`·`console_errors`·`page_errors`·`broken_images` 전부 0 실패) — VIS-141(자유게시판 공감 열)·UB-26(미할당 티켓 동기화 배너)·OrgTree.jsx 타이포그래피를 스크린샷으로 직접 재확인. 상세: `docs/DECISIONS.md` D-77 |
...
| ID | 심각 | 문제 |
|---|---|---|
| FN-41 | High | **`/team-docs` 목록이 범위를 페이지를 자른 뒤에 파이썬으로 거른다**(`team_docs/router.py:124-126`) → 총계는 안 맞고 페이지는 안 채워진다. 사용자가 **「3건」 페이저와 빈 목록을 동시에** 본다 ‖ **보류(MEGA CYCLE I, 코드 확인)**: 검색·타입·프로젝트·기술태그 필터는 이미 SQL에서 페이지 자르기 전에 걸린다(`repository.list_documents`) — 문제는 `doc_in_scope`(부서 범위) 하나만 라우터에서 **그 페이지 결과에만** 사후 적용되고, `total`을 "이번 페이지에서 걸러진 수"로만 보정해 다른 페이지의 잠재적 손실을 못 잡는다. 제대로 고치려면 부서 범위를 SQL로 옮기거나(작성자 해석·`core/scope.py` 판정이 복잡해 어려움) 전체 결과를 먼저 불러와 거른 뒤 파이썬에서 페이지를 잘라야 하는데, `doc_in_scope`가 호출마다 `_verified_id_to_user(db)`를 다시 긋는 기존 N+1 패턴이 있어(격리된 상태로도 존재하던 비효율) 그 리팩터까지 같이 해야 안전하다 — quick-fix 배치 범위를 넘어서 다음 사이클로 미룬다 ‖ **구현완료(2026-08-16)** — 두 부분을 함께 고쳤다. ① `_verified_id_to_user`(N+1)에 대응해 `service.doc_in_scope`가 선택 인자 `id_to_user`를 받게 확장(안 주면 기존처럼 내부에서 직접 구함 — 단건 조회 등 기존 호출부는 무변경). ② 목록 라우터가 이제 검색/타입 등으로 좁힌 결과 **전체**(상한 `_SCOPE_FILTER_FETCH_CAP=5000`, 이 회사 문서 수 규모(수백~두 자릿수 천 단위)를 넉넉히 웃돎)를 한 번에 가져와 `id_to_user`를 한 번만 구해 전량에 범위를 적용한 뒤, 그 결과를 파이썬에서 페이지로 자른다 — `total`이 항상 "범위 안에서 실제로 존재하는 전체 개수"이고 페이지는 있는 만큼 채워진다. 스키마 변경(정규화된 작성자 조인 테이블) 없이 가능한 가장 정확한 수정이라고 판단 — `author_notion_ids`가 구분자 문자열이라 SQL WHERE로 범위를 직접 표현하기 어렵다는 원 진단은 여전히 유효하다(그래서 SQL이 아니라 애플리케이션 레벨 전체 필터링을 택함). 신규 회귀 `tests/security/test_team_docs_pagination_scope.py` 3건 — 제목 정렬 시 범위 안/밖이 번갈아 나오게 만들어(1=밖,2=안,3=밖,4=안,5=밖,6=안) "SQL이 먼저 자른 페이지 안에 범위 밖 문서가 섞인" 상황을 결정적으로 재현, revert-to-verify 확인(옛 순서로 되돌리면 1페이지가 1건으로 짧게 나오고 total이 틀리는 것을 정확히 재현). 관련 스위트(team_docs API·scope·filter·write + 골든 계약) 전체 green |
| FN-42 | High | **프로젝트 Health 가 5개 규칙 중 1개만 판정돼도 `100점`이 되고 그 값이 캐시된다.** 대시보드 `unscored` 는 NULL 만 세므로 **"못 잰 것"으로도 안 잡힌다.** 진실은 상세 GET 만 안다(`unknown`) ‖ **보류(MEGA CYCLE I, 코드 확인)**: `health.py`의 `_Ledger.result()`를 읽어 확인 — `checked`가 비어야만 `None`이고, 1개라도 checked면(감점 0이어도) 나머지 상한(100)에서 감점만 빼 점수를 낸다. 이건 모듈 스스로 선언한 계약과 **일관된 동작**이라 "버그"로 단정하기보다 **신뢰도(몇/5 판정됐는가)를 화면에 드러내지 않는 설계 공백**에 가깝다. 진짜 고치려면 (a) 대시보드가 신뢰도를 알 방법이 없다(캐시된 `health_score`는 정수 하나뿐, `checked` 개수는 스냅샷 계산 시점에만 존재하고 영속화 안 됨 — 마이그레이션 필요) 또는 (b) `compute_health()` 자체의 None 판정 기준(현재 "0개 checked")을 바꾼다(16개 테스트가 현재 계약을 정밀하게 고정해 뒀다 — 임계값을 잘못 고르면 조용히 다른 정상 케이스를 깬다). 둘 다 신중한 별도 검토가 필요해 이번 quick-fix 배치에서는 손대지 않는다 ‖ ✅ **완결(2026-08-16, D-107)** — 재검토 결과 (a)의 전제("checked 개수는 영속화 안 됨")가 틀렸다: `record_health_snapshot`이 `health_score`를 캐시할 때 **같은 트랜잭션**에서 `ProjectHealthSnapshot.reasons_json`에 `checked`/`unknown`까지 이미 같이 적고 있었다(주간 이력 목적으로 원래도 있던 데이터, `health_score`를 쓰는 유일한 경로가 이 함수뿐임을 grep으로 확인). 마이그레이션도 (b)의 임계값 변경도 없이, `service.py::latest_checked_rule_counts()`(프로젝트별 최신 스냅샷의 checked 개수 배치 조회) 신설 + `home/work.py`가 `unscored`와 별개로 `low_confidence`를 세도록 확장 + `Dashboard.jsx`에 구별되는 문구 추가로 닫았다. 백엔드 통합 시험(`test_dashboard_metrics.py` 신규 1건 + 관련 156건) + 프런트(`dashboard-work.test.jsx` 신규 1건 포함 8건) 전부 green |
...
| BKP-05 | Low | 백업이 **원본과 같은 볼륨**(`/var/backups` ↔ `/var/lib`)에 있고 오프사이트 사본이 0이다. 디스크·볼륨 장애 하나에 원본과 백업이 함께 사라진다 | **재확인(2026-08-16), 여전히 사실 — 앱 코드로 못 고침** — TEST SERVER 확인 결과 `/var/backups`·`/var/lib`가 지금도 같은 파일시스템/볼륨이다. 진짜 수정(별도 볼륨 마운트 또는 오프사이트 전송 대상— S3·rsync 원격 등)은 애플리케이션 코드가 아니라 서버 인프라 프로비저닝 결정이라 이 저장소의 변경 범위 밖이다. `RSTR-02`(주기적 백업 수단 부재)와 같은 자리에서 함께 볼 인프라 과제로 남긴다 |
...
| ID | 심각 | 남는 것 | 상태 |
|---|---|---|---|
| VIS-161 | Low | 두 표면의 `z-index` 가 **둘 다 1300** 이고 DOM 순서로만 위아래가 갈린다. 지금은 맞지만 같은 층에 다른 오버레이가 끼면 순서가 흔들릴 수 있다 | **재확인 시도(2026-08-16), 대상 특정 못 함** — 소스에 리터럴 `1300`이 없다(grep 0건) — MUI `Dialog`/`Modal`/`Drawer` 등이 커스텀 없이 테마 기본값(`theme.zIndex.modal=1300`)을 그대로 쓰는 두 컴포넌트 조합으로 추정되나, 원 서술에 구체적 파일 인용이 없어 어느 두 표면인지 지금 특정할 수 없다. 지금 당장 재현되는 결함이 아니라(자체 서술도 "지금은 맞지만") 실브라우저로 여러 오버레이 조합을 직접 열어 보는 전담 조사가 필요 — 이번엔 미룬다 |
| VIS-163 | Med | **`/sprint`(스프린트 회의) 담당자별 티켓 표의 '담당자' 열이 1200×900에서 세로로 무너진다** — `vertical_text_collapse` 하네스 실측(2026-08-17, `ui-qa-user` 계정 실캡처, light+dark 둘 다 재현): 셀 실폭 **61.2px**(줄당 2.7자)로 눌려 "임승환, 김동현" 같은 값이 한 글자씩 세로로 흐른다. 열 정의는 `frontend/src/screens/MyTickets.jsx:151`의 공유 `ticketColumns()`(`assignee_names`, `width: "10rem"`) — DataTable이 아니라 별도 컴포넌트 `GroupedTickets`(`MyTickets.jsx:225`, `useMediaQuery(TABLE_CARD_QUERY)`로 자체 좁은 화면 카드 전환을 이미 갖고 있다)가 그린다. 이 표는 티켓/제목/상태/우선순위/난이도/예상 WD/마감/담당자 8열을 쓰는데, 1200px 컨테이너(실측 846px)에 다 욱여넣으면서 `width` 힌트가 `tableLayout` 자동 계산에서 바닥값 취급돼 담당자 열이 가장 많이 눌린 것으로 보인다(RESP-01이 `kit.jsx::DataTable`에 만든 `hideNarrow`/`TABLE_COMPACT_QUERY` 메커니즘과 같은 종류의 문제이지만, 이 표는 그 컴포넌트를 안 쓰므로 그 수정이 자동으로 안 미친다). `GroupedTickets`는 내 티켓·미할당·팀 티켓·스프린트 4곳이 공유하므로 고칠 때 4곳 다 회귀 확인 필요(`TABLE_CARD_QUERY`의 정확한 breakpoint 값과 1200px이 그 안/밖 어디인지부터 확인할 것) ‖ ✅ **완결(2026-08-17)** — 진짜 원인은 `width` 힌트/`tableLayout`이 아니라 **이 열만 `nowrap: true`가 빠진 것**이었다(재조사로 확정): 나머지 7열(티켓·상태·우선순위·난이도·예상 WD·마감·작업)은 전부 `nowrap: true`가 있어 `overflowWrap: normal`로 렌더되는데, `assignee_names`만 그 플래그가 없어 `overflowWrap: anywhere`가 적용돼 글자 단위로 쪼개졌다(`GIT-4101` 세로 붕괴를 막았던 바로 그 메커니즘, `MyTickets.jsx:127-130` 주석 참고 — 그 수정 당시 이 열은 아직 없었거나 빠뜨렸던 것으로 보인다). `assignee_names` 컬럼 정의에 `nowrap: true` 한 줄 추가로 해결 — `ticketColumns()` 공유 함수라 4개 소비처(내 티켓·미할당·팀 티켓·스프린트) 전부에 자동 적용된다. 담당자가 많아 셀이 넓어지면 다른 nowrap 열과 같은 트레이드오프(가로 스크롤)를 따른다 — 이 저장소가 이미 선택한 트레이드오프와 일관됨. 신규 시험 2건(`grouped-tickets-collapsible.test.jsx`), revert-to-verify 확인함. `ticketColumns` 소비처 11개 테스트 파일(150건) + 프런트 전체 회귀(286파일/1,983건) green |
| VIS-162 | Low | 중첩 모달(992px)이 드로어(992px)를 **완전히 덮어** 무엇을 편집 중인지 뒤 맥락이 안 보인다. 제목이 「업무 도우미 수정」이라 이름으로는 알 수 있다 ‖ **구현완료(2026-08-13)** — 근본 원인은 `DataScreen.jsx`(모든 등록 화면이 공유하는 상세→수정 흐름) 하나였다: 상세(`Modal open={!!sel} size="lg"`)에서 "수정"을 누르면 `setEditing(sel)`만 하고 `sel`은 그대로 둬 상세 Dialog가 DOM에 **그대로 열린 채** 남았다 — 수정 폼(필드 6개 이상이면 똑같이 `size="lg"`=992px)이 그 위에 완전히 겹쳐, 픽셀 하나 안 남기고 덮었다(같은 `runAction`의 `navigate` 분기는 이미 `setSel(null)`로 이 문제를 피하고 있었는데 수정/액션 분기엔 안 걸려 있었다). 상세 Modal의 `open` 조건에 `&& !editing && !actionForm && !subView && !infoView`를 추가 — 중첩 오버레이 넷 중 무엇이 열리든 상세는 화면에서 숨고(sel 값 자체는 유지), 그게 닫히면 같은 상세로 자동 복귀한다(취소해도 방금 보던 상세로 돌아온다). 신규 시험 2개(`data-screen-nested-modal-visibility.test.jsx`) — 처음엔 `getAllByRole("dialog")`로 검사해 **고치기 전 소스로도 통과**하는 오탐 테스트였다(MUI가 겹친 아래 모달을 DOM에서 지우지 않고 `aria-hidden`만 붙이는데, role 기반 조회는 접근성 트리를 따라가 그 상태를 "1개"로 잘못 읽는다) — DOM을 직접 보는 assertion(`aria-hidden="true"`인 `.MuiDialog-root` 존재 여부 + 상세 전용 텍스트가 다이얼로그 안에 남아있는지)으로 정정한 뒤에야 정확히 실패/성공을 구분했다. revert-to-verify 확인함 | 구현완료 |
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| SEC-20 | **High** | **조사 과정에서 sudo 비밀번호를 명령행에 반복 노출했다**(불변규칙 §2-4 위반). 로컬·원격 히스토리, 프로세스 목록, 대화 기록에 남았다. `CLAUDE.md` §10 이 이미 *"대화에 노출된 SSH 비밀번호 변경"* 을 남은 조치로 적어 뒀는데 **내가 그 노출을 크게 늘렸다** ‖ **범위 확장(Product Audit `PA-RC-0003`, 2026-08-15) — 아직 회전되지 않았고, 같은 자격증명이 살아 있는 표면이 하나 더 있다.** `git stash` 의 `stash@{0}` 에 TEST 서버 SSH 비밀번호와 sudo 비밀번호가 **평문 리터럴로** 담긴 `CLAUDE.md` 변경이 보존돼 있다. 그 변경은 자격증명을 Git·tracked docs·명령행에 저장해도 된다고 규칙을 바꾸면서 동시에 **"이 자격증명의 저장/사용을 보안 결함·회전 필요 사유로 재분류하지 않는다"** 고 지시한다 — 탐지 억제 지시가 포함돼 있다. 이전 세션이 적용을 거부하고 사람 검토용으로 stash에 남겼다. **HEAD와 워킹트리는 깨끗하다 — 유출은 stash 안에만 갇혀 있다.** 기존 보안 검사가 워킹트리와 커밋만 보고 **stash/reflog/dangling 객체를 안 보기 때문에** 어떤 스캔도 이것을 못 봤다. **AI 구현 대상이 아니다** — 순서는 ① 사람이 `git stash show -p 'stash@{0}'` 검토 ② **먼저 회전**(stash를 지워도 노출된 값은 회수되지 않는다) ③ `git stash drop` ④ 정책 변경 의도가 실제로 있었다면 `DECISIONS.md`에 근거를 남기고 `CLAUDE.md`를 **값 없이** 고친다. Auditor는 ①~④를 수행하지 않았다(회전은 되돌릴 수 없는 운영 결정이고 `stash drop`은 사람이 보기 전에 증거를 지우는 일이다). 상세: `PRODUCT_AUDIT_HANDOFF.md` `PA-RC-0003` | **탐지 공백은 닫힘(2026-08-16), 회전은 여전히 사용자 조치 필요** — `PA-RC-0003`이 요구한 실제 구현 대상(검사 공백 자체)을 처리했다: `scripts/check_git_secrets.py` 신설(stash·reflog-only 커밋·`git fsck --unreachable`의 dangling 객체 3표면을 자격증명 패턴으로 훑고, **값은 한 글자도 출력하지 않는다** — 어느 객체·어느 파일·몇 번째 줄인지와 패턴 분류명만 보고), `static_checks.sh`에 새 필수 단계로 배선(예외/우회 경로 없음). 격리된 임시 저장소에서 더미 비밀로 revert-to-verify하는 회귀 테스트 4건(`tests/unit/test_git_secrets_scan.py`) — 실제 저장소의 진짜 stash는 시험 과정에서 전혀 건드리지 않았다. **직접 실행해 재확인**: 이 검사를 켜자마자 실제로 `stash@{0}`(위 사건)과 별개로 `reflog-only commit 1b819ce793af`에서 `runner/claude-work-assistant/assistant.py`(현재 코드 아님 — 과거 amend로 브랜치에서는 사라졌지만 reflog가 살려 둔 옛 버전) 1건이 추가로 걸렸다 — 안전 여부는 값을 보지 않고는 판단 불가하므로 미판정으로 남긴다. 또한 이번 조사 중 같은 계열의 초기 임포트 커밋(`f0efc52af4ec`, HEAD에서 reflog로만 도달 가능)에서 `docs/migration_backups/team_docs_tagging_backup.py`의 옛 버전에 Notion 토큰으로 보이는 값이 있었으나(패턴만 확인, 값 미출력) **현재 HEAD의 같은 파일은 이미 `os.environ.get(...)` 참조로 안전하게 고쳐져 있음을 직접 대조 확인**했다 — 다만 그 옛 값 자체는 reflog가 만료되기 전까지 여전히 `.git` 안에 존재한다. **회전은 여전히 이 저장소 밖의 운영 행위라 AI 구현 대상이 아니다** — stash 안내(①~④)는 그대로 유효하고, 이제 추가로 reflog 쪽 옛 커밋도 (필요하다면 새 값으로) 같은 순서(회전 먼저)로 정리 대상이다. `docs/DECISIONS.md`에 근거 기록 ‖ **재정밀화 + 미판정 해소(2026-08-16, WARM 재개)** — 검사를 실행하니 진짜 결함(2건) 외에 `reflog-only commit f0efc52af4ec`(초기 임포트, 커밋 메시지 자체가 "secrets excluded, sudo password scrubbed")에서 약 50건이 오탐됐다(`token = new_session_token()`류 — 값이 아니라 코드). 값을 절대 출력하지 않는 원칙 안에서 매칭된 값의 길이·대소문자·숫자 포함 여부만 boolean으로 확인하는 방식으로 안전하게 진단, `CODE_REFERENCE_RE`(따옴표 없이 식별자+`(`/`[`로 이어지면 참조로 인정) 신설로 오탐 50→29건. 위 "미판정으로 남긴다"고 적었던 `1b819ce793af`도 같은 shape 기반 검증으로 재조사해 `os.environ.get(...)` 패턴임을 확정 — **미판정 → 확인된 안전**. 남은 29건(같은 초기 임포트 커밋)도 샘플링 확인 결과 전부 같은 성격(코드 참조·QA 픽스처의 명백한 더미 값)이나 전수는 안 함 — 회전 후 `git gc`로 결국 사라질 1회성 유물이라 판단. 신규 회귀 1건(`test_code_that_merely_references_a_token_variable_is_not_flagged`), `test_git_secrets_scan.py` 5/5 green. **PA-RC-0003은 구현 관점에서 완결** — 남은 것은 여전히 사람의 회전 조치뿐이다. 상세: `DECISIONS.md` D-82 |
| SEC-21 | Med | **서브에이전트에게 자격증명을 프롬프트로 배포했다.** 위임 시 자격증명을 넘기지 않는 방법(에이전트는 읽기 전용 산출물만 받게 하거나, 서버 접근이 필요한 부분만 내가 실행)이 있었는데 쓰지 않았다 | ⚠️ **조사완료(2026-08-16), 코드 수정 대상 아님** — 이건 diff로 못 고치는 과거 위임 판단(프로세스) 기록이라, 대신 **잔존 노출 여부**를 확인했다: ⓐ `git log --all -p`로 저장소 전체 이력에서 `CLOVIR_TEST_SUDO_PASSWORD` 실값 대입 패턴을 찾았지만 유일한 일치는 문서의 안내문 안 **자리표시자**(`'<비밀번호>'`)였다 — 실제 값 유출 없음. ⓑ `var/`(gitignore됨, git 추적 밖)의 Supervisor 로그·프롬프트 파일(`var/product-audit/logs/*`·`var/runner/logs/*`)도 같은 패턴으로 훑었으나 변수 **이름**만 나올 뿐(사용법 안내) 실제 대입값을 가진 파일은 0건. 즉 이 지적이 가리키는 과거 위임 판단 자체는 남아 있어도(고칠 diff가 없다), 그로 인한 **실제 자격증명 노출 잔존물은 발견되지 않았다.** 이 세션 내내 지켜 온 실제 관행(sudo 필요 명령은 서버 접근이 필요한 부분만 직접 실행하고 `printf '%s\n' "$CLOVIR_TEST_SUDO_PASSWORD" \| ssh ...`로 stdin 전달, 에이전트 위임 프롬프트에는 자격증명을 넣지 않음)이 이미 CLAUDE.md 불변규칙 §4가 요구하는 대로이므로 이것이 사실상의 재발 방지책이다 |
...
| ID | 심각 | 문제 | 고칠 지점 |
|---|---|---|---|
| `OPS-01` | **Critical** | **업로드 디렉터리가 root 소유라 첨부가 안 올라간다**(위 `OPS-01` 절, 내가 실서버에서 확정) | `chown -R` + installer 목록에 `uploads` 추가 + `readyz` 에 쓰기 가능성 ‖ ✅ **실환경검증완료(2026-08-15 재확인)** — 이 행에 표시가 빠져 있었을 뿐 실제로는 이미 고쳐져 있었다. 승인된 TEST SERVER에서 직접 확인: `/var/lib/clovirone-web-assistant/uploads`가 서비스 실행 계정(`clovirone-web`, systemd `User=`와 동일)이 소유(`drwxr-x--- clovirone-web:clovirone-web`)하고, 그 안의 `uploads/ticket/`이 오늘 날짜(Aug 15)로 실제 갱신돼 있어 첨부가 정상 동작 중임을 확인. `install-clovirone-web-assistant.sh:155`의 디렉터리 목록에도 `uploads`가 이미 포함(주석: 앱 자체도 첫 업로드 시 `mkdir(parents=True, exist_ok=True)`로 자가 치유). 문서 표시 누락만 정정 |
...
| `BKP-10` | High | **백업 보존이 "7일"이 아니라 "7개"다.** 배포 백업이 **같은 스크립트·같은 이름 규칙**으로 만들어져 일일 백업과 **카운터 7칸을 공유**한다 → **배포가 잦은 날 하루에 일주일치 복원 지점이 증발한다** | ✅ **구현완료(2026-08-15)** — `scripts/backup-cron.sh`: `KEEP_PLATFORM=7`(개수) → `KEEP_PLATFORM_DAYS=7`(나이, `find -mtime +$KEEP_PLATFORM_DAYS`)로 교체. 배포 빈도와 무관하게 항상 최소 7일치가 남는다. `bash -n` 구문 확인 + 합성 타임스탬프 디렉터리로 로컬 시뮬레이션(revert-to-verify): 같은 픽스처(4일 이내 백업 8개, 그중 3개는 "오늘 배포"로 가정)에 옛 로직을 돌리면 4일 된 백업 하나가 개수 초과로 잘못 지워지고, 새 로직은 8개 전부(전부 7일 이내) 보존함을 직접 확인. 실서버 cron 실행 자체는 미검증(다음 주기적 실행에서 자연히 증명됨) |
...
## `SEC-32`~`DBTX-01` — Whole-product 재감사(invocation=3) 발견, 상세는 `WORK_STATE.md`

승인 취소 알림 누락은 이 재감사에서 함께 나왔지만 기존 승인 파이프라인 섹션에
`APPR-04`로 이미 적었다(§1713 근처) — 여기서는 중복 기재하지 않는다.
...
| SEC-34 | **Critical** | **`admin_scope='org'`(조직 관리자)가 `scope.is_dept`만 보는 곳에서는 전부 `global`과 똑같이 무제한이었다 — SEC-12/13/32와 같은 형제-함수-누락 계열이 org 축에서 재발.** `core/scope.py`(정본, `scope_filter`/`scope_allows_user`)는 이미 3갈래(global/org/dept)를 올바르게 구분하는데, 여러 소비처가 그걸 안 쓰고 `if not scope.is_dept: return True/None`(또는 그 반대) 식으로 직접 재구현하면서 **org를 dept가 아닌 것 = global**로 오판했다. 전수 grep(`not scope.is_dept`/`.is_dept` 전체)으로 확인한 실제 영향 범위: `team_docs/service.py::doc_in_scope`(목록·상세·**휴지통 이동·비공개 지정·댓글 등 쓰기 전부**, `get_doc_in_scope` 하나로 모임) · `tickets/service.py::ensure_in_scope`(상세 + 상태변경·댓글·첨부·배정 등 **6곳 이상의 쓰기**) · 같은 파일 `_scope_assignee_ids`·`_drop_out_of_scope`(팀 티켓 목록·스프린트/어시스턴트 집계) · `trash/repository.py::list_visible`·`visible_to`(휴지통 목록 + **되돌릴 수 없는 영구삭제**). `search/scoping.py`의 두 인스턴스는 직접 대조해 **결함 아님**으로 확인(하나는 이미 올바른 3갈래 분기, 하나는 org 제한이 그 앞단 SQL 절에서 이미 걸려 dept 전용이 의도임을 모듈 docstring이 명시) | ✅ **구현완료(2026-08-16)** — 전용 RBAC 재감사 서브에이전트가 `team_docs` 먼저 발견, 그 뒤 같은 패턴으로 전수 grep해 tickets 3곳·trash 2곳을 추가 발견. 전부 `if scope.is_global: return <무제한값>`으로 정정(진짜 무제한은 global뿐 — org/dept 구분은 이미 올바른 `visible_user_ids`/`scope_filter`에 맡김, 새 분기 로직 없음). 신규 회귀 8건(`test_document_scope.py` 3 · `test_ticket_detail_scope.py` 3 · `test_trash_scope.py` 2, 전부 org 범위 관리자 vs 다른 조직 자원으로 읽기·쓰기 양쪽 확인), revert-to-verify 전부 완료(각 함수를 개별적으로 되돌려 정확히 예측한 증상 재현: 문서 휴지통 이동 200(성공), 티켓 PATCH 200, 휴지통 영구삭제 시도가 스코프를 통과해 버림 — 확인 후 전부 복원). `tests/security/` 전체 + 백엔드 전체 스위트 green |
| SEC-35 | **Critical** | **자유게시판 새 글이 작성자와 무관하게 전부 기본 조직으로 저장됐다 — SEC-34 뒤처리 중 발견.** `app/board/service.py::create_post`가 `Post(...)`를 만들 때 `org_id`를 아예 안 채워, `OrgScopedMixin`의 컬럼 기본값(`DEFAULT_ORG_ID`)에 맡겨졌다. 결과가 양방향: ⓐ 기본 조직이 아닌 사용자가 쓴 글은 **작성 직후 자기 눈에도 안 보인다**(자기 조직으로 거는 조회와 실제 저장값이 안 맞음) ⓑ 기본 조직 사람은 **다른 조직이 쓴 글을 그대로 본다**(1순위 유출 #4가 막으려던 경계가 저장 단계에서부터 안 걸림). `SEC-34`(`_viewer_org_id`가 전역 관리자를 부당하게 좁히던 결함)를 고치는 과정에서 전역 관리자 시험을 작성하다가, 두 조직의 글이 **똑같은 org_id**로 저장된 것을 실측(DB 직접 조회)으로 발견 — 기존 격리 시험들은 전부 우연히 org A(=기본값과 같은 값) 사람이 쓴 글로만 확인해 이 결함을 못 잡고 있었다 | ✅ **구현완료(2026-08-16)** — `create_post`에 `org_id=author.org_id` 추가(단 한 줄). `_viewer_org_id`도 함께 정정: `getattr(me, "org_id", None)`이 전역 관리자에게도 무조건 걸려 **전역 관리자조차 자기 기본 조직으로 부당하게 좁혀지고 있었다**(SEC-34와 반대 방향 — 유출이 아니라 과잉 차단 — 이지만 같은 자리, `core/scope.py::build_scope`를 대신 쓰도록 정정). 신규 회귀 2건(전역 관리자가 전 조직 글을 보는지, B조직 사람의 글이 실제로 B조직 org_id로 저장·조회되는지 DB 직접 대조), 두 결함 각각 독립 revert-to-verify 완료(한쪽만 되돌려도 다른 쪽이 여전히 잡는 교차 확인 포함). `tests/security/test_board_scope.py` 10/10 green |
| SEC-36 | High | **SEC-35(`create_post`가 `org_id` 안 채움)와 똑같은 패턴이 `OrgScopedMixin` 상속 모델 전체에서 3곳 더 있었다 — 다만 지금은 활성 유출이 아니다.** SEC-35 해소 뒤 `OrgScopedMixin`을 상속하는 12개 모델 전부를 전수 점검: `games/service.py::create_room`(`GameRoom`) · `team_chat/service.py`의 세 `ChatRoom` 생성 자리(부서 자동생성 팀방·`create_group`·`create_or_get_direct`) · `team_docs/sync.py::_upsert`(`DocumentCache`, Notion 동기화 경로) 전부 `org_id`를 안 채워 컬럼 기본값에 맡기고 있었다. 나머지(`Project`·`OffboardingRun`·`UsageEvent`·`SearchDocument`·`TicketCache`)는 전부 이미 명시적으로 채우고 있어 결함 아님으로 확인. **SEC-35와의 차이**: `GameRoom.org_id`/`ChatRoom.org_id`/`DocumentCache.org_id`를 읽어 접근을 제한하는 코드가 지금 하나도 없다(놀이방·채팅방은 멤버십으로, 문서는 작성자 Notion id 해석으로 판정) — 즉 컬럼은 선언돼 있지만 아직 아무도 안 읽는 상태라 **지금 당장 유출로 이어지진 않는다.** 다만 board의 `Post.org_id`도 처음엔 똑같이 "언젠가 쓰려고 선언"됐다가 실제로 쓰이기 시작한 뒤에야 이 공백이 진짜 구멍이 됐다 — 나중에 이 세 모델에 조직 스코프를 걸 때 같은 함정을 다시 밟지 않도록 미리 맞게 채워 둔다 | ✅ **구현완료(2026-08-16)** — 4곳 전부 생성자에 `org_id=<생성 주체>.org_id`(팀방은 `dept.org_id`, 문서 동기화는 `tickets/sync.py`와 같은 관용으로 `DEFAULT_ORG_ID`) 추가. 신규 회귀 3건(GameRoom·ChatRoom 각 1건은 `two_orgs`로 실제 다른 조직 값 대조까지 확인하는 진짜 revert-to-verify, `DocumentCache`는 컬럼 기본값과 명시값이 우연히 같아 revert-to-verify가 무의미함을 직접 확인하고 그 한계를 테스트 주석에 정직하게 남김). `tests/security/`+`tests/unit/` 관련 스위트 전체 green. Critical이 아니라 활성 유출이 없어 SEC-34/35처럼 별도 긴급 배포는 안 하고 다음 통합 배포에 포함 |
| SEC-37 | High | **놀이방 대화(chat)에 멤버십 게이트가 없었다 — 형제 함수는 전부 갖고 있는데 이것만 없었다.** `app/games/service.py`의 `set_ready`/`submit_vote`/`submit_number`/`submit_rps`/`submit_quiz_answer`는 전부 `repository.get_member(...) is None → ForbiddenError`를 거치는데 `chat()`만 없었다. 라우터 자신의 주석("대화는 방 안 사람에게만 — 1순위 유출 #10")과 읽기 쪽(`room_state`의 `events` 필터)이 이미 "멤버 전용"을 전제하는데 쓰기 쪽만 그 전제를 안 지켰다 — 방에 한 번도 안 들어온 사용자가 `POST /rooms/{id}/chat`으로 메시지를 "쓸" 수 있었다(자신은 못 읽어도 실제 멤버에게는 보임). `scripts/check_scope_gates.py`(static_checks.sh 필수 단계) 전체 재실행 중 이 경로가 게이트 없음으로 걸리면서 발견 — 이 검사가 실제로 미탐지 결함을 잡은 사례 | ✅ **구현완료(2026-08-16)** — `chat()`에 형제 함수와 같은 멤버십 게이트 추가(관전자는 안 뺌 — 읽기 쪽도 관전자를 안 가리고 관전은 참여의 한 형태). 같은 조사에서 `check_scope_gates.py`의 `GATE_PATTERNS`가 `_ensure_host`(선행 밑줄 + "host" 동의어 미인식)를 놓쳐 `disband`/`start`/`reveal`/`next`/`finish`/`reset` 6곳을 오탐(실제로는 `_ensure_host`가 정상 작동 중)하던 것도 함께 확인 — EXEMPT 6개를 개별 등재하는 대신 정규식 자체를 고쳤다(`ensure_` 앞 `_?` 허용 + `host` 동의어 추가). 이 수정이 옳았다는 근거: 같은 밑줄 문제가 `app/quotas/router.py::_ensure_target_in_scope`·`app/tickets/router.py::_ensure_attachment_ticket_visible`에도 독립적으로 이미 있었다(이름에 scope/visible이 있는데도 인식 못 하고 있었음). 신규 회귀 `test_an_outsider_cannot_post_to_the_room_chat`(`tests/security/test_game_room_chat_scope.py`), revert-to-verify 확인(게이트 제거 시 실제로 200 + 메시지가 방 안 사람에게 보이는 것까지 재현). `tests/integration/test_games_api.py`(43건) 회귀 없음. `check_scope_gates.py` 재실행 `SCOPE_GATES_OK`(새 EXEMPT 추가 없음). 상세: `DECISIONS.md` D-83 |
| DBTX-01 | Med~High | **DB 동시 쓰기 경합 3곳이 정리된 409 대신 처리 안 된 원시 500을 낸다.** 셋 다 "먼저 조회해 있으면 409, 없으면 생성"인데 `begin_nested`+`is_write_conflict` 관례(`app/prompts/service.py::transition` 등이 이미 씀) 없이 조회~쓰기 사이가 안 잠겨 있었다: `app/notion_mapping/service.py::get_or_create_mapping()`(High — `user_id` UNIQUE, 사람이 누른 "검증"과 대량 동기화 잡의 경합) · `app/quotas/router.py::create_quota()`(Med-High — `uq_ai_quota_scope` UNIQUE, 이중 클릭·동시 생성) · `app/trash/service.py::move_to_trash()`(Med — `uq_trash_item` UNIQUE, 같은 항목 동시 삭제) | ✅ **구현완료** — 세 곳 모두 `transition()`과 같은 SAVEPOINT 관례로 감쌈. `get_or_create_mapping()`은 get-or-create 계약이라 409 대신 승자의 행을 돌려주는데, **처음 짠 단순 재조회 버전은 자체 8-way 스트레스 시험에서 바로 `NoResultFound`로 깨졌다** — 낡은 스냅샷 문제(SAVEPOINT 롤백은 스냅샷을 새로 뜨지 않는다, CORE-13)를 그제서야 발견, `create_approval()`이 이미 쓰던 "커밋으로 스냅샷을 새로 뜨고 재시도" 루프로 다시 짬. `create_quota()`/`move_to_trash()`는 재조회가 없는 단순 409라 이 문제 자체가 없음. 신규 회귀 시험 3개(`test_notion_mapping_get_or_create_race.py`·`test_quota_create_race.py`·`test_trash_move_race.py`) — 서비스 함수 둘은 스레드별 별도 엔진의 8-way 타이밍 경주, 라우터에 박힌 `create_quota`는 이 저장소 자체가 이미 겪은 "로그인 지터가 순수 타이밍 경주를 위양성으로 만든다"는 교훈(`test_prompt_create_new_version_race.py`)에 따라 `threading.Barrier`로 결정적으로 겹치게 함. 5회 반복으로 안정성 확인. Revert-to-verify 완료(대상 파일만 stash → 3건 전부 raw `OperationalError: database is locked` 재현 확인 → 복구 → 3/3 재확인). **범위 밖으로 남김**: `record_view()`/`touch_presence()`(팀챗·게임)는 재조회는 하지만 위 재시도 루프 없이 단순 재조회 한 번뿐이라 이론적으로 같은 낡은-스냅샷 틈이 있을 수 있음 — 다만 이미 `begin_nested`+`is_write_conflict` 보호가 있어 "보호 0"인 이 항목의 세 함수와는 다른 부류이고 실사용 트래픽에서 열릴 가능성이 낮아, 확인 없이 같이 고치는 대신 다음 감사로 넘김(`WORK_STATE.md` Finding 5 처리 로그에 상세) |
| DBTX-02 | **Critical** | **잡 핸들러가 느린 아웃바운드 호출(n8n/Notion/LLM) 앞에서 DB를 다시 읽으면, 응답을 받은 뒤 저장이 통째로 롤백된다 — TEST SERVER 실사용(채팅 전송)에서 연속 2회 실측.** DBTX-01과 형제 결함이지만 메커니즘이 다르다: UNIQUE 충돌이 아니라 **WAL 스냅샷 노후화**다. `app/core/db.py`의 DEFERRED BEGIN 아래서, 세션이 먼저 읽어 고정한 스냅샷은 그 사이 **다른 세션이 무엇을 커밋하든**(같은 행이 아니어도) 낡는다 — `busy_timeout`으로 못 구한다("지금 누가 잠갔다"가 아니라 "내가 본 상태가 이미 낡았다"). `chat_message.py`는 아웃바운드 호출 앞에 방어용 `db.commit()`을 이미 갖고 있었는데 `_resolve_chat_endpoint()`(Workflow 레지스트리 조회)가 그 커밋 뒤·호출 앞에 자리해 새 트랜잭션을 도로 열어 방어를 무력화했다 — 실사용 중 흔한 다른 세션의 커밋(예: 30초 heartbeat 스레드, `app/worker_main.py::run_heartbeat_loop`)과 겹치면 n8n이 이미 준 assistant 응답이 저장 직전에 롤백되고, 사용자에게는 "업무 처리 서버와의 연결에 문제가 있어..."라는 오도하는 안내만 남는다. 같은 모양을 `document_generate.py`(커밋 자체가 없어 더 넓게 노출) · `notion_mapping_sync.py`(90초 호출) · `schedule_run.py`(chat_message와 동일: 방어 커밋은 있는데 뒤에 읽기가 낌) · `project_weekly_summary.py`(LLM 호출, 수십 초)에서도 확인 | ✅ **구현완료(2026-08-16)** — 다섯 핸들러 모두 "아웃바운드 호출 바로 앞이 마지막 DB 문장"이 되도록 커밋 위치를 고침(기존 방어 커밋이 있으면 그 앞으로 읽기를 옮기고, 없으면 호출 직전에 새로 넣음). `app/jobs/worker.py::run_once`에도 핸들러 호출 직전 커밋을 추가해 `llm_connection_test`처럼 핸들러 자신은 못 고치는 경우까지 방어. `mail_send.py`는 **의도적으로 제외** — `render_body()`가 만드는 1회용 토큰 행이 "발송 실패 시 롤백과 함께 사라져야 한다"는 기존 계약(파일 주석에 명시)과 정면으로 부딪혀, SMTP 호출이 다른 셋보다 훨씬 짧다는 점을 근거로 이번 범위에서 제외(토큰 수명 재설계는 별도 판단 필요). 신규 회귀 `tests/integration/test_chat_handler.py::test_reply_survives_a_concurrent_write_that_lands_during_the_outbound_call` — n8n 호출이 "나가 있는" 순간 별도 세션이 실제로 커밋하게 만들어 경합을 결정적으로 재현(실제 SQLite WAL 파일, `:memory:` 아님, 스레드 타이밍 불필요 — 순서만으로 결정적). Revert-to-verify 완료: `_resolve_chat_endpoint()`를 원래 위치로 되돌리자 **TEST SERVER에서 실제로 겪은 것과 완전히 같은** `OperationalError: database is locked` / `UPDATE conversations SET backend_conversation_id=?...`로 재현 → 복구 → 재확인. 관련 스위트 전체(`test_chat_handler.py` 17건·`test_notion_mapping_sync_job.py`·`test_project_weekly_llm_summary.py`·`test_project_weekly_report.py`·`test_worker.py`·`test_project_weekly_summary_handler.py`·`test_documents_api.py`·`test_handlers_hardening.py`·`test_jobs_api.py`·`test_document_generation_scope.py`·`test_job_scope.py`·`test_mail_delivery.py`·`test_schedules_api.py`·`test_schedule_zombie_sweep.py`) green. **추가 발견(같은 사이클)**: job 핸들러 밖 웹 요청 경로에도 형제 사례 1건 — `app/chat/service.py::delete_conversation`(`DELETE /api/conversations/{id}`)가 러너 알림(아웃바운드) 앞이 아니라 **뒤**에 최종 커밋이 걸려(`get_db` 요청-종료 커밋), 알림 호출 중 경합하면 자기 docstring이 약속한 "플랫폼 삭제는 항상 진행된다"가 깨지고 있었다 — `get_db`의 D-75 완화(raw 500 대신 재시도 가능 응답)로 job 핸들러 쪽보다 훨씬 덜 심각하지만 같은 근본 원인이라 같이 고침(호출 앞으로 커밋 이동). 전용 재현 시험은 추가하지 않음(메커니즘은 이미 결정적으로 증명됨, 변경 자체가 이미 예정된 커밋 시점을 당기는 것뿐이라 저위험) — 기존 `test_chat_api.py`·`test_chat_handler.py`·`test_chat_message_actions.py`·`test_chat_runner_context_delete.py` 전체 green으로 회귀만 확인. **전수 확장(2026-08-16, 배경 조사 에이전트)**: `outbound_client.request`/`provider.invoke`를 직접 부르는 파일 9개를 라우터 진입점부터 전부 추적해 7곳 추가 확인·수정 — `app/tickets/repository_notion.py`(create/update/save_body/ensure_local, 이 저장소 트래픽에서 **가장 빈도 높은 노출**) · `app/team_docs/repository_notion.py`(create/save_body, 티켓과 동일 모양) · `app/workflows/provider_n8n.py::test`(연결 테스트) · `app/notion_mapping/service.py::verify_mapping`(단건 검증) · `app/notion_console/router.py`(연결 테스트·DB 생성) · `app/assistant/router.py::_with_narrative`(`ai_quotas.consume` 블록 — 이 블록의 Z15 잠금은 DB 트랜잭션이 아니라 순수 프로세스 내 뮤텍스임을 코드로 확인한 뒤에만 블록 안에서 커밋). `app/documents/router.py::generate`는 같은 `consume` 패턴을 쓰지만 동기 아웃바운드 호출이 전혀 없어(비동기 잡 큐 경로) 조사 후 해당 없음으로 확인. 관련 스위트 202건+76건 green(파이프 대신 파일 리다이렉트로 종료 코드 확인 — 아래 별도 항목). 상세: `DECISIONS.md` D-85·D-87 |
| SEC-38 | **Critical** | **`admin_scope`를 한 번도 안 건드린 운영자(operator)/감사자(auditor)가 전역 범위였다 — `build_scope()` 자체의 결함, DBTX-02 뒤 신뢰할 수 있는 Full Regression을 다시 돌리다 발견.** `app/core/scope.py::build_scope()`는 `role==ROLE_USER`만 부서 기반으로 따로 보고 나머지 전 역할(operator/auditor 포함)은 `admin_scope` 컬럼으로 판정하는데, 그 컬럼 기본값이 `global`(0024의 의도된 선택)이고 관리자가 역할을 operator/auditor로 바꿀 때 "관리 범위"를 함께 좁히는 것은 강제되지 않는 선택이다(`update_user`에서 `role`과 `admin_scope`는 독립 파라미터). 모델 주석 자체가 "admin_scope는 관리자 역할일 때만 의미가 있다"고 하는데 코드는 이 구분을 안 지켜, 게시판 모더레이션 등을 위해 "운영자"로만 승격시키고 범위는 안 만진 계정이 **의도와 무관하게 전역 범위**가 되어 다른 조직의 게시판·아이디어·(SEC-34가 확인한 대로 `build_scope`를 공유하는) 팀 문서·티켓·휴지통까지 봤다. 실측: `test_idea_board.py::test_another_organization_neither_sees_nor_moves_an_idea`가 이 정확한 증상(조직 B 운영자가 조직 A 아이디어를 그대로 봄)으로 격리 실행에서도 결정적으로 실패 ‖ **이미 알려져 있었다, 테스트만 고쳐지고 제품은 안 고쳐졌다**: `test_board_scope.py::_login_org_b_moderator`의 주석(RBAC 재감사 2026-08-16, SEC-34/35/36/37과 같은 조사)이 "role만 바꾸고 admin_scope를 그대로 두면 의도와 다르게 전역 범위 운영자가 된다"를 이미 정확히 짚었지만, 그 대응은 그 파일의 테스트 헬퍼에 `admin_scope="org"`를 명시적으로 대입하는 것뿐이었다 — 근본 원인(`build_scope()`)은 그대로 남아 같은 헬퍼를 안 쓰는 다른 테스트가 다시 걸렸다 | ✅ **구현완료(2026-08-16)** — `build_scope()`에 분기 추가: `admin_scope`가 아직 `global`이고 역할이 operator/auditor면 자기 조직(`org_id=user.org_id`)으로 좁힌다. org/dept로 **명시적으로** 좁힌 operator/auditor는 그대로 존중(관리 콘솔이 실제로 그 값을 고를 수 있다). 컬럼만으로는 "한 번도 안 건드림"과 "일부러 global을 골랐음"을 구분할 수 없어 — 이 함수 자신의 기존 fail-closed 원칙과 같게 — 강한 권한은 명시적 선택 쪽으로만 좁혔다. `admin`/`system_admin`은 건드리지 않음(진짜 관리 콘솔 사용자, 0024 원래 근거가 정확히 그들 것). 신규 단위 시험 6건(`tests/unit/test_scope_operator_auditor_default.py`, API 없이 `build_scope()` 직접 호출) + revert-to-verify 완료(`scope.py`만 stash → 원인 테스트가 정확히 같은 증상으로 재현 → 복구 → 재확인). 회귀 없음: `test_idea_board.py`(14건)·`test_board_scope.py`·`test_trash_api.py`·`test_project_scope.py`·`test_regular_user_scope.py`·`test_document_scope.py`·`test_ticket_detail_scope.py`·`test_trash_scope.py`(SEC-34 스코프 시험 포함, 전부 admin_scope를 명시적으로 설정하는 시험이라 이 수정과 안 부딪힘) green, `tests/security/`+`tests/unit/` 전체 재확인 진행 중. **배포 영향(사람 확인 권장, 차단 아님)**: TEST SERVER에 이미 role=operator/auditor이면서 admin_scope를 한 번도 안 좁힌 계정이 있다면 배포 뒤 그 계정의 범위가 전역→자기 조직으로 좁아진다(의도한 방향) — 배포 전 `SELECT role, admin_scope FROM users WHERE role IN ('operator','auditor')`로 실제 영향 계정 수 확인 권장. **범위 밖으로 남긴 것**: `app/profiles/router.py::me()`(`/api/me`)가 `ScopeBar.jsx` 발견성 표시용으로 `user.admin_scope` **원본 컬럼**을 그대로 돌려준다 — operator/auditor가 아직 global인 경우 이제 실제 접근 범위(자기 조직)와 화면 표시("전체")가 어긋난다. 보안 결함은 아니다(실제 데이터 접근은 이미 좁혀졌다, 화면 라벨만 낡음) — 폭넓게 쓰이는 `/api/me`를 건드리는 위험 대비 이득이 낮아 이번 수정 범위에서 뺐다. 고칠 지점: `_scope_names`/`me()`가 원본 컬럼 대신 `build_scope()`의 실제 판정을 반영하게 한다. ✅ **구현완료(2026-08-16)** — `build_scope()`에 분기 추가: `admin_scope`가 아직 `global`이고 역할이 operator/auditor면 자기 조직(`org_id=user.org_id`)으로 좁힌다. org/dept로 **명시적으로** 좁힌 operator/auditor는 그대로 존중(관리 콘솔이 실제로 그 값을 고를 수 있다). 컬럼만으로는 "한 번도 안 건드림"과 "일부러 global을 골랐음"을 구분할 수 없어 — 이 함수 자신의 기존 fail-closed 원칙과 같게 — 강한 권한은 명시적 선택 쪽으로만 좁혔다. `admin`/`system_admin`은 건드리지 않음(진짜 관리 콘솔 사용자, 0024 원래 근거가 정확히 그들 것). 신규 단위 시험 6건(`tests/unit/test_scope_operator_auditor_default.py`, API 없이 `build_scope()` 직접 호출) + revert-to-verify 완료(`scope.py`만 stash → 원인 테스트가 정확히 같은 증상으로 재현 → 복구 → 재확인). 회귀 없음: `test_idea_board.py`(14건)·`test_board_scope.py`·`test_trash_api.py`·`test_project_scope.py`·`test_regular_user_scope.py`·`test_document_scope.py`·`test_ticket_detail_scope.py`·`test_trash_scope.py`(SEC-34 스코프 시험 포함, 전부 admin_scope를 명시적으로 설정하는 시험이라 이 수정과 안 부딪힘) green, `tests/security/`+`tests/unit/` 전체 재확인 진행 중. **배포 영향(사람 확인 권장, 차단 아님)**: TEST SERVER에 이미 role=operator/auditor이면서 admin_scope를 한 번도 안 좁힌 계정이 있다면 배포 뒤 그 계정의 범위가 전역→자기 조직으로 좁아진다(의도한 방향) — 배포 전 `SELECT role, admin_scope FROM users WHERE role IN ('operator','auditor')`로 실제 영향 계정 수 확인 권장. 상세: `DECISIONS.md` D-86 |
| SEC-39 | **Critical** | **내(이 세션)가 `CLOVIR_TEST_SUDO_PASSWORD` 값을 내 자신의 도구 출력(화면)에 노출시켰다 — 불변규칙 §3-4 위반, `SEC-20`과 같은 계열의 사고.** Supervisor가 이번 invocation에 TEST SERVER(`cloviradmin@10.100.64.71`) sudo 비밀번호를 환경변수로 제공했다(§4단계 권한). `sudo -S`로 첫 번째 비밀번호(sudo 자신용)를 stdin으로 넘기고, 같은 파이프에서 **두 번째 값**(QA 테스트 계정 새 비밀번호)을 잇달아 넘기려 했는데 비-TTY 파이프에서 `sudo -S`가 그 이후 stdin을 통째로 삼켜 버렸다(자식 프로세스가 빈 입력을 받음 — 실제로 확인함, `LEN=0`). 그 버퍼링 문제를 "고치려고" `ssh -tt`(강제 PTY 할당)로 재시도했는데, PTY는 기본적으로 로컬 echo를 켜서 **내가 stdin으로 넣은 sudo 비밀번호 값 자체가 명령 출력(stdout)에 그대로 찍혀 내 도구 결과·대화 기록에 남았다.** 이 세션 안에서 즉시 알아채고 그 접근 패턴을 바로 중단, 남은 임시 파일(`/tmp/.uiqa_pw`, 결국 QA 계정 비밀번호도 못 담고 있었다 — 같은 버퍼링 문제로 내용이 비어 있었음)을 정리하고, 더 이상 그 자격증명으로 실험하지 않았다 | 노출된 값: `CLOVIR_TEST_SUDO_PASSWORD`(TEST SERVER `cloviradmin` sudo 비밀번호). 노출 위치: 이 대화 세션의 도구 실행 결과(터미널 화면 출력) — 커밋·git 추적 파일·로그 파일에는 안 남았다(직접 확인: `git grep`/추적 파일 어디에도 그 값이 없다, 이 세션이 만든 것은 임시 SSH 명령뿐이었다) | **미해결 — 사람 조치 필요(회전), AI 구현 대상 아님.** 순서는 `SEC-20`과 동일: ① 이 세션이 이 값을 다시 안 쓴다(이미 지킴 — 이후 모든 SSH 호출은 단일 비밀 stdin 패턴만 사용, 두 번째 비밀은 파일 리다이렉트로 우회). ② **`CLOVIR_TEST_SUDO_PASSWORD`(=`cloviradmin` 계정의 실제 sudo 비밀번호)를 회전한다** — 이 저장소 밖의 운영 행위, 사람만 할 수 있다. ③ 회전 후 그 값을 쓰는 Supervisor 환경변수 설정도 새 값으로 갱신한다(안 그러면 다음 invocation의 sudo가 깨진다). **회전 전까지 이 자격증명을 신뢰하지 않는다** — 이 세션은 이미 완료한 배포(정적 파일, 체크섬 검증됨)에서 이 자격증명을 더 안 썼다. 상세: `docs/DECISIONS.md` D-136 |
...
| `GM-10` | **High** | ✅ 구현완료 — `_finish_number`·`_finish_vote`·단판 `_finish_rps`를 `_cas_update_state`로 감쌌다(`_finish_rps_tournament`가 FN-20에서 이미 쓰던 것과 동일한 패턴 — 검증하며 그 사실을 발견해 재사용). 결과 계산을 `mutate(state)` 순수 함수로 분리하고 `if "result" in state: return state`로 멱등하게 만들어, 재시도(다른 요청이 먼저 확정)가 다시 계산하지 않고 이미 확정된 값을 그대로 돌려받게 했다. 조사 중 같은 뿌리(`maybe_autoresolve`가 read-then-write 가드 하나로 여러 종결 로직을 부름)의 **다섯 번째가 아니라 여섯 번째 인스턴스**를 추가로 찾았다 — `_reveal_quiz`(퀴즈 채점)도 동일하게 무방비였다(`submit_quiz_answer`는 이미 CAS를 쓰는데 정작 채점만 안 썼다). 같은 패턴으로 `_cas_update_state`로 감싸되, 기존 "이미 공개된 문제입니다" 오류 계약은 유지(호스트가 직접 두 번 누르면 여전히 오류)하고 `maybe_autoresolve`(자동 폴링 경로)만 그 오류를 조용히 삼키게 분리했다. 신규 회귀 시험 1건(`test_concurrent_autoresolve_does_not_recompute_a_different_winner`, 실제 동시 DB 세션 2개로 재현) — revert-to-verify 결과 되돌린 코드는 assertion 실패가 아니라 `sqlite3.OperationalError: database is locked`로 죽었다(CAS의 재시도/충돌 처리 없이는 쓰기 경합 자체를 못 버틴다는 추가 증거) |
| `GM-11` | High | ✅ 구현완료 — 시딩(`_open_rps_tournament`)과 강제 마감(`_tournament_advance`의 `force` 분기) 둘 다 `_present_players`로 통일했다. 신규 시험 2건: `test_tournament_seeding_excludes_stale_ghost`(시딩), `test_tournament_force_finish_excludes_stale_ghost`(강제 마감 — 코인플립 분기의 난수를 고정해 revert 시 우연히 통과하지 않고 항상 실패하게 함). revert-to-verify 둘 다 확인 |
| `CONC-01` | High | ✅ 구현완료 — `Users.jsx:50`의 `diffFields`를 `frontend/src/lib/diffFields.js`로 옮겨 `DataScreen.jsx`의 공용 수정 폼(등록 화면 전체)에도 적용했다. `toApiBody` 변환 **뒤**의 값으로 비교(승인 정책처럼 여러 폼 필드가 객체 하나로 합쳐지는 화면에서 무관한 필드만 바뀌어도 diff가 그 객체 전체를 "바뀜"으로 오판하지 않게). 옮기며 값 비교를 `String(obj)`(`"[object Object]"`로 뭉개져 객체 필드는 내용이 달라도 항상 "같음"으로 오판하던 버그, Users.jsx는 원시값뿐이라 안 드러났었다)에서 JSON 직렬화 비교로 고쳤다. **범위**: `editMethod`가 PUT인 화면(`schedules`·`templates`)은 제외했다 — 백엔드가 부분 스키마가 아니라 전체 표현을 요구하는 진짜 REST PUT이라(`app/schedules/router.py`의 `ScheduleRequest` 확인함) 부분 body를 보내면 검증 실패·기본값 초기화로 이어질 수 있다(그대로 전체 재전송, 남은 범위로 문서에 남김). 새 시험 `lib/diffFields.test.js`(9건)·`screens/data-screen-edit-diff.test.jsx`(2건, 실제 렌더링) 추가, revert-to-verify 확인. `expected_config_version` 왕복(서버가 stale write를 409로 직접 거부)까지는 안 갔다 — diffFields만으로 "무관한 필드 덮어쓰기"의 핵심 시나리오는 막히고, 같은 필드를 두 사람이 동시에 고치는 진짜 충돌은 여전히 남는 남은 범위(CONC-02가 가장 위험한 그 인스턴스는 별도로 닫음) |
| `CONC-02` | High | ✅ 구현완료 — 러너 등록 화면(`integrations.js`)의 일반 `edit.fields`에서 `maintenance_state`를 뺐다(diffFields만으로는 "이 필드를 사람이 의도적으로 바꾼 값"과 "그 사이 자동 판정이 다시 바꾼 값"이 겹치는 충돌까지는 못 막는다). 대신 `activeToggle`(actions.js)과 같은 "단일 필드 전용 PATCH" 패턴의 전용 액션 "점검 상태 변경"을 추가 — 값이 셋(정상/성능 저하/점검)이라 고정 body 대신 작은 select 입력 폼 하나, `confirm`이 현재 상태를 말하고 폼보다 먼저 뜬다(`DataScreen.jsx runAction`이 이미 그 순서를 보장). 도움말 문구("‘수정’에서 점검 상태를 바꾸세요")도 새 액션을 가리키게 정정. 새 시험 `runner-maintenance-action.test.jsx`(2건, 실제 렌더링 — 수정 폼에 필드가 없음을 확인 + 확인→폼→PATCH 전 과정), revert-to-verify 확인(필드를 되돌리자 정확히 실패). 프런트 전체 회귀 228 파일/1541건 green |
| `ATT-01` | High | ✅ 구현완료 — `Board.jsx`의 파일별 try/catch → `failed[]` 패턴을 `TicketAttachments.jsx`(티켓 첨부, `Board.jsx` 외 유일하게 여러 파일을 순차 루프로 올리는 곳 — `ChatPane.jsx`/`Profile.jsx`의 `FormData` 사용은 둘 다 단일 파일이라 이 결함군 대상이 아님을 확인함)로 이식했다. `mutationFn`이 파일별 실패를 삼켜 더 이상 mutation 전체를 reject하지 않으므로 `onSuccess`(→`refresh()`)가 항상 돈다. 신규 회귀 시험 1건(두 장 중 하나만 실패 → 나머지는 반영 + 실패 파일명 안내), revert-to-verify로 되돌린 코드가 정확히 `refresh()` 미호출로 실패하는 것 확인 | 완료 |
| `OPS-05` | High | 파일시스템 오류가 그대로 500(위 `OPS` 절) | `save_upload` 에서 `OSError` → 사용자 언어 503 ‖ ✅ **이 행 자체가 중복(교차 확인, 2026-08-17)** — 같은 ID가 이 파일 위쪽(1행, ✅ 구현완료 2026-08-11)에 이미 있고 `StorageUnavailableError`로 실제 구현됨(`save_upload` 확인). 이 표는 어느 감사 라운드의 원본 findings 표라 별도로 다시 실었을 뿐, 새 항목이 아니다 |
...
| ID | 심각 | 문제 | 상태 |
|---|---|---|---|
| OPS-06 | **Critical** | **실서버 `n8n` 계정 Claude CLI 미인증** — LLM 의존 AI 도우미 응답 전체가 대체 문구로 떨어진다. 코드 결함 아님, 배포와 무관할 가능성이 더 높음(근거 위 서술). **사용자에게 알려야 할 항목** | ✅ **해결**(2026-08-10 11:00 KST) — 사용자가 `sudo -iu n8n` → `claude` → `/login` 재인증. 러너와 동일 조건 실호출로 `result:"PROBE_OK"` + 실제 토큰 소모 확인, `expiresAt` 정상화, 신규 `claude_cli_failed` 0건. ~~**단 웹 AI 도우미 대화 E2E 는 미검증**~~ — **그 공백도 닫힘(2026-08-17, D-118 Phase 2/3)**: 실브라우저로 실제 채팅 메시지를 여러 차례 전송해 n8n→Claude CLI 경로 전체(이 항목이 고친 그 인증)를 종단 확인함 — 배치 워커를 완전히 내린 채로도 확인, 서로 다른 대화 3개 동시 처리까지 실측. 상세: `docs/DECISIONS.md` D-124/D-125 |
...
| QAH-05 | Low | QAH-03 조사 중 발견(하네스 표본엔 안 걸림, 라우트당 5건 상한 때문일 수 있음) — 작은 글자에 raw `primary.main`을 직접 쓰는 같은 패턴이 `Board.jsx:411` · `game-room/GameStage.jsx:70,132` · `game-room/LadderBoard.jsx:90` · `game-room/MembersList.jsx:47` · `game-room/Scoreboard.jsx:23` · `game-room/StageShared.jsx:86` 7곳에 더 있다 ‖ **구현완료(2026-08-13)** — 하네스 편입 대신 "손대기 전에 수동으로 대비를 재야 함"을 문자 그대로 했다: 4개 accent 프리셋 × 라이트/다크 × 각 자리의 **실제 렌더 배경**(순수 `background.paper`/`default`거나, Paper 없이 `alpha(color, 0.03~0.14)` 워시가 바로 페이지 배경 위에 뜨는 경우 — 뒤쪽이 앞쪽보다 대비가 더 나쁘다)으로 WCAG 비율을 전수 계산(Node 스크립트, 공식은 `theme-link-contrast.test.js`와 동일). 결과: `primary.main`은 다크 모드 전량 + 워시가 낀 라이트 모드 상당수가 4.5:1 미달(최저 3.24), QAH-03이 이미 만들어 둔 `primary.dark`(=`primaryStrong`)로 바꾸면 전 조합에서 통과(최저 4.77) — 그래서 실측 없이 하네스 편입부터 하자는 원래 계획 대신 계산으로 충분히 검증됐다고 판단해 바로 수정. 같은 파일들을 고치면서 **직접 2곳을 추가로 더 발견**(원래 7곳에 없던 것) — `GameStage.jsx:303`(퀴즈 정답 표시가 `success.main`)과 `StageShared.jsx:23`(Countdown의 `urgent` 삼항이 `error.main`도 같이 씀) — 같은 계산법으로 확인 후 `success.strong`/`error.strong`로 함께 수정(9곳 전부). `bgcolor:`/`borderColor:`/SVG `stroke:` 형태의 `primary.main`은 손대지 않음(텍스트 대비 문제가 아니다, `StageShared.jsx`의 `WinnerName` 배지는 `primary.contrastText`와 이미 짝이 맞는 의도된 조합). 신규 시험 73건(`theme-link-contrast.test.js`) — 9곳 전부 소스 배선 확인(정규식으로 실제 `.dark`/`.strong` 참조 확인) + 4프리셋×2모드×7개 배경 시나리오(워시 포함) 계산된 대비 ≥4.5 확인, revert-to-verify 확인함(소스 6파일만 되돌리면 9개 배선 시험이 정확히 "아직 `.main`"으로 실패, 계산 시험은 테마 토큰 자체는 안 바뀌었으므로 그대로 통과 — 예상대로). **후속 발견**: 같은 `color: "X.main"` 패턴이 game-room/Board 밖에서도 저장소 전체에 최소 15개 파일에 더 있다(`AppShell.jsx`·`UserMenu.jsx`·`kit.jsx`(전역 필수 표시 `*`!)·`DevReport.jsx` 등) — 일부는 `bgcolor:`오탐(내 grep이 `bgcolor:`도 같이 잡음)이었지만 `kit.jsx`의 필수 표시 asterisk처럼 진짜 텍스트색 후보도 있다. 다만 이 자리들은 QAH-03의 519표본 하네스 실행에 이미 포함됐을 공유/고빈도 컴포넌트라(반면 game-room은 하네스 라우트 목록에 아예 없어서 빠졌다), QAH-03이 이미 잡아서 고쳤거나 실제로는 통과하는 조합일 가능성이 game-room보다 높다 — 각각 이번과 같은 수동 대비 계산이 필요해 범위를 넓히지 않고 새 행(`QAH-07`)으로만 남긴다 | 구현완료 |
| QAH-07 | Low | QAH-05 수정 중 발견 — `color: "X.main"`(X=primary/success/warning/error) 텍스트색 raw 사용이 game-room·Board 밖에서도 저장소 전체 최소 15개 파일에 더 있다: `AppShell.jsx`·`UserMenu.jsx`·`ui/kit.jsx`(**필수 입력 표시 `*` — 전역 공유, 폼마다 나온다**)·`ui/Mascot.jsx`·`DevReport.jsx`·`Games.jsx`·`Profile.jsx`·`SetupWizard.jsx`·`TeamChatWidget.jsx`·`TeamDocs.jsx`·`chat/MessageThread.jsx`·`chat/WelcomeStatus.jsx`·`ChatPane.jsx`·`ChatRooms.jsx`·`settings/AccentPicker.jsx` ‖ **구현완료(2026-08-13)** — QAH-05와 같은 방법(정확한 렌더 배경 확인 + 4프리셋×2모드 WCAG 계산, 배경 조사 에이전트로 15개 파일 전부 개별 확인)으로 전수 재검증했다. `kit.jsx` 필수 표시(폭이 가장 넓어 최우선 확인 대상이었다)는 `error.main`이 이미 전 조합 통과(최저 4.69) — **손대지 않음**. 나머지 14개 파일 중 진짜 텍스트색인 것만 추리면(`bgcolor:`·`borderColor:`·`outlineColor:`·SVG 아이콘 `fill`은 대비 문제가 아니라 제외 — `AppShell.jsx:169,514`·`Mascot.jsx:230`은 원래 서술대로 확인된 오탐) **5곳이 실제로 미달**이었다: `TeamDocs.jsx:343,572`(제목 링크 hover)·`ChatPane.jsx:413`(읽음 표시)·`settings/AccentPicker.jsx:51`(선택 체크 ✓) 4곳은 dark 모드 4개 accent 전부 미달(최저 2.81, `background.paper` 기준) — `chat/WelcomeStatus.jsx:15`(QuickPrompts hover)는 배경이 Card가 아니라 `Chat.jsx`가 다시 칠하는 `background.default`라 **light 모드도 accent 2/4가 미달**(4.37·4.38), dark는 전량 미달(최저 3.07)로 이번 QAH 시리즈 중 가장 나쁜 조합이었다. 전부 `primary.dark`로 교체(QAH-03/05와 같은 보강 토큰), `border`/`outline` 절반은 대비 문제가 아니므로 그대로 둠. 나머지(`UserMenu.jsx`·`DevReport.jsx`·`Games.jsx`·`Profile.jsx`·`SetupWizard.jsx`의 간접 참조 `TONE_COLOR` 맵 포함·`TeamChatWidget.jsx`·`chat/MessageThread.jsx`·`ChatRooms.jsx`)는 실측 결과 전부 **이미 통과**(최저 4.57, `AppShell.jsx`의 `ConsoleSwitch`는 CTR-02가 이미 검증 중이던 자리와 동일해 중복 확인만) — 애매해서 못 정한 자리는 없었다. `theme-link-contrast.test.js`에 `describe("QAH-07 ...")` 신설(소스 배선 정규식 4개 + 4프리셋×2모드×2배경 계산 48개 = 52건), revert-to-verify 확인함(4파일 되돌리면 배선 시험 4건이 정확히 실패, 계산 시험은 theme.js를 안 건드렸으므로 그대로 통과 — 예상대로) | 구현완료 |
...
## PA — Whole Product Audit Cycle `PA-20260812-171558-56c5befa` (§11 Handoff, 2026-08-15)
...
의 해당 `PA-RC-*` 블록 전체(문제·기대·실제·구현 방향·제약·회귀 범위·Acceptance Criteria·필수 시험)를 읽어라.
원본 증거는 `PRODUCT_AUDIT_FINDINGS.md`(PA-F-001~039), 의도 근거는 `PRODUCT_AUDIT_FEATURE_CONTRACTS.md`.

Handoff Root Cause 8건 중 **7건이 아래 신규 행**이고, `PA-RC-0003`은 신규 행을 만들지 않고
**기존 `SEC-20`을 갱신**했다(같은 자격증명·같은 회전 조치라 중복 생성이 아니라 범위 확장이다).

| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| PA-01 | **High** | **글자 크기 스케일이 선언만 되고 소비되지 않는다**(`PA-RC-0001`). `tokens.css`가 6단계를 정의하는데 화면을 실제로 그리는 MUI `sx`/`styled` 층에서 이 토큰을 읽는 곳이 **0곳**이라, 비테스트 소스에 `fontSize` 리터럴이 **31종·278회** 쌓여 본문이 11~17px의 1px 연속체가 됐다. 같은 구조가 `RADIUS`(참조 2회 vs 표현 27종·108회)에서도 반복된다. **본문 크기 SSOT가 둘로 갈려 서로 다른 값을 주장한다** — `tokens.css:243` "body 15px" vs `theme.js:271` `body1: 14px`. 처방은 규율 요구가 아니라 **간격(`p: 2`)과 같은 수준의 일급 타이포 API를 주는 것**이다(PA-F-013 대조 실험: 같은 `sx` 층에서 간격 준수율 96% vs 글자 0%) | `frontend/src/styles/tokens.css:243-253` · `frontend/src/ui/theme.js:37,271` · `var/product-audit/design_scan.json` · 스캐너 `scan_design.py` | **대부분 구현완료** — SSOT 충돌 해소: `tokens.css`의 `--font-size-base`(15px, 정본 어디에도 없던 값)를 지움, `--font-size-md`(14px)가 이미 정확한 본문 값이었다. `theme.js`에 RD-1~RD-3 확정값 그대로 `FONT_SIZE`(6단계)·`RADIUS.full`(4번째 의미 슬롯) 신설 + `sectionTitle`·`statValue` MUI typography variant 추가(`FONT_WEIGHT`는 DS-05가 이미 5단계로 신설해 둔 것을 재사용). 1차로 `kit.jsx`(23곳)·`SectionTitle`을 마이그레이션(`theme-baseline.test.js` 신규 3건, revert-to-verify). **2차 확장**: 소스 전체에서 6단계 스케일과 **정확히 일치하는** 리터럴(`0.75/0.8125/0.875/1.0625/1.25rem`, `fontWeight 400/500/600/700/800`, `borderRadius "999px"`/`999`)을 5개 파일 클러스터 병렬 작업 + 직접 마무리로 전부 토큰 참조로 옮겼다 — 계산값이 동일해 시각적 변화 없음, 58개 파일 일괄 커밋 + borderRadius 잔여 3파일 별도 마무리(총 60여 파일). 전체 `npm test`(257개 파일·1,764건) green(과정에서 이 마이그레이션이 `theme-link-contrast.test.js`의 원시 소스 정규식 시험 3건을 조용히 깨뜨린 것을 전체 스위트로 발견·수정). **남은 범위**: 6단계 스케일 밖의 "사이값"(15px/16px/11px 등, `fontSize` 잔여 약 90회) 및 `fontWeight` 750/650/720/620/780(잔여 약 57회)은 **어느 단계로 재양자화할지 시각적 판단이 필요**해 이번에 손대지 않음(값 하나하나가 잘못된 단계로 옮겨지면 실제 시각 변화=회귀가 된다 — 실측/스크린샷 검증 없이 기계적으로 못 옮긴다). `static_checks.sh`의 `fontSize:`/`fontWeight:` 리터럴 금지 검사(acceptance_criteria 5)는 이 사이값 정리가 끝난 뒤에나 추가 가능. 부수 발견: `QAH-07`(2026-08-15 이전 커밋)이 `ChatPane.jsx`에 JSX 주석 구문 오류를 남겨 전체 vitest 20개 파일이 조용히 깨져 있었다(개별 타겟 실행만으론 안 잡힘) — 이번에 전체 스위트를 처음 돌리며 발견해 정정, `jsx-comments.test.js`에 이 패턴(`(` 바로 뒤 `{/*`) 재발 방지 테스트 추가 ‖ **다음 세션 착수용 분류(2026-08-15, 코드 변경 없음)** — `fontSize` 잔여를 다시 세어 보니 91회(`fontWeight` 잔여는 54회). `fontSize` 91회 중 최소 13회는 grep만으로도 확인되는 **아이콘 크기 지정**(`<XxxIcon sx={{ fontSize: "1rem" }} />` 류 — MUI 아이콘은 글꼴 크기로 치수를 잡는 게 정상 관례라 텍스트 스케일과 다른 축이다, 나머지도 상당수 아이콘일 가능성이 높음 — 소스만으론 "이 fontSize가 텍스트 줄인지 아이콘 옆 장식인지" 완전히 자동 판별은 안 됨). `fontWeight` 잔여 54회는 `AssistantPanel.jsx`(5)·`ProjectMetrics.jsx`(4)·`MyTickets.jsx`(4)·`Dashboard.jsx`(4)·`chat/TicketCard.jsx`(3)·`DevReport.jsx`(3)·`AssistantDrawer.jsx`(3) 순으로 집중돼 있어 이 7파일이 절반 가까이(26/54)를 차지 — 여기부터 손대면 효율이 높다. 착수 시 순서 제안: (1) 아이콘 `fontSize`는 우선 그대로 두고 텍스트 `<Typography>`/`sx` 라벨류만 골라낸다, (2) 위 7파일부터 스크린샷 대조(`scripts/ui_qa/baseline.py` 또는 실제 배포 화면)로 각 값이 6단계 중 어디에 가장 가까운지 확인, (3) 하나씩 옮기며 `revert-to-verify` 대신 **배포 전후 스크린샷 diff**로 검증(순수 리터럴 치환이라 유닛 테스트로는 시각 회귀를 못 잡는다) ‖ ✅ **`fontWeight` 잔여 전부 구현완료(2026-08-15, 커밋 `b214102`)** — 620/650→semibold, 720/750→bold, 780→extrabold. 정확한 중간값(650/750)은 산술이 아니라 저장소 안의 형제 패턴 증거로 정했다: `theme.js` 자신의 `sectionTitle` variant가 이미 같은 역할(작은 부제목)에 `bold`를 쓰고 있었고(`h5`/`button` MUI variant 정의 자체도 720/750 리터럴이었다 — 직접 고침), `TeamDocs.jsx`의 이미 토큰화된 "클릭 가능한 행 제목" 패턴이 `MyTickets.jsx`의 650 인스턴스와 구조가 거의 동일해 semibold 쪽 증거가 됐다. 29개 파일을 4개 클러스터 병렬 처리, import 누락 8곳 보강, repo 전체 grep 잔여 0건 재확인, 전체 `npm test`(261파일/1,783건) green. `fontSize`도 최대 클러스터(`fontSize: "1rem"`=16px, 29회)를 분석한 결과 대부분(23곳)이 `component="h2" variant="h6"` 카드/패널 부제목 관례 또는 그 형제 패턴("클릭 가능한 카드 제목"/"굵은 이름표")이라 같은 근거로 `FONT_SIZE.sectionTitle`(17px, 1px 차이라 시각 변화 없음)로 스냅 진행 중 — 나머지 6곳(입력창 2·서체 본문 1·아이콘 2·자격증명 표시 1)은 각기 다른 정당한 이유(iOS 확대 방지용 16px 입력창, 낭독용 큰 서체 본문, 아이콘 자체 크기, 비밀번호 글자 구분 목적)로 이번 스코프에서 명시적으로 제외. `fontSize: "0.9375rem"`(15px) 클러스터도 구현완료(2026-08-15, 커밋 `1911b3e`) — 17곳을 `FONT_SIZE.body`(14px)로. 이번엔 `Projects.jsx:136`이 정확히 같은 구조(굵게+`KO_WORD_BREAK`)에 이미 `FONT_SIZE.body`를 썼고 `ProjectMetrics.jsx`는 같은 flex 행 바로 옆 요소가 이미 `FONT_SIZE.body`라는 저장소 내부 직접 증거로 정했다. 같은 값이지만 다른 자리(서체 본문 4·입력창 1·브랜드 워드마크 1·반응형 clamp 조건부 1·아이콘 다수)는 제외. **`fontSize`의 마지막 큰 클러스터 `0.6875rem`(11px, 12회)는 의도적으로 미착수 — 위험 신호 발견**: 그중 하나(`kit.jsx` `StatCard` 심각도 배지 "주의"/"위험")가 이전 PA-RC-0001 작업에서 이미 "12px(`caption`)로 올리면 실측 폭 12.6px에서 재현되는 줄바꿈 결함 위험, 실측 없이는 안 바꾼다"는 근거로 명시적 예외 처리돼 있다 — `caption`(12px)이 스케일의 최하단이라 내려갈 곳도 없다. 이 사실 하나로 클러스터 전체를 "1px 차이는 안 보이니 올려도 된다"는 앞선 두 클러스터의 논리로 일괄 적용할 수 없다는 것이 확인됐다 — 나머지 11곳도 비슷하게 좁은 폭(kbd 단축키, 칩, 시간 표시 등)에서 쓰이고 있어 개별 실측(`scripts/ui_qa`로 실제 배포 화면 스크린샷 대조) 없이는 손대지 않는 것이 안전하다. ~15개 단일값(1.125rem/1.5rem 등 각 1~3회)도 미착수 — 일부(사람/조직 카드 큰 숫자, 캘린더 날짜 헤딩)는 `statValue`/`pageTitle`과 `sectionTitle` 사이 애매한 자리라 이것도 실측 필요 ‖ **재확인(2026-08-16, `DECISIONS.md` D-78)** — `0.9375rem`을 "합병 대상 사이값"으로 다시 보다가, 위 2026-08-15 판단(서체 본문·입력창·브랜드 워드마크는 명시적 제외)이 정확히 옳았음을 다른 근거(`RichText.jsx:9`의 기존 주석)로 독립 재확인함 — 이 그룹은 더 다툴 필요 없음. `acceptance_criteria(3)`(8단계 이하) 기준 정확한 잔여 인벤토리를 D-78에 남김: 공식 6단계 + 사이값 7종(10·11·15·16·18·22·24px) = 13단계. **이어서(같은 날) 24px를 실제로 전용 토큰화 시도 → `theme-baseline.test.js`의 "FONT_SIZE는 정확히 6단계다(RD-1)" 회귀 테스트에 걸려 되돌림(코드 변경 0, 주석만 보강) — RD-1의 6단계는 닫힌 집합이라 7번째 공식 토큰은 만들 수 없다는 것을 확인.** 16/18/22px도 직접 열어 보니(`ChatPane.jsx:518`은 애초에 이모지 피커라 대상 아님) **전부 이미 예외 주석이 있어 `acceptance_criteria(1)`은 장꼬리 전체에서 이미 충족돼 있다** — 남은 `acceptance_criteria(3)` 미달은 구현 과제가 아니라 "8단계"가 실제 렌더링 값 수를 세는지 정당한 예외를 허용하는지의 정책 질문이라 다음 Audit Cycle/사람 검토 대상으로 넘긴다(D-78 상세) |

**6차 확장(`0.6875rem` 13곳 낱개 재검증, 2026-08-16, `PA-RC-0001` 재검증 대응)** — "위험 신호"로 미착수 남겨 둔 13곳을 **하나씩 실제 소스 문맥으로** 대조했다(줄 번호가 아니라 `grep`으로 매번 현재 위치를 다시 찾음 — 이전 세션들에서 `copy_inventory.json` 같은 스냅숏이 최대 9줄까지 밀려 있던 것을 이번 PA-02 재검증에서 직접 겪었기 때문). 결과: **11곳은 안전하게 이전할 수 있는 근거가 확인**됐다 — 전부 kbd 단축키 힌트·그룹 헤더 라벨·타임스탬프·+N건 오버플로 안내 같은 **폭 제약이 없는 평문 라벨**이었고, 그중 다수는 **같은 파일 안에 이미 `FONT_SIZE.caption`을 쓰는 구조적으로 동일한 형제 요소**가 있어 그 값이 맞다는 직접 증거가 됐다(`AppShell.jsx`의 사이드바 배지 vs 그룹 헤더, `CommandPalette.jsx`의 "Esc" kbd 힌트 vs 이미 토큰화된 검색결과 그룹 헤더, `NotificationBell.jsx`/`ChatRoomMembers.jsx`의 caption 형제 등). 폭 제약이 있어 보였던 2곳(`ChatRoomMembers.jsx`의 "방장" Chip, `SchedulerCalendar.jsx`의 날짜 칸 안 일정 칩)도 확인 결과 `flexShrink:0`+형제가 대신 줄어드는 구조 또는 이미 `overflow:hidden`+`textOverflow:ellipsis`로 방어돼 있어 안전했다 — **StatCard의 실패 사례(폭 보호 없이 11→12px 올렸다가 세로로 무너짐)와 근본적으로 다른 조건**임을 직접 대조로 확인한 뒤에 옮겼다. 나머지 **2곳은 그대로 둔다**: `kit.jsx`의 StatCard 심각도 배지(이미 실측된 위험, 주석 유지)와 `theme.js`의 `MuiTableCell` head(기준선 파일 `th{font-size:11px}`과 정확히 일치하는 전역 스타일 — 이건 애초에 "재양자화 대상"이 아니라 **기준선을 정확히 따르는 의도된 값**이었다, 손대면 오히려 기준선에서 벗어난다). 신규 회귀 `typography-scale-migration.test.js`(2건: 예외 2파일을 뺀 나머지에 리터럴이 없는지 + 그 2파일엔 여전히 있는지, 후자는 예외가 조용히 사라지는 것도 잡는다), revert-to-verify 확인. 프런트 전체 `npm test`(267파일·1,808건) green. **`0.9375rem`(15px, 10곳)과 `1rem`(16px, 12곳)도 이번에 전수 재확인** — 전부 이미 정당한 예외 범주(아이콘 `fontSize`·서체 본문(prose, `lineHeight 1.6~1.75`+`overflowWrap:anywhere`)·입력창(iOS 확대 방지 또는 4K 대응 주석 있음)·브랜드 워드마크·자격증명 표시(`Users.jsx`의 임시 비밀번호, monospace+`letterSpacing`+`userSelect:all`))에 정확히 들어맞아 **추가로 옮길 것이 없음을 확인**(코드 변경 없음). **남은 진짜 미착수**: `1.125rem`(6)·`1.5rem`(3)·`2rem`/`2.75rem`/`2.125rem`/`1.375rem`/`0.625rem`/`0.8125rem`(각 1) — Handoff 자신이 "다음 세션 착수용"으로 분류해 둔 바로 그 장꼬리(long-tail) 단일값 클러스터로, 개별 실측이 필요해 이번에도 손대지 않았다. acceptance_criteria(3) "실사용 단계 ≤8개"는 이 장꼬리를 포함해도 핵심 6단계 + 아이콘/서체본문/입력창/워드마크/자격증명표시라는 **이유가 있는** 소수 예외 축이라 사실상 충족에 가깝다고 판단하지만, 정확한 단계 수는 `scan_design.py` 재실행으로 다음 세션이 확정할 것 |

**7차 확장(장꼬리 8종 낱개 판정 + 스캐너 사각지대 발견, 2026-08-16)** — `var/product-audit/scan_design.py`(값이 아니라 **표현식 문자열**로 집계 — `grep`보다 정확해 `fontWeight: isOrg ? 800 : 600`처럼 삼항연산자에 감싸인 리터럴도 잡는다)를 실행해 남은 8종을 실제 소스로 하나씩 확인했다. **아이콘/이모지/아바타 글자 크기**(`<XxxIcon sx={{fontSize:...}}/>`·`aria-hidden` 이모지 span·MUI Avatar 이니셜, 총 6곳 — `TopSearch.jsx`·`GameStage.jsx`·`RpsViews.jsx`·`Profile.jsx`·`Mascot.jsx`)는 이미 확립된 패턴대로 대상에서 뺐다(글꼴 크기로 아이콘/글리프 치수를 잡는 것은 MUI 정상 관례). **텍스트인데 6단계 밖인 7곳**은 근거 없이 스냅하지 않고 각 자리에 **왜 의도된 예외인지 직접 주석을 남겼다**(acceptance_criteria 1의 "예외 사유가 주석으로 있다" 요건을 리터럴 단위로 충족): `Home.jsx`(보조 통계 숫자, 18px — StatCard의 30px는 과함, 17/20px 사이 자리), `SchedulerCalendar.jsx`(월 이동 h2, 18px, 같은 이유), `ChatPane.jsx`(읽음 표시, 10px — 메신저 관용상 본문보다 작아야 하는 보조 지표, caption(12px)로 올리면 오히려 본문과 경쟁), `WelcomeStatus.jsx`(채팅 환영 제목, 22px — MUI 기본 h5(24px)보다 의도적으로 작게 함), `GameStage.jsx`/`ProjectWbs.jsx`/`ui/charts/Donut.jsx`(24px+extrabold **3곳이 완전히 같은 값+굵기** — 우연이 아니라 이미 자리 잡은 관용이라는 상호 증거, 서로를 참조하는 주석으로 연결). **스캐너 사각지대에서 진짜 수정 1건 발견**: `OrgTree.jsx`가 `fontWeight: isOrg ? 800 : 600, fontSize: isOrg ? "0.9375rem" : "0.875rem"`를 raw 리터럴로 쓰고 있었는데 — `800`/`600`은 정확히 `FONT_WEIGHT.extrabold`/`semibold`와, `"0.875rem"`은 정확히 `FONT_SIZE.body`(14px)와 같은 값이었다. 삼항연산자 안이라 `grep`도 이전 `fontWeight` 전수 정리(4차 확장, "잔여 0건 재확인")도 놓쳤던 것 — 값이 같으니 시각 변화 없이 토큰으로 교체(`FONT_SIZE`/`FONT_WEIGHT` import 추가). `org-tree-arrow-keys.test.jsx` green. `isOrg` 쪽 `0.9375rem` 분기는 이미 검토한 그 클러스터의 연장이라 손대지 않음. 프런트 전체 `npm test`(267파일·1,808건) green(신규 테스트 없음 — 순수 리터럴 대 토큰 치환은 계산값이 같아 시각 회귀가 없고, 주석 추가는 동작 변화가 없다). **acceptance_criteria(1) 사실상 충족**: 텍스트 성격의 raw 리터럴은 이제 전부 0.6875rem 2곳(코드 근거) + 이번 7곳(코드 근거)뿐이고, 아이콘/이모지/아바타류는 구조적으로 자명해 개별 주석이 관행상 불필요하다(다른 아이콘 20여 곳도 마찬가지 관례). **acceptance_criteria(5)(static_checks.sh 린트)는 여전히 켜지 않는다** — 이 장꼬리 값들이 "예외로 남긴다"는 판단 자체가 방금 확정된 것이라, 린트를 지금 켜면 그 판단이 뒤집힐 때마다(다음 세션이 전용 토큰을 신설하기로 하면) 다시 풀어야 한다. 다음 세션이 이 장꼬리에 대한 최종 설계 결정(전용 토큰 신설 vs 예외 유지)을 내리면 그때 켜는 것이 순서 |

**8차 확장(acceptance_criteria(5) 게이트 신설, 2026-08-16, WARM 재개 직후)** — 7차 확장이 "예외로 남긴다는 판단이 방금 확정됐다"고 적은 그 상태를 근거로, 이제는 안전하다고 보고 재유입 방지 게이트를 걸었다. `scripts/check_typography_literals.py` 신설(`fontWeight` 0건 유지 + `fontSize`는 `FONT_SIZE.*` 토큰이거나 이미 낱개 검증된 15종 예외 값만 허용, 새 값은 무조건 실패) + `static_checks.sh` 필수 단계 배선, `tests/unit/test_typography_literals_scan.py` 7건(revert-to-verify 포함) green. 만드는 과정에서 삼항연산자 조건까지 leaf로 오판·JSDoc 주석 안 언급을 코드로 오판하는 검사 자신의 결함 2건을 실측으로 잡아 고쳤다. **acceptance_criteria(1)(2)(4)(5)(6) 충족 확인, (3)은 D-78의 "정책 질문" 결론을 그대로 수용**(6단계는 `theme-baseline.test.js`가 닫힌 집합으로 못박아 뒀고 장꼬리는 개별 근거가 있는 예외라 인위적 재양자화가 오히려 근거 없음) — **PA-RC-0001 실행 관점에서 완결로 판단**. 상세: `DECISIONS.md` D-81 |
| PA-02 | **High** | **UX Writing 규칙 문서가 존재하지 않는다**(`PA-RC-0002`, 저장소 전체 검색 0건). 그래서 문구가 매번 호출부 즉흥 판단으로 갈렸다 — ⓐ 실패 문구 167건 중 `[무엇을 하라]`를 주는 것이 24건(14%), `[왜]`는 5건, **3요소를 다 갖춘 것은 0건** ⓑ 같은 문장이 마침표 유무로 동시 존재 ⓒ 같은 개념에 동사가 균등 분포(생성 84·등록 72·추가 64·만들 49). `lib/api.js` **한 파일 안에서** 16행은 막다른 길이고 54행은 회복 경로가 있다 | `frontend/src/lib/api.js:10,16,35,54` · `frontend/src/app/AdminRoutes.jsx:40` · `var/product-audit/errcopy_scan.json` · 스캐너 `scan_copy.py`·`scan_errcopy.py` | **부분 구현완료(기초)** — `docs/UX_WRITING.md` 신설(acceptance_criteria 1): 오류 2문장 패턴 + 실패 종류별 회복 절 선택표, 마침표(문장형 O/라벨형 X) 규칙, 표준 동사표(추가/수정/삭제/비활성화/활성화/저장 6종 확정), 길이·괄호 규칙. **공용 진입점부터**(implementation_direction 4): `lib/api.js`의 막다른 길 2곳(알 수 없는 상태 코드, 네트워크 단절) 수정 — 이 파일 하나가 앱 전체 API 호출의 오류 문구 출처라 여기 고치면 다수 화면이 따라온다. 신규 단위 테스트 3건(revert-to-verify 확인). "권한이 없습니다" 마침표 불일치(원 근거)는 재확인 결과 **버그 아님**으로 재분류 — `title=` prop(헤딩)과 문장(토스트/본문)이 이 문서의 마침표 규칙대로 이미 올바르게 갈려 있었다. **남은 범위**(전면 정렬 전): 막다른 길 나머지 약 165건, 동사표 위반 검사, `static_checks.sh` 린트 3종(acceptance_criteria 5) — 지금 걸면 기존 위반이 광범위해 즉시 실패한다. 순서상 전면 정렬 뒤에 추가.

**2차 확장**(kit.jsx 내부 점검에서 시작): 감사가 "공용 키트 `ui/kit.jsx`조차 마침표 관용이 6:5로 자기 안에서 갈라져 있다"고 지목한 걸 따라가 보니 실제로는 kit.jsx 2곳(붙여넣기 초과 경고, 선택지 없음 오류)뿐 아니라 화면 **20개·26곳**에 "`-습니다,` [다음 문장]" 식으로 독립된 두 문장을 마침표 대신 쉼표로 이어붙이는 같은 패턴이 있었다(`links.jsx`·`MessageThread.jsx`·`chat-helpers.js`·`Chat.jsx`·`Dashboard.jsx`·`DataScreen.jsx`·`SubListDrawer.jsx`·`Diagnostics.jsx`·`ServiceStatusPanel.jsx`·`Maintenance.jsx`·`authoring.js`(7)·`automation.js`(3)·`governance.js`(2)·`org.js`(4)·`integrations.js`·`shared.js`(2)·`platform.js`(2)·`SettingEditor.jsx`(2)·`settingsRegistry.js`·`SettingsMain.jsx`(2)) — kit.jsx의 6:5는 저장소 전체 패턴의 일부였을 뿐이다. 목록 나열(예: "중단, 응답 없음")은 서술어 종결 어미로 안 끝나 대상이 아님을 확인하며 전부 눈으로 개별 검토 후 수정, `teamdoc.test.jsx`의 대응 assertion도 함께 갱신. 전체 `npm test`(253개 파일·1,754건) green. 이 하위 패턴은 이제 저장소 전체에서 **0건**이라 즉시 lint를 걸 수 있었다: `frontend/src/ui/ux-writing-punctuation.test.js`(신규, revert-to-verify 확인) + `static_checks.sh` 신규 step(같은 정규식, 독립 게이트 — static_checks.sh는 npm test를 안 부른다) 둘 다 green. **동사표/막다른 길 축은 아직 남았지만, 마침표 축은 이제 기계 검사로 고정됐다**.

**3차 확장(막다른 길 대량 수정)**: `var/product-audit/scan_errcopy.py`를 현재 소스로 재실행해 최신 수치를 얻고(고유 실패 문구 167건, 막다른 길 141건 — 원 감사와 근사, 드리프트 없음 확인), kit.jsx를 제외한 141건을 파일 클러스터 5개(chat/game-room · board/comments/teamdoc · project/assistant/devreport · users/notion/tickets · registry/settings/ops/misc)로 나눠 병렬 에이전트 5개에 배분. 각 에이전트에게 kit.jsx의 `ErrorState`(title+help+별도 액션 버튼) 같은 스캐너 오탐 패턴을 명시적으로 알려 스킵 기준을 맞췄다. 5개 전부 완료 후 에이전트들이 스스로 보고한 "자기 담당 파일인데 후보 목록엔 없던" 동일 패턴 잔여 건들(`useGameRoomController.js` 2곳·`BoardPost.jsx`/`TeamDoc.jsx`/`TeamDocs.jsx`/`NotionConsole.jsx` 각 1곳·`Home.jsx` 1곳)을 직접 찾아 수정, 이어서 재스캔이 텍스트 중복 제거로 가려져 있던 다음 켜의 동일 문자열(`MyTickets.jsx`·`TeamTickets.jsx`(신규 발견 파일, 어느 클러스터에도 없었음)·`useChat.js`·`Diagnostics.jsx`·`kit.jsx:836`·`Ticket.jsx`(신규 발견 파일)·`ServiceStatusPanel.jsx`)를 4라운드에 걸쳐 추가로 발견·수정 — 매 라운드 재스캔 수치가 141→69→65→63으로 수렴하다 63에서 안정됐다(더 이상 새 파일이 안 나옴). 전체 `npm test`(254개 파일·1,755건) 매 라운드 green 재확인. 남은 63건은 개별 검토 결과 대부분 스캐너 오탐으로 확인됨(확인 사유는 대응 리뷰: 인접 액션 버튼 있음·confirm 대화상자 질문·서술형 설명/안내문·성공 메시지·업무 규칙 서술 — 실패 통지가 아님). **동사표 위반 검사·나머지 스캐너 오탐 재검토는 여전히 미착수, static_checks.sh 린트(acceptance_criteria 5)는 그 뒤에나 추가 가능** |

**4차 확장(동사표 정렬, 2026-08-15)**: `docs/UX_WRITING.md` §3의 6개 개념 중 화면 텍스트에서 실제로 관측된 5개 축(생성/등록/만들기→추가, 편집/변경→수정, 제거/지우기→삭제, 끄기/중지/정지→비활성화, 켜기→활성화)을 저장소 전체(JSX 텍스트 자식 기준)에서 정렬했다. 배경 에이전트에 위임했는데 Windows Supervisor가 프로세스를 두 번 재시작하며 완료 알림이 두 번 유실됐다 — 에이전트 자신의 트랜스크립트·파일 변경은 공유 워킹트리에 그대로 남아 있었으므로(에이전트 도구 호출도 나와 같은 파일시스템에 직접 쓴다), 세 번째로 다시 재개하는 대신 Main Agent가 직접 diff를 검토해 마무리했다(CLAUDE.md §4 "Main Agent가 최종 diff를 검토한다"에 해당). **에이전트 결과**: 76개 파일·326곳(문법상 완전 대칭 — 순수 단어 치환, 구조 변경 없음), `npm test` 1건 실패(등록→추가로 바뀐 소스에 낡은 테스트 기대값 1곳, `registry-documents-empty-help.test.js`)만 수정하면 나머지는 UX_WRITING.md §3 규칙과 명시된 예외(역할 변경, 필터/검색어 지우기 등)를 정확히 지켰음을 6개 개념 전체에 대해 직접 재검토(각 개념별 JSX 텍스트 재스캔)로 확인 — 실제 위반 0건. **새 기계 검사 2개 신설**(에이전트가 못 끝낸 부분, comma-splice 패턴과 같은 이중 게이트): `frontend/src/ui/ux-writing-verb-table.test.js`(vitest, 5건) + `static_checks.sh` 신규 step. 둘 다 `>...텍스트...<` JSX 텍스트 자식만 본다 — 설계하면서 두 가지를 직접 발견했다: (1) 첫 버전은 `[^<]*`가 줄바꿈을 안 막아 JS 비교 연산자(`a > b`)의 `>`가 수백 줄 뒤 무관한 `<`와 잘못 짝지어지는 오탐을 냈다(폭 제한 `{0,120}`으로 해결, vitest는 추가로 주석을 먼저 지운다 — 주석이 금지어를 설명하며 언급하는 일이 잦아서다). (2) 범위를 `registry/*.js`의 객체 속성 정의(`label:`/`help:`)까지 넓히려다가 포기했다 — `registry/authoring.js`·`automation.js`에 "문서 생성"(자동 문서 생성 기능 자체의 고정 이름)이 수십 곳에 정당하게 쓰이고 있어 허용 목록만으로는 안전하게 못 걸렀다. 이 범위 제한과 각 예외 문구의 근거는 두 파일의 주석에 남겼다. revert-to-verify 확인(JSX 자식 단순 케이스 + 여러 줄에 걸친 케이스 둘 다). 6번째 축(반영/적용→저장)은 예외가 너무 다양해(설정 즉시반영·외부 동기화·필터 범위 설명 등) 이번에도 기계 검사를 안 걸었다 — 사람이 계속 심사한다. 전체 `npm test`(258개 파일·1,769건) green. **적용한 skill 없음**(순수 기계적 용어 정렬이라 UX Writing/humanize-korean 대상이 아니었다 — 이미 확정된 규칙표를 소스에 맞추는 작업) |

**5차 확장(막다른 길 전수 재검증, 2026-08-16, `PA-RC-0002` 재검증 대응)** — Audit이 "3차 확장의 63건을 다시 전수로 읽으면 20건은 실패 서술 자체가 아니다"라고 지목한 것을 계기로, 63건(재스캔 결과 62건, `copy_inventory.json` 드리프트 없음 확인) **전부**를 `file:line`이 아니라 **실제 소스 문맥**으로 하나씩 대조했다 — 스캐너는 텍스트 리터럴만 보고 ① 문자열 연결(`+`)로 뒤에 이어붙는 회복 절 ② 같은 JSX의 형제 `<Button onClick=.../<Link href=...` ③ `ErrorState`(`onRetry`)/`EmptyState`(`steps`/`action`)가 이미 그리는 회복 UI를 못 본다는 것을 낱개로 확인했다(예: `Board.jsx`는 `"...못했습니다: " + failed.join(", ") + ". 다시 시도해 주세요."`처럼 연결돼 있는데 스캐너는 앞 조각만 캡처, `MyTickets.jsx`의 `EmptyState`는 `steps={[...]}`로 실제 행동 목록을 이미 주고 있음 등). 그 결과 **62건 중 52건이 이미 충족**(스캐너가 놓친 회복 절이 실제로 있음)이거나 **실패 서술 자체가 아님**(설명문·규칙 문구·확인 대화·성공 알림·인라인 폼 검증·필드 값 자리표시자)으로 확인됐고, 진짜 새로 고칠 것은 **`SystemOps.jsx`·`useChat.js` 딱 2곳**뿐이었다(둘 다 `onError` 폴백에 "잠시 후 다시 시도해 주세요." 추가, 신규 회귀 2건 — `system-ops-toast.test.jsx`에 추가한 케이스는 네트워크 실패로 `onError`를 실제로 태워 토스트 문구를 확인하고, `chat-delete-conv-error-copy.test.jsx`는 `useToast`를 스파이로 바꿔 호출 인자를 직접 확인 — 둘 다 revert-to-verify 확인). 남은 8건(`DataScreen.jsx`의 삭제됨/권한없음 안내, `ChatPane.jsx`의 폴링 중 안내, `Diagnostics.jsx`의 이전 자료 폴백 안내 2곳, `SystemOps.jsx`의 도우미 부재 안내, `NotificationBell.jsx`의 아이콘 호버 툴팁)는 **재시도가 실제로 무의미하거나 이미 자동 재시도 중**이라는 구체적 사유와 함께 예외 목록에 등재했다(acceptance_criteria 2가 명시적으로 허용하는 경로 — "다시 시도해 주세요"를 기계적으로 남발하지 않는 쪽을 골랐다). `var/product-audit/scan_errcopy.py`를 v3로 다시 써서 이 판정 전부(사유 포함, 값 없음)를 코드로 고정했고, ACTION 정규식도 넓혔다(원래 "고쳐 주세요"/"줄여 주세요"/"만들 수 있습니다" 같은 활용형을 놓치고 있었다 — `project-format.js`·`SettingEditor.jsx`·`org.js`가 이 정규식 공백 때문에 억울하게 걸려 있었다). **재측정 결과: 회복 절 비율 96~100%**(계산 방식에 따라, 예외 목록을 "충족"으로 넣으면 100%, 순수 [무엇을 하라] 검출만으로도 96%) — 기준 90%를 넘겼다. `var/product-audit/scan_errcopy.py`는 gitignore 대상(`var/`)이라 이 판정 근거는 여기 이 문단에 영구 기록한다. `docs/QA_COVERAGE.md` §15 `T2` 갱신. **acceptance_criteria(5)는 의도적으로 부분만 채운다** — 마침표 중복·동사표 위반 2종은 이미 `static_checks.sh`에 기계 게이트가 있지만, 회복 절 비율 자체는 안 걸었다: 이번 재검증에서 직접 겪었듯 이 판정은 문자열 연결·형제 JSX·공용 컴포넌트 문맥을 함께 봐야 정확한데, 순수 정규식 게이트로 굳히면 앞으로 추가되는 정상 문구(예: `ErrorState`로 감싼 새 화면)마다 오탐으로 막힐 위험이 이번에 실측한 오탐률(62건 중 52건)만큼 크다. 이 축은 사람(또는 다음 감사)이 계속 심사하는 쪽으로 남긴다 |

**6차(재검증, 2026-08-16, WARM 재개 직후)** — Handoff 요약표가 아직 "열림(90% 미달)"로 적힌 것을 보고 5차 확장의 96~100% 결론을 다시 직접 대조했다: `docs/UX_WRITING.md` 존재 확인(criteria 1) · `static_checks.sh`의 comma-splice·표준 동사표 2개 게이트 green 재확인(criteria 5의 2/3). 세 번째 게이트(회복 절 비율)는 5차 확장의 판단(순수 정규식으로 굳히면 문맥 의존 판정을 오탐낸다)을 그대로 수용해 **새로 걸지 않는다** — fontSize와 달리 "아직 안 만들어서"가 아니라 "정확히 자동화 못 하는 종류의 판단이라서"라 같은 재검토를 반복하지 않는다. **PA-RC-0002 실행 관점에서 완결로 판단**. 상세: `DECISIONS.md` D-81 |
| PA-03 | **High** | **SQLite 쓰기 경합 재시도가 공용 유틸 없이 호출부마다 손으로 쓰여 있다**(`PA-RC-0008`). 예산이 2·5·10·12로 네 가지고 backoff/jitter를 쓰는 곳은 7곳 중 1곳뿐이다. `app/prompts/service.py::new_version_from`(예산 5·jitter 없음)이 8-way 경합에서 재시도를 소진하고 **처리되지 않은 `OperationalError: database is locked`로 500**이 난다. 결정적 증거는 이 저장소가 **이미 그 교훈을 실측해 적어 뒀다**는 것 — `auth/router.py:505` *"지터 없이 10회 재시도로도 5번 중 1번은 여전히 실패했다"*. 지식이 옆 파일로 전파되지 않았다 | `app/prompts/service.py:123,148-178` · `app/auth/router.py:476,505-506` · `app/core/db.py:148-176`(`is_write_conflict`는 정상) · `tests/integration/test_prompt_create_new_version_race.py` | **구현완료** — `app/core/db.py`에 공용 헬퍼(`DEFAULT_WRITE_CONFLICT_RETRIES=10`, `write_conflict_backoff` — auth.py 로그인 실측값 승격) 신설, 7개 호출부(prompts/approvals/team_chat/games/notion_mapping/auth.py/sessions.py는 의도적 제외) 통일. 예산 소진 시 raw 500 대신 `ConflictError`(prompts/approvals/notion_mapping — 신규 exhaustion 테스트 3건, revert-to-verify 확인). `approvals`/`team_chat`은 관측된 경합이 더 잦아 12 유지(사유 주석). 최대 지연 상한 계산해 `db.py`에 문서화(예산10→2.25s, 12→3.3s, `proxy_read_timeout 180s` 대비 무시할 수준). race 계열 6개 파일을 **20회 연속 반복 실행해 실패 0건** 확인. `tests/unit`+`tests/regression`+`tests/security`+`tests/integration`(2,900+건) green(청크 실행, `scripts/run_full_regression.sh` 신설 — PA-RC-0009와 공유) ‖ ✅ **구현완료·Audit 재검증됨(2026-08-16)** — `app/core/db.py:199,202`에 공용 `DEFAULT_WRITE_CONFLICT_RETRIES=10`·`write_conflict_backoff()` 신설, 호출부 10곳 전부 backoff 사용(8곳이 공용 상수 채택, `approvals`·`team_chat`의 12는 의도적 상향). **결정적 증거: race 테스트 5회 연속 통과** — 이 Audit이 원래 약 40% 실패를 재현했던 바로 그 테스트다 |
| PA-04 | Med | **입력 길이 정책이 서버에만 존재한다**(`PA-RC-0005`). 백엔드는 길이 제약을 452건 선언하는데 프런트 `maxLength`는 21건뿐이고 **전부 사용자 콘솔 수제 화면**이다. 관리자 화면 28개를 그리는 공용 폼 경로(`kit.jsx::FormField`·`DataScreen.jsx`·`registry/*.js`)에는 길이 개념이 **0건** — 관리자는 상한을 넘겨 입력하고 저장을 누른 뒤에야 거절당한다. `UX-40`(422 사유가 화면에 안 옴)과 겹치면 보이는 것은 영어 상수 `Invalid request data` 하나뿐이라 **입력을 잃는다** | `frontend/src/ui/kit.jsx:766,830`(`inputProps` 설정하면서 `maxLength`는 없음) · `var/product-audit/limits_scan.json` · 스캐너 `scan_limits.py` | **구현완료(등록 화면 전체)** — 공용 메커니즘 + `create:` 있는 등록 화면 **13/13** 배선 완료: `prompts`·`policies`·`templates`·`departments`·`job-titles`·`organizations`·`schedules`·`approval-delegations`·`integrations`·`runners`·`workflows`·`announcements`·`ai-quotas`(전부 create/edit 필드명을 실제 Pydantic 스키마와 대조 확인, 핀 고정 회귀 테스트 13건). `app/core/field_limits.py`가 Pydantic `model_json_schema()`로 손으로 안 옮기고 직접 유도(`FORM_SCHEMAS`가 registry key → 스키마 매핑, 새 화면은 한 줄만 추가하면 됨) → `scripts/generate_field_limits.py`가 `frontend/src/generated/fieldLimits.json`으로 빌드 시 내보냄(런타임 API 아님) → `scripts/check_field_limits_fresh.py`가 `static_checks.sh`에서 드리프트를 잡음(revert-to-verify 확인) → `kit.jsx::FormField`가 `maxLength` 렌더 + 남은 글자 수 + 붙여넣기 초과 토스트(revert-to-verify 확인). `runners.description`/`workflows.purpose`처럼 edit 스키마에 제약이 없는(create만 있는) 비대칭 필드도 introspection이 자동으로 올바르게 구분함(수작업이었다면 놓치기 쉬운 지점, 핀 테스트로 고정). **남은 범위**: `Users.jsx`(공용 `FormModal`/`FormField`를 안 타는 손수 제작 화면, 별도 처리 필요) — `feature-flags`는 create/edit 폼 자체가 없는 화면(토글 전용)이라 이 RC의 대상이 아니다 ‖ ✅ **구현완료·Audit 재검증됨(2026-08-16)** — `frontend/src/lib/fieldLimits.js` 신설(백엔드 Pydantic 스키마에서 유도), `kit.jsx:783` `FormField`가 `maxLength` 수용 + `:789` 붙여넣기 초과 경고 + `:800` 글자수 표시, `:1033`에서 `maxLengthFor(screenKey, formKind, f.name)`로 배선. 프런트 `maxLength` 21 → 37건 ‖ ✅ **자기모순 정정(2026-08-16, `PA-RC-0014`)** — 위 "남은 범위" 문구가 틀렸다: `Users.jsx`는 실제로 `FormModal`을 그대로 쓰고 있었다(`fields` 배열만 손으로 구성). `screenKey="users"`/`formKind` prop을 안 넘긴 배선 누락이었을 뿐이라, `PA-RC-0014`가 그 두 prop과 `FORM_SCHEMAS["users"]`(+`"offboarding"`, `Offboarding.jsx`는 진짜로 `FormModal` 밖이라 별도 배선) 한 줄로 닫았다. 이제 이 표 전체가 정확히 **완료**다 — Users.jsx도 예외가 아니다 |
| PA-05 | Med | **승인된 TEST 서버가 저장소보다 5일·131커밋 뒤처져 있다**(`PA-RC-0007`). 배포본은 2026-08-10 16:22 빌드이고 이후 `app/`·`frontend/`를 건드린 커밋이 131개다. 번들 asset 공통 33개 중 **내용 해시가 같은 것은 4개뿐**. 제품 결함이 아니라 **검증 체계의 결함**이다 — 지금 Chrome E2E를 돌리면 현재 코드가 아니라 08-10 빌드를 검증하게 되고, green이 나와도 현재 제품에 대해 아무것도 말하지 않는다. 실제로 배포본에는 이미 고쳐진 `AI-11`·`AI-08`·`UA-25`·`VIS-162`가 **그대로 살아 있다** | 원격 번들·백엔드 소스 mtime·서비스 기동 시각 3중 대조 + asset 내용 해시 대조(`10.100.64.71`) | **구현완료** — 2026-08-15 17:19 KST, `scripts/build-bundle.sh`(로컬 HEAD `01e9687`)로 번들을 새로 만들어 `scripts/upgrade-clovirone-web-assistant.sh`로 통합 배포(백업 `/var/backups/clovirone-web-assistant/20260815_171907` 생성 확인, `UPGRADE_OK`, alembic 마이그레이션 적용, worker/web 헬스체크 통과). 배포 뒤 3중 확인: ⓐ `healthz`/`readyz` 둘 다 OK ⓑ 서버의 `BUILD_STAMP.json` source_hash가 로컬 재빌드 직후 값과 일치 ⓒ 실제 서빙되는 `index.CETw8jl4.js` 자산 해시가 로컬 빌드 산출물과 정확히 일치(정적 경로로 직접 확인, `/` 는 인증 리다이렉트라 우회). 재기동 이후 `journalctl -p warning` 0건. 부수 정리: 배포본에 오래전 삭제된 `app/policies/__pycache__` 빈 잔재가 rsync `--delete`로 안 지워지고 있었다(현재 소스에 `app/policies` 자체가 없음, 기능 영향 없는 죽은 캐시) — 직접 제거 ‖ ✅ **해소 확인(2026-08-16 Audit 실측)** — TEST 서버 재배포됨(번들 mtime 08-15 23:38, 서비스 기동 08-16 00:32). 배포본 `source_hash`가 **HEAD 커밋본과 정확히 일치**하고 `check_bundle_fresh.py` → `BUNDLE_FRESH_OK`. 원래 드리프트 5일·131커밋 → **0커밋** |
| PA-06 | Med | **백엔드 전체 회귀는 실제로 전부 통과한다 — 문제는 결과가 아니라 실행 수단이다**(`PA-RC-0009`). 이 Audit이 청크 분할 전경 실행으로 2,903건 전부 `EXIT=0`을 확인했다(`regression` 331·`security` 498·`unit` 761·`integration` 1,313). 단일 호출로는 45분+가 걸려 세션 경계를 못 넘고(두 번 시도해 27%·49%에서 잘림), 그 결과 `WORK_STATE.md`가 **세 사이클 동안 이것을 "진짜 행(hang)"으로 잘못 기록**해 아무도 전체 회귀를 돌리지 않았다. 여기에 flaky 1건(`PA-03`의 race 테스트, 약 40% 실패)이 겹쳐 **1회 green을 완료 근거로 쓸 수 없다** | 4청크 전경 실행 5회, 모두 `EXIT=0` · 이 Audit이 직접 재현 | **구현완료** — 이 Audit이 처음에 "Full Regression green이 확립된 적 없다"(High)로 썼다가, 스스로 처방한 방법을 실행해 전부 통과함을 확인하고 Medium으로 내렸다. `scripts/run_full_regression.sh`(청크 스크립트, PA-RC-0008이 신설)로 **3회 연속 배경 실행**해 완료 기준을 채웠다: 1회차 `EXIT=0`(1826초≈30분26초, PA-RC-0005 13/13 확장 커밋 직후), 2회차 `EXIT=0`(1802초≈30분2초), 3회차 `EXIT=0`(1927초≈32분7초) — 06:32:51Z~08:05:26Z 연속 실행, 실패 0건. (첫 시도는 invocation 경계에서 3회차 도중 중단됐다 — 애매한 상태를 완료 근거로 쓰지 않고 처음부터 다시 시작함, `docs/DECISIONS.md` 참고 불필요할 만큼 단순한 재시작.) `PA-03`이 지목한 flaky race 테스트도 이번 3연속에서 전부 통과해 재발 없음 확인 ‖ ✅ **구현완료·Audit 재검증됨(2026-08-16)** — `scripts/run_full_regression.sh`(청크 스크립트) 신설, Full Regression **3연속 green**(30m26s/30m2s/32m7s, 전부 exit=0). 이 Audit이 지적한 "45분+라 세션 경계를 못 넘는다"는 실행 수단 문제가 스크립트로 닫혔다 |
| PA-07 | Med | **서버 렌더 로그인 화면이 다크 모드를 전혀 따르지 않는다**(`PA-RC-0010`). Chromium 151로 `prefers-color-scheme: dark`를 주고 실측하니 `body` 배경·글자색이 light와 **완전히 동일**했고 `<html data-theme>`는 `null`이었다. 원인은 값이 아니라 **활성화 경로의 부재**다 — `app/static/css/tokens.css:154`에 `[data-theme="dark"]` 블록이 실제로 있는데 `app/templates_html/**` 전체에 `data-theme`·`prefers-color-scheme`가 **0건**이라 켜는 주체가 없다. 그 파일의 다크 토큰 20여 개는 자신을 읽는 유일한 화면에서 영원히 활성화되지 않는다 | 실브라우저 측정 `var/product-audit/probe_login2.py` · `app/static/css/tokens.css:154` · `app/templates_html/**` grep 0건 | **구현완료(범위 재확인, `DECISIONS.md` D-74)** — 나머지 3화면 실측 결과 감사의 "전부 같은 문제" 가정이 부분적으로 틀렸다: `change-password`는 이미 `theme.js`로 다크 지원 중(감사가 정적 grep만 해서 런타임 JS 세팅을 놓침), `login`은 `login.css`의 `color-scheme: light` + `test_theme_on_all_authed_pages.py`로 **의도적으로 라이트 고정**(히어로가 그 전제로 설계됨) — 그대로 둔다. 진짜 공백은 `forgot-password`/`reset-password` 둘뿐이었다. `app/static/css/tokens.css`에 `@media (prefers-color-scheme: dark) { :root:not([data-theme]) {...} }` 추가(값은 `[data-theme="dark"]`와 의도적으로 중복 — 전처리기 없이 선택자 간 참조 불가 — `test_css_says_what_it_does.py`의 신규 drift 테스트가 어긋남을 잡음, revert-to-verify 확인). `:not([data-theme])` 가드로 SPA·change-password는 건드리지 않고 login은 `login.css`가 더 구체적 선택자로 이미 이김 — 셋 다 Playwright 실측으로 확인. body 텍스트/배경 대비 신규 케이스 2건(라이트 14.00·다크 17.04) 추가 ‖ ✅ **Audit 재검증 종결(2026-08-16)** — 구현 Phase의 판단이 옳았음을 실측으로 확인했다. 서버 렌더 4화면 light/dark 실측: `/forgot-password`·`/reset-password`는 **다크가 켜진다**(`rgb(9,14,29)`), `/login`은 동일(의도된 라이트 고정 — `login.css:19` `color-scheme: light` + `test_theme_on_all_authed_pages.py`가 계약으로 못박음), `/change-password`는 `theme.js`로 저장된 선택을 따른다(정상). **원 감사(PA-RC-0010)의 "로그인 화면이 빠졌다"는 오탐이었고**, 구조적 주장만 두 화면에 대해 옳았다. 내가 이 행에 한때 "되돌려라"라고 적었던 것도 `/login` 하나만 재고 내린 오판이라 함께 철회한다. 상세: `PA-F-041` |
| PA-08 | High | **부서·직책·조직 생성이 SAVEPOINT/재시도 없는 check-then-insert다**(2026-08-15, 이 세션의 배경 RBAC 재감사 — PA-RC-0008과 같은 계열이지만 8건 승격 때 이 3개 호출부만 빠졌다). `app/org/service.py::create_item`(부서·직책)과 `app/org/router.py::create_organization`이 `find_by_name`/slug 조회로 중복을 사전 검사한 뒤 재시도 없이 바로 `db.add`+`flush`한다 — `Department.(org_id,name)`·`JobTitle.name`(전역)·`Organization.slug`가 전부 실제 DB UNIQUE라, 동시에 같은 이름/slug로 두 번 POST하면 사전 검사를 둘 다 통과하고 나중 요청의 `flush()`가 처리 안 된 `IntegrityError`/`OperationalError`("database is locked")로 원시 500을 냈다. `docs/BACKLOG.md` PROJ-01(2026-08-11)이 이미 고친 것과 정확히 같은 결함 모양이다 | `app/org/service.py:244-330`(`create_item`) · `app/org/router.py:361-407`(`create_organization`) · `app/org/models.py`(Department/JobTitle/Organization의 실제 UNIQUE 제약) | **구현완료** — 두 호출부 모두 `app/core/db.py`의 공용 `DEFAULT_WRITE_CONFLICT_RETRIES`/`write_conflict_backoff`/`is_write_conflict`로 SAVEPOINT 재시도 래핑, 예산 소진 시 각 호출부가 **원래 사전 검사와 같은 예외 타입**으로 통일(create_item→`ConflictError` 409, create_organization→`ValidationAppError` 422 — 사전 검사가 이미 그 타입을 쓰고 있었으므로 그대로 맞춤). 신규 회귀 `tests/integration/test_org_create_race.py`: 8-way 실 스레드 경합 2건(Department 조직-스코프/JobTitle 전역-스코프, 서로 다른 dedup 분기를 태움) + monkeypatch 소진 시험 2건(revert-to-verify 확인 — `db.add`를 항상 `IntegrityError`로 고정해 예산만큼 정확히 재시도하는지 확인). **부수 발견(별도, 이번엔 안 고침)**: 이 새 시험을 8-way로 처음 돌리다가 재시도 사이의 "스냅샷 새로 뜨기" `db.commit()`(`create_item`/`create_organization` 내부)이 그 자체로도 드물게 경합에 거부될 수 있음을 실측 — 이건 고쳤다(같은 `is_write_conflict` 판정으로 감싸고 `db.rollback()`으로 세션 정리). 그런데 그 상위, `app/core/deps.py::get_db`(모든 엔드포인트가 쓰는 요청-스코프 **바깥** commit, `create_item`이 성공을 반환한 *다음* 자리)에는 재시도가 전혀 없다는 **더 넓은 기존 결함**을 함께 찾았다 — 이건 이 두 호출부만의 문제가 아니라 `get_db`를 쓰는 모든 라우트에 걸리고, 제대로 고치려면 커밋 재시도가 아니라 **요청 전체 재실행**이 필요하다(단순 커밋 재시도는 이미 flush된 행을 rollback으로 조용히 버린다 — 시험 작성 중 직접 확인). 범위가 이 RC를 훨씬 넘어서 이번엔 손대지 않고 `test_org_create_race.py` 모듈 docstring에 남겨 뒀다 — 다음에 이 결이 다시 나오면 `get_db` 자체를 다룰 차례다 | **추가(같은 날, 2026-08-15)**: PA-14(D-75 부분)로 `get_db` 바깥 commit 쪽은 분류만 고친 뒤, 되짚어 보니 org의 두 호출부와 **똑같은 "재시도 사이 스냅샷 새로 뜨기 commit이 무방비"** 패턴이 `PA-RC-0008`이 승격한 재시도 루프 중 3곳에 더 있었다: `app/approvals/service.py::create_approval`(230행), `app/notion_mapping/service.py::get_or_create_mapping`(74행), `app/prompts/service.py::new_version_from`(188행) — 전부 `db.commit()  # 스냅샷을 새로 뜬다` 뒤에 아무 방어가 없었다. `app/games/service.py`·`app/team_chat/service.py`의 재시도 루프는 이 패턴이 아니라(`db.refresh()`/`db.rollback()`만 씀, commit 없음) 대상이 아님을 확인했고, `app/auth/router.py` 로그인 재시도는 애초에 이 자리에 commit이 하나뿐이라(재시도 사이 별도 commit이 없음) 처음부터 안전했다. 세 곳 다 org와 같은 패턴(`try: db.commit() except (IntegrityError, OperationalError): if not is_write_conflict: raise; db.rollback()`)으로 통일. 기존 회귀(`test_approval_create_race.py`·`test_notion_mapping_get_or_create_race.py`·`test_prompt_create_new_version_race.py`·`test_prompt_publish_race.py`, 총 12건) 전부 green — 이 패턴이 원래도 드물게만 터져 기존 시험이 우연히 못 잡았을 뿐, 새 시험을 추가하진 않음(이미 각 파일에 8-way 실경합 시험이 있고, 이번 수정은 그 동일한 코드 모양을 한 번 더 복제한 것뿐이라 서로 다른 새 시나리오가 없다) |
| PA-09 | Med | **조직도 트리가 `role="tree"`를 선언했는데 화살표 키 이동이 없다**(2026-08-15, 배경 접근성 재감사). `OrgTree.jsx`의 `onKeyDown`이 Enter/Space(선택)·ArrowRight/Left(펴기/접기)만 처리하고 위/아래/Home/End가 없어, 키보드 사용자는 화살표 대신 Tab으로 노드마다(펴진 부모는 펴기 버튼까지 포함해 둘씩) 거쳐야 했다 — 부서 20개짜리 트리에서 25회+ Tab | `frontend/src/screens/OrgTree.jsx:114-144` | **구현완료** — WAI-ARIA treeview 패턴대로 위/아래/Home/End가 `role="tree"` 안의 "보이는" `role="treeitem"`(접힌 자식은 DOM에 없어 자동으로 제외) 사이를 DOM 순서로 이동. 신규 회귀 `org-tree-arrow-keys.test.jsx`(3건, revert-to-verify 확인) |
| PA-10 | Med | **허용 이메일 도메인 칩의 삭제가 키보드로 안 된다**(2026-08-15, 배경 접근성 재감사 — 이 저장소가 `AssistantDrawer.jsx` 주석에 이미 적어 둔 함정: "MUI Chip의 onDelete는 아이콘이 tabIndex=-1이라 키보드로 못 뗀다"). `StructuredObjectFields.jsx`(설정 화면)만 그 교훈을 안 따르고 `Chip onDelete`+커스텀 `deleteIcon` span을 그대로 썼다 | `frontend/src/screens/settings/StructuredObjectFields.jsx` | **구현완료** — `AssistantDrawer.jsx`/`Chat.jsx`와 같은 패턴(독립 `IconButton`)으로 교체. 신규 회귀 테스트 2건 |
| PA-11 | High | **채팅 말풍선 시각이 KST가 아니라 뷰어의 실행 환경 로컬 시간대로 표시됐다**(2026-08-15, 배경 wiring-gap 재감사, 불변규칙 §3-7 위반). `lib/format.js::fmtTimeShort`(팀 채팅)와 `game-room/timeUtils.js::fmtTime`(게임방 채팅) 둘 다 `toLocaleTimeString("ko-KR", {...})`을 `timeZone` 없이 불렀다 — 같은 화면군(AI 도우미 채팅)의 세 번째 구현 `chat-helpers.js::fmtTime`만 Asia/Seoul을 명시하고 있었다. 개발 서버가 KST라 로컬에서는 안 보이고, VPN·해외 출장·시계 오설정으로 KST가 아닌 뷰어에게만 시각이 몇 시간 밀려 보이는 결함이라 실사용 중 발견되기 어렵다 | `frontend/src/lib/format.js` · `frontend/src/screens/game-room/timeUtils.js` | **구현완료** — 두 곳 다 `timeZone: "Asia/Seoul"`을 명시하는 `Intl.DateTimeFormat`으로 교체. 신규/보강 회귀 테스트가 `vi.stubEnv("TZ", ...)`로 실행 환경을 KST가 아닌 값으로 강제 고정해 놓고도 KST가 나오는지 직접 확인(실행기가 우연히 KST라 그 사실에 기대는 검사로는 재발을 못 잡는다 — 실제로 시험 작성 중 처음 버전이 이 이유로 재현을 못 잡는 것을 확인하고 고쳤다), revert-to-verify로 두 파일 다 결함 재현 확인 후 복원. **남긴 것**: 근본적으로 같은 일(UTC → KST HH:MM)을 하는 구현이 이제 셋(위 둘 + `chat-helpers.js`)이다 — 하나로 합치면 이 드리프트 계열(`DS-17`도 같은 모양) 자체가 재발할 수 없지만, 통합은 이번 범위를 넘어서는 리팩터라 하지 않았다. 세 번째 드리프트가 나오면 통합을 고려한다 |
| PA-12 | Med | **로그인 화면 비밀번호 표시 버튼이 WCAG AA 4.5:1에 근소 미달했다**(2026-08-15, 배포 후 Chrome Whole-product E2E 690페이지 실행 — `post_20260815`, 5뷰포트×2테마 10개 조합 전부에서 재현). `app/static/css/login.css::.password-toggle`이 `color: var(--primary)`(`#536CD6`)를 `--surface-soft`(`#F7F9FF`) 배경 위에 썼다 — 실측 4.449:1. 로그인 화면은 `auth.css` 주석이 명시하듯 승인된 디자인 원본을 글자 하나 안 바꾸는 것이 전제라, 원본 팔레트 자체(`--primary` 값)는 건드리지 않았다 | `app/static/css/login.css:284` · `app/static/css/auth.css:26-27`(`--primary`/`--primary-dark` 정의) | **구현완료** — 이미 같은 팔레트 안에서 `:hover`가 쓰던 `--primary-dark`(`#435CBE`)를 기본색으로 올렸다(계산 확인: 5.679:1). 원본 팔레트에 없던 색을 새로 만들지 않고 이미 승인된 두 톤 중 하나를 골랐다는 점에서 "원본 재해석 금지" 제약을 지켰다. `:hover`는 그대로 두어(더 이상 색이 안 바뀌지만) 최소 변경을 지켰다 |
| PA-13 | Med | **`/me` 홈 화면의 "티켓 동기화 확인 필요" 캡션이 WCAG AA에 근소 미달했다**(2026-08-15, 같은 E2E 실행, 라이트 테마 390~1920px 3개 뷰포트). `Home.jsx::Freshness`가 `color="warning.main"`을 카드 밖 `background.default`(`#F5F7FC`) 위에 직접 얹어 실측 4.48:1 — `theme.js`가 이미 이 정확한 경계 문제를 위해 만들어 둔 `warning.strong`(2026-08-11 QAH-03 감사가 근거)을 이 호출부만 안 쓰고 있었다 | `frontend/src/screens/Home.jsx:264-271` · `frontend/src/ui/theme.js:250-256`(`warning.strong` 정의와 QAH-03 근거 주석) | **구현완료** — `color="warning.strong"`으로 교체(계산 확인: `background.default` 위 5.99:1). 새 토큰을 만들지 않고 이미 있던, 바로 이 문제를 위해 설계된 토큰을 이 호출부에 연결한 것뿐이다 |
| QA-15 | Med | **QA 하네스의 `content_clipped` 검사가 닫힌 모바일 드로어를 오탐했다**(2026-08-15, 같은 E2E 실행 — `user_chat`, 390×844, 라이트/다크 둘 다). `/chat`의 `ConversationSidebar`(대화 목록)는 좁은 화면에서 `position:absolute + transform:translateX(-100%) + inert + aria-hidden="true"`로 화면 밖에 둔다 — CSS transform은 시각 위치만 옮길 뿐이라 조상 Paper의 `scrollWidth` 계산에는 여전히 이 드로어의 폭이 잡혀, 화면은 멀쩡한데(실측: 조상 자체가 `onscreen=true`) "가로 79px 넘침"으로 오검출됐다. 실제로는 `inert`라 포커스도 스크린리더도 닿을 수 없는 내용이라 사용자가 볼 방법이 없다 — 직접 작성한 Playwright 진단 스크립트로 실서버에서 재현·확인(자작 프로브라 개별 표본부터 의심하는 원칙 적용) | `scripts/ui_qa/assertions.py:239-268`(`PROBE_JS`의 `out.clipped` 수집부) | **구현완료** — 후보 상자 안에 `[inert]` 자손이 있으면 건너뛰도록 한 줄 추가. 같은 페이지에서 수정 전/후 재실측으로 확인(전: `clipped_count=1`, 후: `clipped_count=0`). `PROBE_JS`는 실브라우저 없이 단위 시험이 안 되므로(이 파일의 다른 시험들과 같은 이유) `test_ui_qa_harness_honesty.py`에 제외 조건 소스가 사라지지 않았는지 고정하는 시험만 추가. 이 패턴(닫힌 드로어+inert)을 쓰는 다른 컴포넌트(예: `AssistantDrawer`)도 이제 같은 오탐에서 벗어난다 |
...
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| PA2-01 | ~~Med~~ **완결** | **`variant="h6"`이 시각 선택이면서 DOM 구조까지 결정한다**(`PA-RC-0012`). 관리자 화면 5개가 `h1 → h6`으로 네 단계를 건너뛴다. 스크린리더의 문서 개요가 깨진다 | `PRODUCT_AUDIT_HANDOFF.md` `PA-RC-0012` · `PA-F-046` | ✅ **구현완료(2026-08-16)** — `theme.js`에 `MuiTypography.defaultProps.variantMapping`(`sectionTitle→h2`) 신설(MUI 소스로 부분 매핑이 h1~h6 기본값을 안 건드림을 직접 확인). 5화면 17개 호출부 + `SetupWizard.jsx`(h3→h2) + `BodyEditor.jsx`(사용자 저작 h1/h2/h3 블록, 반대 방향의 같은 결함)에 `component=` 추가. 신규 정적 검사 `check_heading_variant_mapping.py`(h3~h6) — **처음엔 `Offboarding.jsx`의 `variant="subtitle1"`을 놓쳤다**(MUI가 subtitle1/2도 `<h6>`로 매핑하는 걸 몰랐음), 렌더 결과를 직접 보는 신규 `heading-order.test.jsx`(6화면 대표 렌더 + heading 레벨 열 검증)가 그 구멍을 실제로 잡아 정적 검사를 subtitle1/2까지 넓혔다 — 정적 검사와 렌더 시험을 같이 둔 이유가 실제로 증명된 사례. 시험 13+91건 신규 green, 프런트 전체 회귀 278파일 1910건 green. **TEST SERVER 배포(`UPGRADE_OK`) + 실측 완료** — `verify_pa_rc_0012.py`로 6화면(`/llm-console`·`/notion-console`·`/mail`·`/system`·`/setup`·`/offboarding`) 전부 heading 열 건너뜀 0건, 글자 크기(20px/700·17px/700)도 실측으로 수정 전과 동일 확인. 상세: `DECISIONS.md` D-99 |
| PA2-02 | ~~Med~~ **완결** | **`/users`만 목록 상태(검색·필터·페이지)를 URL에 안 싣는다**(`PA-RC-0013`). 새로고침·공유에 잃는다. `/team-docs`·`/board`·`/team-tickets`·`/audit` 넷은 정상이라 **단독 예외**다 | `PA-RC-0013` · `PA-F-048` · `probe_deeplink2.json` | ✅ **구현완료+배포+실측(2026-08-16)** — 새 메커니즘 안 만들고 `DataScreen.jsx`가 이미 쓰는 `datascreen-view.js` 순수 함수(`buildViewQuery`/`withHashQuery`)를 5개 필터 키 config로 재사용. `setSearchParams` 대신 raw `history.replaceState`(같은 이유: 기존 `?id=` 소비 효과와의 되먹임 루프 회피, 이 파일 자신의 기존 주석이 이미 경고). **구현 전 코드 리뷰로 실제 버그 하나 미리 잡음** — "필터 바뀌면 1쪽" 효과가 마운트 때도 돌아 URL에서 막 복원한 page를 조용히 1로 되돌릴 뻔했다, `useRef` 플래그로 첫 실행만 건너뛰게 수정. 신규 `users-url-state.test.jsx` 6건(쓰기/읽기 양방향 + 기존 인바운드 딥링크 회귀 없음) + 기존 `users-*` 7파일 30건 green, 프런트 전체 회귀 278파일 1911건 green. TEST SERVER 배포 + 라이브 확인(검색 → hash 즉시 반영, 새로고침 → 복원, 둘 다 PASS). 상세: `DECISIONS.md` D-101 |
| PA2-03 | Med | **Pydantic 스키마 422가 영문 그대로 노출되고 필드에 연결되지 않는다**(`PA-RC-0014`). 서버는 정확히 거절하나 사용자는 어느 칸이 문제인지 모른다 | `PA-RC-0014` · `PA-F-054` · `probe_limits.json` | ✅ **구현완료(2026-08-16)** — `app/core/errors.py`가 `err["msg"]` 대신 `err["type"]`+`ctx`로 한국어 문구 생성. `kit.jsx::FormModal`이 `details[].loc`를 필드에 연결(`aria-invalid`+`aria-describedby`=실제 문구), 등록 화면 13개 전부에 적용(클라이언트 검증 실패도 동시에 개선). `Users.jsx`는 `FormModal`을 이미 쓰고 있었음이 드러나 `screenKey` prop 배선만으로 해결(`PA-04` 자기모순 정정). `Offboarding.jsx`(진짜 손수 제작)는 별도 배선. 백엔드 통합 테스트 2건(실제 등록된 핸들러 경유)·`field_limits` 핀 2건, 프런트 신규 4건, 백엔드 전체 회귀 green(29분51초)·프런트 전체 회귀 280파일 1915건 green. TEST SERVER 배포 + 라이브 Chrome 확인(영문 0건, `aria_invalid`=1, 문구 연결 확인, 계정 미생성). 상세: `DECISIONS.md` D-102 |
| PA2-04 | Low | **장애 배너 경과 시간이 항상 '분'이라 12.5일이 "17976분"으로 나온다**(`PA-RC-0015`). 가장 심각한 배너가 가장 안 읽힌다. 같은 저장소의 백업·프로젝트 경로는 이미 '일 전'을 쓴다 | `PA-RC-0015` · `PA-F-056` · `verify_banner.json` | ✅ **구현완료(2026-08-16)** — `app/core/elapsed.py::format_elapsed_korean` 신설(분/시간/일 자동 승급). `observability` 배너·`backups` 정체 알림이 공용 헬퍼를 씀(`projects/health.py`는 date 뺄셈이라 애초에 하루 미만 정밀도가 없어 의도적으로 제외, 이유를 주석에 남김). 신규 단위 시험 10건(경계값 5종 + 실측 사고 크기 12일), revert-to-verify(예전 로직으로 되돌리면 "18080분"으로 정확히 재현). 상세: `DECISIONS.md` D-91 |
| PA2-05 | **High** | **전역 배너 스택이 모든 화면 첫 화면의 31%(관리자)·20%(사용자)를 고정 비용으로 가져간다**(`PA-RC-0016`). 관리자 14라우트 전부 배너 5장 331.5px, 사용자 9라우트 전부 4장 216px — **편차 0**. 닫을 수 있는 것은 2/5·1/4뿐이라 상시 표시가 되고 진짜 장애도 배경으로 읽힌다. 같은 셸에서 **본문 상한(max-width)이 없어** 4K 본문이 3500px까지 늘어난다(활용률 86.3→88.3→91.1%) — `RESP-01`·`RESP-02`가 좁은 폭만 다뤄 이 축은 미조사였다. **175% 가로 넘침은 기존 `RESP-01`이므로 여기서 중복 계상하지 않는다**(함께 고치되 `RESP-01`의 기존 진단을 먼저 읽을 것). `RESP-04`(1024~1200 사이드바 264px, 축소 레일 필요)는 `PA2-06`이 함께 닫는다 | `PA-RC-0016` · `PA-F-058`·`067` · `probe_shell.json` · `design_capture2.json` viewports[] · 기존 `RESP-01`·`RESP-04` | ✅ **구현완료(REDESIGN, 2026-08-16)** — 배너 스택을 헤더 상태 칩(`StatusChip`, 활성 항목 0이면 안 그려짐) + CRITICAL 전용 한 줄(≤40px, `CriticalStatusLine`)로 재설계(`frontend/src/app/StatusNotices.jsx` 신설). 같은 원인(동기화 정지 등, `sync.` id 접두어 + 같은 심각도)은 한 줄로 병합 — 문구 매칭이 아니라 식별자 기준. 판정 조건(WARNING/CRITICAL 임계, 공지 노출 대상)·두 엔드포인트 응답 구조는 전혀 안 바꿈. 공지 닫기는 기존 서버 저장 그대로, 시스템 상태 닫기는 서버 저장소가 없는 임시/자가치유 성격이라 `localStorage`로 의도적으로 다르게 처리(근거는 D-91) — 닫힘 키에 심각도를 포함해 "심각도 상승 시 재노출"을 새 판정 로직 없이 만족. 실측(1920×1080): `h1` y좌표가 관리자 447→116, `/me` 92까지 개선(기준 200px 이하 크게 충족). 4K 본문 폭은 `usedWidth` 기준 처음부터 3040px로 정확했음을 직접 DOM 실측으로 확인(하네스의 `mainWidth` 필드—flex 컨테이너 자체 크기—와 `usedWidth`—실제 쓰는 폭—를 혼동해 존재하지 않는 결함을 잠깐 쫓았던 경위도 D-91에 정직하게 남김). 신규 vitest 18건(병합·칩 노출/은닉·패널·양쪽 닫기 방식·심각도 재노출·안전링크·부분 API 실패), revert-to-verify(병합 로직 무력화 시 정확히 예측한 시험 실패). 기존 AppShell 의존 스위트 20개 파일 69건 전체 green. TEST SERVER 통합 배포 2회 + Chrome 실측. **175% 오버플로는 이 RC 범위 밖**(Handoff 명시) — `RESP-01`이 이미 별도로 열려 있고 이 RC가 그것까지 닫았다고 주장하지 않는다. 상세: `DECISIONS.md` D-91 |
| PA2-06 | **High** | **관리자 IA가 8그룹 39목적지 평면이라 내비가 839px 창으로 1976px를 보여준다(42%)**(`PA-RC-0017`). 설정이 6화면에 흩어져 `/settings` 안내 4문단이 그 사실을 설명한다. 사용자 콘솔은 23링크·접힘 0으로 정상 — **관리자만의 문제** | `PA-RC-0017` · `PA-F-059`·`064` · `verify_nav.json` | ✅ **구현완료(2026-08-16) — acceptance_criteria 8개 전부 자동 시험 + TEST SERVER 실배포·실브라우저로 확인.** `NAV` 8그룹 39항목 → 5그룹 35항목(운영/사용자와 권한/자동화/연동/감사, 전부 정확히 7항목). 설정 6화면 중 4개(시스템 설정·유지보수·Notion 관리·AI 관리)를 `/settings` 1화면 + 4탭(시스템 정책/OS와 서비스 동작/연동/AI)으로 통합(`SettingsShell.jsx` 신설) — 초기 설정은 체크리스트 마법사라 탭 그릇이 안 맞아 독립 화면으로 남기고 감사 그룹으로 재분류. 안내 4문단 삭제. 옛 URL 4개는 `/settings?tab=*`로 리다이렉트, 나머지 35개는 경로 불변. 레일 필터 신설(관리자 셸만). breadcrumb 3단. **RBAC**: 역할 4종(user·operator·auditor·system_admin) × 신규 탭 allow/deny를 자동 시험(`settings-shell.test.jsx`)과 revert-to-verify로 확인. **실배포 후 실브라우저 검증(16/16 통과, 스크린샷 확인)에서 유닛 테스트로는 못 보는 진짜 결함을 잡았다**: 그룹 이름 변경으로 기존 사용자의 사이드바 접힘 기록이 새 이름과 안 맞게 돼, 배포 후 사실상 전원이 첫 방문에서 5그룹 전부 펼쳐진 채(scrollHeight 1716 vs clientHeight 794)로 보는 상태였다 — "기록 없음=펼침"이던 기본값을 "기록 없음=접힘, 활성 그룹만 강제 펼침"으로 고치고, 그 과정에서 발견한 2차 결함(접힌 그룹 클릭이 안 먹던 `toggle()` 뒤집기 공식 버그)도 같이 고침. 재배포 후 재검증: scrollHeight===clientHeight(794=794), 5그룹 헤더 전부 fold 안. 회귀 중 이전 `PA-RC-0016`(`StatusNotices.jsx`)의 금지 문자(가운뎃점·em대시) 누락도 발견해 고침. 신규 vitest 30건(사이드바 기본값 회귀 포함) + 기존 스위트 갱신, 전체 프런트 회귀 274파일 1864건 green, `static_checks.sh` green(git-secrets의 기존 SEC-20 인간 전담 항목 제외). TEST SERVER 통합 배포 2회(`UPGRADE_OK`) + Playwright 실측 16/16 + 스크린샷 7장 육안 확인(`dist/pa_rc_0017_verify/`, 로컬 전용). **범위 밖으로 남긴 것**(이 RC의 acceptance_criteria엔 없음): `RESP-04`(축소 레일, `PA2-05`의 교차 참조일 뿐), `probe_rbac_gate.py` 재실행(이 RC가 안 건드린 라우트를 잰다). 상세: `DECISIONS.md` D-92 |
| PA2-07 | **High** | **대시보드가 결정 화면이 아니라 균일 카드 벽이다**(`PA-RC-0018`, 판정 **REBUILD**). 카드 40장·기본 동작 0개·2.38화면, 같은 수치가 최대 3회 반복(`3`이 3구역, `42.6%`·`0`이 각 2구역). 화면이 「이 줄은 요약입니다…」로 자기 IA를 설명한다. `/me`·`/my-stats`·`/projects`도 같은 병 | `PA-RC-0018` · `PA-F-060` · `probe_shell.json` dup 스캔 | ✅ **완결(2026-08-16) — TEST SERVER 실배포 + Playwright 실측 13/13 + 스크린샷 5장 육안 확인.** 「확인이 필요한 항목」을 StatCard 격자에서 행 목록(`ActionQueue`/`ActionRow`)으로 교체 — 각 행 「무엇이(라벨)+영향(값·심각도)」+ 기본 조치 버튼, danger 우선 정렬로 **첫 행만 `primary`, 나머지 `default`**(`PA-RC-0023`의 "화면당 contained 1개" 규범과의 충돌을 이 화면 안에서 국소 해결 — 상세는 `DECISIONS.md` D-94). 경보 판단 로직은 `buildAlerts(d,role)`로 원문 그대로 추출(값 재현 시험 20건). 「지금 상태」 카드 5장을 `HealthyStrip`(한 줄, 테두리 구분자) 로 대체 — 지금 경보 중인 지표는 스트립에서 자동 제외돼 같은 값이 두 번 안 뜬다. 「인벤토리」·「현재 큐 상태」 구역 완전 삭제(`/workflows`·`/schedules`·`/runners`·`/jobs`가 이미 더 상세한 목록/자체 요약을 가짐), 「작업 지표」는 성공률 타일만 제거(스트립과 중복), 「백업」은 유지하되 버튼을 항상 `default`로 낮추고 시스템 관리자에게 이미 경보로 뜬 "며칠 전" 배지만 중복 억제. 「이 줄은 요약입니다…」 문구 완전 삭제. `/projects`: 0건이면 8타일 요약 자체를 안 그림(`Summary` 조기 반환). `/me`: 팀 채팅·게시판 카드 제거(사이드바 "채팅방"/"자유게시판"으로 이미 대체 경로 있음), `TeamChatWidget.jsx` 파일 자체는 존치(자체 테스트 보유, 범위 밖 삭제 안 함). **관리자 로그인 착지**: `login.js` 추적 결과 `next` 없는 일반 로그인은 항상 물리 경로 `"/"`로 가는데 `App.jsx`가 물리 경로만 보고 역할을 안 봐 **관리자도 실제로는 항상 `/me`에 떨어지고 있었다** — `initialLandingPath(pathname,role)` 추출해 역할 기반으로 수정(acceptance_criteria 11). 신규/갱신 vitest: `dashboard-helpers.test.js`(+20, `buildAlerts`/`dashboardNav`), `dashboard-render.test.jsx`(전면 재작성 — 중복 부재·0값 무채색·RBAC 버튼 비활성 포함), `app-landing-path.test.js`(신규 4), `home.test.jsx`/`projects.test.jsx` 갱신. 전체 프런트 회귀 277파일 1900건 green, `static_checks.sh` green(SEC-20 인간 전담 항목 제외, `PA-RC-0017`/`0022`와 동일 관용). **TEST SERVER 배포**: `CLOVIR_TEST_SUDO_PASSWORD`(런타임 환경변수, CLAUDE.md §9가 이미 승인한 이 서버 전용 사용)로 `sudo -S` 인증 — 값은 명령행 인자로 안 넘기고 stdin 파이프로만 전달(`sshpass` 금지 규칙과 같은 이유). `build-bundle.sh` → `scp` → 체크섬 대조 → `upgrade-clovirone-web-assistant.sh`(백업→정지→재설치→마이그레이션→검증→실패시 자동 롤백) → `UPGRADE_OK`, 외부 `verify_deploy.sh` → `DEPLOY_VERIFY_OK`(정적 자산 33/33 해시 일치). `verify_pa_rc_0018.py` 1차 실행 13개 중 4개 실패 → 실측으로 원인 규명: **①진짜 결함**(문서 높이 1805px, 예산 1620px 초과 — WorkSection의 차질 프로젝트/지연 마일스톤 상세 목록 2카드가 원인, 위 StatCard가 이미 개수+`/projects` 링크를 가지므로 카드 자체를 지워 1576px로 해결) **②~④검사 스크립트 오탐 3건**(전역 어시스턴트 전송 버튼을 `contained` 카운트에 오포함 → `:not([type=submit])`로 제외, "게시판" 텍스트 검사가 사이드바 nav "자유게시판"까지 잡음 → heading role로 좁힘, 다크 모드 스크린샷 대기시간 300ms가 이 화면엔 부족해 라이트로 찍힘 → 1000ms로 늘림, 셋 다 제품 결함 아님). 수정 후 재배포, **13/13 PASS**. 상세: `DECISIONS.md` D-94. |
| PA2-08 | ~~Med~~ **완결** | **어시스턴트 FAB(z=1050)이 표 화면 행 「상세」 버튼을 실제로 덮어 클릭을 가로챈다**(`PA-RC-0019`). 관리자 9/10 · 사용자 1/5 라우트에서 `elementFromPoint`가 FAB을 반환. **z-index만 낮추지 말 것** — `PA2-09`가 원인을 없앤다 | `PA-RC-0019` · `PA-F-061` · `verify_fab.json` | ✅ **구현완료+배포+실측(2026-08-16)** — Handoff 권장대로 `PA2-09`(진입점을 헤더 칩 하나로 모으고 FAB 제거)로 원인 소멸시켜 함께 닫았다. 재확인 결과 원래 대상은 사라졌지만 FAB이 행 전체(12건)를 덮고 있어 문제가 여전했음을 먼저 실측 확인, 그 뒤 FAB 자체를 없애 해결. TEST SERVER 재배포 후 `verify_pa_rc_0019.py` 재실행 — 전 10라우트 `no FAB present`, `total_blocked=0`. 상세: `DECISIONS.md` D-100 |
| PA2-09 | ~~Med~~ **완결** | **어시스턴트가 이름 4종·진입점 3개로 존재한다**(`PA-RC-0020`). `/chat` 한 화면에서 breadcrumb 「AI 도우미」와 `h1` 「채팅」이 불일치. 진입점은 헤더 칩·사이드바 카드(264×121)·FAB. **미확정 1건**: 「업무 도우미」 동일성 확인 필요 | `PA-RC-0020` · `PA-F-062` | ✅ **구현완료+배포+실측(2026-08-16)** — 「업무 도우미」는 확인 결과 표시명이 아니라 별개 시스템의 워크플로 조회 키(`CHAT_WORKFLOW_NAME`)라 대상 아님, 손 안 댐. 실제로 어긋난 자리는 예상보다 좁았다 — 사이드바·breadcrumb·`/me` 섹션은 이미 전부 "AI 도우미"였고 `/chat`의 h1+대화 제목 막대(별개 리터럴 2곳) 만 "채팅" 폴백이 남아 있어 통일. "클로비"는 인격 이름으로 유지(헤더 칩 텍스트·드로어 머리말). 진입점 3→1(헤더 칩만) — FAB·사이드바 카드 컴포넌트째 삭제, 전역 단축키(Ctrl/Cmd+/) 신설로 대체. `/org-tree`의 h1/사이드바 라벨 불일치는 `OrgConsole.jsx`의 의도된 3-라우트 통합 설계(사용자 요청으로 만들어짐, 기존 시험이 h1을 계약으로 고정)라 강제로 안 맞추고 의도된 예외로 문서화(D-100). 신규 `assistant-hotkey.test.jsx` 3건 + 관련 시험 갱신, 대상 사라진 시험 2개 삭제, 프런트 전체 회귀 277파일 1905건 green. TEST SERVER 배포 + 라이브 확인(헤더 칩 정확히 1개, 사이드바 카드 0개, `Ctrl+/`로 드로어 실제로 열림, `/chat` 사이드바·breadcrumb "AI 도우미" 일치, 스크린샷 확인). 상세: `DECISIONS.md` D-100 |
| PA2-10 | Med | **색 토큰이 다크 테마에 참여하지 않는다**(`PA-RC-0021`). 온보딩 다이얼로그가 다크에서 흰색, 알림 배지 2.23:1, `/me` 활성 탭 3.76:1(기준 4.5), `prefers-color-scheme` 첫 로드 무시. **다크 팔레트 자체는 잘 만들었다** — 참여하지 않는 표면만 문제 | `PA-RC-0021` · `PA-F-063` · `verify_dark.json` | ✅ **구현완료(2026-08-16)** — 조사 결과 Handoff의 항목 4개 중 셋은 실제 원인이었고 **하나는 오탐**이었다: 온보딩 다이얼로그는 실측 결과 이미 `rgb(17,24,45)`(정상 다크 표면)로 렌더돼 재현 불가(감사 자신의 스크린샷 두 장이 같은 HEAD·90초 간격으로 서로 다른 결과 — 타이밍 오탐으로 판단). 실제 원인 셋: (1) `MuiTab`/`MuiTabs`에 색 지정이 없어 MUI 기본값(`primary.main`)을 그대로 써 탭 소비처 3곳(AssistantPanel `/me`·Project·SettingsShell) 전부 실패 — `MuiLink`/`MuiButton`이 이미 쓰는 `primaryStrong` 토큰으로 테마 컴포넌트 층에서 통일 배선. (2) `NavBadge`가 `color: "common.white"`를 하드코딩 — `error.contrastText`(모드별 자동 계산, 커스텀 override 없음)로 교체, jsdom 실제 CSS 엔진으로 두 모드 다 계산해 확인. (3) `prefers-color-scheme` 미반영의 **진짜 원인은 Handoff가 지목한 파일이 아니었다** — SPA의 `theme-store.js`는 이미 정확히 확인하고 있었고, 레거시 `app/static/js/theme.js`(`/change-password`용, 신규/재설정 계정이 로그인 직후 반드시 거치는 화면)가 그 확인 없이 "light"를 공유 부팅 키에 먼저 써 버리고 있었다 — 그 파일에 같은 `matchMedia` 확인을 추가. `theme-link-contrast.test.js`에 배선+대비 계산 회귀 137건(신규분 포함) green, 관련 화면 회귀 16파일 283건 green. TEST SERVER 배포 + 라이브 확인: `scripts/ui_qa/contrast.py` 대표 8화면×2테마(요구 5화면의 60% 초과) **위반 0건**, `verify_pa_rc_0021.py`로 온보딩 다이얼로그 실측(다크 배경 확인)·실제 배포된 `theme.js`로 OS 다크 선호 첫 로드 확인·명시적 선택이 OS 선호를 덮어쓰는 회귀 없음까지 전부 PASS. 상세: `DECISIONS.md` D-103 |
| PA2-11 | Med | **화면이 자기 IA를 산문으로 설명한다**(`PA-RC-0022`). 안내 패널 8화면. `/settings`는 전역 정책 표와 브라우저 로컬 개인 설정(「화면 강조색」)이 한 화면에 있고 그 차이도 산문으로만 구분된다. 설정 표가 백엔드 키를 사용자용 열로 노출 | `PA-RC-0022` · `PA-F-064` · `probe_shell.json` intro[] | ✅ **구현완료(2026-08-16) — TEST SERVER 실배포 + 실브라우저 14/14 확인.** `PageHeader`에 `help`/`helpTone` prop 신설(제목 옆 도움말 토글, 기본 접힘, `Collapse` 안 기존 `Callout` 재사용) — `DataScreen.jsx` 한 곳을 고쳐 registry 28개 화면 전부(Handoff가 이름 붙인 3개보다 훨씬 넓다) + `Offboarding.jsx`/`Users.jsx`(손으로 짠 JSX 안내)가 한 번에 옮겨졌다, 문구는 한 글자도 안 바꿈. `/system`·`/diagnostics`는 직접 재확인 결과 정적 안내 패널 자체가 없었다(전자는 런타임 조건부 배너뿐, 후자는 `aria-live` 실시간 장애 요약이라 접으면 활성 장애를 숨기는 회귀가 된다) — Handoff 수치와 현재 코드가 안 맞는 충돌로 기록하고 손대지 않음. 화면 강조색을 `/my-display`(신규 `DisplaySettings.jsx`, `UserRoutes.jsx`/`navConfig.js USER_SEG_PATHS` 등록)로 이전, 계정 메뉴("내 화면 설정")는 관리자·사용자 두 콘솔이 공유해 역할 무관 접근이 자동 충족(acceptance_criteria 5). 다크/라이트 토글은 의도적으로 안 옮김(UserMenu.jsx의 기존 단일 소스 원칙 유지). 설정 표: 백엔드 키 열 기본 숨김 + "백엔드 키 표시" 체크박스(완전 제거 아님, constraints 준수), "기본값" 배지 제거(변경 안 된 항목은 배지 없음, "수정됨"만 표시), 열 라벨 항목명/현재 값으로 개칭. "적용 범위"·"마지막 변경" 열은 만들지 않음 — 실제 API 응답에 그 데이터가 없고 `api: 없음(응답 구조 불변)`이 명시적 제약이라, 없는 데이터로 열만 만들면 거짓 정보가 된다(판단 근거는 `DECISIONS.md` D-93). 오프보딩 후보 목록: `notion_mapping_status`가 unmapped인 행에 `app/offboarding/service.py::_onboarding_checklist`의 notion `help` 문자열을 **그대로** 캡션으로 추가(새 문구 안 씀) — 실서버 스크린샷으로 QA 계정 5명의 실제 미연결 사유가 목록 단계에서 바로 보임을 확인. 신규/갱신 vitest(`kit.test.jsx` 6건, `settings-accent.test.jsx` 이전, `settings-key-column-toggle.test.jsx` 4건 신규, `user-menu-display-settings-link.test.jsx` 신규, `offboarding.test.jsx` +2건), 전체 프런트 회귀 276파일 1878건 green, `static_checks.sh` green(SEC-20 인간 전담 항목 제외). TEST SERVER 통합 배포(`UPGRADE_OK`) + Playwright 실측 14/14 + 스크린샷 4장 육안 확인(`dist/pa_rc_0022_verify/`, 로컬 전용) — `/rbac` 도움말 토글 기본 접힘/펼침, `/settings` 키 열 토글 + 배지 제거, 계정 메뉴 → `/my-display` 접근, 오프보딩 목록의 실제 차단 사유 노출 전부 실서버에서 확인. **남은 것**: acceptance_criteria 8(강조색 새로고침 유지)은 관련 훅·컴포넌트 전혀 무수정이라 기존 시험 계약이 그대로 지킨다고 판단해 별도 재확인 안 함. 상세: `DECISIONS.md` D-93 |
| PA2-12 | ~~High~~ **완결** | **동작 위계 규범이 퍼지지 않아 양쪽 극단이 동시에 존재한다**(`PA-RC-0023`). 한쪽은 `contained` 0개(관리자 10/16 · 사용자 4/10), 다른 쪽은 상세 모달에 3개이고 `/departments`는 그중 하나가 **「삭제」**다. 표의 행 「상세」 버튼은 행 클릭과 중복. **`/users` 목록과 생성 모달이 같은 제품 안의 정답 대조군** | `PA-RC-0023` · `PA-F-065` · `design_capture2.json` detail[] | **진행 중(2026-08-16) — 핵심 기반 + 실제 결함 2건 완료, 전체 범위(표 열 제거·RBAC 4역할·화면별 기본 동작 전수)는 남음.** Explore 조사로 Handoff 수치 일부 재확인·일부 정정: `/departments` 「삭제」는 이미 `variant:"danger"`(주장과 달리 `primary` 아님) — registry 전수 확인 결과 파괴적 라벨(삭제·비활성화·보관·거절·취소·롤백·연결 해제·위임 거두기) 전부 이미 `danger`, 예외 0건. 대신 **실제 결함 2건**을 새로 찾아 고침: ① `onoff()`/`activeToggle()`(부서·직책·워크플로·스케줄·러너·연동 공유)의 "활성화"가 `primary`라 편집 가능한 비활성 행 상세에서 "수정"(DataScreen 고정 primary)과 동시에 채운 버튼 2개 — `default`로 내려 Users.jsx의 "복구"와 통일. ② `notion-mapping` "자동 동기화"가 `primary:true`인데 `variant:"primary"`가 짝이 안 맞아 평소 툴바에서 외곽선으로 보임 — 짝 맞춤. `DataTable`(관리자 28+화면 공유) 행에 키보드 도달(tabIndex+Enter/Space+aria-label) 추가 — 「상세」 버튼 열 제거의 전제 조건(acceptance_criteria 4-b)이자 아직 열은 안 지웠다(안전한 순서: 도달 경로 먼저 추가 → 나중에 옛 경로 제거). TEST SERVER 실측으로 `/offboarding` 후보 클릭 결함(`PA-F-077`)이 낡은 발견임을 확인(이미 정상 동작, `/users`와 같은 패턴) — 배포하면 키보드 경로도 자동으로 같이 열린다. 신규 정적 검사 `scripts/check_button_hierarchy.py`(`check_typography_literals.py`와 같은 구조) — 파괴적 라벨 primary 금지(예외 없음) + `primary:true`/`variant:"primary"` 짝 검증, `static_checks.sh` 배선. 시험: `datatable-row-keyboard.test.jsx`(5) · `test_button_hierarchy_scan.py`(9) · `admin-uiux.test.jsx` 신규 1건(부서 상세 primary 정확히 1개 실측). 전체 프런트 회귀 278파일 1905건, 백엔드 `tests/unit/` 전체 green. **의도적으로 보류**: `SystemOps.jsx`는 재시작 5개+설정변경 N개가 전부 동등한 무게라 인위적 primary 지정 대신 예외로 남긴다(강제로 만들지 말라는 Handoff 자신의 경고, `implementation_direction`(2)). 나머지 0-primary 화면(`audit`·`audit-anomalies`·`rbac`·`prompt-usage`·`policy-usage`·`restore-drills`[이미 `forceOnboarding`]·`DisplaySettings`·`MyStats`·`Activity`)은 전부 순수 조회 화면이라 잠정 예외로 기록, 정식 예외 등재 메커니즘(`check_button_hierarchy.py`에 화면 단위 축 추가)은 아직 안 만듦. **아직 안 한 것**: 「상세」 버튼 열 실제 제거(전 표 대상), RBAC 4역할 실측(신규 기본 동작이 권한 없는 역할에 노출 안 되는지), `probe_write_gate.py` 재실행, TEST SERVER 재배포+실브라우저 확인. 상세: `DECISIONS.md` D-95. ‖ **거의 완결(2026-08-16, 같은 invocation 이어서) — 남은 건 TEST SERVER 재배포 + 실측뿐.** 「상세」 버튼 열을 실제로 지웠다(`ui/kit.jsx` `DataTable`) — 파급으로 20개 파일 49건이 한꺼번에 깨졌고(전부 「상세」 버튼을 다른 걸 시험하는 **수단**으로 썼던 시험), 전부 행 클릭/키보드로 바꿔 고쳤다(대다수 기계적 치환, `users-row-open-label.test.jsx`·`board.test.jsx`·`datatable-detail-button.test.jsx`[삭제]는 판단 필요). 프런트 전체 회귀 **277파일 1904건 green**. D-95가 "아직 안 만듦"으로 남긴 화면 단위 정적 검사 2개를 실제로 만듦 — acceptance_criteria (1) 화면당 contained 최소 1개(`ZERO_PRIMARY_EXCEPTIONS` 코드 등재, `audit`·`audit-anomalies`·`rbac`·`prompt-usage`·`policy-usage`·`restore-drills`+**신규 `org-tree`** 7개, `SystemOps` 등 registry 밖 독립 화면은 이 검사가 원천적으로 못 봄을 docstring에 명시) · (2) 화면·오버레이당 contained 2개 미만(영역별로 셈 — registry 전수 재확인 위반 0건, D-95가 고친 activeToggle 충돌 유형을 revert-to-verify로 재현). `test_button_hierarchy_scan.py` 9→19건 green. 화면 단위 검사를 실물에 처음 돌려 **신규 실제 결함 1건 추가 발견**: `notifications` "모두 읽음"이 `variant` 필드 자체가 없어 항상 외곽선이었다(Handoff가 예시로 직접 든 화면인데 원래 안 됐었다) — `variant:"primary"` 추가, `roles:`는 원래도 없어(self-service 동작) RBAC 노출 범위 불변. 이번 배치는 `roles:` 필드를 한 곳도 안 건드려 4역할 실측 대신 diff 근거로 대신함(다음에 roles를 건드리면 반드시 역할 4종 실측으로 되돌아간다). 빌드+번들 재생성, `static_checks.sh` 전부 green(유일한 예외는 배치와 무관한 기존 `PA-RC-0003`/`SEC-20` 사람 회전 대기). 신규 `var/product-audit/verify_pa_rc_0023.py`(PA-RC-0022 패턴) 작성 완료, TEST SERVER 배포 후 실행 예정. 상세: `DECISIONS.md` D-96. ‖ **완결(2026-08-16) — TEST SERVER 배포(`UPGRADE_OK`) + 실측 완료.** `verify_pa_rc_0023.py` 14/14 green(`/users`·`/offboarding`·`/departments`·`/notion-mapping`·`/notifications` 전부 실측, 스크린샷 5장). `/offboarding` 이력 표만 서버에 이력 데이터가 없어 라이브 클릭은 못 했고 공유 컴포넌트+목업 단위시험 근거로 대신함(정직하게 기록, 완전한 라이브 확인 아님). 캐시된 `operator` 세션으로 실측 RBAC 2역할 추가 확인 — `operator`가 `/departments`(`SCREEN_ROLES` 라우트 게이트)와 notion-mapping "자동 동기화"(`WRITE_ROLES` 액션 게이트) 둘 다 정상적으로 못 보는 것을 실측으로 재확인(`PA-F-053` 유지), `auditor`/`user`는 이번 배치가 `roles:` 필드를 안 건드려 diff 근거로 대신함. `verify_deploy.sh` 전부 OK. acceptance_criteria 8개 중 7개 완전 충족, 1개(4-b 이력 표+RBAC 4종)는 부분 실측+근거 대체. 상세: `DECISIONS.md` D-97. |
| PA2-13 | Med | **"상세 보기"가 콘솔별로 다른 것이다**(`PA-RC-0024`). 사용자는 `:id` 라우트 6개, 관리자는 URL 없는 모달 — 딥링크·새로고침·뒤로가기 불가. `/users/<uuid>`는 9초 뒤 대시보드로 조용히 이동(같은 제품의 `/board/999999`는 정상 not-found). **라우트 신설로 직접 진입 경로가 열리므로 RBAC/IDOR negative 테스트 필수** | `PA-RC-0024` · `PA-F-066` · `UserRoutes.jsx:60-83` vs `AdminRoutes.jsx` | ✅ **구현완료(2026-08-16)** — `AdminRoutes.jsx`에 `/users/:id`·`/departments/:id`·`/audit/:id`(목록과 같은 element, react-router 7에서 컴포넌트 인스턴스 유지 실측 확인) 신설. 감사 로그는 단건 조회 자체가 없어(Handoff의 "새 API 불필요" 전제가 틀림) `GET /api/admin/audit/{id}` 신규(같은 범위 판정, 없는 id/범위 밖 id 동일 404 — `test_audit_detail_scope.py` 6건). `DataScreen.jsx`는 `config.hasIdRoute` 옵트인 화면(audit·departments)에만, `Users.jsx`(손수 제작)는 직접 배선. catch-all을 `ErrorState`(사용자 콘솔 `:id` 6개가 이미 쓰는 것 재사용, 새 컴포넌트 없음)로, `UserRoutes.jsx`의 catch-all도 navConfig.js의 NAV로 "관리자 전용"과 "진짜 모름"을 갈라 권한거부/not-found로 분리. 시험이 실제 경합 조건(딥링크 첫 렌더에서 sel이 아직 null인 순간 반대 방향 효과가 오해해 URL을 지웠다 되돌리는 깜빡임)을 잡아 `routeIdSettledRef`로 수정. **라이브 검증이 로컬 목이 못 잡은 진짜 결함 발견**: `departments/:id`가 매번 404 — 서버 단건 GET이 `{"department":{...}}`로 감싸는데 `selectKey` 배선이 빠져 있었다(목이 감싸지 않은 응답을 흉내 내 못 잡음) — 수정 후 재배포·재검증 통과. 프런트 전체 회귀 284파일 1951건 green. TEST SERVER 배포 2회 + 실제 데이터로 라이브 검증 11개 전부 PASS. 상세: `DECISIONS.md` D-104 |

| PA2-15 | Low | **공유 `DataScreen`이 성공 문구를 조립해 저장소 자신의 UX Writing 규칙을 어긴다**(`PA-RC-0025`). `docs/UX_WRITING.md:54`가 토스트를 문장형(마침표 O)으로 못박는데 `DataScreen.jsx:287`이 `라벨 + " 완료"`로 조립해 「삭제 완료」를 만든다 — 명사형이라 확인 문장이 아니라 상태 배지처럼 읽힌다. 손으로 쓴 화면은 규칙을 지킨다(저장소 한국어 알림 문자열 87종 중 **서술형 62 대 명사형 2**) — **규칙을 강제해야 할 공유 프레임워크가 유일한 위반자**다. `/departments` 한 화면에서 생성 「추가했습니다.」·수정 「저장했습니다.」인데 삭제만 「삭제 완료」로 갈린다. 라벨을 문자열 결합으로 활용시키지 말고 **사전**을 둘 것(「보관 복구」·「세션 해제」 같은 복합 라벨이 실재한다) | `PA-RC-0025` · `PA-F-073`·`076` · `docs/UX_WRITING.md:54,74` · `DataScreen.jsx:287` | ✅ **구현완료(2026-08-16, 격리 worktree agent)** — `data-screen/successMessages.js` 신설(라벨→문장 사전 23항목 + 일반 기본형), `DataScreen.jsx`·`SubListDrawer.jsx` 둘 다 `successMessageFor(label)`로 교체(두 곳이 다시 갈리지 않게 공용 함수 하나). `docs/UX_WRITING.md` §3-1에 같은 표 등재. `scripts/check_success_toast_labels.py` 신설(`check_typography_literals.py` 선례, 사전에 없는 새 라벨을 `static_checks.sh`에서 잡음). Handoff가 예외로 지목한 「보관 복구」·「세션 해제」는 이 사전과 무관한 `Users.jsx`/`UsersBulk.jsx`의 별도 경로임을 소스로 확인, `a.result(res)` 옵트아웃(대량 활성화 커스텀 문구)은 음성 대조군 시험으로 안 건드렸음을 확인. `PA-RC-0024`와 같은 파일(`DataScreen.jsx`, 다른 구간)을 동시에 고쳤으나 병합 충돌 0(사전에 `git merge-tree`로 확인) — 병합 후 두 RC 시험 함께 재확인. 라이브 검증(TEST SERVER에 일회용 부서로 실측): 삭제 토스트가 실제로 "삭제했습니다."(예전 "삭제 완료" 재현 지점) — 자작 프로브가 처음엔 페이지 상시 안내 배너(`role="alert"`)를 토스트로 오인해 위양성을 냈던 것도 잡아 고침(`kit.jsx`의 실제 토스트는 이중 낭독 방지로 `role="presentation"`). 상세: `DECISIONS.md` D-105 |
| PA2-14 | ~~Med~~ **완결** | **「진단」 화면이 조회 행위인데 설정 변경 권한으로 막혀 있다**(`PA-RC-0026`). `/diagnostics`가 프런트(`AdminRoutes.jsx:123`)·백엔드(`health/router.py:80`) 양쪽에서 `CONSOLE_WRITE_ROLES`를 요구하는데, 제품 자신의 능력 표는 `console.ops` 「운영 동작(실행, 재시도, **헬스체크**, 드라이런)」을 운영자에게 준다. 번들은 spec §14.7로 마스킹이 보장되고 그 조각들(대시보드·설정·실패 잡)은 운영자가 이미 각각 본다. `/maintenance`는 바로 다음 줄에서 운영자를 포함한다 — **고치는 화면은 열리고 진단하는 화면이 닫혀 있다.** 권한을 넓히는 변경이라 **번들 내용 항목별 감사가 선행 조건**이다 | `PA-RC-0026` · `PA-F-078` · `authz.py:145,149,153` · `health/service.py:382` | ✅ **구현완료(2026-08-16)** — 게이트를 `CONSOLE_WRITE_ROLES`(admin+) → `CONSOLE_OPS_ROLES`(operator 포함, `/jobs`와 동일 집합)로 낮추기 전 번들 5개 키를 전부 항목별로 감사(요구된 선행 조건). `settings`·`tenant_config`·`mail`·`recent_job_errors.last_error` 4개는 operator가 이미 다른 화면에서 같거나 더 넓은 형태로 보고 있어 안전 확인(특히 `settings`는 오히려 진단 쪽이 `mask_sensitive`로 더 가려져 있고, `/api/admin/settings` 원본은 마스킹이 아예 없다). **`dashboard`(내장 `build_dashboard()` 호출) 1개에서 실제 결함 발견** — `include_critical_audit`를 안 넘겨 기본값 `True`가 그대로 적용돼, `/api/admin/dashboard` 라우터가 operator에게 명시적으로 가리는 감사 슬라이스(`recent_critical_audit`, 누가 역할 변경/롤백/승인했나)를 진단 번들이라는 옆문으로 우회 노출할 뻔했다 — `include_critical_audit` 파라미터를 새로 뚫어 `role in SENSITIVE_READ_ROLES` 판정을 명시적으로 전달해 막음. 프런트 `AdminRoutes.jsx`(role 배열 갱신 + `help` 문구 추가)·`navConfig.js`(내비 role 배열) 동기화. `auditor`는 `console.ops`가 원래 안 담아 제외. 신규 `test_diagnostics_bundle_rbac.py` 9건(역할 5종 매트릭스, critical-audit 우회 차단을 revert-to-verify로 실제 재현·확인) + 기존 RBAC/보안 시험 66/66 green, 프런트 전체 회귀 277파일 1904건 green. **TEST SERVER 배포(`UPGRADE_OK`) + 실측 완료** — 캐시된 `operator` 세션으로 `/api/admin/diagnostics/bundle` 200(전엔 403), `recent_critical_audit` 빈 배열로 정확히 가려짐, `/#/diagnostics` 화면이 실제 데이터를 렌더(스크린샷 확인, 권한 거부 아님). `system_admin` 무변화 확인. `auditor`/`user`는 원격 계정 없어 로컬 role-matrix 시험(9건)으로 대신 확정. 상세: `DECISIONS.md` D-98 |
...
## PA3 — Whole Product Audit Cycle `PA-20260817-072224-24b91505` (Handoff 13건, 2026-08-17)

> 상세 구현 계약은 **`docs/product-audit/PRODUCT_AUDIT_HANDOFF.md`** 의 각 `PA-RC-*` 블록이 정본이다.
> Backlog 한 줄만 보고 구현하지 마라 — Acceptance Criteria·Regression 범위·Target Design 이 거기 있다.
>
> 이 Cycle 은 **Backlog 가 완전히 닫힌 상태**(278행 전부 ✅)에서 시작했다. 따라서 아래 13건은
> 정의상 **아직 아무도 적지 않은 것**이고, 기존 278행과 대조해 중복이 없음을 확인했다.
> 관측 대상은 TEST SERVER(`10.100.64.71`)이며 배포본이 HEAD 임을 `app/**/*.py` 314개 파일 해시로 확인했다.

| ID | 심각도 | 요약 | 근거 | 상태 |
|---|---|---|---|---|
| PA3-01 | **High** | **최초 로그인 강제 관문이 SQLite 잠금에서 raw 500 을 낸다**(`PA-RC-0032`). `POST /change-password` 가 오늘 **31건 중 3건(약 10%)** 500. `app/chat/`(대화 전송 포함) 세 파일 전부 `is_write_conflict` 사용 **0회**. 같은 파일의 `login()` 은 공용 관용의 **기준 구현**을 갖고 있는데 쓰기를 더 많이 하는 `change_password()` 에만 없다. **`PA-03`(PA-RC-0008)이 만든 공용 규약을 적용받지 못한 마지막 두 경로** — 중복이 아니라 그 계열의 잔여분이다 | `PA-RC-0032` · `PA-F-091` · `auth/router.py:689` traceback · TEST SERVER journalctl 02:25·05:09·08:12·08:14 | ✅ **배포완료** — `auth/router.py::change_password`·`chat/router.py::post_message` SAVEPOINT 재시도 적용, 신규 시험 6건 + revert-to-verify, TEST SERVER 배포+코드 존재 확인(grep). 동시 부하 재현 브라우저 E2E는 미시도(`visual_change_required` 아님) |
| PA3-02 | **High** | **`/me`·`/my-stats` 개수 타일이 "모른다"를 `0` 으로 그린다**(`PA-RC-0027`). API 가 `mapped:false` 를 알면서 버킷을 하드 `0` 으로 보낸다. 같은 응답의 `sprint` 는 `null` 로 옳고, 형제 `work.py` 는 가드를 걸어 「셀 수 없습니다」라 말한다. **불변식을 명시한 주석이 두 파일에 있는데** `service.py` 버킷과 `profiles/router.py` 두 호출부에만 가드가 없다. 프런트는 이미 옳다(`Home.jsx:309`). TEST SERVER 23명 중 **10명**이 이 경로. **기존 테스트가 이 결함을 고정하고 있다**(`test_stats_survive_an_unmapped_account` 가 `totals["all"] == 0` 단언) | `PA-RC-0027` · `PA-F-082`·`084`·`085` · `DASHBOARD_METRICS.md` §3 | 🟡 **배포완료, 브라우저 검증대기** — `home/service.py`·`profiles/router.py`·`MyStats.jsx`·`AssistantPanel.jsx` 수정, 기존 고정 시험 정정 + 신규 시험(revert-to-verify로 실크래시 확인) + TEST SERVER 배포. `visual_change_required: true`(Handoff) — 매핑 없음/있음 두 계정으로 `/me`·`/my-stats` 라이트/다크 스크린샷은 **아직 안 함**, PA-RC-0033 일괄 배치에 포함 |
| PA3-03 | Med | **`/dashboard` 와 `/diagnostics` 가 본문 68% 중복**(`PA-RC-0028`). `최근 주요 변경` 16/16 문자열 동일, `백업` 은 CTA 까지 동일. `build_diagnostic_bundle` 이 `build_dashboard()` 를 통째로 품는 구조(docstring 명시). 같은 8개 서비스를 두 화면이 다르게 분류. **번들(데이터)은 옳고 화면이 그것을 그대로 펼쳐 그리는 것이 문제** — `include_critical_audit` 역할 분기를 화면 정리 중에 잃지 않는 것이 최대 위험 | `PA-RC-0028` · `PA-F-083` · `pa2_dup.py` 문자열 계측 | 🟡 **구현완료, 브라우저 실측대기** — `Diagnostics.jsx`에서 `최근 주요 변경`·`백업` 섹션 렌더 제거(데이터는 번들에 그대로, `/dashboard`로 가는 링크 한 줄로 대체), 두 화면 모두 도넛→섹션 제목 옆 `정상 N/M` 텍스트로 교체, `Dashboard.jsx`의 서비스 카드를 진단과 같은 `서비스 상태`/`외부 연동` 2분류로 분리. `opsHelpers.js`의 중복 상수(`COMP_LABELS`)를 없애고 `serviceLabel()` 단일 소스로 통일(두 화면 라벨이 실제로 같은 함수를 쓴다), 죽은 `integrationMix()` 제거. 백엔드·RBAC 게이트 무수정(`include_critical_audit` 분기 확인). 신규 시험: 번들 페이로드 불변 회귀(백엔드), 두 섹션 미렌더 컴포넌트 시험, 서비스 라벨 어휘 교차검증 시험(둘 다 revert-to-verify로 실제 발산을 잡는지 확인) + 기존 스위트 297/2054 green. 정적 검사 통과 중 무관한 기존 결함 2건 추가 발견해 함께 고침(가운뎃점 금지 위반 `kit.jsx`, UX 동사표 오탐 1건). `pa2_dup.py` 재실행·실측 스크린샷은 PA3-08/11/13과 일괄 배치(`DECISIONS.md` D-133) |
| PA3-04 | Med | **관리자 메뉴 분류가 업무와 어긋난다**(`PA-RC-0031`). `감사` 그룹 6개 중 **3개가 감사가 아니다**(기능 플래그·공지 배너·복구 리허설). `정책`(연동) 대 `정책 사용 통계`(감사)로 같은 명사가 갈리고, 구조가 같은 사용 통계 화면 둘이 다른 그룹. `백업`(운영)과 `복구 리허설`(감사)이 갈려 한 업무에 두 그룹을 오간다. `업무용어(기술용어)` 병기 관례가 **34항목 중 3개**에만 | `PA-RC-0031` · `PA-F-090` · `pa2_ia.py` DOM 트리 덤프 | 🟡 **구현완료, 브라우저 실측대기** — `navConfig.js` 4항목 재배치(운영 9·자동화 8·연동 6·감사 5), 괄호 라벨 3개 통일, 사용자 콘솔 휴지통→`TeamDocs.jsx` 버튼 이동(5→4그룹). role·배지·아이콘은 diff 스크립트로 무결성 확인(`DECISIONS.md` D-129 — `/setup`을 감사에 남긴 근거, Handoff의 34 vs 실제 35항목 불일치 진단 포함) |
| PA3-05 | Med | **`/settings` 탭 게이트에서 주소와 화면이 갈라진다**(`PA-RC-0030`). `admin`·`operator` 가 `/system` 으로 오면 주소는 `?tab=os` 인데 화면은 `시스템 정책` 이고 **거부 안내가 없다**(역할 3종 × 라우트 4종 12조합 실측). 같은 콘솔의 라우트 게이트는 「권한이 없습니다」를 명시하므로 **한 제품에 권한 거부 어휘가 두 벌**. `system_admin` 2명을 뺀 전원이 이 경로. 보안 결함은 아니다 — 내용은 정확히 막힌다 | `PA-RC-0030` · `PA-F-088` · `SettingsShell.jsx:61-67` | 🟡 **구현완료, 브라우저 실측대기** — "모르는 탭"(주소 정정)과 "역할 거부"(`RequireRole`과 같은 `EmptyState`)를 분리, 탭 1개면 스트립 숨김. 옛 결함을 정답으로 고정하던 기존 시험 2개 갱신 + revert-to-verify(`DECISIONS.md` D-130) |
| PA3-06 | Med | **관리자 상세 3종이 없는 레코드에 침묵한다**(`PA-RC-0033`). `/users/:id`·`/departments/:id`·`/audit/:id` 가 없는 UUID 로 들어와도 목록만 그린다(주소는 그 레코드를 계속 가리킨다). 감사 로그가 특히 나쁘다 — 사건을 특정해 공유하는 링크인데 항목이 없으면 8,138자 전체 목록이 뜬다. **사용자 콘솔 5종은 같은 상황에서 옳게 답한다** | `PA-RC-0033` · `PA-F-092` · `pa2_badid.py` 9라우트 실측 | 🟡 **구현완료, 배포·브라우저 검증대기** — `DataScreen.jsx`(`/audit`+`/departments` 공용, `OrgConsole.jsx`는 무수정으로 확인) + `Users.jsx` 각각 `routeIdNotFound`+기존 `ErrorState` 재사용. `admin-detail-routes.test.jsx` 3라우트 found/not-found 10건 + 관련 파일 전체(16개 101건) 회귀 green. Handoff의 "/departments는 오른쪽 패널" 지시는 실측과 어긋나 세 라우트 모두 모달로 통일(`DECISIONS.md` D-127 근거). 번들 빌드·배포·TEST SERVER 브라우저 캡처는 나머지 PA3 항목과 일괄 배치 예정 |
| PA3-07 | Med | **전역 QueryClient 에 `retry` 가 없어 확정적 4xx 를 4회 호출한다**(`PA-RC-0034`). `main.jsx:17` 이 `retry` 를 생략해 React Query 기본값 3이 적용된다. `/board/<없는 id>` 가 **20초 시점에 아직 로딩, 30초에야** 「찾을 수 없습니다」. `useQuery` **80곳 중 20곳**이 이 기본값 상속, **나머지 60곳은 각자 덮어썼다** — 60곳이 기본값을 덮는다는 사실 자체가 기본값이 틀렸다는 증거다. 개별 화면에 하나씩 넣지 말고 기본값을 고칠 것 | `PA-RC-0034` · `PA-F-092` · 네트워크 404 4회 관측 | 🟢 **구현+시험완료** — `lib/queryRetry.js` 신설(4xx 즉시 포기, 5xx/네트워크는 2회까지), `main.jsx`에서 한 줄로 연결. 단위 시험(순수 함수)+통합 시험(실제 QueryClient+react-query 재시도 엔진, 404/403 1회·500 3회 확인) 8건, 기존 `api.test.js`·`auth-401-invalidates-me.test.jsx` 회귀 확인. `visual_change_required` 없음(요청 횟수·시간이 관심사) — TEST SERVER Network 탭 실측(1회 호출·3초 이내)만 배치 대기(`DECISIONS.md` D-131) |
| PA3-08 | Med | **좁은 폭에서 행을 식별하는 열이 가장 먼저 죽는다**(`PA-RC-0037`). `/settings` `항목명` 이 1920 에서 140px·잘림 0/11 인데 **1366 에서 91px·잘림 8/11**(같은 표의 `설명` 은 566px 유지). `/users`·`/offboarding` 이메일도 1366 에서 83·127px(필요 181). 가로 스크롤이 없어 드러낼 방법이 화면에 없다. 7뷰포트 × 5라우트 35조합 실측 — **가로 넘침은 전 조합 0**, 문제는 셀 내부다 | `PA-RC-0037` · `PA-F-095`·`087` · `pa2_resp_dark.py`·`pa2_verify_no.py` | 🟡 **구현완료, 브라우저 실측대기** — `kit.jsx` `DataTable`에 `identifier`(최소 폭)+숨은 열 안내 캡션 추가, 미지정 화면은 렌더 불변(전체 회귀 291/2013 green, `DECISIONS.md` D-128). `pa2_resp_dark.py`·`pa2_verify_no.py` 재실행은 PA3-11/13과 일괄 배치 |
| PA3-09 | Med | **기본 필터가 걸린 화면이 「없습니다」를 무조건형으로 말한다**(`PA-RC-0038`). `/approvals` 가 `?status=pending` 으로 열리는데 0건일 때 「승인 요청이 없습니다」 + **필터 지우기 버튼 없음**. 실제 상태는 「대기 중인 것이 없다」다. 같은 제품의 `/users` 는 「조건에 해당하는 사용자가 없습니다」 + 필터 지우기 2곳으로 정확히 구분한다. 판정 기준에 **「기본 필터도 활성 필터로 센다」** 를 명시하는 것이 핵심(`useQueryState` 가 기본값을 주소에 안 쓴다) | `PA-RC-0038` · `PA-F-097` · Blind pass 1 | 🟡 **구현완료, 브라우저 실측대기** — `DataScreen.jsx`의 공유 `hasFilter` 분기에서 기본값 제외 로직을 없애 config 기본 필터도 활성으로 센다(REGISTRY 전체 27화면에 자동 적용, 화면별 예외 없음). `필터 지우기`는 기존 핸들러(`setFilters({})`) 그대로라 기본값 복귀가 아니라 진짜 전체를 보여준다 — 지운 뒤에도 0건이면 `filters`가 다시 `{}`가 되어 자연히 기존 온보딩 빈 상태로 떨어진다(두 빈 상태가 스스로 갈라져 답한다, 새 config 필드 없음). 기본 필터를 가진 화면 4개(`approvals`·`audit-anomalies`·`prompts`·`policies`) 전수 확인, 그 집합을 `registry-default-filters.test.jsx`로 고정(acceptance 4). acceptance (5) 조사: `/approvals` 목록은 "결재자 배정" 개념이 없고(`approver_id`는 결정 시점에만 채워지는 기록 필드) `requested_by` 기준 조직 범위 스코프다 — 백엔드 시험 1건(`test_approval_scope.py`)으로 고정. 신규 프런트 시험 10건(합성 config 5 + 실제 REGISTRY.approvals 3 + enumeration 2) 전부 revert-to-verify로 구코드 FAIL 확인, 전체 프런트 회귀 300파일/2064건 green. 실측(TEST SERVER `/approvals` 캡처, `/announcements` 등 무필터 화면 대조)은 나머지 PA3 항목과 일괄 배치(`DECISIONS.md` D-134) |
| PA3-10 | Med | **일반 사용자 화면이 「관리자」를 breadcrumb 뿌리로 보여준다**(`PA-RC-0039`). 공용 `PageHeader` 의 `crumbRoot` **기본값이 `"관리자"`**(`kit.jsx:1165`)라 사용자 콘솔 화면이 prop 을 잊으면 조용히 관리자가 된다. `role=user` 실측 — `/team-docs` 「관리자」, `/notifications` 「관리자 › 운영」. 정적 분석(호출 32곳 중 미전달 2곳)과 브라우저(12화면 중 2화면)가 **정확히 일치**. `Trash.jsx`·`Profile.jsx` 가 명시적으로 넘기는 것이 개발자도 알고 있었다는 증거 | `PA-RC-0039` · `PA-F-098` · Blind pass 2 · `pa2_crumb.py` | 🟡 **구현완료, 브라우저 실측대기** — Handoff의 (b)안(콘솔+navConfig 그룹에서 유도) 채택. `kit.jsx`에 `CrumbRootCtx`+`CrumbRootProvider`, `AppShell.jsx`가 계산해 주입, `navConfig.js`에 `groupForPath`(기존 `activeNavPath` 재사용 — 사이드바 강조와 항상 같은 값). `Trash.jsx`의 하드코딩 제거(PA-RC-0031과의 연쇄를 위해 필요), `Profile.jsx`는 무수정. 관리자 콘솔 회귀 없음 확인(`DECISIONS.md` D-129, `crumb-root.test.jsx`) |
| PA3-11 | Med | **`/users` 이메일 열이 20/20행 잘린다**(`PA-RC-0029`). 1920 에서 열 폭 131px·필요 198px 인데 같은 표의 `역할` 이 295px, `최근 로그인` 이 287px. 가로 스크롤 없음. `/audit`·`/jobs` 는 잘림 0이라 표 계열 전체가 아니라 이 화면의 열 정의 문제. 기존 UI QA 의 `horizontal_overflow` 는 표가 화면을 넘는지만 봐서 이것을 통과시킨다 | `PA-RC-0029` · `PA-F-087` · `pa2_cols.py` | 🟡 **구현완료, 브라우저 실측대기** — `email` 열에 `identifier:true`, `역할`/`최근 로그인` width 축소로 폭 회수(`Users.jsx`). `/audit`·`/jobs`는 무수정(회귀 없음, 열 정의 자체를 안 건드림). `pa2_cols.py` 재실행은 PA3-08/13과 일괄 배치 |
| PA3-12 | Low | **`/new-ticket` 담당자가 인원수만큼 길어지고 안내가 입력과 함께 사라진다**(`PA-RC-0035`). 체크박스 14개 평면 나열(검색·그룹 없음), `MuiFormHelperText` **0회** — 안내가 전부 placeholder 에 있어 입력 시작하면 사라진다. 정작 `제목` 필드는 placeholder 를 예시로만 쓰는 옳은 패턴을 이미 보여준다. (계측의 「무레이블 4건」은 MUI Select 숨은 input 으로 **결함 아님** 확인) | `PA-RC-0035` · `PA-F-093` · 입력 25개 전수 계측 | 🟡 **구현완료, 브라우저 실측대기** — 공유 `AssigneePicker`(수정 모달과 공용)를 MUI Autocomplete(검색+칩)로 재설계, 담당자 배정 값 전송 형식 불변. 설명 placeholder는 예시로, 서식 안내는 캡션으로. 시험 9건 전부 revert-to-verify로 구코드 FAIL 확인, 기존 화면 8파일 72건 무손상(`DECISIONS.md` D-132) |
| PA3-13 | Low | **`/offboarding` 대상 표가 같은 문장을 8/20행 반복한다**(`PA-RC-0036`). 행 높이가 **97px 대 35px(2.8배)** 로 갈리고 상태가 513px 열에 **481px 막대** — `/users` 는 같은 값을 작은 배지로 그린다. 워크플로 자체(선택 → 실행 → 이력 → **되돌리기**)와 빈 상태 문구는 이 제품에서 가장 잘 된 축이라 건드리지 말 것 | `PA-RC-0036` · `PA-F-094` · 행 높이 분포 계측 | 🟡 **구현완료, 브라우저 실측대기** — 반복 문장을 표 위 1회 안내로 이동(`NOTION_UNMAPPED_HELP` 문구 재사용), 상태를 `/users`와 같은 Badge 하나로 통일, 회수한 폭을 이메일 열(`identifier:true`)로. `실행 이력`·되돌리기·검색·잘림 고지 무수정. 실측 스크린샷은 PA3-08/11과 일괄 배치 |
| PA3-14 | Med | **`PA-RC-0031`이 사이드바 그룹만 옮기고 breadcrumb `area`는 안 옮겼다**(배경 재감사 발견, Handoff에 없던 항목). `복구 리허설`·`기능 플래그`·`공지 배너`·`프롬프트 사용 통계` 4화면이 사이드바는 새 그룹을 강조하는데 breadcrumb는 옛 그룹 이름을 계속 말한다 — `PA-RC-0039`가 구조적으로 없애려던 결함이 그 배선이 안 닿는 형제 필드(`config.area`)로 재발 | `registry/platform.js`·`authoring.js` `area` 필드, `D-135` | ✅ **구현+시험완료** — 4개 필드를 사이드바와 일치시켜 수정, `REGISTRY` 전체 `area`를 `groupForPath()`와 교차 대조하는 회귀 시험 신설(`registry-area-matches-nav-group.test.js` 29건), revert-to-verify로 정확히 그 4개 화면만 실패함을 확인. 브라우저 실측은 PA3-04/10과 일괄 배치 |
| PA3-15 | Med | **`identifier` 열 최소 폭 메커니즘(`D-128`)이 3화면에만 적용되고 REGISTRY 대다수는 안 받았다**(배경 재감사 발견, Handoff에 없던 항목). `runners`(10열)·`notion-mapping`(첫 열이 `user_email`) 등 일부는 원래 `/users`를 고치게 만든 사례보다 더 나쁘다 | `D-128`(메커니즘 신설), `D-135` | ✅ **구현+시험완료** — REGISTRY 22화면의 식별 열(항상 첫 열은 아니다 — `rowName`/SEM-01 근거로 판단)에 `identifier:true` 적용, 새 메커니즘 없이 기존 것만 실제로 씀. `audit`·`jobs`는 `PA-RC-0029`가 실측으로 이미 "잘림 0" 확정한 화면이라 의도적으로 제외, `backup`/`restore-drills`/`subList`도 확신 낮아 보류. 22화면·26열 집합 + audit/jobs 제외를 함께 고정하는 회귀 시험 신설(`registry-identifier-columns.test.js` 24건), revert-to-verify 확인. 전체 프런트 회귀 302파일/2117건 green. 브라우저 실측은 PA3-08/11/13과 일괄 배치 |

### 착수 순서 — 같은 파일을 건드리는 것끼리 묶어라

1. **`PA3-01`(High) 먼저.** 필수 관문이 10% 실패한다. 다른 어떤 것보다 사용자를 막는다.
2. **`PA3-02`(High).** 백엔드 두 호출부 + 테스트 단언 정정. **테스트가 빨개지는 것이 정상**이다.
3. **표 3건을 한 묶음으로** — `PA3-08`·`PA3-11`·`PA3-13` 은 전부 열 정의/공용 표를 건드린다.
   따로 하면 같은 파일을 세 번 만지고 서로 덮어쓴다.
4. **IA 2건을 한 묶음으로** — `PA3-04`(메뉴 taxonomy)와 `PA3-10`(breadcrumb 뿌리)은 둘 다
   `navConfig` 를 쓴다. `PA3-10` 을 navConfig 유도 방식으로 고치면 `PA3-04` 가 그룹을 바꿀 때
   breadcrumb 가 자동으로 따라온다 — **`PA3-10` 을 먼저** 하는 편이 싸다.
5. `PA3-03`(대시보드/진단 분리)은 `include_critical_audit` 역할 분기를 잃지 않는 것이 전부다.
6. 나머지(`PA3-05`·`06`·`07`·`09`·`12`)는 서로 독립이다.

### 이 13건이 실제로 몇 개의 병인가

- **공용 규약이 마지막 몇 호출부에 안 닿았다** — `PA3-01`. `PA-03`(PA-RC-0008)이 규약을
  만들고 `PA-08`·`PA-15`·`d5ba3f9` 가 하나씩 적용해 왔는데, 인증 관문과 AI 대화가 남았다.
  **어느 쓰기 경로가 보호되지 않았는지 세는 정적 검사가 없어서** 새 경로가 무방비로 추가돼도
  아무것도 빨개지지 않는다.
- **「모른다」를 「0/없다」로 말한다** — `PA3-02`·`PA3-09`. 값과 문장이 같은 상태를 다르게 말한다.
- **화면이 자기 위치·대상을 사실과 다르게 말한다** — `PA3-05`·`PA3-06`·`PA3-10`.
  주소·breadcrumb·상세가 각각 다른 것을 가리킨다.
- **좋은 패턴이 있는데 전부에 적용되지 않았다** — `PA3-09`(빈 상태)·`PA3-06`(ErrorState)·
  `PA3-13`(배지)·`PA3-12`(placeholder 대 helper). 전부 **같은 제품 안에 정답 대조군이 있다.**
- **폭·정보 배분** — `PA3-08`·`PA3-11`·`PA3-13`.
- **정보의 소속** — `PA3-03`·`PA3-04`.

### 검사가 「실패」만 보고 「틀림」은 안 본다 (이 Cycle 전체를 관통하는 QA 공백)
...
- `UI-R31` 내부 구현 정보 노출 (지시 36) — **SMTP·백업 일정 완료**(D-156·D-167): 구조화 편집기 +
  검증 문장 재작성 + 요약의 `starttls` → 이름. 남은 것: `backup_schedule`(cron 원문 편집),
  Notion/LLM 콘솔의 경로 노출(AI 실행 파일은 D-158 로 강등 완료). 작업 큐의 원문은 **그대로
  둔다**(D-162 판정: 거기는 기술 화면이고 원문이 내용이다)
...
- `UI-R45` **티켓 폼 둘이 공용 부품을 안 쓴다** (지시 17). `FormField` 는 라벨을 입력 위로
  올렸는데(D-165) 티켓 **수정**(`te-*`)과 **새 티켓**(`nt-*`)은 MUI TextField 를 직접 그려
  그 둘만 떠 있는 라벨로 남았다 — 한 제품에 폼 언어가 둘이다. 배포 화면 캡처에서 확인했다
  (`dist/ui-qa/deploy2/light/1920x1080/user_new-ticket.png`).
  변환이 단순하지 않은 이유: 선택지를 `MenuItem` 자식으로 그리고(불러오는 중 문구 포함),
  `EMPTYABLE_SELECT`·`step`·`disabled` 를 쓴다. `FormField` 에 그 능력을 더할지, 두 폼을
  옵션 배열로 옮길지 먼저 정해야 한다 — 반쯤 바꾸면 가장 많이 쓰는 화면이 깨진다.
