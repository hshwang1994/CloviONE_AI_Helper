...
| PA2-08 | Med | **어시스턴트 FAB(z=1050)이 표 화면 행 「상세」 버튼을 실제로 덮어 클릭을 가로챈다**(`PA-RC-0019`). 관리자 9/10 · 사용자 1/5 라우트에서 `elementFromPoint`가 FAB을 반환. **z-index만 낮추지 말 것** — `PA2-09`가 원인을 없앤다 | `PA-RC-0019` · `PA-F-061` · `verify_fab.json` | **재확인(2026-08-16) — `PA-RC-0023`이 「상세」 버튼 열을 지운 뒤 전제가 바뀌어 라이브로 재검증했다(Handoff constraints가 명시한 절차).** 원래 가려지던 대상(작은 버튼)은 당연히 사라졌지만 **문제 자체는 안 없어졌다** — `var/product-audit/verify_pa_rc_0019.py`(신규, TEST SERVER 대상)로 실측하니 FAB이 이제 **행 전체(`tr[tabindex]`, 상세를 여는 새 클릭 대상)를 덮는다**: `/users` 2줄·`/audit` 3줄·`/jobs` 3줄·`/offboarding` 3줄, 총 12건 `blocked`(`elementFromPoint`가 FAB을 반환). Handoff `implementation_direction`(1)의 권장 순서대로 `PA2-09`(`PA-RC-0020`, 진입점을 헤더 칩 하나로 모으고 FAB 자체를 제거)를 먼저 처리해 **원인 소멸로 함께 닫는다** — 착수함(같은 invocation). |
