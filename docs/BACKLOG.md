...
| PA-03 | **High** | **SQLite 쓰기 경합 재시도가 공용 유틸 없이 호출부마다 손으로 쓰여 있다**(`PA-RC-0008`). 예산이 2·5·10·12로 네 가지고 backoff/jitter를 쓰는 곳은 7곳 중 1곳뿐이다. `app/prompts/service.py::new_version_from`(예산 5·jitter 없음)이 8-way 경합에서 재시도를 소진하고 **처리되지 않은 `OperationalError: database is locked`로 500**이 난다. 결정적 증거는 이 저장소가 **이미 그 교훈을 실측해 적어 뒀다**는 것 — `auth/router.py:505` *"지터 없이 10회 재시도로도 5번 중 1번은 여전히 실패했다"*. 지식이 옆 파일로 전파되지 않았다 | `app/prompts/service.py:123,148-178` · `app/auth/router.py:476,505-506` · `app/core/db.py:148-176`(`is_write_conflict`는 정상) · `tests/integration/test_prompt_create_new_version_race.py` | **발견** — race 테스트는 1회 실행이 무의미하다(이번 건도 60%는 통과). 반복 실행으로 검증할 것 |
