...
| USE-06 | Low | **`web.sqlite3-wal`이 4.7MB로 본체 6.0MB에 육박한다.** WAL 체크포인트가 밀리고 있다는 신호이고, 이 상태에서의 백업·복구 동작은 확인된 바 없다(`backups` 1건, `restore_rehearsals` 0건) | **재확인(2026-08-16), 여전히 사실 — TEST SERVER 실측 갱신** — 방금 확인한 실제 값은 `web.sqlite3` 6,692,864바이트(6.4MB) 대비 `web.sqlite3-wal` 4,799,832바이트(4.6MB) — 원 관찰과 거의 동일한 비율로, **한 시점의 우연이 아니라 지속되는 패턴**임을 확인했다. `backups`/`restore_rehearsals`는 그새 각각 2건/1건으로 늘었다(`RSTR-02` 참고 — 완전히 0은 아니게 됨). WAL 체크포인트 정체의 실제 원인(자동 체크포인트 임계값 미도달? 장기 보유 읽기 트랜잭션? 지속적 고빈도 쓰기?)은 라이브 서비스에 `PRAGMA wal_checkpoint`를 함부로 실행하는 방식으로 조사하지 않는다 — DB transaction 계층은 CLAUDE.md 불변규칙 §3-10 대상이라 락 경합·서비스 중단 위험을 먼저 이해하는 전담 조사가 필요하다 |
