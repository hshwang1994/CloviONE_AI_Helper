...
| PA3-11 | Med | **`/users` 이메일 열이 20/20행 잘린다**(`PA-RC-0029`). 1920 에서 열 폭 131px·필요 198px 인데 같은 표의 `역할` 이 295px, `최근 로그인` 이 287px. 가로 스크롤 없음. `/audit`·`/jobs` 는 잘림 0이라 표 계열 전체가 아니라 이 화면의 열 정의 문제. 기존 UI QA 의 `horizontal_overflow` 는 표가 화면을 넘는지만 봐서 이것을 통과시킨다 | `PA-RC-0029` · `PA-F-087` · `pa2_cols.py` | ⬜ 미착수 |
...
| PA3-13 | Low | **`/offboarding` 대상 표가 같은 문장을 8/20행 반복한다**(`PA-RC-0036`). 행 높이가 **97px 대 35px(2.8배)** 로 갈리고 상태가 513px 열에 **481px 막대** — `/users` 는 같은 값을 작은 배지로 그린다. 워크플로 자체(선택 → 실행 → 이력 → **되돌리기**)와 빈 상태 문구는 이 제품에서 가장 잘 된 축이라 건드리지 말 것 | `PA-RC-0036` · `PA-F-094` · 행 높이 분포 계측 | ⬜ 미착수 |
