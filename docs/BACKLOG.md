...
| `user_team-doc-detail` | 3 | 파괴적 동작이 화면에서 가장 무거운 버튼이고, 가장 자주 하는 동작인 본문 편집은 카드 안쪽 외곽선 버튼으로 · 페이지 헤더가 브레드크럼과 같은 단어를 제목으로 반복하고, 실제 문서 제목은 카드 안 두 번째 위치로 밀린다 · ~~본문 안 URL 이 본문과 같은 색·굵기의 평문이라 클릭할 수 없다(링크가 아니다)~~ ‖ **구현완료(2026-08-13)**: 팀 채팅 말풍선이 이미 쓰는 `linkifyText`(`chat/links.jsx` — 허용 도메인은 실제 링크, 그 외는 복사 버튼 폴백, 텍스트 노드만 써 innerHTML 아님)를 `TeamDoc.jsx::DocBlock`의 문단뿐 아니라 헤딩·목록·인용·콜아웃·토글·이미지 캡션까지 텍스트를 그리는 모든 자리에 적용(코드 블록은 원문 유지 — `RichText.jsx`의 같은 판단과 동일). `DocBody`는 `TicketBody.jsx`가 그대로 재사용하는 공용 렌더러라 **티켓 상세 본문도 같은 수정으로 함께 고쳐진다**(한 곳 수정, 두 화면 해결). 신규 시험 4건(`teamdoc.test.jsx` — Notion URL 실제 링크·비허용 도메인은 복사 버튼·코드 블록은 미적용·목록 항목도 적용), 관련 회귀(`teamdoc`·`ticket-detail`) + 프런트 전체 회귀(241파일/1601건) green |
