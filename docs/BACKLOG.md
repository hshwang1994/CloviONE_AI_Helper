...
| PA-02 | **High** | **UX Writing 규칙 문서가 존재하지 않는다**(`PA-RC-0002`, 저장소 전체 검색 0건). 그래서 문구가 매번 호출부 즉흥 판단으로 갈렸다 — ⓐ 실패 문구 167건 중 `[무엇을 하라]`를 주는 것이 24건(14%), `[왜]`는 5건, **3요소를 다 갖춘 것은 0건** ⓑ 같은 문장이 마침표 유무로 동시 존재 ⓒ 같은 개념에 동사가 균등 분포(생성 84·등록 72·추가 64·만들 49). `lib/api.js` **한 파일 안에서** 16행은 막다른 길이고 54행은 회복 경로가 있다 | `frontend/src/lib/api.js:10,16,35,54` · `frontend/src/app/AdminRoutes.jsx:40` · `var/product-audit/errcopy_scan.json` · 스캐너 `scan_copy.py`·`scan_errcopy.py` | **발견** — `docs/UX_WRITING.md`를 **먼저** 만들고 `static_checks.sh` 린트로 못박은 뒤 일괄 정렬. UX Writing 확정 **후에** 한국어 humanization 적용(순서 역전 금지) |
