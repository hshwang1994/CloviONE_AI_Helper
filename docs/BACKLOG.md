...
| PA3-04 | Med | **관리자 메뉴 분류가 업무와 어긋난다**(`PA-RC-0031`). `감사` 그룹 6개 중 **3개가 감사가 아니다**(기능 플래그·공지 배너·복구 리허설). `정책`(연동) 대 `정책 사용 통계`(감사)로 같은 명사가 갈리고, 구조가 같은 사용 통계 화면 둘이 다른 그룹. `백업`(운영)과 `복구 리허설`(감사)이 갈려 한 업무에 두 그룹을 오간다. `업무용어(기술용어)` 병기 관례가 **34항목 중 3개**에만 | `PA-RC-0031` · `PA-F-090` · `pa2_ia.py` DOM 트리 덤프 | ⬜ 미착수 |
| PA3-05 | Med | **`/settings` 탭 게이트에서 주소와 화면이 갈라진다**(`PA-RC-0030`). `admin`·`operator` 가 `/system` 으로 오면 주소는 `?tab=os` 인데 화면은 `시스템 정책` 이고 **거부 안내가 없다**(역할 3종 × 라우트 4종 12조합 실측). 같은 콘솔의 라우트 게이트는 「권한이 없습니다」를 명시하므로 **한 제품에 권한 거부 어휘가 두 벌**. `system_admin` 2명을 뺀 전원이 이 경로. 보안 결함은 아니다 — 내용은 정확히 막힌다 | `PA-RC-0030` · `PA-F-088` · `SettingsShell.jsx:61-67` | ⬜ 미착수 |
...
| PA3-10 | Med | **일반 사용자 화면이 「관리자」를 breadcrumb 뿌리로 보여준다**(`PA-RC-0039`). 공용 `PageHeader` 의 `crumbRoot` **기본값이 `"관리자"`**(`kit.jsx:1165`)라 사용자 콘솔 화면이 prop 을 잊으면 조용히 관리자가 된다. `role=user` 실측 — `/team-docs` 「관리자」, `/notifications` 「관리자 › 운영」. 정적 분석(호출 32곳 중 미전달 2곳)과 브라우저(12화면 중 2화면)가 **정확히 일치**. `Trash.jsx`·`Profile.jsx` 가 명시적으로 넘기는 것이 개발자도 알고 있었다는 증거 | `PA-RC-0039` · `PA-F-098` · Blind pass 2 · `pa2_crumb.py` | ⬜ 미착수 |
