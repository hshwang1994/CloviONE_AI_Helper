...
| ID | 심각 | 문제 |
|---|---|---|
| VIS-53 | **High**(정정) | **백업이 꺼진 채 20일이 지났는데 아무도 통보받지 않았다.** 설정 `자동 백업 일정 = 꺼짐` + 마지막 백업 20일 전 + `FN-09`대로 **백업 실패 알림 없음**(예외를 삼킨다). ※ **정정**: 처음엔 "각 화면이 조각만 말해 아무도 전체 그림을 못 본다"고 적었는데 **틀렸다** — `/diagnostics` 최상단 오류 상자가 `위험 마지막 백업이 오래됨(20일 전)`을 **한 자리에 모아서 말한다.** 남는 진짜 문제는 **아무도 그 화면을 열어 보라는 신호를 못 받는다**는 것이다(알림 없음). 즉 "정보가 없다"가 아니라 "**밀어 주는 채널이 없다**" ‖ **착수용 조사 완료(2026-08-15), 구현은 다음으로 — 정확한 설계가 필요해 서두르지 않음.** 확인한 사실: (1) `BACKUP_STALE_DAYS`(`opsHelpers.js:37`, 7일) 판정이 **프런트에만** 있다 — 백엔드는 "며칠 지났는지"를 계산하지 않으므로 화면을 실제로 여는 사람이 없으면 아무도 그 사실을 계산조차 안 한다. (2) 알림 발송 자체는 이미 있는 인프라(`app/notifications/service.py::notify_admins(db, type_, title, body, related, now)` — admin/system_admin 전원에게 발송, `AUDIENCE_ADMIN`)라 재사용만 하면 된다. (3) 진짜 어려운 부분은 **"언제 계산을 트리거하는가"** — 이 저장소엔 사용자가 만드는 `schedules`(문서 생성용) 말고 **시스템 자체의 주기 점검** 메커니즘이 없다. 워커의 `run_heartbeat_loop`(`app/worker_main.py:63`, 30초 간격)이 유일한 상시-반복 지점인데 그 주기 그대로 쓰면 낭비이고(파일시스템/DB 조회를 30초마다), 한 번 통보한 뒤 **다음 통보까지 억제하는 상태(dedup)** 도 필요하다(매 30초 재알림은 스팸). **다음 세션 제안 설계**: 하트비트 루프에 "N번째 비트마다만" 던지는 카운터를 추가하거나 별도의 훨씬 낮은 빈도 타이머를 두고, 마지막 통보 시각을 `settings`류 기존 key-value 저장소(전용 테이블 신설 없이)에 남겨 재통보 주기(예: 1일)를 지킨다. 알림 `type_`을 새로 만들면 `AppShell.jsx`의 `BADGE_TYPES`(사이드바 배지 매핑)에도 등록해야 사이드바에서 보인다 — 등록을 빠뜨리면 알림함엔 뜨는데 배지엔 안 뜨는 절반짜리가 된다(이 저장소가 이미 여러 번 겪은 "형제 함수 누락" 패턴, SEC-32와 같은 종류) |
