...
| DS-21 | Med | **`.c-screen`이 유령 클래스** — 39곳에 붙어 있는데 기본 규칙이 없고 자식 margin 2줄뿐. 9개 화면은 아예 안 붙어 있어 페이지 리듬이 다르다. **주목할 상관관계: 시각 QA 밖에 있던 `SystemOps`·`SetupWizard`·`NotionConsole`·`LlmConsole` 4화면이 전부 `c-screen` 0건이고 그중 3개는 `EmptyState`도 0건이다** — 아무도 안 본 화면이 규약에서 가장 멀리 떠내려갔다. 검사 공백과 품질 드리프트가 같은 자리에 있다 | `styles/screens.css:307-308`; 4화면 직접 대조 | 재검증 결과 그대로 유효 — 39곳 사용, 9화면 미사용 확인(정확한 목록 재확인: Search·Dashboard·Settings·SetupWizard·SystemOps·NotionConsole·LlmConsole·Ops·DevReport). 실제 결과 예시 확인: `SystemOps.jsx`의 최상위 `Callout`이 다른 화면보다 아래 여백이 좁게 렌더됨. 고치는 방법(4~9개 화면에 `className="c-screen"` 한 줄씩)은 정해졌으나 이번 사이클엔 구현 안 함 — 후속 배치의 쉬운 항목으로 남김 |
| DS-22 | Low | **`ui/Pager.jsx`가 `Activity.jsx`에 복붙**됐고 동작이 갈라졌다(원본은 1페이지에서 `null`, 복사본은 항상 렌더) | `Activity.jsx:175-189` vs `ui/Pager.jsx:18-24` | 재검증 결과 그대로 유효하나 원 서술보다 조금 더 복잡함 — `Activity.jsx`는 `total==null`일 때 `Pager.jsx`에 없는 폴백 표시("{page}페이지", `items.length<PAGE_SIZE`로 다음 페이지 추정)를 이미 갖고 있어, 단순 교체가 아니라 `Pager.jsx`가 그 경우를 먼저 지원해야 함. 이번 사이클엔 구현 안 함 |
