...
| ID | 심각 | 문제 | 근거 | 상태 |
|---|---|---|---|---|
| PA-01 | **High** | **글자 크기 스케일이 선언만 되고 소비되지 않는다**(`PA-RC-0001`). `tokens.css`가 6단계를 정의하는데 화면을 실제로 그리는 MUI `sx`/`styled` 층에서 이 토큰을 읽는 곳이 **0곳**이라, 비테스트 소스에 `fontSize` 리터럴이 **31종·278회** 쌓여 본문이 11~17px의 1px 연속체가 됐다. 같은 구조가 `RADIUS`(참조 2회 vs 표현 27종·108회)에서도 반복된다. **본문 크기 SSOT가 둘로 갈려 서로 다른 값을 주장한다** — `tokens.css:243` "body 15px" vs `theme.js:271` `body1: 14px`. 처방은 규율 요구가 아니라 **간격(`p: 2`)과 같은 수준의 일급 타이포 API를 주는 것**이다(PA-F-013 대조 실험: 같은 `sx` 층에서 간격 준수율 96% vs 글자 0%) | `frontend/src/styles/tokens.css:243-253` · `frontend/src/ui/theme.js:37,271` · `var/product-audit/design_scan.json` · 스캐너 `scan_design.py` | **발견** — 재양자화 확정값은 Findings `RD-1`~`RD-3`. `DS-05`(굵기)와 **같은 배치**로 처리할 것 |
