...
| PA-07 | Med | **서버 렌더 로그인 화면이 다크 모드를 전혀 따르지 않는다**(`PA-RC-0010`). Chromium 151로 `prefers-color-scheme: dark`를 주고 실측하니 `body` 배경·글자색이 light와 **완전히 동일**했고 `<html data-theme>`는 `null`이었다. 원인은 값이 아니라 **활성화 경로의 부재**다 — `app/static/css/tokens.css:154`에 `[data-theme="dark"]` 블록이 실제로 있는데 `app/templates_html/**` 전체에 `data-theme`·`prefers-color-scheme`가 **0건**이라 켜는 주체가 없다. 그 파일의 다크 토큰 20여 개는 자신을 읽는 유일한 화면에서 영원히 활성화되지 않는다 | 실브라우저 측정 `var/product-audit/probe_login2.py` · `app/static/css/tokens.css:154` · `app/templates_html/**` grep 0건 | **발견** — `DS-18`(정적 사본 값 동기화)과 **같은 배치**로 처리할 것(값만 맞추고 안 켜지면 무의미, 켜기만 하고 값이 낡으면 이상하게 보인다). **§3-6이 inline script를 금지**하므로 `<html>`에 테마를 심는 인라인 스크립트 방식은 착수 전 제약 확인 필요 |
