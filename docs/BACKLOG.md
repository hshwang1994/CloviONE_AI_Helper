...
| PA3-06 | Med | **관리자 상세 3종이 없는 레코드에 침묵한다**(`PA-RC-0033`). `/users/:id`·`/departments/:id`·`/audit/:id` 가 없는 UUID 로 들어와도 목록만 그린다(주소는 그 레코드를 계속 가리킨다). 감사 로그가 특히 나쁘다 — 사건을 특정해 공유하는 링크인데 항목이 없으면 8,138자 전체 목록이 뜬다. **사용자 콘솔 5종은 같은 상황에서 옳게 답한다** | `PA-RC-0033` · `PA-F-092` · `pa2_badid.py` 9라우트 실측 | ⬜ 미착수 |
