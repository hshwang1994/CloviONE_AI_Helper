...
| 실제 기능 실행 | `F` | **~** | 승인·복구 리허설·저장된 뷰·오프보딩·공지 배너·AI 쿼터·AI 대화·중첩 모달 실행(2026-08-11 갱신 — 휴지통은 기존 `trash_e2e.py`가 빈 상태만 확인했고 실제 왕복은 아직이라 이 목록에서 뺌). 문서 생성·스케줄은 D-21/DGEN-02로 보류 |
...
| 화면 간 반영 | `L` | **~** | 복구 리허설 3단계 확인 + `WF2` 무효화 8건 조사 + `WF7-L01`(승인→users/integrations/runners/schedules/documents 5화면 무효화, 2026-08-11). 전수는 아님 — `CROSS_SCREEN_KEYS`에 등록된 6개 매핑(notifications/organizations·departments/jobs/announcements/schedules/approvals)은 전부 시험이 있지만, 그 지도 자체가 전체 화면 쌍을 놓쳤을 가능성은 남아 있다 |
| 반응형·Theme | `V` | **O** | 768/1024/1200/1366/1920/3840 × 라이트·다크 |
| **실사용 이력** | `U` | **~** | 12기능 중 8 확인(전부 정상 — 저장된 뷰·대리 보기·복구 리허설·승인·메일·오프보딩·공지 배너·AI 쿼터). 3기능(문서 생성·스케줄·주간 리포트)은 D-21/DGEN-02로 실행 보류(원인 규명됨). 휴지통 1개만 실제 왕복 미실행(`trash_e2e.py`는 빈 상태만 확인했다) |
...
**남은 큰 공백 3개**
1. `U` — 2026-08-11 WF7 후속으로 오프보딩·공지 배너·AI 쿼터 3종을 로컬 dev 서버에서 실행
   완료(전부 정상, `docs/BACKLOG.md` §OFFB·ANN·QUOTA). 남은 것: 휴지통 왕복 미실행(문서를
   실제로 trash→list→restore까지 왕복시켜야 하는데, 새 문서 생성은 Notion 쓰기라 D-21
   위험이 있고 기존 실문서를 잠시 trash하는 것도 실고객 화면에 순간 영향을 준다 — 아직
   보류) · 문서 생성(`DGEN-02`, 이 설치에 적합한 워크플로 자체가 없다) · 스케줄·주간 리포트
...
3. `L` — 전수 무효화 매트릭스는 없다(`WF2` 가 8건을 표본으로 봤을 뿐).
