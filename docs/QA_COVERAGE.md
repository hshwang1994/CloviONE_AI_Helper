...
| 실제 기능 실행 | `F` | **~** | 승인·복구 리허설·저장된 뷰·휴지통·AI 대화·중첩 모달 실행. 미실행 5기능 남음 |
...
| **실사용 이력** | `U` | **~** | 12기능 중 6 확인(4정상·2보류). 5기능 미실행 |
| **텍스트 대비** | `K` | **~** | 8화면 실측. **그라디언트 영역은 판정 불가**(화면당 34~50요소) |
| **키보드·포커스** | `B` | **O** | 4화면 × 45탭 + 합성 대비 계산 |
| **접근성 시맨틱** | `S2` | **O** | 8화면. **강점** — 이름 없는 버튼 0/125 · `th[scope]` 100% |

**남은 큰 공백 3개**
1. `U` — 미실행 5기능(오프보딩·메일·AI 상한·공지 배너·주간 리포트). 셋은 외부 부작용이 있어
   [D-21](DECISIONS.md) 확인 후에만.
2. `K` — 그라디언트 배경 위 텍스트(상단바 전체)는 **아직 아무도 재지 못했다**.
