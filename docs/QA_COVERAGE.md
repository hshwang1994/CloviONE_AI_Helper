...
|---|---|---|---|
| `T1` 타이포 토큰 소비 | 선언된 글자 크기/굵기/radius 토큰이 **실제로 소비되는가**(리터럴 잔존 수) | `S`(화면)·`K`(대비) 축은 "보기에 이상한가"만 본다. 스케일이 1px 연속체로 번져도 스크린샷은 통과한다 — 그래서 `fontSize` 리터럴 31종·278회가 **VIS 162건을 거치고도** 안 잡혔다(`PA-RC-0001`) | **부분 검증** — 토큰 자체(`FONT_SIZE`/`FONT_WEIGHT`/`RADIUS`, RD-1~RD-3 확정 단계 수)는 `theme-baseline.test.js`가 고정. 소비 측은 `kit.jsx`(23곳)만 확인됨 — 나머지 약 270여 곳은 아직 리터럴(`docs/BACKLOG.md` PA-01이 추적). 실사용 리터럴 잔존 수 재측정(`scan_design.py`)은 전면 마이그레이션 후에나 유의미 |
| `T2` 오류 문구 3요소 | 실패 문구가 `[무엇이 실패]·[왜]·[무엇을 하라]`를 주는가 | 화면별 시각·기능 축은 있으나 **문구 축이 없다.** 실패 문구 167건 중 회복 경로를 주는 것이 24건(14%)인 상태로 전 화면이 QA를 통과했다(`PA-RC-0002`) | **대부분 검증완료** — `docs/UX_WRITING.md`에 오류 2문장 패턴(실패+회복 절) 확정. 공용 진입점 `lib/api.js` 수정(회귀 3건) 뒤, 5개 병렬 에이전트 + 직접 4라운드 추가 발견으로 막다른 길을 141건→63건까지 줄임([무엇을 하라] 있음 26건(15%)→109건(63%)). 남은 63건은 개별 검토로 대부분 스캐너 오탐 확인(인접 액션 버튼·confirm 질문·서술문·성공 메시지 — 실패 통지가 아님, `docs/BACKLOG.md` PA-02 상세). 전체 `npm test`(254파일·1,755건) 매 라운드 green. 동사표 위반 검사는 미착수 |
...
| 축 | 상태 변화 | 근거 |
|---|---|---|
| `T1` 타이포 토큰 소비 | **미검증 → 부분 충족** | `theme.js`에 `FONT_SIZE` 6단계 신설, 타이포 `fontSize` 259회 중 **206회가 토큰 참조**. 남은 것은 6단계 밖 사이값 약 30~40회 |
| `T2` 오류 문구 3요소 | **미검증 → 부분 충족** | `docs/UX_WRITING.md` 신설, 회복 절 14%→63%(오탐 제외 시 약 72%). 기준 90% 미달이라 축은 아직 안 닫힘 |
| `T3` 용어 표준 준수 | **미검증 → 검사 도입됨** | `static_checks.sh`에 표준 동사표·쉼표접속 린트 추가 |
| `T4` 저장소 위생 | **미검증 유지** | `stash@{0}` 여전히 존재. 검사 자체가 아직 없다 — `PA-RC-0003`이 이 축을 겨눈다 |
