...
|---|---|---|---|---|---|
| `/api/assistant/weekly-digest` | **200**(당시) | **200**(당시) | 200 | 200 | ⚠️ **문서 갱신(2026-08-17)** — 이 표는 2026-08-08 캡처다. `BACKLOG UA-01`이 2026-08-10 `visible_user_ids` 스코프 필터를 배선해 **구현완료**(현재 소스 `app/assistant/facts.py:123` 직접 확인) — 응답은 여전히 200이지만 이제 호출자 범위로 좁혀진 데이터를 준다(게이트가 없던 것과 다름). 이 서버는 조직 1개·부모-자식 부서뿐이라 "범위 밖 사람이 실제로 안 보인다"를 증명할 고립 인구가 없어 **실서버 재검증은 여전히 미완**(합성 two-org 시험으로만 확인, `test_org_axis.py`) — 표의 200 자체를 다시 찍어 봐야 근본적으로 다시 증명되지 않는다 |
| `/api/sprint/summary` | **200**(당시) | **200**(당시) | 200 | 200 | ⚠️ **문서 갱신(2026-08-17)** — 위와 같음. `BACKLOG UA-02`가 2026-08-10 `sprints/service.py::_visible_ids`·`tickets/service.py::drop_out_of_scope_dtos`의 `is_dept` 조건을 없애 **구현완료**(현재 소스 확인) — 실서버 재검증 미완 사유도 UA-01과 동일 |
...
### 이 표가 드러낸 가장 중요한 것 (2026-08-08 캡처 당시)
**제품에 정책이 있는데 옆문이 그 정책을 무시했다.** `dev-monthly`는 같은 전사 집계를
`SENSITIVE_READ_ROLES`로 막아 **operator조차 403**인데, `weekly-digest`는 **아무 게이트가 없어
평범한 `user`에게도 같은 `team` 합계와 이름 붙은 상위 기여자를 줬다.** 즉 이것은 "게이트를
깜빡했다"가 아니라 **명시적으로 정한 기밀 등급을 다른 경로가 무효화하는** 상태였다.

나머지는 전부 설계대로다 — `auditor`가 `jobs`에서 빠지고 `impersonation/sessions`에 들어가는
비대칭까지 코드 의도와 일치한다. **RBAC 뼈대는 건강했고, 구멍이던 위 두 개(UA-01/UA-02)도
2026-08-10 코드 레벨로 닫혔다** — 위 표의 갱신 주석 참고. 이 서버 토폴로지(단일 조직) 때문에
"200이 이제 좁혀진 데이터인지"를 이 매트릭스 방식(역할별 status code만 비교)으로는 앞으로도
증명할 수 없다 — 재검증하려면 최소 2개 조직의 격리된 데이터가 있는 환경이 필요하다(합성
시험은 이미 있음, `tests/security/test_org_axis.py`).
...
| `contrast`(WCAG AA) | 408페이지 중 343개(**68/68 라우트**)가 최소 1건 fail. 표본 519건 | ✅ [QAH-03](BACKLOG.md)/[QAH-05](BACKLOG.md)/[QAH-07](BACKLOG.md) 전부 구현완료 — 표본 90%(466/519)를 차지하는 두 지배적 패턴(`a.MuiButtonBase-root`·`span.MuiBox-root`)은 공유 theme 수준에서 수정, 나머지 소수 패턴(53/519)도 수정 완료(`button.password-toggle`(로그인 화면, 6건)만 승인된 디자인 베이스라인 고정 계약으로 의도적 보류). **정정 이력**: 2026-08-13에 이 표본(라우트당 5건 상한, 408페이지)이 하네스 라우트 목록에 없는 게임방(`GM-01`)·`Board.jsx`의 raw `primary.main` 사용 7곳을 놓친 것을 발견해 `QAH-05`로 등록했는데, **같은 날 실측 대신 4프리셋×2모드×실제 렌더 배경 전수 계산으로 수정까지 완료**됐다(수정 중 2곳 추가 발견, 총 9곳). 그 작업 중 game-room/Board **밖**에서도 같은 패턴이 15개 파일에 더 있는 것을 발견해 `QAH-07`로 등록했고, 그것도 같은 날 전수 재검증(5곳만 실제 미달로 확인돼 수정, 10곳은 이미 통과)으로 마무리됐다. 이 축은 더 이상 실측 대비값 부재 상태가 아니다(계산 기반 시험 `theme-link-contrast.test.js`가 QAH-03/05/07 전부 합쳐 고정) |
...
| `T12` 관리자 상세 딥링크 | 관리자 콘솔 상세가 **직접 URL 진입·새로고침·뒤로가기**를 지원하는가 | 사용자 콘솔은 `:id` 라우트 6개로 이미 지원하는데 관리자 상세는 URL 없는 모달이라 전부 안 됐다 — 모르는 상세 id는 설명 없이 대시보드로 튕겼다(`PA-RC-0024`, `PA-F-066`/`072`) | **검증완료(2026-08-16)** — `AdminRoutes.jsx`에 `/users/:id`·`/departments/:id`·`/audit/:id`(감사는 단건 조회 자체를 신설) 배선, 미등록 경로는 대시보드 대신 `ErrorState`(사용자 콘솔과 공유). `UserRoutes.jsx`도 "관리자 전용"과 "진짜 모름"을 구분해 권한거부/not-found로 응답(예전엔 둘 다 `/me`로 조용히 이동). 라이브 검증(`var/product-audit/verify_pa_rc_0024.py`, 실제 데이터)이 로컬 목 시험은 못 잡은 결함(부서 상세 단건 응답 봉투 처리 누락)을 실측으로 잡아 배포 전 수정 — **"모의 응답의 정확성 자체가 검증되지 않으면 그 위 시험이 헛것일 수 있다"**는 사례로 기록 |
...
| 14 | **화면 역할 게이트가 능력 표(`rbac_matrix`)와 일치하는가** | F축을 지금까지 *"프런트가 숨긴 것을 백엔드도 막는가"*(느슨한 방향)로만 봤다. **공표한 것보다 더 막는 반대 방향**은 검사한 적이 없어 `/diagnostics`가 지금까지 안 보였다 | Blind pass 2에서 `operator`로 화면을 돌다가 발견 → 프런트·백엔드 소스 대조 |
| 15 | **장애 알림에서 진단 화면까지의 경로** | 배너가 증상을 말하는지만 봤고 **거기서 다음 화면으로 갈 수 있는지**는 축이 없었다 | Blind pass 2 — 배너 `links: []` 확인 |
...
## T10~T16 — Product Audit Cycle `PA-20260817-072224-24b91505` 가 드러낸 검증 공백

> 이 Cycle 의 Root Cause 13건 중 **8건**이 「화면이 성공적으로 렌더되는데 내용이 틀린」 유형이다.
> 기존 검증 축(HTTP 4xx/5xx · console error · `horizontal_overflow` · 테스트 통과)은 그것을
> 전부 통과시킨다. 아래는 그래서 필요한 축이다.

| 축 | 무엇을 보는가 | 왜 없으면 못 잡는가 | 관련 |
|---|---|---|---|
| `T10` | **쓰기 경합에서 각 엔드포인트가 어떻게 답하는가** — 공용 재시도 관용을 쓰지 않는 쓰기 경로를 기계적으로 열거 | 지금은 알림 경로 하나에만 경합 테스트가 있다. 인증 관문과 AI 대화가 무방비인 것을 아무 검사도 알려주지 않았다 | `PA-RC-0032` |
| `T11` | **「모른다」와 「0」의 구분** — 미매핑/소스 장애 상태에서 각 화면의 숫자·문장이 서로 같은 말을 하는가 | 세 화면(`/me`·`/my-stats`·`/dashboard`)이 같은 상태에서 다른 말을 하는지 비교하는 검증이 없었다 | `PA-RC-0027` |
| `T12` | **주소와 화면이 같은 것을 말하는가** — 딥링크·탭·상세 id 진입 후 주소와 렌더 내용의 일치 | 잘못된 화면이 **성공적으로** 열리는 것은 4xx/5xx·console error 검사를 전부 통과한다 | `PA-RC-0030`·`0033`·`0039` |
| `T13` | **오류가 확정된 뒤 화면이 얼마나 빨리 말하는가** — 4xx 확정 후 표시까지의 시간 | 기존 검증은 최종적으로 올바른 화면이 뜨는지만 본다. 올바른 답을 30초 걸려 주는 것은 통과한다 | `PA-RC-0034` |
| `T14` | **좁은 폭에서 식별자 열이 읽히는가** — 셀 내부 잘림(절대 위치 자식·스크린리더 라벨 제외) | `horizontal_overflow` 는 표가 **화면을** 넘는지만 본다. 표가 컨테이너 안에 얌전히 들어가 있고 셀 안에서만 잘리면 통과한다 | `PA-RC-0029`·`0037`·`0036` |
| `T15` | **필터로 0건인가 데이터가 0건인가** — 기본 필터를 활성 필터로 세는가 | `useQueryState` 가 기본값을 주소에 안 쓰므로 주소만 보면 필터가 없어 보인다 | `PA-RC-0038` |
| `T16` | **같은 데이터가 여러 화면에서 같은 컴포넌트로 표현되는가** | 개별 화면은 각각 통과하므로 `/users` 의 배지와 `/offboarding` 의 481px 막대가 같은 값이라는 사실이 드러나지 않는다 | `PA-RC-0036`·`0028` |

### 이 Cycle 이 실제로 실행한 검증 (다음 Cycle 이 중복하지 않도록)

| 무엇 | 범위 | 도구 |
|---|---|---|
| 표면 실렌더 + 레이아웃 계측 | **28표면**(관리자 16 · 사용자 12), console error 0 · HTTP 4xx/5xx 0 | `pa2_design.py` |
| RBAC 3계층 대조 | 관리자 라우트 **21** × 역할 **3** = 63조합, 불일치 **0** | `pa2_rbac.py` |
| 없는 레코드 진입 | 상세 라우트 **9** | `pa2_badid.py` |
| 뷰포트·배율 | **7뷰포트 × 5라우트** = 35조합, 가로 넘침 0 | `pa2_resp_dark.py` |
| 라이트/다크 대비 | 5라우트 전 텍스트 노드 WCAG 계산, 실질 실패 **0**(1건은 disabled — WCAG 예외) | `pa2_resp_dark.py` |
| 한국어 문구 | UI 문자열 **1,481개** 전수, S1 1 · S2 0 · D/G/I 0 | `scan_r_axis.py` |
...
## 17. 0060/0061 Resource Ownership — 로컬 Chrome E2E (2026-08-18)

> `DECISIONS.md` D-138/D-139/D-140 · `BACKLOG.md` §IA-RBAC 과 짝이다.
> **TEST SERVER 가 아니라 로컬 uvicorn(`127.0.0.1:8098`) 을 대상으로 했다** — 이유는 §17-4.

### 17-1. 표본을 어떻게 만들었나 (헛것 방지)

운영 데이터 사본(`var/web.sqlite3` → `var/e2e.sqlite3`, 사용자 25 · 티켓 1056 · 문서 104)에
`alembic upgrade head` 를 실제로 돌린 뒤 그 위에 세계를 심었다. 빈 DB 로 하면 마이그레이션의
backfill 이 아무것도 안 하므로 **fail-closed 규모를 못 본다**.

마이그레이션 직후 실측(= 배포하면 그날 벌어질 일):

| 대상 | 결과 |
|---|---|
| `users.membership_kind` | `department` 4 · **`unassigned` 21** |
| `ticket_cache.project_link` | **`unresolved` 1056 / 1056** (`projects` 표가 0행이라 해석 대상이 없다) |
| `document_cache.owner_kind` | **`unset` 104 / 104** (이 DB 에 `ClovirONE팀` 부서가 없어 이관이 no-op) |

그 위에 부서 트리(본부 > 개발팀 > 프런트팀 · 영업팀), 부서별 프로젝트 4 + 조직 공통 1,
티켓 60건 연결, 문서 40건 소속 지정, 계정 6개(조직직속 · 프런트팀원 · 프런트팀 관리자 ·
조직 관리자 · 전체 관리자 · **소속 미지정**)를 심었다.

### 17-2. 무엇을 실제로 눌러 확인했나

| 확인 | 결과 |
|---|---|
| 로그인 착지 | 일반 사용자·관리자 **둘 다 `/#/me`**. 관리자는 사이드바에 `사용자/관리자` 전환이 뜬다 |
| 조회 줄기 | 프런트팀원이 프로젝트 **4건**(본부·개발팀·프런트팀·조직공통) — **영업팀 제외** |
| 부서 필터 | 후보가 `본부 / 본부 › 개발팀 / 본부 › 개발팀 › 프런트팀` (영업팀 없음). `개발팀` 선택 → **총 2건**(자기 ∪ 후손), 주소에 `?dept=` 로 남는다 |
| 요약 타일 일치 | 필터 후 타일도 2로 함께 좁혀진다(고치기 전에는 "전체 4 / 총 2건") |
| 팀 티켓 | 1056건 중 **37건**(내 줄기 프로젝트의 활성 티켓)만 |
| 소속 미지정 계정 | 「소속 미지정 … 조직 데이터가 보이지 않습니다」 경고 + **총 0건**, 401/500 아님 |
| 진단 → 일괄지정 → 회복 | `/integrity` 에서 그 계정을 `조직 직속`으로 지정 → 같은 계정이 **5건**을 본다. 감사 1행(`integrity.assign_membership`) |
| 알림 콘솔 분리 | 사용자 `/notifications`(사이드바가 `내 업무`), 관리자 `/admin-notifications`(사이드바가 `운영`). 제목·빈 상태 문구도 다르다 |
| 개인 결재함 | `/my-approvals` 가 일반 사용자에게 200. 「결재 권한이 없습니다 … 부재 중 위임을 받으면」 |
| 관리자 대시보드 | 개인 업무 섹션 없음(운영·서비스·연동·작업지표·백업만) |
| 관리 범위 vs 내 소속 | `/users` 에서 `관리 범위: 본부 › 개발팀 › 프런트팀`, 목록은 프런트팀 3명만(상위 부서 사람 없음). 사용자 화면에서는 같은 계정이 `내 소속` 을 본다 |
| 계정 메뉴 | `내 소속 굿모닝아이텍 › 본부 › 개발팀 › 프런트팀` 상시 표시 |
| 새 티켓 | `프로젝트 *` 필수, 「프로젝트를 먼저 고르세요…」로 담당자 잠금, 고르면 **공유 범위** 안내 표시 |
| 테마 | 문서 목록 라이트/다크 모두 정상(부서 필터 포함) |

### 17-3. 이 E2E 가 찾은 실결함 4건 (전부 이 자리에서 수정)

| # | 결함 | 왜 시험이 못 잡았나 |
|---|---|---|
| 1 | `GET /api/approvals/mine` **500** (`Approval.created_at` 없는 열) | 그 경로를 부르는 시험이 하나도 없었다. 셸 배지가 부르므로 **로그인한 전원**이 겪는다 |
| 2 | 프로젝트 목록 `부서` 열이 일반 사용자에게 전부 `-` | 이름 맵을 관리자 전용 API 로 받았다 — 관리자 화면에서는 정상이라 안 보였다 |
| 3 | 새 티켓 공유 범위가 부서 프로젝트를 **"조직 전체 공통"이라고 반대로** 안내 | 같은 원인(경로를 화면이 만들기로 돼 있었다) |
| 4 | 부서 필터가 조직 공통을 계속 포함해 **아무것도 안 좁혔다** | 조회 규칙을 그대로 쓴 결과. 시험에는 조직 공통 표본이 없었다 |

전부 회귀 시험을 함께 넣었다: `tests/integration/test_my_approvals.py`(6),
`tests/security/test_department_filter_scope.py`(5), `frontend/.../ticket-share-scope.test.jsx`(4),
`frontend/.../department-filter.test.jsx`(5), `frontend/.../scope-bar.test.jsx`(+2).

### 17-4. 아직 안 한 것 (정직하게)

* **TEST SERVER 배포 뒤 E2E 가 아니다.** 번들은 만들어 올려 스테이징까지 했고
  (1735/1735 해시 일치, `0060`·`0061` 포함) 남은 것은 **sudo 한 줄**뿐이다 — 에이전트가
  sudo 비밀번호를 다룰 수 없어 사람이 실행해야 한다(`docs/DEPLOY_NOW.md`).
* **8뷰포트 반응형 매트릭스**는 이 회차에서 안 돌렸다. 라이트/다크는 봤다.
* **놀이·채팅·게시판·스프린트**는 이번 축(Ownership)의 변경 대상이 아니라 회귀 시험으로만
  확인했고 브라우저로는 안 돌았다.

---

## 2026-08-19 — 승인된 TEST SERVER 배포 후 실환경 검증

대상: `https://clovirone-ai.gooddi.lab` (10.100.64.71) · 배포 커밋 `2e301069` 기반 번들 ·
`UPGRADE_OK` → `DEPLOY_VERIFY_OK`(7/7).

### 화면 × 테마 × 뷰포트 행렬 — 288 캡처, 실패 0

| 실행 | 라우트 | 뷰포트 | 테마 | 결과 |
|---|---|---|---|---|
| `deploy-full` | 72 | 1920x1080 | light·dark | 144 캡처, 치명 실패 0 |
| `deploy-4k-mobile` | 72 | 3840x2160 · 390x844 | light | 144 캡처, 치명 실패 0 |

검사별(두 실행 합산): `auth_ok` 288 · `console_errors` 288 · `page_errors` 288 ·
`horizontal_overflow` 288 · `broken_images` 288 · `duplicate_ids` 288 ·
`vertical_text_collapse` 288 · `fab_overlap` 288 · `image_cropped` 288 ·
`content_clipped` 288 · `rail_wider_than_prose` 288 · `contrast` 288 — **전부 통과**.

**이번에 처음 실제로 돈 검사들**:
* `tiny_text` 72 / `narrow_main` 72 — 3840x2160 에서만 게이트가 열린다. 1920 만 돌리던
  예전 실행은 이 둘이 늘 "건너뜀"이었고 요약만 보면 통과로 읽혔다.
* `modal_*` 7종 × 55개 모달 — `--modals` 없이는 한 번도 안 돌던 축이다. 전부 통과.
* `theme_applied` 는 288 중 2건 건너뜀 = 로그인 화면(라이트 고정, 설계대로).

### 흐름 검증 (Screen → Action → API → Backend → DB → 결과 → 감사 → 원상복구)

| 기능 | 실측 | 상태 |
|---|---|---|
| 저장된 뷰 | 필터 → 뷰 저장 → 새로고침 후 유지 → `GET /api/me/views` 로 DB 확인 → 삭제 → 0건 | ✅ |
| 공지 배너 | 생성 201(비활성) → 목록 노출 → 삭제 → 사라짐 → `announcement.create` 감사 기록 | ✅ |
| AI 사용 상한 | 생성 201 → 목록(사용량 필드 포함) → 사용량 조회 → 삭제 → 사라짐 → 감사 기록 | ✅ |
| TLS 인증서 상태 | 화면의 "만료까지 329일 남았습니다, 자체 서명 인증서입니다" 가 서버 실값과 일치 | ✅ |
| 시스템 설정 값 | 호스트 이름 `ai-n8n-svr` · 타임존 `Asia/Seoul` · 시각 동기화 = 특권 헬퍼 실값 | ✅ |

저장된 뷰·공지·쿼터는 **셋 다 이 설치에서 실행 이력이 0이던 기능**이다(USE-01).
전부 만든 뒤 지웠고, 지운 것까지 확인했다(지시 68).

### 아직 안 한 것

* **사람의 브라우저에서의 대화형 확인** — 이 서버는 자체 서명 인증서라 Chrome 이
  경고 화면에서 멈춘다. 인증서 신뢰 설정은 시스템 보안 설정이라 사람이 해야 한다.
  화면 캡처·검사·흐름 검증은 위 하네스(Playwright, 프로젝트 자산)로 대신했다.
* Notion 을 실제로 쓰는 흐름(문서 휴지통 왕복, 오프보딩)과 역할 변경 승인 흐름은
  실고객 워크스페이스·실계정을 건드리므로 이번 실행에서 제외했다(D-21 과 같은 이유).
