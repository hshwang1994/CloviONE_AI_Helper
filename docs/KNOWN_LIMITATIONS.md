...
- 테스트 스위트는 API/보안/회귀(pytest) + React 컴포넌트 유닛(vitest, `frontend/`) 중심이다.
  유닛 레벨은 커버되지만, HTML+JS UI의 통합 테스트는 페이지 응답과 정적 자원까지만 확인한다.
- **2026-08-12부터 최소 실 브라우저(E2E) 골든 패스가 `pytest -m smoke`로 존재한다**
  (`tests/smoke/` — 로그인→사용자 콘솔 홈→관리자 콘솔 대시보드→문서 목록, 실제 Chromium +
  콘솔 오류·4xx/5xx 네트워크 응답 수집). 기본 `pytest` 실행은 여전히 `-m "not smoke"`로
  제외한다(라이브 서버가 있어야 돌기 때문) — `SMOKE_BASE_URL` 환경변수로 대상을 지정해
  수동/CI 게이트에서 별도 실행한다. **70라우트 전수·역할별·반응형 매트릭스는 여전히
  `scripts/ui_qa/run.py`의 몫이다** — `tests/smoke`는 그 반대 극단(초 단위 최소 게이트)이지
  대체재가 아니다.
