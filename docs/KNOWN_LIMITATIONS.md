...
- 테스트 스위트는 API/보안/회귀 중심이다. HTML+JS UI는 통합 테스트가 페이지 응답과
  정적 자원까지만 확인하며, **실 브라우저(E2E) 렌더링·상호작용 검증은 아직 수행되지
  않았다** (`tests/smoke`는 라이브 인스턴스 필요, 기본 deselect)
- UI 변경 후에는 수동으로 로그인→채팅 왕복→관리자 콘솔 주요 섹션을 확인할 것

## 8. 기타 소소한 제약

- 채팅 응답은 폴링 방식(1~5s backoff) — WebSocket/SSE 아님. 지연 체감은 수 초 이내
- 대화 목록은 최근 100개, 보존 기간 설정(`conversation_retention_days`)은 존재하나
  자동 정리 잡은 아직 없다
- run-now/스케줄 payload는 template 그대로 전달 — 변수 치환(예: `{week}`)은
  workflow(n8n) 쪽 책임
- 감사 로그·알림의 자동 아카이브 미구현 (보존 일수 설정만 존재)
...
- **채팅 화면의 알림 진입점 없음**: 알림 목록 화면은 관리자 콘솔에만 있다
  (`/admin#notifications`). 그래서 상단바의 알림 벨은 관리자 콘솔에 들어갈 수 있는
  역할(operator 이상)에게만 보인다. 일반 사용자에게 개수를 보여주면서 갈 곳을 안 주는 것은
  안 보여주는 것보다 나쁘다고 판단했다. 채팅 쪽 알림 화면이 생기면 그때 모두에게 연다.
- **채팅 사이드바 부가 항목**: Search/Help/Profile 전용 진입점은 미구현
  (현재 대화 목록·새 대화, 상단바에 화면 전환·비밀번호 변경·로그아웃 제공).
