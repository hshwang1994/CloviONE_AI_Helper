...
| **S11** ✅ | n8n · 외부 Runner 제거 | 워크플로 22개 Export 보관 · 러너 소비자 셋을 **Gateway 로 이관**(D-266) · 채팅이 `app/ai/retrieval` 로 답한다 · n8n + 3 runner 정지·**유닛 제거** · 표 넷 + 컬럼 둘 하향(`0009`) · 포트/유닛/백업/테스트/문서 정리 | **완료 (2026-08-23)** — 다섯 포트 **미청취** · `list-unit-files` **0건**(재부팅 안전) · Installer·backup-cron·validate 에 n8n 흔적 **0**. 지운 것이 돌아오지 않게 회귀로 못 박았고(`test_external_automation_removed.py` 9건), 채팅의 새 계약을 시험으로 다시 세웠다(7건 — **모델에게 넘어간 문자열**로 권한을 증명한다). 결정 **D-265~D-268**, 원장 [`EVIDENCE/S11/`](EVIDENCE/S11/README.md) |
...
| **S14** ✅ | **Cutover + Legacy 제거** (단독) | 최종 Backup → Maintenance → 마지막 Delta → PG 전환 → File/Relation/Application 검증 → AI Index → Open → **Notion·SQLite Runtime 차단** → Legacy 코드·문서·Harness 제거 | **완료 (2026-08-24)** — 운영이 PostgreSQL 위에서 돈다(`/healthz` → `ticket_source=native`). 이관 검사 **64건 전부 통과**하고, 적재된 운영 DB 에 **직접 물어** 다시 셌으며(티켓 1,133 · 이름 1,120 · 프로젝트 22/22 코드), 읽기 경로 **16개가 200** 으로 답했다. 🔴 **결정성이 실제로 성립했다**: 서로 데이터를 공유하지 않는 **세 데이터베이스**에서 프로젝트 코드 22건과 티켓 이름 1,133건이 한 글자도 다르지 않다(D-282). 복구 리허설이 **실 데이터로 8단계 전부 통과**했고 복원본을 물고 띄운 앱이 읽기 경로 13개를 전부 200 으로 답했다. Cutover 가 **설치기 결함 다섯과 제품 결함 셋**을 찾아냈고, 런타임을 걷어내는 과정이 **「자체 행이 이류 시민」 결함 여덟**을 더 드러냈다(진행률 재계산이 부르는 사람을 잃은 것 · 자체 프로젝트가 자기 티켓을 못 찾는 것 · 자체 티켓이 검색에서 전역 관리자에게만 보이는 것 · 판에서 안 열리고 못 옮기고 안 버려지는 것 · 프로젝트 상세의 티켓 탭이 목록을 아예 안 부르는 것 · 알림 이름이 폐기한 `GIT-*` 인 것). 그리고 휴지통의 영구 삭제 확인 창이 **되돌릴 수 없는 동작을 「30일 내 복구 가능」이라고** 말하고 있었다. 전부 고쳐서 성질로 못박았다. 옛 slug 운영 스크립트 **아홉**과 옛 유닛·nginx 사본을 걷어냈고(INSTALLATION.md §9 가 S14 로 배정한 것), 그 과정에서 **새 설치기 스냅샷이 사용자 업로드를 안 담는 갭**을 찾아 고쳤다 — 두 벌을 유지하는 동안 옛 스크립트의 시험이 계속 초록이라 안 보였다. 🔴 **완료 보고 뒤 실 화면 재점검에서 결함이 더 나왔다** — 행 수 검증만으로는 이관 품질을 증명하지 못한다는 것이 결론이다. `scripts/audit_migration_fidelity.py` 로 원본 대비 축마다 대조해 **손실 42,321건·17개 축 → 44건·6개 축**(전부 설명됨)까지 줄였고, 그 과정에서 진짜 결함 셋(`data:` URI 이미지, 티켓 속성 첨부가 안 붙던 것, 안전 재이관 경로가 첨부를 아예 안 건드리던 것)과 재배포 뒤 화면 결함 둘(태그 필터 주소 오타, 문서 목록이 분류·태그를 안 실은 것)을 찾아 고치고 운영에 다시 적용·재검증했다. 상세는 [`WORK_STATE.md`](WORK_STATE.md) §8. 결정 **D-282~D-284**, 원장 [`EVIDENCE/S14/`](EVIDENCE/S14/README.md) |
...
| **S16** ✅ | Table / Grid / Metadata / Alignment + Chart | W6 | **완료 (2026-08-25)** — 열이 폭 대신 의미를 말한다(D-288). 네 Table Assertion 이 `--fail-on` 으로 걸린 채 초록이고 `numeric_alignment` 는 8 → 0. 결정 **D-288~D-293** |
| **S17** ✅ | Empty / Loading / Error / Feedback + Clovi + Detail Metadata | W7 | **완료 (2026-08-25)** — 🔴 클로비가 작았던 원인은 «작게 줬다» 가 아니라 **«박스를 줬다»** 였다: 포즈 자산의 세로 잉크 비율이 0.666~0.850 으로 벌어져 같은 `size` 가 자산마다 다른 크기로 보인다(D-294). 이제 호출부가 **자리 이름**만 말하고 박스는 «보이는 크기 / 여백 비율» 로 파생한다 — 상단바의 보이는 캐릭터가 22px → 34px 이 됐고 얼굴이 자는 포즈에서 웃는 포즈로 바뀌었다. 🔴 **그것을 재던 검사도 틀려 있었다**: 프로브가 알파 문턱 8 에 부유 픽셀 제거 없이 재서 `clovi-talking` 의 잉크 비율을 1.000(참값 0.817)으로 봤다 — 그만큼 부풀어 **정말 작은 마스코트가 통과한다**(D-295). 정의를 `mascot-bounds.json` 에 잠그고 생성기·`static_checks` 신선도·Python 회귀 6건으로 고정한 뒤 `mascot_visible_size` 를 `--fail-on` 으로 승격해 **60페이지 초록**을 받았다. 값이 0건이면 차트·표·Pager 가 **실제로 언마운트**되고(D-296, 렌더 시험 11건) 로딩 자리는 차트 «모양» 스켈레톤이 잡는다 — 점선 상자는 폐기했다. 빈 상태 밴드의 순환(분자가 분모를 정하던 것)을 정리했고(D-297), 상세 상단 속성이 «먼저 판단할 값» 과 «참조용 값» 두 층으로 갈렸다(D-298) — 긴 프로젝트명 실데이터로 1920/2560/3072/3840 × 2테마 캡처. 게이트 **UI_RENEWAL_COVERAGE_OK (stage=wave, wave=W7)**. 프런트 시험 **2,439 PASS / 0 FAIL**(P-34 의 셋이 닫혔고 셋 다 화면이 아니라 시험이 틀렸다). 결정 **D-294~D-298** |
