...
| **P-14** | Work Domain — Project Key · Ticket 3층 식별자 · 채번 · Relation · Workflow | S6 | **DONE** | ~~`project_key_registry`(예약 `GIT`)~~(D-282 가 내렸다) · `ticket_cache` → `tickets`(D-238) · `seq`/`canonical_key` + BEFORE 트리거(D-236) · `project_ticket_counters` 채번(D-196) · ~~`ticket_key_aliases`~~(D-283 이 내렸다) · `ticket_relations`(계층 정본, D-239) · `ticket_statuses` + `app/work/workflow.py`. **동시 12건에서 1..12 가 정확히 한 번씩**(반례 포함) · 롤백 시 미소비 · Exception 임의 배정 0. **Project Code 정책은 2026-08-24 에 바뀌었다**(D-282): 서버가 짓고 아무도 못 고친다 |
...
| **P-23** | Migration Tool + Dry Run (Notion + SQLite → 임시 PG) | S13 | **DONE** | `app/migration/` 이 Extract → Transform → Load → Validate → Report 를 진다. 실 운영 SQLite + 실 Notion → 임시 PG 에서 **검사 64건 전부 통과** · **재실행 2회차 신규 0**. 티켓 1,133(번호 1,120 · 예외 13) · 문서 110(본문 103) · 관계 121 · 첨부 6 · `legacy_mapping` 1,372. 🔴 재실행을 실제로 돌려 **표 복사와 재채번이 같은 컬럼의 주인**이던 것을 찾았다(D-275) — `projects.code` 쪽은 오류를 안 내고 조용히 Key 를 지운다. 그리고 「문서 분류가 전부 비어 있다」의 원인이 **Notion 속성 이름**이었음을 찾았다(D-277). 원장 [`EVIDENCE/S13/`](EVIDENCE/S13/README.md) |
...
| **P-33** | **세션 쿠키 이름에 옛 정체성이 남아 있다** (`clovirone_session`) | S14 | **DONE** | `app/core/sessions.py::SESSION_COOKIE_NAME`. 바꾸는 순간 **전원이 로그아웃**되므로 S4 가 건드리지 않았다(D-226). Cutover 는 어차피 세션이 끊기는 자리라 그때 함께 바꾼다. MASTER_PLAN §10-13 「Product-owned Artifact 에 Legacy Identity 잔존 없음」이 이 한 건을 본다. **S14 가 바꿨다**: 이름을 손으로 적지 않고 `app/core/product.py::SLUG` 에서 만든다 — 손으로 적었기 때문에 S4 가 열두 자리를 옮길 때 이 한 줄만 남았다. 브라우저에 남아 있던 옛 정체성 여섯 개도 함께 갈았다 (`clovirone_theme`·`clovirone_accent`·`clovirone_nav_collapsed`·`clovirone_login_welcome`·`clovirone_dismissed_status_notices_v1`, 그리고 관리 콘솔이 **없는 유닛**의 로그를 보라고 안내하던 옛 systemd 이름) |
| **P-24** | **Cutover + Legacy 제거** (단독 Session) | S14 | **DONE** | 운영이 PostgreSQL 위에서 돈다. Notion 런타임 모듈 **열다섯**을 지웠고 런타임 SSRF 목록에 `api.notion.com` 이 없다 — 이관 도구는 자기 목록을 따로 든다. SQLite 런타임 의존 0. Rollback 지점은 [`EVIDENCE/S14/`](EVIDENCE/S14/README.md) 가 적는다. 남은 Notion 이름들(`notion_page_id`·`assignee_notion_ids`·`user_notion_mappings`)은 **데이터**라 지우려면 이전이나 화면과 함께 움직여야 한다 — 무엇이 왜 남았는지는 D-284 가 표로 적었다 |

## Phase E — UI Renewal 재개 (동결 해제)

| ID | 작업 | Session | 원 Wave | 상태 |
|---|---|---|---|---|
| **P-25** | Search/Filter **기능 정확성** — 새 PG Backend 대상 | S15 | W5B (REDEFINE) | **DONE** — 조건 사슬을 목록 일곱 곳에서 실측했고(`tests/regression/test_filter_chain.py` 38회차 · 기대값은 전량 목록에서 직접 셌다) 결함 여섯을 뿌리에서 고쳤다: 담당자 축이 새 계정을 못 가리키던 것(D-285) · 주소에 이미 질의가 있는 목록의 조건이 서버에 안 닿던 것(D-286) · 스프린트의 프로젝트 조건이 언제나 0건이던 것(D-287) · 문서 검색 상자가 주소와 갈라지던 것 · 게시판·결재함이 20건에서 조용히 잘리던 것 · 문서 목록 정렬이 전순서가 아니던 것. 게이트 `--stage wave`(W5B) 통과 |
| **P-26** | Table / Grid / Metadata / Alignment + Chart | S16 | W6 | **DONE** — 열이 폭 대신 **의미**를 말한다(D-288, `columnTypes.js` 타입 열넷). `numeric_alignment` 8건이 0 이 됐고 네 Table 검사가 `--fail-on` 으로 걸린 채 초록이다. 열 제거는 **호출부가 준 질의 계약**으로만 한다(D-289, `resultScope`) — 없으면 아무 열도 안 사라진다. 그 자리 편집의 공통 계약이 섰고(D-290, 배선은 티켓 도메인 회차) 그림 열둘이 각자 답하는 업무 질문을 갖는다(D-291). Brand 두 충돌을 결정으로 닫았다(D-292 `highlight` · D-293 `selected_state`) — `brand_role_coverage` 가 W6 부터 `--fail-on` 이고 초록이다. 🔴 실브라우저가 **`/work-board` 가 아무에게도 안 열리고 있던 것**을 잡았다 |
| **P-27** | Empty / Loading / Error / Feedback + Clovi + Detail Metadata | S17 | W7 | **DONE** — 클로비가 «보이는 크기» 로 말하고 박스는 자산 여백에서 파생한다(D-294) — 상단바의 보이는 캐릭터가 22px → 34px 이 됐고 얼굴이 자는 포즈에서 웃는 포즈로 바뀌었다. 잉크 정의가 틀려 있던 것을 고쳤다(D-295): 프로브가 `clovi-talking` 의 잉크 비율을 **1.000**(참값 0.817)으로 봐서 작은 마스코트가 통과하고 있었다. `mascot_visible_size` 가 W7 부터 `--fail-on` 이고 60페이지 초록이다. 값이 0건이면 차트·표·페이저를 **언마운트**한다(D-296, 렌더 시험 11건) — 점선 상자를 폐기하고 로딩 자리는 차트 «모양» 스켈레톤이 잡는다. 빈 상태 밴드의 순환(분자가 분모를 정하던 것)을 정리했고(D-297), 상세 상단 속성이 «먼저 판단할 값» 과 «참조용 값» 두 층으로 갈렸다(D-298, 긴 프로젝트명 실데이터로 1920/2560/3072/3840 × 2테마 캡처). 게이트 `--stage wave`(W7) 통과. 범위 밖 발견 둘을 P-41 · P-42 로 등록했다 |
...
| **P-34** | **프런트 시험 셋이 이미 빨갛다 — S14 이전부터** | S17 | **DONE** (S17) — 셋 다 **화면이 아니라 시험이 틀렸다**: ① `jsx-comments` 가 `ko-wordbreak.test.jsx` 의 **문자열 리터럴** `"{/*"` 를 열린 주석으로 읽었다(앞 글자가 따옴표면 건너뛴다 — 진짜 JSX 주석 앞에 따옴표가 오는 경우는 없다). ② `SettingVersions.jsx` 의 「그 변경이 새 기록으로」를 「그 사실이 새 기록으로」로 고쳤다(표준 동사표의 «변경» 금지). ③ `settings-coerce` 가 `/검증 통과/` 를 찾고 있었는데 화면 문구는 「검증을 통과했습니다. 저장할 수 있습니다.」다 — 문구가 완전한 문장으로 다듬어진 뒤로 그 조각은 어디에도 없다. 화면은 옳게 동작하고 있었다 | S14 가 `eec4886c` 기준으로 확인한 **범위 밖 Finding** 이고, S14 의 변경이 만든 것이 아니다(변경을 stash 한 채로 같은 셋이 실패한다). ① `src/ui/jsx-comments.test.js` — `ko-wordbreak.test.jsx:182` 의 **문자열 안**에 있는 `{/*` 를 안 닫힌 JSX 주석으로 읽는다(검사의 위양성이지 화면 결함이 아니다). ② `src/ui/ux-writing-verb-table.test.js` — `SettingVersions.jsx:75` 이 표준 동사표가 막는 동의어(「롤백하면」)를 쓴다. ③ `src/screens/settings-coerce.test.jsx` — bool 설정 편집기가 「검증 통과」를 안 그린다. ①②는 한 줄짜리이고 ③은 실제 화면 동작이라 원인을 봐야 한다 |
| **P-35** | **옛 문서 미러 표(`document_cache` 등)를 완전히 내린다** | 플랫폼 축 | TODO | S14 가 문서 정본을 `documents`(Knowledge Domain)로 통합했다 — 검색·홈·부서별 문서 수·정합성 진단이 전부 그쪽을 읽는다(D1). `/team-docs` 화면은 지웠고 주소만 리다이렉트로 남겼다(D2, 옛 알림 딥링크·북마크 보존 목적 — 그 리다이렉트는 이 작업과 무관하게 계속 남는다). **표 자체(`document_cache`·`document_sync_state`, 그리고 `document_favorites`/`document_comments`/`document_recent_views`의 옛 `notion_page_id` 축)는 S14 가 일부러 안 건드렸다** — 화면 통합과 DB 스키마 정리를 한 세션에 묶으면 범위가 너무 커진다. 이 항목은 그 **완전 제거**다: (1) 남은 소비자가 정말 0인지 다시 확인(코드 grep + 운영 실측), (2) `document_favorites`/`document_comments`/`document_recent_views`가 전부 `documents.id` 축으로만 조회되는지 확인(S14 의 alembic `0016_document_axis_to_documents`가 이관은 했으나 옛 칸을 안 지웠을 수 있다), (3) alembic revision 으로 표·칼럼을 DROP, (4) `app/team_docs/**`에 그때도 남아 있는 코드가 있으면 함께 지운다. 완료의 정의: `document_cache` 관련 표·모델·라우터 코드가 저장소에 없고, 회귀가 그대로 통과한다 |
| **P-37** | **담당자 축을 하나로 합친다 — 옛 토큰 → `users.id` 데이터 이전** | 플랫폼 축 | TODO | S15 가 서비스 계층에서 「옛 짝이 있으면 그것, 없으면 자기 id」로 해석해(D-285) 사용자가 겪는 결함은 전부 없앴다. 하지만 `tickets.assignee_notion_ids` 에는 여전히 **두 종류의 토큰**이 섞여 있고, 뒤늦게 누가 verified 매핑을 붙이면 그 사람의 토큰이 바뀌어 이미 쌓인 행이 그 사람의 「내 티켓」에서 빠진다. 완료의 정의: (1) alembic revision 이 `assignee_notion_ids` 의 옛 토큰을 `users.id` 로 옮긴다(매핑이 있는 사람만, 없는 값은 그대로 둔다 — 외부 사람·퇴사자의 흔적이다), (2) `assignee_token` 이 언제나 `users.id` 를 돌려주도록 좁힌다, (3) `user_notion_mappings` 가 담당자 축에서 완전히 빠진다(그 표 자체는 이관 이력이라 P-35 와 함께 판단한다), (4) 회귀가 그대로 통과한다. **되돌리기 어려운 데이터 변경이라 자기 회차를 가져야 한다** |
| **P-39** | **백엔드 회귀 넷이 이미 빨갛다 — S14 가 남겼다** | 플랫폼 축 | TODO | S15 가 확인한 것: **자기 변경을 stash 한 채로 같은 넷이 뜬다.** ① `test_baseline_carries_every_legacy_index.py` — 옛 체인이 만든 인덱스·제약 일부가 모델에 없다(기준선을 모델에서 만들면 그대로 사라지고 오류를 안 낸다). ②③ `test_migration_0025_self_id_refs.py[document_favorites]`·`[document_recent_views]` — `notion_page_id` 칸이 사라졌다. S14 의 `0016_document_axis_to_documents` 가 그 축을 정본 문서로 옮기면서 칸을 내린 것으로 보이는데, 그 시험은 「옛 축을 지키자」는 계약이라 **어느 쪽이 지금 옳은지를 사람이 정해야** 한다(P-35 와 같은 묶음이다). ④ `test_ui_qa_route_registry_completeness.py` — `/team-docs`·`/team-docs/:pageId` 가 QA 하네스에 없다. 앞은 `<Navigate>` 라 `ALIAS_ROUTES` 자리이고 뒤는 실제 컴포넌트다. **하네스 id 와 Surface id 의 경로 표기(`{id}` ↔ `:pageId`)를 함께 맞춰야** 게이트의 C1 과 이 시험이 동시에 초록이 된다 |
| **P-38** | **`static_checks.sh` 가 시험 강도 선언 미비로 빨갛다 — S13·S14 가 남겼다** | 플랫폼 축 | TODO | `check_test_strength.py` 가 기준 `eec4886c`(S2) 대비 **15건**을 든다: 지워진 시험 파일 아홉이 대체 파일을 명명하지 않았고(`teamdoc*.test.jsx` 여덟 + `test_team_docs_record_view_race.py`), 단언이 줄어든 여섯이 사유를 안 적었다(`modal-dirty-guard-vis88` · `theme-link-contrast` · security 넷). S15 가 확인한 것: **자기 변경을 stash 한 채로 같은 열다섯이 그대로 뜬다** — S15 가 만든 한 건은 그 자리에서 선언했다. 고치는 방법은 둘뿐이고 **둘 다 사실이어야 한다**: 대체 파일이 정말 그 계약을 지키면 그 파일 머리에 `qa-contract-replaced-by:` 를, 계약이 바뀐 것이면 `qa-contract-change:` 로 **왜** 바뀌었는지를 적는다. 검사를 달래려고 아무 파일이나 명명하면 그 순간 이 검사는 뜻을 잃는다 |
| **P-40** | **중립 램프의 divider 가 Brand 색으로 세어진다** | S22 (W15) | TODO | `scripts/ui_qa/brand.py` 의 판정 상수로 재면 `divider #DCDFEC`(H228.8 · S0.296 · 폭0.063 · L0.894)와 `dividerStrong #BCC2D9` 가 **Brand 로 통과한다.** 새 중립 램프가 인디고 계열이라 색상각과 채도는 통과하고, 「아주 밝은 색은 채널 폭이 넉넉할 때만 유채색으로 센다」는 보정이 `EXTREME_L_HIGH = 0.90` 에서 끊기는데 divider 는 0.894 로 **간발의 차로 그 아래**다. 그래서 어떤 role 이든 색이 divider 이면 「브랜드다」로 세어진다. S16 이 캡처한 화면에서는 실제로 그 경로로 통과한 role 이 없었지만(전부 다른 색으로 판정), 문턱이 절벽이라 다음에 누가 divider 로 칠하면 조용히 통과한다. 소유는 `brand_presence` 를 완료 조건으로 갖는 **S22** 다 — 그 문턱을 만지면 `ai_surface`(brandTint `#E9ECFA` · 폭0.067)가 함께 움직이므로 AI 면이 실재하는 화면을 찍을 수 있는 회차가 해야 한다. 완료의 정의: 문턱이 절벽이 아니라 명도에 따라 연속으로 오르고, 반례(divider 는 실패 · rail `#A9BAFF` 와 focusRing `#C3CEFF` 는 통과 · brandTint 는 사람이 정한 대로)가 `probe_selftest.py` 에 있다 |
| **P-36** | **`/knowledge`의 삭제(archived)를 되살릴 방법이 없다 — 휴지통이 두 갈래로 남았다** | 플랫폼 축 | TODO | S14 가 문서 정본을 `/knowledge`로 통합하면서(P-35 와 같은 정리의 일부) 옛 `/team-docs`의 진짜 휴지통(복구 가능한 soft-trash)을 새 화면에 못 접었다 — `/knowledge`의 "삭제"는 `documents.archived = true` 뿐이고 그것을 되돌리는 un-archive API 가 없다. 기능 손실은 아니다: `/team-docs/trash`(티켓 휴지통과 공유하는 화면)가 그대로 남아 있고 `Knowledge.jsx`에 "휴지통" 버튼으로 입구가 있다. 다만 삭제 개념이 두 표(`documents.archived` 와 `document_cache` 기반 trash)로 갈라진 채다 — `documents` 에서 archived 처리해도 team_docs 관점의 휴지통에는 안 나타난다. P-35(옛 표 제거)와 **함께 처리해야** 한다: un-archive API + `/knowledge` 안의 통합 휴지통 UI 를 만들거나, 최소한 두 삭제 개념의 관계를 문서로 명시한다 |
| **P-41** | **Notion 이 화면에는 아직 살아 있다 — 런타임은 S14 가 걷었다** | S20 (W11+W12) | TODO | S17 이 담당자 후보 안내에서 「Notion에 연결된 사용자 없음」이라는 **틀린 원인**을 찾아 고치는 과정에서 드러난 것이다. 런타임은 없는데 화면에는 남아 있다: 사이드바 항목 「Notion 사용자 연결」(`navConfig.js`)과 그 화면 전체(`registry/org.js` 의 `notion-mapping` — 열·상세칸·폼·「연결 해제」 동작), `/notion-mapping` 라우트(QA 하네스에도 Surface 로 등록돼 있다), `Profile.jsx` 의 「Notion 연결」 행, `Offboarding.jsx` 의 「Notion 연결」 열, `SetupWizard.jsx` 의 「Notion 연결 화면 열기」, `format.js`·`registry/shared.js` 의 이름표들. **먼저 답해야 하는 것은 백엔드다**: `/api/admin/notion-mapping` 이 아직 서 있는지, `user_notion_mappings` 표가 담당자 축에서 정말 빠졌는지(P-37 과 같은 묶음). 지우는 것과 남기는 것을 함께 정해야 한다 — 이관 이력으로서의 표는 남기고 **화면만** 내리는 답도 가능하다. 소유가 S20 인 이유: 사이드바·registry·Surface 목록을 한 번에 만지는 일이라 Admin IA 회차가 아니면 반쪽만 지워진다 |
| **P-42** | **`/dev-report` 의 판이 자기 내용을 못 채운다** | S20 (W11+W12) | TODO | S17 캡처가 처음으로 이 화면을 찍었고 `oversized_empty_surface` 가 light/dark × 1920/1366 **네 장 전부에서 2건**을 든다: ① 도넛 + 범례가 든 판이 `1606×275` 인데 잉크가 17%(내용 폭 비 0.36) — 그림이 왼쪽에 몰리고 오른쪽 절반이 빈다. ② 기간 선택 줄이 `1606×68` 에 잉크 18%(내용 폭 비 0.26). 데이터가 없어서가 아니다(「24건 · 완료 6건(25%)」이 그려져 있다) — **판의 폭을 내용이 못 쓰는** 배치 문제이고 지시 0-10 이 「이유 없는 대형 Blank」로 지목한 형태다. S17 범위(공유 부품)가 아니라 그 화면의 배치라 화면 소유 회차가 닫는다. 완료의 정의: 그 두 자리의 `oversized_empty_surface` 가 0 이 되거나, 억제라면 `QA_SUPPRESSIONS.md` 에 만료일과 근거가 있다 |
| **P-43** | **값이 0 이면 그 화면의 판독값이 회색이 된다 — 실데이터에서만 보인다** | S22 (W15) | TODO | S17 배포 뒤 **운영 서버 스모크**가 처음 잡았다(`dist/ui-qa/s17-deployed-smoke/`). `/me` 의 `brand_role_coverage` 가 «회색인 자리: highlight» 로 실패한다 — 판독값이 `#161A2C`(본문 잉크)다. 원인은 규칙 둘이 만나는 자리다: ① `MetricStrip` 은 **값이 0 이면 판독 슬롯을 안 준다**(W4 — 「0 오늘 마감」이 40px 을 차지하는 것은 위계가 반대인 것이다) ② 프로브는 판독 줄에서 **가장 큰 글자**를 그 화면의 지배값으로 읽는다. 그래서 오늘 할 일이 없는 날의 홈은 「지배값이 회색이다」로 세어진다. 임시 데이터베이스로 찍는 로컬 캡처에서는 값이 전부 0 이 아니라 이 조합이 안 나온다 — **배포하고 실데이터를 봐야 보이는 종류**다. S17 이 만든 것이 아니다(S17 은 `MetricStrip` 의 판독 슬롯 규칙을 안 건드렸다). 고르는 답은 셋 중 하나다: (a) 슬롯을 못 받은 줄은 `data-brand-role` 을 아예 안 내보내고 프로브가 `unknown` 으로 읽게 한다 (b) 「모든 칸이 0」인 판독 줄은 그 사실을 말하는 다른 표현으로 바꾼다(`emptyCause` 와 같은 부류) (c) 홈이 0 이 아닌 지배값을 갖도록 지표 자체를 다시 고른다. 소유가 S22 인 이유: `brand_presence`(절대)를 완료 조건으로 갖는 회차이고, 그 회차가 실데이터 위에서 전량 캡처를 한 번 돌린다. 완료의 정의: 「할 일이 없는 날」의 홈이 실패가 아니고, 그 판정이 반례(정말 회색인 판독값은 여전히 실패)로 확인된다 |
...
(20개 Project Key 는 2026-08-24 에 **정책 자체가 폐기돼** 목록에서 빠졌다 — D-282.)
상세는 [`MASTER_PLAN.md`](MASTER_PLAN.md) §13.

| 항목 | 필요 시점 | 없을 때 |
|---|---|---|
| ~~20개 Project Key 명명~~ | ~~S6~~ | **대상 소멸 (2026-08-24)** — 코드는 서버가 짓는다(D-282). 확정표·`PROJECT_KEYS.md`·`apply_confirmed()` 는 전부 폐기했다 |
