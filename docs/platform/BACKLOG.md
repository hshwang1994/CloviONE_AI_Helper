...
| **P-10** | Product Identity · Hostname · TLS | S3 | **DONE** | CN/SAN 이 `clovirassist.gooddi.lab` 로 일치하고 `ssl_verify_result=0` 이다 — **반례도 함께 남겼다**(기준점 없이 18 · 이름 다르면 1). `DEFAULT_VERIFY = True` 로 켰고, 자체서명이라 켜는 것만으로는 부족하다는 것이 이 세션의 발견이다(D-223): 신뢰 기준점을 `UI_QA_TLS_CA` 로 준다. **브라우저 프로브만 남았다** — Chromium 은 운영체제 신뢰 저장소를 보므로 인증서를 실행 머신에 넣어야 한다(아래 P-10a) |
| **P-10a** | **브라우저 프로브의 신뢰 기준점을 실행 머신에 넣는다** | 운영/실행 환경 | TODO | Playwright 의 `new_context()` 에는 신뢰 기준점 인자가 없다(D-223). 그래서 브라우저 288페이지는 인증서를 **실행 머신의 신뢰 저장소**에 넣기 전까지 `--insecure` 로만 돈다 — 그동안 호스트명 불일치를 그 레그는 구조적으로 못 본다. Windows 는 CurrentUser\Root 추가에 대화 상자를 요구해 비대화형 세션에서 못 넣는다. 사람이 한 번 실행하면 끝난다: `Import-Certificate -FilePath dist\ops\clovirassist.gooddi.lab.crt -CertStoreLocation Cert:\CurrentUser\Root`. 서버에서 돌린다면 `libnss3-tools` + `~/.pki/nssdb` |
