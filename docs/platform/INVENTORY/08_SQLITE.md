...
| 10 | 문자열 날짜 컬럼 | **S7 이 닫았다** (P-14a · D-248) — 16컬럼을 한 번에 옮겼다: 달력일 11(`date`) · 시각 5(`timestamp`). 경계는 `app/core/dates.py` 하나 | **완료 (2026-08-22)** — 화면 계약(ISO 문자열)은 그대로다. 이 전환이 조용한 결함 둘을 드러냈다: `_last_activity_on` 의 `isinstance(str)` 갈래와 `projects/sync.py::_apply` 의 값 비교 |
