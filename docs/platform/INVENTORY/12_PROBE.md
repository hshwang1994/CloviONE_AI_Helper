...
| 미확인 | Owner | 내용 |
|---|---|---|
| 위 **8건** 수정 | **S1** (P-01) | 각 수정마다 **양방향 반례**를 `probe_selftest` 에 추가한다. 8번(TLS)은 S1 이 코드를 준비하고 **S3 에서 켠다** |
| 21 checker + 26 ui_qa 모듈의 self-test 보유 여부 전수 표시 | **각 Owner Session** (필요 시) | **S1 은 고치는 8건만 본다.** 나머지를 전수 표로 만드는 것은 S1 필수 작업이 아니다 |
