...
3. 각 파일은 `측정` 절과 **`완성도` 절**을 갖는다. `완성도` 는 **S1 이 무엇을 마저 해야 하는지**를
   지목한다 — S0 는 Plan Mode 실측치를 옮겨 적었을 뿐 재조사하지 않았다.
4. **해결되지 않은 불일치는 지우지 않고 표시한다.** 지금 하나 있다 — `11_TEST.md`.
...
| 3 | [`03_COMPONENT.md`](03_COMPONENT.md) | 공유 UI 부품과 소비처 | 부분 — 목록 열거는 S1 |
| 4 | [`04_API.md`](04_API.md) | FastAPI Route / Router | 개수 확보, **경로별 열거는 S1** |
| 5 | [`05_DB.md`](05_DB.md) | 현행 SQLite 스키마 · 데이터 규모 · 품질 | 확보 (가장 두꺼운 실측) |
| 6 | [`06_DEPENDENCY.md`](06_DEPENDENCY.md) | Python / Node 의존과 신규 도입 후보 | 부분 — 전량 열거는 S1 |
...
| 11 | [`11_TEST.md`](11_TEST.md) | Backend / Frontend / Runner 테스트 | **불일치 1건 미해결 — S1** |
