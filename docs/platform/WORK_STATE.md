...
- s1~s10_commit: `95a89189` · `eec4886c` · `e88e4de3` · `6909bb96` · `aa9c8c63` ·
  `971a31fa` · `87dd8707` · `0240c661` · `ba68ef2c` · `e3953192`
- working_tree: clean
...
| 대상 | 결과 |
|---|---|
| 백엔드 전 회귀 (PostgreSQL) | `run_full_regression.sh` 완주. ⚠️ **`pytest tests` 를 백그라운드로 직접 돌리면 안 된다**(P-09e). 그리고 **파이프 뒤에서 결과를 읽지 마라** — 이번에도 첫 회차가 `\| tail` 뒤에서 조용히 죽어 결과를 한 줄도 안 남겼다 |
