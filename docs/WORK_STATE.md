...
다음은 VIS-81(알림 딥링크가 서버가 이미 계산해 주는 `related_route`를 안 쓰고 필터 없는
전체 목록으로 감)로 이어간다 — 자기완결적이고 사용자 가치가 크다. 그 다음은
VIS-107R/108R/109R(장애 카운터에 시간창이 없고 재시도/알림 워크플로 자체가 없음)·VIS-90
(빈 상태 품질 불균일, D-22가 이미 정답 패턴을 지목함) 순으로 계속한다. 사용자 확인 대기 없이
진행한다.
