...
**보류(4건, 사유 BACKLOG.md에 각각 기록)**: `FN-41`(부서 범위 필터가 SQL 페이지네이션
뒤에 걸림 — `doc_in_scope`의 기존 N+1과 얽혀 있어 리팩터 동반 필요) · `FN-42`(Health
점수의 "낮은 신뢰도" 노출은 스키마 마이그레이션 또는 `compute_health` 계약 변경이 필요,
16개 테스트가 현재 계약을 정밀 고정) · `FN-12`(배지가 폴링을 새로 안 만든다는 명시적
설계 원칙 위에 있어, "제대로" 고치려면 그 트레이드오프를 되돌려야 함) · 나머지
"기존 백엔드에 UI만 없음" 부류(FN-03/05/06/09/10/11/14, SEC-04/05/10/11)는 넓은
조사에서 식별만 하고 아직 손대지 않음(FN-01/04가 이 부류의 실제 착수 사례 — 다음
사이클에서 계속 가능).

**검증**: 신규 회귀 테스트 다수(러너 영향 없음, 백엔드 통합·보안 다수, 프런트 vitest 8)
전부 revert-to-verify. 백엔드 전체(~2659건)·프런트 vitest(201파일/1337건)·러너(287건)
전부 green, `STATIC_CHECKS_OK`. 커밋 `b563eec`(SEC-31/12/13/02/03·FN-02/07) →
`28ce70a`(FN-04, 프런트 전용 후속).

**배포 — Blocker(외부)**: 위 §D-54 이후 배너에 기록된 대로, 사용자가 채팅에 평문
SSH/sudo 비밀번호를 제공하며 비대화형 자동 배포를 지시했으나 CLAUDE.md §2 불변규칙 #4
위반이라 거부 — 그 비밀번호는 회전 필요. MEGA CYCLE H·I 둘 다 구현·테스트·정적검사는
끝났고 커밋 대기 또는 완료 상태이나, 실서버 배포·Chrome E2E는 스코프 사용자의 다음
조치(NOPASSWD sudoers 구성 또는 직접 배포) 대기 중이다. 이 사이 다른 독립 작업(다음
MEGA CYCLE 조사·구현)은 계속한다.
