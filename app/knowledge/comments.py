...
그건 부모 문서의 성질이고, 이 모듈은 문서를 모른다. 범위는 부르는 쪽인
`app/knowledge/service.py` 가 `get_scoped_document_or_404` 로 **권한 판정보다 먼저** 하고,
목록, 작성, 수정, 삭제가 전부 그 한 함수를 지난다. 티켓 댓글에서 정확히 이 자리가 뚫려
있었다: 목록에는 범위가 걸렸는데 삭제는 comment_id 만 받아 그냥 지나갔다
(→ tests/security/test_ticket_comment_write_scope.py).

## 조회 키가 `document_id` 다 (S14 · C2)

예전에는 옛 미러의 `notion_page_id` 로 붙어 있었다. 그 선택의 이유는 미러 동기화가
문서를 한 회차 prune 하면 캐시 행이 사라져 댓글이 딸려 나갔기 때문인데, 정본이
`documents` 로 넘어오면서 그 prune 자체가 없어졌다 -- 지우는 동작은 보관(archived)이고
행은 남는다. 다리가 둘이면 언젠가 서로 다른 답을 내므로 키는 하나만 둔다.
...
def list_comments(db: Session, *, document_id: str, me: User) -> dict:
...
def create_comment(
    ...
    *,
    ...
    """부모 문서는 **부르는 쪽이 이미 범위 안에서 찾아 왔다**(모듈 docstring)."""
