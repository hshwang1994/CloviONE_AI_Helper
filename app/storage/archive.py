...
**안 한다**: 일정 · 보존 정책 · manifest · PostgreSQL 덤프. 그것은 S12 가 세웠고
(`app/backups/`), 이 모듈은 그쪽이 부르는 조각으로 남았다 — `run_backup` 이 백업 회차마다
`archive_to_backup()` 을 부른다. 정책이 두 곳에 있으면 어긋나므로 여기서는 안 만든다.
