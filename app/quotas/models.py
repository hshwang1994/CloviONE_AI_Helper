...
**채팅 전송·폴링이 아닌** 두 곳뿐이다: AI 도우미 문장 생성(`/api/assistant/*?narrate=true`)과
문서 자동 생성 요청(`POST /api/admin/documents/generate`). 놀이 퀴즈 AI 는 이미 분당 5건
레이트리밋이 걸려 있고 그 모듈은 관측성 import 자체가 금지돼 있어(테스트가 고정) 여기 들어
오지 않는다 — 지어내지 않고 그대로 적어 둔다.
